...
Milex.removeConfigValue = function(action, el) {
    Milex.executeAction(action, function(response) {
    	if (response.success) {
 ...
 */
Milex.parseQuery = function (query) {
...
Milex.parseUrlHashParameter = function(url) {
    var url = url.split('#');
...
Milex.observeConfigTabs = function() {

    if (!mQuery('#config_coreconfig_last_shown_tab').length) {
        return;
    }

    var parameters = Milex.parseQuery(window.location.search.substr(1));
    ...
    mQuery('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
        var tab = Milex.parseUrlHashParameter(e.target.href);
        if (tab) {
...
Milex.resetEmailsToNotification = function(obj) {
...
mQuery(Milex.observeConfigTabs);
