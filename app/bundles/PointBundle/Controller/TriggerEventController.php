...
use Milex\PointBundle\Form\Type\TriggerEventType;
...
class TriggerEventController extends CommonFormController
{
    ...
    public function newAction()
    {
        ...
        $form   = $this->get('form.factory')->create(TriggerEventType::class, $triggerEvent, [
            'action'   => $this->generateUrl('milex_pointtriggerevent_action', ['objectAction' => 'new']),
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $actions            = $session->get('milex.point.'.$triggerId.'.triggerevents.modified');
                    ...
                    $session->set('milex.point.'.$triggerId.'.triggerevents.modified', $actions);
                }
            }
        }
        ...
        if ($cancelled || $valid) {
            ...
            $form = (isset($triggerEvent['settings']['formTheme'])) ?
                $this->setFormTheme($form, 'MilexPointBundle:Event:form.html.php', $triggerEvent['settings']['formTheme']) :
                $form->createView();
        ...
        $passthroughVars = [
            'milexContent' => 'pointTriggerEvent',
            'success'       => $success,
            'route'         => false,
        ];
        ...
        if (!empty($keyId)) {
            ...
            $template = (empty($triggerEvent['settings']['template'])) ? 'MilexPointBundle:Event:generic.html.php'
        ...
        return $this->ajaxAction([
            'contentTemplate' => 'MilexPointBundle:Event:form.html.php',
    ...
    public function editAction($objectId)
    {
        ...
        $events       = $session->get('milex.point.'.$triggerId.'.triggerevents.modified', []);
        $success      = 0;
        $valid        = $cancelled = false;
        ...
        if (null !== $triggerEvent) {
            ...
            $form = $this->get('form.factory')->create(TriggerEventType::class, $triggerEvent, [
                'action'   => $this->generateUrl('milex_pointtriggerevent_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
            ...
            if ('POST' == $method) {
                if (!$cancelled = $this->isFormCancelled($form)) {
                    if ($valid = $this->isFormValid($form)) {
                        ...
                        $session->set('milex.point.'.$triggerId.'.triggerevents.modified', $events);
    ...
    public function deleteAction($objectId)
    {
        ...
        $events    = $session->get('milex.point.'.$triggerId.'.triggerevents.modified', []);
        $delete    = $session->get('milex.point.'.$triggerId.'.triggerevents.deleted', []);
        ...
        if ('POST' == $this->request->getMethod() && null !== $triggerEvent) {
            ...
            if (!in_array($objectId, $delete)) {
                $delete[] = $objectId;
                $session->set('milex.point.'.$triggerId.'.triggerevents.deleted', $delete);
            }
