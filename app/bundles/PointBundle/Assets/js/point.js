//PointBundle
Milex.pointOnLoad = function (container) {
    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'point');
    }
};

Milex.pointTriggerOnLoad = function (container) {
    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'point.trigger');
    }
    ...
    if (mQuery('#triggerEvents')) {
        ...
        mQuery('#triggerEvents').sortable({
            ...
            stop: function(i) {
                mQuery.ajax({
                    type: "POST",
                    url: milexAjaxUrl + "?action=point:reorderTriggerEvents",
...
Milex.pointTriggerEventOnLoad = function (container, response) {
...
Milex.getPointActionPropertiesForm = function(actionType) {
    Milex.activateLabelLoadingIndicator('point_type');
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.html != 'undefined') {
                ...
                Milex.onPageLoad('#pointActionProperties', response);
            }
        },
...
Milex.EnablesOption = function (urlActionProperty) {
