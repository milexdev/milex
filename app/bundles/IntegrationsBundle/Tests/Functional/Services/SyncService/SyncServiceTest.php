...
namespace Milex\IntegrationsBundle\Tests\Functional\Services\SyncService;
...
class SyncServiceTest extends MilexMysqlTestCase
{
    ...
    public function testSync(): void
    {
        ...
        $prefix             = $this->container->getParameter('milex.db_table_prefix');
        ...
        $syncIntegrationsHelper = $this->container->get('milex.integrations.helper.sync_integrations');
        $syncIntegrationsHelper->addIntegration($exampleIntegration);

        /** @var SyncService $syncService */
        $syncService = $this->container->get('milex.integrations.sync.service');

        $syncService->processIntegrationSync(ExampleIntegration::NAME, true);
        $payload = $dataExchange->getOrderPayload();

        // Created the 48 known contacts already in Milex
        ...
        // All should be mapped to the Milex contact object
        $qb      = $this->connection->createQueryBuilder();
