...
namespace Milex\IntegrationsBundle\Tests\Functional\Services\SyncService\TestExamples\Sync\SyncDataExchange;
...
class ExampleSyncDataExchange implements SyncDataExchangeInterface
{
    ...
    /**
     * This pushes to the integration objects that were updated/created in Milex. The "sync order" is
    ...
    /**
     * This fetches objects from the integration that needs to be updated or created in Milex.
        ...
        foreach ($requestedObjects as $requestedObject) {
            ...
            foreach ($updatedPeople as $person) {
                ...
                foreach ($person as $field => $value) {
                    // Normalize the value from the API to what Milex needs
                    $normalizedValue = $this->valueNormalizer->normalizeForMilex(self::FIELDS[$field]['type'], $value);
