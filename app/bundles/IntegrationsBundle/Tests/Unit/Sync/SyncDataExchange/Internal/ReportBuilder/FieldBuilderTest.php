...
namespace Milex\IntegrationsBundle\Tests\Unit\Sync\SyncDataExchange\Internal\ReportBuilder;
...
use Milex\IntegrationsBundle\Sync\SyncDataExchange\Internal\ReportBuilder\FieldBuilder;
...
class FieldBuilderTest extends TestCase
{
    ...
    {
        $field = $this->getFieldBuilder()->buildObjectField('milex_internal_id', ['id' => 1], new ObjectDAO('Test'), 'Test');
        $this->assertEquals('milex_internal_id', $field->getName());
    ...
    public function testDoNotContactFieldIsAdded(): void
    {
        ...
        $field = $this->getFieldBuilder()->buildObjectField('milex_internal_dnc_email', ['id' => 1], new ObjectDAO('Test'), 'Test');

        $this->assertEquals('milex_internal_dnc_email', $field->getName());
        $this->assertEquals(FieldDAO::FIELD_CHANGED, $field->getState());
    ...
    public function testTimelineFieldIsAdded(): void
    {
        ...
        $field = $this->getFieldBuilder()->buildObjectField('milex_internal_contact_timeline', ['id' => 1], new ObjectDAO('Test'), 'Test');

        $this->assertEquals('milex_internal_contact_timeline', $field->getName());
        $this->assertEquals(FieldDAO::FIELD_CHANGED, $field->getState());
        $this->assertEquals(0, $field->getValue()->getNormalizedValue());
    }
