...
namespace Milex\IntegrationsBundle\Tests\Unit\Sync\SyncDataExchange\Internal\ObjectHelper;
...
class ContactObjectHelperTest extends TestCase
{
    ...
    public function testUpdate(): void
    {
        ...
        $companyValue->setType(MilexSyncDataExchange::OBJECT_COMPANY);
        ...
        $this->connection->expects($this->once())
            ->method('executeQuery')
            ->with(
                'SELECT c.companyname FROM '.MILEX_TABLE_PREFIX.'companies c WHERE c.id = :id',
                ['id' => $companyId]
            )
        ...
        $contact1->expects($this->exactly(2))
            ...
            ->withConsecutive(
                ['email', 'john@doe.com'],
                [MilexSyncDataExchange::OBJECT_COMPANY, 'Company A']
            );
    ...
    public function testDoNotContactIsAdded(): void
    {
        ...
        $objectChangeDAO->addField(new FieldDAO('milex_internal_dnc_email', new NormalizedValueDAO(NormalizedValueDAO::INT_TYPE, 1)));
    ...
    public function testDoNotContactIsRemoved(): void
    {
        ...
        $objectChangeDAO = new ObjectChangeDAO('Test', Contact::NAME, 1, 'MappedObject', 1, new \DateTime());
        $objectChangeDAO->addField(new FieldDAO('milex_internal_dnc_email', new NormalizedValueDAO(NormalizedValueDAO::INT_TYPE, 0)));
    ...
    public function testUnrecognizedDoNotContactDefaultsToManualDNC(): void
    {
        ...
        $objectChangeDAO = new ObjectChangeDAO('Test', Contact::NAME, 1, 'MappedObject', 1, new \DateTime());
        $objectChangeDAO->addField(new FieldDAO('milex_internal_dnc_email', new NormalizedValueDAO(NormalizedValueDAO::INT_TYPE, 4)));
