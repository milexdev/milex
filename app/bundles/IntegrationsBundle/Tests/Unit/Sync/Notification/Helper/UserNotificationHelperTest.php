...
use Milex\IntegrationsBundle\Sync\Notification\Helper\RouteHelper;
use Milex\IntegrationsBundle\Sync\Notification\Helper\UserHelper;
...
class UserNotificationHelperTest extends TestCase
{
    ...
    public function testNotificationSentToOwner(): void
    {
        ...
        $this->translator->expects($this->exactly(2))
            ->method('trans')
            ->withConsecutive(
                ['milex.integration.sync.user_notification.header', $this->anything()],
                ['milex.integration.sync.user_notification.sync_error', $this->anything()]
            )
            ->willReturn('test');
