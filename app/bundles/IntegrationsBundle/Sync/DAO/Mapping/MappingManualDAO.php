...
namespace Milex\IntegrationsBundle\Sync\DAO\Mapping;
...
class MappingManualDAO
{
    ...
    /**
     * Get a list of fields that should sync from Milex to the integration.
     *
     * @throws ObjectNotFoundException
     */
    public function getInternalObjectFieldsToSyncToIntegration(string $internalObjectName): array
    {
        ...
        foreach ($integrationObjectsNames as $integrationObjectName) {
            ...
            foreach ($fieldMappings as $fieldMapping) {
                if (ObjectMappingDAO::SYNC_TO_MILEX === $fieldMapping->getSyncDirection() && !$fieldMapping->isRequired()) {
                    // Ignore because this field is a one way sync from the integration to Milex nor is required
                    continue;
                }
    ...
    /**
     * Get a list of fields that should sync from the integration to Milex.
     *
     * @throws ObjectNotFoundException
     */
    public function getIntegrationObjectFieldsToSyncToMilex(string $integrationObjectName): array
    {
        if (!array_key_exists($integrationObjectName, $this->integrationObjectsMapping)) {
            throw new ObjectNotFoundException($integrationObjectName);
        }
        ...
        foreach ($internalObjectsNames as $internalObjectName) {
            ...
            foreach ($fieldMappings as $fieldMapping) {
                if (ObjectMappingDAO::SYNC_TO_INTEGRATION === $fieldMapping->getSyncDirection() && !$fieldMapping->isRequired()) {
                    // Ignore because this field is a one way sync from Milex to the integration nor a required field
                    continue;
                }
