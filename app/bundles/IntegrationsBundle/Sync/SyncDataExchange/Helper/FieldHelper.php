...
namespace Milex\IntegrationsBundle\Sync\SyncDataExchange\Helper;
...
class FieldHelper
{
    ...
    public function getSyncFields(string $objectName): array
    {
        ...
        $event                                     = new MilexSyncFieldsLoadEvent($objectName, $this->syncFields[$objectName]);
        $event                                     = $this->eventDispatcher->dispatch(IntegrationEvents::INTEGRATION_MILEX_SYNC_FIELDS_LOAD, $event);
        $this->syncFields[$event->getObjectName()] = $event->getFields();

        // Add ID as a read only field
        $this->syncFields[$objectName]['milex_internal_id'] = $this->translator->trans('milex.core.id');
        ...
        // Milex contacts have "pseudo" fields such as channel do not contact, timeline, etc.
        $channels = $this->channelListHelper->getFeatureChannels([LeadModel::CHANNEL_FEATURE], true);
        foreach ($channels as $label => $channel) {
            $this->syncFields[$objectName]['milex_internal_dnc_'.$channel] = $this->translator->trans('milex.integration.sync.channel_dnc', ['%channel%' => $label]);
        }

        // Add the timeline link
        $this->syncFields[$objectName]['milex_internal_contact_timeline'] = $this->translator->trans('milex.integration.sync.contact_timeline');
