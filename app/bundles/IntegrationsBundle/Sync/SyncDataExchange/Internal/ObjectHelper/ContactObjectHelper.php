...
class ContactObjectHelper implements ObjectHelperInterface
{
    ...
    {
        if (MilexSyncDataExchange::OBJECT_COMPANY === $value->getType() && 0 < $value->getValue()) {
            try {
    ...
    public function findObjectsByFieldValues(array $fields): array
    {
        ...
        foreach ($fields as $col => $val) {
            // Use andWhere because Milex treats conflicting unique identifiers as different objects
    ...
    public function findOwnerIds(array $objectIds): array
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'leads', 'c');
    ...
    private function getCompanyNameById(int $id): string
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'companies', 'c');
        $qb->where('c.id = :id');
    ...
    private function processPseudoFields(Lead $contact, array $fields, string $integration): void
    {
        foreach ($fields as $name => $field) {
            if (0 === strpos($name, 'milex_internal_dnc_')) {
                $channel   = str_replace('milex_internal_dnc_', '', $name);
