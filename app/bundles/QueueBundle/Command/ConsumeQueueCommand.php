...
namespace Milex\QueueBundle\Command;
...
class ConsumeQueueCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:queue:process')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $queueService = $container->get('milex.queue.service');

        if (!$queueService->isQueueEnabled()) {
            $output->writeLn('You have not configured milex to use queue mode, nothing will be processed');
