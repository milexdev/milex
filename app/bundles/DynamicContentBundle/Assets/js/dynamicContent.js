...
Milex.toggleDwcFilters = function () {
    ...
    if (mQuery("#dwcFiltersTab").hasClass('hide')) {
        ...
        Milex.dynamicContentOnLoad();
    }
};

Milex.dynamicContentOnLoad = function (container, response) {
    if (typeof container !== 'object') {
        if (mQuery(container + ' #list-search').length) {
            Milex.activateSearchAutocomplete('list-search', 'dynamicContent');
        }
    }

    var availableFilters = mQuery('div.dwc-filter').find('select[data-milex="available_filters"]');
    Milex.activateChosenSelect(availableFilters, false);

    Milex.dynamicFiltersOnLoad('div.dwc-filter');
};

Milex.dynamicFiltersOnLoad = function(container, response) {
    ...
    if (mQuery('#' + prefix + '_filters').length) {
        mQuery('#available_filters').on('change', function() {
            if (mQuery(this).val()) {
                Milex.addDwcFilter(mQuery(this).val(),mQuery('option:selected',this).data('field-object'));
                mQuery(this).val('');
...
Milex.addDwcFilter = function (elId, elObj) {
    ...
    if (isSpecial) {
        ...
        if (fieldCallback && typeof Milex[fieldCallback] == 'function') {
            var fieldOptions = filterOption.data("field-list");
            Milex[fieldCallback](filterIdBase + 'filter', elId, fieldOptions);
        } else {
            filter.attr('data-target', alias);
            Milex.activateLookupTypeahead(filter.parent());
        }
        ...
        if (fieldCallback && typeof Milex[fieldCallback] == 'function') {
            var fieldOptions = filterOption.data("field-list");
            Milex[fieldCallback](filterIdBase + 'display', elId, fieldOptions);
        }
    } else {
    ...
    Milex.convertDwcFilterInput('#' + filterIdBase + 'operator');

    // Reposition if applicable
    Milex.updateFilterPositioning(mQuery('#' + filterIdBase + 'glue'));
};

Milex.convertDwcFilterInput = function(el) {
    var prefix = 'leadlist';
    ...
    if (mQuery(filterId).is('select')) {
        ...
        Milex.destroyChosen(mQuery(filterId));

        mQuery(filterId).attr('data-placeholder', placeholder);

        Milex.activateChosenSelect(mQuery(filterId));
    }
};

Milex.standardDynamicContentUrl = function(options) {
    if (!options) {
        return;
    }
...
Milex.disabledDynamicContentAction = function(opener) {
...
if (typeof MilexIsDwcReady === 'undefined') {
    var MilexIsDwcReady = true;
    ...
    if (
        ...
        document.addEventListener("DOMContentLoaded", Milex.dynamicContentOnLoad);
    }
}
