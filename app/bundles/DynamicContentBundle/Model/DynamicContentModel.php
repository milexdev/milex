...
namespace Milex\DynamicContentBundle\Model;
...
use Milex\DynamicContentBundle\Entity\DynamicContentRepository;
use Milex\DynamicContentBundle\Entity\Stat;
use Milex\DynamicContentBundle\Event\DynamicContentEvent;
use Milex\DynamicContentBundle\Form\Type\DynamicContentType;
...
class DynamicContentModel extends FormModel implements AjaxLookupModelInterface
{
    ...
    public function getRepository()
    {
        ...
        $repo = $this->em->getRepository('MilexDynamicContentBundle:DynamicContent');
    ...
    /**
     * @return \Milex\DynamicContentBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('MilexDynamicContentBundle:Stat');
    }
    ...
    public function setSlotContentForLead(DynamicContent $dwc, Lead $lead, $slot)
    {
        ...
        $qb->insert(MILEX_TABLE_PREFIX.'dynamic_content_lead_data')
            ->values([
                'lead_id'            => $lead->getId(),
    ...
    public function getSlotContentForLead($slot, $lead)
    {
        ...
        $qb->select('dc.id, dc.content')
            ->from(MILEX_TABLE_PREFIX.'dynamic_content', 'dc')
            ->leftJoin('dc', MILEX_TABLE_PREFIX.'dynamic_content_lead_data', 'dcld', 'dcld.dynamic_content_id = dc.id')
    ...
    {
        $q->join('t', MILEX_TABLE_PREFIX.'dynamic_content', 'd', 'd.id = t.dynamic_content_id')
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || 'total_and_unique' === $flag) {
            ...
            $chart->setDataset($this->translator->trans('milex.dynamicContent.show.total.views'), $data);
        }
        ...
        if ('unique' === $flag || 'total_and_unique' === $flag) {
            ...
            $data = $query->loadAndBuildTimeData($q);
            $chart->setDataset($this->translator->trans('milex.dynamicContent.show.unique.views'), $data);
        }
