...
$view->addGlobal('translationBase', 'milex.dynamicContent');
$view->addGlobal('milexContent', 'dynamicContent');
...
if (!empty($id)) {
    ...
    $header = $view['translator']->trans('milex.dynamicContent.header.edit', ['%name%' => $name]);
} else {
    $header = $view['translator']->trans('milex.dynamicContent.header.new');
}
                ...
                <ul class="bg-auto nav nav-tabs pr-md pl-md">
                    ...
                    <li class="<?php echo ($form->vars['value']->isCampaignBased() || isset($form['updateSelect'])) ? 'hide' : ''; ?>" id="dwcFiltersTab">
                        <a href="#filters" role="tab" data-toggle="tab"<?php echo $filterErrors; ?>>
                            <?php echo $view['translator']->trans('milex.core.filters'); ?>
                            <?php if ($filterErrors): ?>
                                <i class="fa fa-warning"></i>
                            <?php endif; ?>
                        </a>
                                                    ...
                                                    <optgroup label="<?php echo $view['translator']->trans('milex.lead.'.$header); ?>">
                                                        <?php foreach ($field as $value => $params):
                                                            ...
                                                            $choices   = ('boolean' === $params['properties']['type'])
                                                                ?
                                                                array_flip(\Milex\LeadBundle\Helper\FormFieldHelper::parseBooleanList($list))
                                                                :
                                                                \Milex\LeadBundle\Helper\FormFieldHelper::parseListForChoices($list);
...
<div class="hide" id="templates">
    <?php foreach ($templates as $dataKey => $template): ?>
        <?php $attr = ('tags' == $dataKey) ? ' data-placeholder="'.$view['translator']->trans('milex.lead.tags.select_or_create')
            .'" data-no-results-text="'.$view['translator']->trans('milex.lead.tags.enter_to_create')
            .'" data-allow-add="true" onchange="Milex.createLeadTag(this)"' : ''; ?>
