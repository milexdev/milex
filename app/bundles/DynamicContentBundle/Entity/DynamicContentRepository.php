...
class DynamicContentRepository extends CommonRepository
{
    ...
    public function getEntities(array $args = [])
    {
        $q = $this->_em
            ->createQueryBuilder()
            ->select('e')
            ->from('MilexDynamicContentBundle:DynamicContent', 'e', 'e.id');
    ...
    public function upSentCount($id, $increaseBy = 1)
    {
        ...
        $q->update(MILEX_TABLE_PREFIX.'dynamic_content')
    ...
    public function getDynamicContentForSlotFromCampaign($slot)
    {
        ...
        $qb->select('ce.properties')
            ->from(MILEX_TABLE_PREFIX.'campaign_events', 'ce')
            ->leftJoin('ce', MILEX_TABLE_PREFIX.'campaigns', 'c', 'c.id = ce.campaign_id')
