...
class StatRepository extends CommonRepository
{
    ...
    public function getSentStats($dynamicContentId)
    {
        ...
        $q->select('s.lead_id')
            ->from(MILEX_TABLE_PREFIX.'dynamic_content_stats', 's')
    ...
    public function getLeadStats($leadId = null, array $options = [])
    {
        ...
        $query->select('dc.id AS dynamic_content_id, s.id, s.date_sent as dateSent, dc.name, s.sent_details as sentDetails, s.lead_id')
            ->from(MILEX_TABLE_PREFIX.'dynamic_content_stats', 's')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'dynamic_content', 'dc', 'dc.id = s.dynamic_content_id');
    ...
    {
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MILEX_TABLE_PREFIX.'dynamic_content_stats')
    ...
    {
        $this->_em->getConnection()->delete(MILEX_TABLE_PREFIX.'dynamic_content_stats', ['id' => (int) $id]);
    }
