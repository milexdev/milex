...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    protected function addSmsEvents(LeadTimelineEvent $event, $state)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.sms.timeline.status.'.$state);
        ...
        /** @var \Milex\SmsBundle\Entity\StatRepository $statRepository */
        $statRepository        = $this->em->getRepository('MilexSmsBundle:Stat');
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($stats['results'] as $stat) {
                if (!empty($stat['sms_name'])) {
                    ...
                    $label = $this->translator->trans('milex.sms.timeline.event.custom_sms');
                }

                $eventName = [
                        'label'      => $label,
                        'href'       => $this->router->generate('milex_sms_action', ['objectAction'=>'view', 'objectId' => $stat['sms_id']]),
                    ];
                ...
                $event->addEvent(
                    [
                        ...
                        'contentTemplate' => 'MilexSmsBundle:SubscribedEvents\Timeline:index.html.php',
