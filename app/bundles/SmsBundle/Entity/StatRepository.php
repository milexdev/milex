...
class StatRepository extends CommonRepository
{
    ...
    public function getSentStats($smsId, $listId = null)
    {
        ...
        $q->select('s.lead_id')
            ->from(MILEX_TABLE_PREFIX.'sms_messages_stats', 's')
            ->where('s.sms_id = :sms')
    ...
    public function getSentCount($smsIds = null, $listId = null)
    {
        ...
        $q->select('count(s.id) as sent_count')
            ->from(MILEX_TABLE_PREFIX.'sms_message_stats', 's');
    ...
    public function getLeadStats($leadId, array $options = [])
    {
        ...
        $query->from(MILEX_TABLE_PREFIX.'sms_message_stats', 's')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'sms_messages', 'e', 's.sms_id = e.id');
    ...
    {
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MILEX_TABLE_PREFIX.'sms_message_stats')
    ...
    {
        $this->_em->getConnection()->delete(MILEX_TABLE_PREFIX.'sms_message_stats', ['id' => (int) $id]);
    }
