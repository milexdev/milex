...
namespace Milex\SmsBundle\Entity;
...
class SmsRepository extends CommonRepository
{
    ...
    public function getEntities(array $args = [])
    {
        $q = $this->_em
            ...
            ->from('MilexSmsBundle:Sms', $this->getTableAlias(), $this->getTableAlias().'.id');
    ...
    public function getSegmentsContactsQuery(int $smsId)
    {
        ...
        $q->from(MILEX_TABLE_PREFIX.'sms_message_list_xref', 'sml')
            ->join('sml', MILEX_TABLE_PREFIX.'lead_lists', 'll', 'll.id = sml.leadlist_id and ll.is_published = 1')
            ->join('ll', MILEX_TABLE_PREFIX.'lead_lists_leads', 'lll', 'lll.leadlist_id = sml.leadlist_id and lll.manually_removed = 0')
            ->join('lll', MILEX_TABLE_PREFIX.'leads', 'l', 'lll.lead_id = l.id')
            ->where(
                $q->expr()->andX(
    ...
    public function upCount($id, $type = 'sent', $increaseBy = 1)
    {
        try {
            ...
            $q->update(MILEX_TABLE_PREFIX.'sms_messages')
