/** SmsBundle **/
Milex.smsOnLoad = function (container, response) {
    const smsMessage = mQuery('#sms_message');

    if (smsMessage.length) {
        Milex.setSmsCharactersCount(smsMessage);
        smsMessage.on('input', () => {
            Milex.setSmsCharactersCount(smsMessage);
        });
    }
    ...
    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'sms');
    }
...
Milex.setSmsCharactersCount = function (smsMessage) {
    mQuery('#sms_nb_char').text((smsMessage.val().length))
};

Milex.selectSmsType = function(smsType) {
    if (smsType == 'list') {
        mQuery('#leadList').removeClass('hide');
        mQuery('#publishStatus').addClass('hide');
        mQuery('.page-header h3').text(milexLang.newListSms);
    } else {
        mQuery('#publishStatus').removeClass('hide');
        mQuery('#leadList').addClass('hide');
        mQuery('.page-header h3').text(milexLang.newTemplateSms);
    }
...
Milex.standardSmsUrl = function(options) {
    if (!options) {
        return;
    }
...
Milex.disabledSmsAction = function(opener) {
