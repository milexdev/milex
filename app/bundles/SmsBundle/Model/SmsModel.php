...
namespace Milex\SmsBundle\Model;
...
use Milex\SmsBundle\Event\SmsEvent;
...
class SmsModel extends FormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\SmsBundle\Entity\SmsRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexSmsBundle:Sms');
    }

    /**
     * @return \Milex\SmsBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('MilexSmsBundle:Stat');
    }
    ...
    public function sendSms(Sms $sms, $sendTo, $options = [])
    {
        ...
        if (!$sms->isPublished()) {
            foreach ($contacts as $leadId => $lead) {
                $results[$leadId] = [
                    'sent'   => false,
                    'status' => 'milex.sms.campaign.failed.unpublished',
                ];
            }
        ...
        if (!empty($dnc)) {
            foreach ($dnc as $removeMeId => $removeMeReason) {
                $results[$removeMeId] = [
                    'sent'   => false,
                    'status' => 'milex.sms.campaign.failed.not_contactable',
                ];
        ...
        if (!empty($contacts)) {
            ...
            if ($queued) {
                foreach ($queued as $queue) {
                    $results[$queue] = [
                        'sent'   => false,
                        'status' => 'milex.sms.timeline.status.scheduled',
                    ];
            ...
            if (count($contacts)) {
                ...
                foreach ($contacts as $lead) {
                    ...
                    if (empty($leadPhoneNumber)) {
                        $results[$leadId] = [
                            'sent'   => false,
                            'status' => 'milex.sms.campaign.failed.missing_number',
                        ];
                    ...
                    $sendResult = [
                        'sent'    => false,
                        'type'    => 'milex.sms.sms',
                        'status'  => 'milex.sms.timeline.status.delivered',
                        'id'      => $sms->getId(),
                        'name'    => $sms->getName(),
    ...
    {
        $q->join('t', MILEX_TABLE_PREFIX.'sms_messages', 's', 's.id = t.sms_id')
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || 'total_and_unique' === $flag) {
            ...
            $chart->setDataset($this->translator->trans('milex.sms.show.total.sent'), $data);
        }
        ...
        if (!$flag || 'failed' === $flag) {
            ...
            $data = $query->loadAndBuildTimeData($q);
            $chart->setDataset($this->translator->trans('milex.sms.show.failed'), $data);
        }
