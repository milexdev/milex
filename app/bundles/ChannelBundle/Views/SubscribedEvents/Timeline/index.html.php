...
$getChannelOutput = function ($channel) use ($view, $event, $log, $eventSettings) {
    ...
    if (!empty($log['metadata']['dnc'])) {
        switch ($log['metadata']['dnc']) {
            case \Milex\LeadBundle\Entity\DoNotContact::BOUNCED:
                $msg = 'milex.lead.event.donotcontact_bounced';
                break;
            case \Milex\LeadBundle\Entity\DoNotContact::UNSUBSCRIBED:
                $msg = 'milex.lead.event.donotcontact_unsubscribed';
                break;
            case \Milex\LeadBundle\Entity\DoNotContact::MANUAL:
    ...
    $template                     = 'MilexCampaignBundle:SubscribedEvents\Timeline:index.html.php';
    $channelEvent                 = $event;
