...
namespace Milex\ChannelBundle\Model;

use Milex\ChannelBundle\ChannelEvents;
use Milex\ChannelBundle\Entity\MessageQueue;
use Milex\ChannelBundle\Event\MessageQueueBatchProcessEvent;
use Milex\ChannelBundle\Event\MessageQueueEvent;
use Milex\ChannelBundle\Event\MessageQueueProcessEvent;
...
class MessageQueueModel extends FormModel
{
    ...
    /**
     * @return \Doctrine\ORM\EntityRepository|\Milex\ChannelBundle\Entity\MessageQueueRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexChannelBundle:MessageQueue');
    }
    ...
    public function processFrequencyRules(
        ...
        /** @var \Milex\LeadBundle\Entity\FrequencyRuleRepository $frequencyRulesRepo */
        $frequencyRulesRepo     = $this->em->getRepository('MilexLeadBundle:FrequencyRule');
    ...
    public function queue(
        ...
        foreach ($leads as $lead) {
            ...
            if ($campaignEventId) {
                $messageQueue->setEvent($this->em->getReference('MilexCampaignBundle:Event', $campaignEventId));
            }
            ...
            $messageQueue->setLead(
                ($lead instanceof Lead) ? $lead : $this->em->getReference('MilexLeadBundle:Lead', $leadId)
            );
    ...
    public function sendMessages($channel = null, $channelId = null)
    {
        ...
        while ($queue = $this->getRepository()->getQueuedMessages($limit, $processStarted, $channel, $channelId)) {
            ...
            $this->em->clear(\Milex\CampaignBundle\Entity\Event::class);
        }
