...
use Milex\StageBundle\Entity\LeadStageLogRepository;
...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    public function onTimelineGenerate(LeadTimelineEvent $event)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.stage.event.changed');
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($logs['results'] as $log) {
                if (isset($log['reference']) && null != $log['reference']) {
                    $eventLabel = [
                        'label'      => $log['eventName'],
                        'href'       => $this->router->generate('milex_stage_action', ['objectAction' => 'edit', 'objectId' => $log['reference']]),
                        'isExternal' => false,
                    ];
                } else {
