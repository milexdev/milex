...
namespace Milex\StageBundle\Entity;
...
class LeadStageLogRepository extends CommonRepository
{
    ...
    public function updateLead($fromLeadId, $toLeadId)
    {
        ...
        $results = $this->_em->getConnection()->createQueryBuilder()
            ->select('pl.stage_id')
            ->from(MILEX_TABLE_PREFIX.'stage_lead_action_log', 'pl')
        ...
        $q->update(MILEX_TABLE_PREFIX.'stage_lead_action_log')
        ...
        if (!empty($actions)) {
            ...
            $this->_em->getConnection()->createQueryBuilder()
                ->delete(MILEX_TABLE_PREFIX.'stage_lead_action_log')
                ->where('lead_id = '.(int) $fromLeadId)
                ->execute();
        } else {
            $q->execute();
