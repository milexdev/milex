...
use Milex\NotificationBundle\Entity\StatRepository;
...
class ReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $columns              = [
            $prefix.'heading' => [
                'label' => 'milex.notification.mobile_notification.heading',
                'type'  => 'string',
            ],
            $prefix.'lang' => [
            ...
            $prefix.'read_count' => [
                'label' => 'milex.mobile_notification.report.read_count',
                'type'  => 'int',
            ],
            'read_ratio' => [
                'alias'   => 'read_ratio',
                'label'   => 'milex.mobile_notification.report.read_ratio',
                'type'    => 'string',
                'formula' => 'CONCAT(ROUND(('.$prefix.'read_count/'.$prefix.'sent_count)*100),\'%\')',
            ],
            $prefix.'sent_count' => [
                'label' => 'milex.mobile_notification.report.sent_count',
                'type'  => 'int',
            ],
            'hits' => [
                'alias'   => 'hits',
                'label'   => 'milex.mobile_notification.report.hits_count',
                'type'    => 'string',
                'formula' => $channelUrlTrackables.'hits',
            ],
            'unique_hits' => [
                'alias'   => 'unique_hits',
                'label'   => 'milex.mobile_notification.report.unique_hits_count',
                'type'    => 'string',
                'formula' => $channelUrlTrackables.'unique_hits',
            ],
            'hits_ratio' => [
                'alias'   => 'hits_ratio',
                'label'   => 'milex.mobile_notification.report.hits_ratio',
                'type'    => 'string',
            ...
            'unique_ratio' => [
                'alias'   => 'unique_ratio',
                'label'   => 'milex.mobile_notification.report.unique_ratio',
                'type'    => 'string',
        ...
        $columns = array_merge(
            $columns,
            $event->getStandardColumns($prefix, [], 'milex_mobile_notification_action'),
            $event->getCategoryColumns()
        );
        $data = [
            'display_name' => 'milex.notification.mobile_notifications',
            'columns'      => $columns,
        ];
        ...
        if ($event->checkContext(self::MOBILE_NOTIFICATIONS_STATS)) {
            ...
            $statColumns = [
                $statPrefix.'date_sent' => [
                    'label'          => 'milex.mobile_notifications.report.stat.date_sent',
                    'type'           => 'datetime',
                    'groupByFormula' => 'DATE('.$statPrefix.'date_sent)',
                ],
                $statPrefix.'date_read' => [
                    'label'          => 'milex.mobile_notifications.report.stat.date_read',
                    'type'           => 'datetime',
            ...
            $data = [
                'display_name' => 'milex.mobile_notification.stats.report.table',
            ...
            // Register Graphs
            $event->addGraph($context, 'line', 'milex.mobile_notification.graph.line.stats');
            $event->addGraph($context, 'table', 'milex.mobile_notification.table.most.mobile_notifications.sent');
            $event->addGraph($context, 'table', 'milex.mobile_notification.table.most.mobile_notifications.read');
            $event->addGraph($context, 'table', 'milex.mobile_notification.table.most.mobile_notifications.read.percent');
        }
    }
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        switch ($event->getContext()) {
            case self::MOBILE_NOTIFICATIONS:
                $qb->from(MILEX_TABLE_PREFIX.'push_notifications', 'pn');
            ...
            case self::MOBILE_NOTIFICATIONS_STATS:
                $qb->from(MILEX_TABLE_PREFIX.'push_notification_stats', 'pns')
                    ->leftJoin('pns', MILEX_TABLE_PREFIX.'push_notifications', 'pn', 'pn.id = pns.notification_id');
    ...
    public function onReportGraphGenerate(ReportGraphEvent $event)
    {
        ...
        foreach ($graphs as $g) {
            ...
            switch ($g) {
                case 'milex.mobile_notification.graph.line.stats':
                    ...
                    $chart->setDataset($options['translator']->trans('milex.mobile_notification.sent.mobile_notifications'), $sends);
                    $chart->setDataset($options['translator']->trans('milex.mobile_notification.read.mobile_notifications'), $reads);
                    $data         = $chart->render();
                    $data['name'] = $g;
                ...
                case 'milex.mobile_notification.table.most.mobile_notifications.sent':
                    ...
                    $graphData['link']      = 'milex_mobile_notification_action';
                    $event->setGraph($g, $graphData);
                    break;

                case 'milex.mobile_notification.table.most.mobile_notifications.read':
                ...
                case 'milex.mobile_notification.table.most.mobile_notifications.read.percent':
