...
namespace Milex\NotificationBundle\Entity;
...
class StatRepository extends CommonRepository
{
    ...
    public function getSentStats($notificationId, $listId = null)
    {
        ...
        $q->select('s.lead_id')
            ->from(MILEX_TABLE_PREFIX.'push_notification_stats', 's')
    ...
    public function getLeadStats($leadId, array $options = [])
    {
        ...
        $query->select('IDENTITY(s.notification) AS notification_id, s.id, s.dateRead, s.dateSent, e.title, s.isRead, s.retryCount, IDENTITY(s.list) AS list_id, l.name as list_name, s.trackingHash as idHash, s.clickDetails')
            ->leftJoin('MilexNotificationBundle:Notification', 'e', 'WITH', 'e.id = s.notification')
            ->leftJoin('MilexLeadBundle:LeadList', 'l', 'WITH', 'l.id = s.list')
            ->where(
                $query->expr()->andX(
    ...
    {
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MILEX_TABLE_PREFIX.'push_notification_stats')
    ...
    {
        $this->_em->getConnection()->delete(MILEX_TABLE_PREFIX.'push_notification_stats', ['id' => (int) $id]);
    }
