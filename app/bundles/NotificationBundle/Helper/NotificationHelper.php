...
namespace Milex\NotificationBundle\Helper;
...
class NotificationHelper
{
    ...
    /**
     * @var \Milex\LeadBundle\Model\DoNotContact
     */
     ...
     */
    public function __construct(EntityManager $em, AssetsHelper $assetsHelper, CoreParametersHelper $coreParametersHelper, IntegrationHelper $integrationHelper, Router $router, RequestStack $requestStack, \Milex\LeadBundle\Model\DoNotContact $doNotContact)
    {
        $this->em                   = $em;
    ...
    {
        /** @var \Milex\LeadBundle\Entity\LeadRepository $repo */
    ...
    {
        if ($this->hasScript()) {
            return 'MilexJS.insertScript(\'https://cdn.onesignal.com/sdks/OneSignalSDK.js\');
...
var postUserIdToMilex = function(userId) {
    var data = [];
    data['osid'] = userId;
    MilexJS.makeCORSRequest('GET', '{$leadAssociationUrl}', data);
};
            ...
            OneSignal.on('subscriptionChange', function(isSubscribed) {
                if (isSubscribed) {
                    OneSignal.getUserId(function(newUserId) {
                        postUserIdToMilex(newUserId);
                    });
                }
            });
        } else {
            postUserIdToMilex(userId);
        }
    });
