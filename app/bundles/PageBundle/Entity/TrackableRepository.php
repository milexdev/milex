...
class TrackableRepository extends CommonRepository
{
    ...
    public function findByChannel($channel, $channelId)
    {
        ...
        return $q->select('r.redirect_id, r.url, r.id, '.$tableAlias.'.hits, '.$tableAlias.'.unique_hits')
            ->from(MILEX_TABLE_PREFIX.'page_redirects', 'r')
            ->innerJoin('r', MILEX_TABLE_PREFIX.'channel_url_trackables', $tableAlias,
                $q->expr()->andX(
    ...
    public function upHitCount($redirectId, $channel, $channelId, $increaseBy = 1, $unique = false)
    {
        ...
        $q->update(MILEX_TABLE_PREFIX.'channel_url_trackables')
    ...
    public function getCount($channel, $channelIds, $listId, ChartQuery $chartQuery = null, $combined = false, $countColumn = 'ph.id')
    {
        $q = $this->_em->getConnection()->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'channel_url_trackables', 'cut')
            ->innerJoin('cut', MILEX_TABLE_PREFIX.'page_hits', 'ph', 'ph.redirect_id = cut.redirect_id AND ph.source = cut.channel AND ph.source_id = cut.channel_id');
        ...
        if ($listId) {
            if (!$combined) {
                $q->innerJoin('ph', MILEX_TABLE_PREFIX.'lead_lists_leads', 'cs', 'cs.lead_id = ph.lead_id');
