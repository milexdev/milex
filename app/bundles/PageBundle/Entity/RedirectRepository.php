...
namespace Milex\PageBundle\Entity;
...
class RedirectRepository extends CommonRepository
{
    ...
    public function upHitCount($id, $increaseBy = 1, $unique = false)
    {
        ...
        $q->update(MILEX_TABLE_PREFIX.'page_redirects')
    ...
    public function getMostHitEmailRedirects(
        ...
        $q->addSelect('pr.url')
            ...
            ->join('ph', MILEX_TABLE_PREFIX.'page_redirects', 'pr', 'pr.id = ph.redirect_id')
            ->join('ph', MILEX_TABLE_PREFIX.'email_stats', 'es', 'ph.source = "email" and ph.source_id = es.email_id and ph.lead_id = es.lead_id')
            ->join('es', MILEX_TABLE_PREFIX.'emails', 'e', 'es.email_id = e.id')
        ...
        $q->leftJoin('es', MILEX_TABLE_PREFIX.'campaign_events', 'ce', 'es.source = "campaign.event" and es.source_id = ce.id')
