...
use Milex\PageBundle\Event\PageEvent;
use Milex\PageBundle\Event\PageHitEvent;
use Milex\PageBundle\Form\Type\PageType;
...
class PageModel extends FormModel
{
    ...
    /**
     * @return \Milex\PageBundle\Entity\PageRepository
     */
    public function getRepository()
    {
        $repo = $this->em->getRepository('MilexPageBundle:Page');
    ...
    /**
     * @return \Milex\PageBundle\Entity\HitRepository
     */
    public function getHitRepository()
    {
        return $this->em->getRepository('MilexPageBundle:Hit');
    }
    ...
    public function generateUrl($entity, $absolute = true, $clickthrough = [])
    {
        ...
        return $this->buildUrl('milex_page_public', ['slug' => $slug], $absolute, $clickthrough);
    }
    ...
    public function generateSlug($entity)
    {
        ...
        if ($this->catInUrl) {
            ...
            $catSlug  = (!empty($category))
                ? $category->getAlias()
                :
                $this->translator->trans('milex.core.url.uncategorized');
        }
    ...
    public function hitPage($page, Request $request, $code = '200', Lead $lead = null, $query = [])
    {
        ...
        if ($hit) {
            $this->cookieHelper->setCookie('milex_referer_id', $hit->getId() ?: null);
        }
    ...
    public function processPageHit(Hit $hit, $page, Request $request, Lead $lead, $trackingNewlyGenerated, $activeRequest = true)
    {
        ...
        if (!empty($clickthrough)) {
            ...
            if (!empty($clickthrough['email'])) {
                $emailRepo = $this->em->getRepository('MilexEmailBundle:Email');
        ...
        if (!$trackingNewlyGenerated) {
            $lastHit = $request->cookies->get('milex_referer_id');
            if (!empty($lastHit)) {
    ...
    {
        $q->join('t', MILEX_TABLE_PREFIX.'pages', 'p', 'p.id = t.page_id')
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || 'total_and_unique' == $flag) {
            ...
            $chart->setDataset($this->translator->trans('milex.page.show.total.visits'), $data);
        }
        ...
        if ('unique' == $flag || 'total_and_unique' == $flag) {
            ...
            $data = $query->loadAndBuildTimeData($q);
            $chart->setDataset($this->translator->trans('milex.page.show.unique.visits'), $data);
        }
    ...
    public function getDeviceGranularityData(\DateTime $dateFrom, \DateTime $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('count(h.id) as count, ds.device as device')
            ->from(MILEX_TABLE_PREFIX.'page_hits', 'h')
        ...
        foreach ($results as $result) {
            $label = empty($result['device']) ? $this->translator->trans('milex.core.no.info') : $result['device'];
    ...
    public function getPopularPages($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('COUNT(DISTINCT t.id) AS hits, p.id, p.title, p.alias')
            ->from(MILEX_TABLE_PREFIX.'page_hits', 't')
            ->join('t', MILEX_TABLE_PREFIX.'pages', 'p', 'p.id = t.page_id')
            ->orderBy('hits', 'DESC')
            ->groupBy('p.id')
            ->setMaxResults($limit);
    ...
    public function getPageList($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('t.id, t.title AS name, t.date_added, t.date_modified')
            ->from(MILEX_TABLE_PREFIX.'pages', 't')
            ->setMaxResults($limit);
    ...
    private function setUtmTags(Hit $hit, Lead $lead): void
    {
        ...
        if ($queryHasUtmTags && $lead) {
            ...
            $repo = $this->em->getRepository('MilexLeadBundle:UtmTag');
    ...
    private function getPageUrl(Request $request, $page)
    {
        ...
        // Use the current URL
        $isPageEvent = false;
        if (false !== strpos($request->server->get('REQUEST_URI'), $this->router->generate('milex_page_tracker'))) {
            // Tracking pixel is used
            if ($request->server->get('QUERY_STRING')) {
                parse_str($request->server->get('QUERY_STRING'), $query);
                $isPageEvent = true;
            }
        } elseif (false !== strpos($request->server->get('REQUEST_URI'), $this->router->generate('milex_page_tracker_cors'))) {
