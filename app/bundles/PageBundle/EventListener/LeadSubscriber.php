...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    public function onTimelineGenerate(LeadTimelineEvent $event)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.page.event.hit');
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($hits['results'] as $hit) {
                $template = 'MilexPageBundle:SubscribedEvents\Timeline:index.html.php';
                $icon     = 'fa-link';
                ...
                if (!empty($hit['page_id'])) {
                    ...
                    $eventLabel = [
                        'label' => $page->getTitle(),
                        'href'  => $this->router->generate('milex_page_action', ['objectAction' => 'view', 'objectId' => $hit['page_id']]),
                    ];
                } else {
                    $eventLabel = [
    ...
    public function onTimelineGenerateVideo(LeadTimelineEvent $event)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.page.event.videohit');
        $event->addEventType($eventTypeKey, $eventTypeName);
        $event->addSerializerGroup('pageList', 'hitDetails');
        ...
        if (!$event->isEngagementCount()) {
            // Add the hits to the event array
            foreach ($hits['results'] as $hit) {
                $template   = 'MilexPageBundle:SubscribedEvents\Timeline:videohit.html.php';
