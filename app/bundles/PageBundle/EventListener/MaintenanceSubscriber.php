    ...
    private function cleanData(MaintenanceEvent $event, $table)
    {
        ...
        if ($event->isDryRun()) {
            $qb->select('count(*) as records')
              ->from(MILEX_TABLE_PREFIX.$table, 'h')
              ->join('h', MILEX_TABLE_PREFIX.'leads', 'l', 'h.lead_id = l.id')
            ...
            $subQb->select('id')->from(MILEX_TABLE_PREFIX.'leads', 'l')
              ->where($qb->expr()->lte('l.last_active', ':date'));
            ...
            while (true) {
                ...
                $rows += $qb->delete(MILEX_TABLE_PREFIX.$table)
                  ->where(
                    $qb->expr()->in(
                      'lead_id', $leadsIds
                    )
