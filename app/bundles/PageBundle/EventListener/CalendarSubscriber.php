...
class CalendarSubscriber implements EventSubscriberInterface
{
    ...
    public function onCalendarGenerate(CalendarGeneratorEvent $event)
    {
        ...
        $query->from(MILEX_TABLE_PREFIX.'pages', 'p')
            ->leftJoin('p', MILEX_TABLE_PREFIX.'categories', 'c', 'c.id = p.category_id AND c.bundle=:bundle')
        ...
        foreach ($eventTypes as $eventKey => $eventType) {
            ...
            foreach ($results as &$object) {
                ...
                $eventTitle           = $this->translator->trans('milex.page.event.'.$eventKey, ['%page%' => $object['title']]);
                ...
                $object['url']        = $this->router->generate('milex_calendar_action', [
                    'objectAction' => 'edit',
                    'source'       => 'page',
                    'objectId'     => $object['page_id'],
                    'startDate'    => $date->toLocalString(),
                ], true);
                $object['viewUrl'] = $this->router->generate('milex_page_action', ['objectAction' => 'view', 'objectId' => $object['page_id']], true);
                $object['attr']    = [
                    'data-toggle' => 'ajaxmodal',
                    'data-target' => '#CalendarEditModal',
                    'data-header' => $eventTitle,
                ];
                $object['description'] = $this->translator->trans('milex.page.event.'.$eventKey.'.description', ['%page%' => $object['title']]);
    ...
    public function onCalendarEventGenerate(EventGeneratorEvent $event)
    {
        ...
        $event->setContentTemplate('MilexPageBundle:SubscribedEvents\Calendar:modal.html.php');
