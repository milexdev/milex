//UserBundle
Milex.userOnLoad = function (container) {
    if (mQuery(container + ' form[name="user"]').length) {
        if (mQuery('#user_position').length) {
            Milex.activateTypeahead('#user_position', {displayKey: 'position'});
        }
    } else {
        if (mQuery(container + ' #list-search').length) {
            Milex.activateSearchAutocomplete('list-search', 'user.user');
        }
    }
};

Milex.roleOnLoad = function (container, response) {
    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'user.role');
    }

    if (response && response.permissionList) {
        MilexVars.permissionList = response.permissionList;
    }
};
 ...
 */
Milex.togglePermissionVisibility = function () {
 ...
 * @param bundle
 */
Milex.onPermissionChange = function (changedPermission, bundle) {
    var granted = 0;
    ...
    if (mQuery('.' + bundle + '_granted').length) {
        var granted = 0;
        var levelPerms = MilexVars.permissionList[bundle];
