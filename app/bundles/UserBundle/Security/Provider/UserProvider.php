...
namespace Milex\UserBundle\Security\Provider;

use Milex\CoreBundle\Helper\EncryptionHelper;
use Milex\UserBundle\Entity\PermissionRepository;
use Milex\UserBundle\Entity\User;
use Milex\UserBundle\Entity\UserRepository;
...
class UserProvider implements UserProviderInterface
{
    ...
    public function loadUserByUsername($username)
    {
        ...
        if (empty($user)) {
            $message = sprintf(
                'Unable to find an active admin MilexUserBundle:User object identified by "%s".',
                $username
            );
    ...
    public function saveUser(User $user, $createIfNotExists = true)
    {
        ...
        if (!$user->getRole()) {
            throw new AuthenticationException('milex.integration.sso.error.no_role');
        }

        if (!$user->getUsername()) {
            throw new AuthenticationException('milex.integration.sso.error.no_username');
        }

        if (!$user->getEmail()) {
            throw new AuthenticationException('milex.integration.sso.error.no_email');
        }

        if (!$user->getFirstName() || !$user->getLastName()) {
            throw new AuthenticationException('milex.integration.sso.error.no_name');
        }
