...
$view['slots']->set('milexContent', 'role');

$view['assets']->addScriptDeclaration('MilexVars.permissionList = '.json_encode($permissionsConfig['list']), 'bodyClose');

$objectId = $form->vars['data']->getId();
if (!empty($objectId)) {
    $name   = $form->vars['data']->getName();
    $header = $view['translator']->trans('milex.user.role.header.edit', ['%name%' => $name]);
} else {
    $header = $view['translator']->trans('milex.user.role.header.new');
}
...
<div class="box-layout">
	<div class="col-xs-12 bg-white height-auto">
		...
		<ul class="bg-auto nav nav-tabs pr-md pl-md">
			...
			<li class=""><a href="#permissions-container" role="tab" data-toggle="tab"><?php echo $view['translator']->trans('milex.user.role.permissions'); ?></a></li>
		</ul>
		<!--/ tabs controls -->
    ...
    echo ' class="hide"';
				...
				<div id="isAdminMessage"<?php if (!$hidePerms) {
					...
					<div class="alert alert-warning">
						<h4><?php echo $view['translator']->trans('milex.user.role.permission.isadmin.header'); ?></h4>
						<p><?php echo $view['translator']->trans('milex.user.role.permission.isadmin.message'); ?></p>
					</div>
				</div>
			</div>
		</div>
