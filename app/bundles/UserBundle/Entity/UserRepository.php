...
class UserRepository extends CommonRepository
{
    ...
    public function setLastLogin($user)
    {
        ...
        $conn->update(MILEX_TABLE_PREFIX.'users', [
            'last_login'  => $datetime,
    ...
    public function setLastActive($user)
    {
        $now  = new DateTimeHelper();
        $conn = $this->_em->getConnection();
        $conn->update(MILEX_TABLE_PREFIX.'users', ['last_active' => $now->toUtcString()], ['id' => (int) $user->getId()]);
    }
    ...
    protected function addSearchCommandWhereClause($q, $filter)
    {
        ...
        switch ($command) {
            ...
            case $this->translator->trans('milex.core.searchcommand.isunpublished'):
                ...
                break;
            case $this->translator->trans('milex.user.user.searchcommand.isadmin'):
                case $this->translator->trans('milex.user.user.searchcommand.isadmin', [], null, 'en_US'):
            ...
            case $this->translator->trans('milex.core.searchcommand.email'):
                ...
                break;
            case $this->translator->trans('milex.user.user.searchcommand.position'):
                case $this->translator->trans('milex.user.user.searchcommand.position', [], null, 'en_US'):
                $expr            = $q->expr()->like('u.position', ':'.$unique);
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.user.user.searchcommand.username'):
                case $this->translator->trans('milex.user.user.searchcommand.username', [], null, 'en_US'):
                $expr            = $q->expr()->like('u.username', ':'.$unique);
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.user.user.searchcommand.role'):
                case $this->translator->trans('milex.user.user.searchcommand.role', [], null, 'en_US'):
    ...
    public function getSearchCommands()
    {
        $commands = [
            ...
            'milex.user.user.searchcommand.isadmin',
            'milex.core.searchcommand.name',
            'milex.user.user.searchcommand.position',
            'milex.user.user.searchcommand.role',
            'milex.user.user.searchcommand.username',
        ];
