...
use Milex\UserBundle\Event\RoleEvent;
use Milex\UserBundle\Form\Type\RoleType;
...
class RoleModel extends FormModel
{
    ...
    {
        return $this->em->getRepository('MilexUserBundle:Role');
    }
    ...
    public function saveEntity($entity, $unlock = true)
    {
        ...
        if (!$isNew) {
            //delete all existing
            $this->em->getRepository('MilexUserBundle:Permission')->purgeRolePermissions($entity);
        }
    ...
    public function deleteEntity($entity)
    {
        ...
        $users = $this->em->getRepository('MilexUserBundle:User')->findByRole($entity);
        if (count($users)) {
            throw new PreconditionRequiredHttpException($this->translator->trans('milex.user.role.error.deletenotallowed', ['%name%' => $entity->getName()], 'flashes'));
        }
