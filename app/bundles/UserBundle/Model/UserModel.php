...
namespace Milex\UserBundle\Model;
...
use Milex\UserBundle\Enum\UserTokenAuthorizator;
use Milex\UserBundle\Event\UserEvent;
use Milex\UserBundle\Form\Type\UserType;
use Milex\UserBundle\Model\UserToken\UserTokenServiceInterface;
...
class UserModel extends FormModel
{
    ...
    public function checkNewPassword(User $entity, UserPasswordEncoder $encoder, $submittedPassword, $validate = false)
    {
        if ($validate) {
            if (strlen($submittedPassword) < 6) {
                throw new \InvalidArgumentException($this->translator->trans('milex.user.user.password.minlength', 'validators'));
            }
        }
    ...
    public function getEntity($id = null)
    {
        ...
        if ($entity) {
            ...
            $entity->setActivePermissions(
                $this->em->getRepository('MilexUserBundle:Permission')->getPermissionsByRole($entity->getRole())
            );
        }
    ...
    {
        $adminRole = $this->em->getRepository('MilexUserBundle:Role')->findOneBy(['isAdmin' => true]);
    ...
    public function sendResetEmail(User $user)
    {
        ...
        $resetLink  = $this->router->generate('milex_user_passwordresetconfirm', ['token' => $resetToken->getSecret()], UrlGeneratorInterface::ABSOLUTE_URL);

        $mailer->setTo([$user->getEmail() => $user->getName()]);
        $mailer->setSubject($this->translator->trans('milex.user.user.passwordreset.subject'));
        $text = $this->translator->trans(
            'milex.user.user.passwordreset.email.body',
        ...
        $this->emailUser(
            $user,
            $this->translator->trans('milex.user.user.passwordreset.subject'),
            $html
        );
    }
