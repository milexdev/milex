...
use Milex\UserBundle\Form\Type\ContactType;
...
class UserController extends FormController
{
    ...
    {
        if (!$this->get('milex.security')->isGranted('user:users:view')) {
            return $this->accessDenied();
        }

        /** @var PageHelperFactoryInterface $pageHelperFacotry */
        $pageHelperFacotry = $this->get('milex.page.helper.factory');
        $pageHelper        = $pageHelperFacotry->make('milex.user', $page);

        $this->setListFilters();

        $limit      = $pageHelper->getLimit();
        $start      = $pageHelper->getStart();
        $orderBy    = $this->get('session')->get('milex.user.orderby', 'u.lastName, u.firstName, u.username');
        $orderByDir = $this->get('session')->get('milex.user.orderbydir', 'ASC');
        $search     = $this->request->get('search', $this->get('session')->get('milex.user.filter', ''));
        $search     = html_entity_decode($search);
        $this->get('session')->set('milex.user.filter', $search);
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('milex_user_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect([
                ...
                'contentTemplate' => 'MilexUserBundle:User:index',
                'passthroughVars' => [
                    'activeLink'    => '#milex_user_index',
                    'milexContent' => 'user',
                ],
            ]);
        }
        ...
        return $this->delegateView([
            'viewParameters'  => [
                ...
                'tmpl'        => $tmpl,
                'permissions' => [
                    'create' => $this->get('milex.security')->isGranted('user:users:create'),
                    'edit'   => $this->get('milex.security')->isGranted('user:users:editother'),
                    'delete' => $this->get('milex.security')->isGranted('user:users:deleteother'),
                ],
            ],
            'contentTemplate' => 'MilexUserBundle:User:list.html.php',
            'passthroughVars' => [
                'route'         => $this->generateUrl('milex_user_index', ['page' => $page]),
                'milexContent' => 'user',
            ],
        ]);
    }
    ...
    {
        if (!$this->get('milex.security')->isGranted('user:users:create')) {
            return $this->accessDenied();
        }
        ...
        $returnUrl = $this->generateUrl('milex_user_index');

        //set the page we came from
        $page = $this->get('session')->get('milex.user.page', 1);

        //get the user form factory
        $action = $this->generateUrl('milex_user_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('milex.core.notice.created', [
                        '%name%'      => $user->getName(),
                        '%menu_link%' => 'milex_user_index',
                        '%url%'       => $this->generateUrl('milex_user_action', [
                            'objectAction' => 'edit',
                            'objectId'     => $user->getId(),
                        ]),
                    ]);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexUserBundle:User:form.html.php',
            'passthroughVars' => [
                'activeLink'    => '#milex_user_new',
                'route'         => $action,
                'milexContent' => 'user',
            ],
        ]);
    ...
    {
        if (!$this->get('milex.security')->isGranted('user:users:edit')) {
            return $this->accessDenied();
        }
        $model = $this->getModel('user.user');
        ...
        //set the return URL
        $returnUrl = $this->generateUrl('milex_user_index', ['page' => $page]);
            ...
            return $this->postActionRedirect(
                array_merge($postActionVars, [
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'milex.user.user.error.notfound',
                            'msgVars' => ['%id%' => $objectId],
                        ],
                    ],
                ])
        ...
        $action = $this->generateUrl('milex_user_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($user, $this->get('form.factory'), $action);
    ...
    {
        if (!$this->get('milex.security')->isGranted('user:users:delete')) {
            return $this->accessDenied();
        }
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if ((int) $currentUser->getId() !== (int) $objectId) {
                ...
                $flashes[] = [
                    'type' => 'error',
                    'msg'  => 'milex.user.user.error.cannotdeleteself',
                ];
            }
    ...
    public function contactAction($objectId)
    {
        ...
        if (null === $user) {
            return $this->postActionRedirect([
                'returnUrl'       => $this->generateUrl('milex_dashboard_index'),
                'contentTemplate' => 'MilexUserBundle:User:contact',
                'flashes'         => [
                    [
                        'type'    => 'error',
                        'msg'     => 'milex.user.user.error.notfound',
        ...
        $action = $this->generateUrl('milex_user_action', ['objectAction' => 'contact', 'objectId' => $objectId]);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            $returnUrl = $formUrl ? urldecode($formUrl) : $this->generateUrl('milex_dashboard_index');
            $valid     = false;
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('milex.user.user.notice.messagesent', ['%name%' => $user->getName()]);
                }
            }
            if ($cancelled || $valid) {
            ...
            $returnUrl  = InputHelper::clean($this->request->get('returnUrl', $this->generateUrl('milex_dashboard_index')));
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexUserBundle:User:contact.html.php',
            'passthroughVars' => [
                'route'         => $action,
                'milexContent' => 'user',
            ],
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            foreach ($ids as $objectId) {
                ...
                } elseif (!$this->get('milex.security')->isGranted('user:users:delete')) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'milex.user.user.notice.batch_deleted',
                    'msgVars' => [
