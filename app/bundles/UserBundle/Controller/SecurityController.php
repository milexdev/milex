...
class SecurityController extends CommonController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        if ($authChecker->isGranted('IS_AUTHENTICATED_FULLY') ||
            ...
            $redirectUrl = $this->generateUrl('milex_dashboard_index');
    ...
    public function loginAction()
    {
        ...
        if ($this->request->cookies->has('milex_update')) {
            $step = $this->request->cookies->get('milex_update');
            if ('clearCache' == $step) {
                // Run migrations
                $this->request->query->set('finalize', 1);

                return $this->forward('MilexCoreBundle:Ajax:updateDatabaseMigration',
                    [
                ...
                // Done so finalize
                return $this->forward('MilexCoreBundle:Ajax:updateFinalization',
                    [
                        'request' => $this->request,
                    ]
                );
        ...
        if (!empty($error)) {
            if (($error instanceof Exception\BadCredentialsException)) {
                ...
                $msg = 'milex.user.auth.error.disabledaccount';
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexUserBundle:Security:login.html.php',
            'passthroughVars' => [
