...
trait FieldsTypeTrait
{
    ...
    protected function buildFormFields(
        ...
        $builder->addEventListener(
            ...
            function (FormEvent $event) use ($options, $integrationFields, $milexFields, $fieldObject, $limit, $start) {
                $form = $event->getForm();
                $index = 0;
                $choices = [];
                $requiredFields = [];
                ...
                foreach ($milexFields as $key => $value) {
                    if (is_array($milexFields)) {
                        $milexFields[$key] = array_flip($value);
                    }
                }
                ...
                foreach ($paginatedFields as $field => $details) {
                    ...
                    if (isset($options['enable_data_priority']) and $options['enable_data_priority']) {
                        ...
                        $disabled = (isset($fieldData[$fieldsName][$field])) ? $options['integration_object']->isCompoundMilexField($fieldData[$fieldsName][$field]) : false;
                        $data = isset($fieldData[$updateName][$field]) ? (int) $fieldData[$updateName][$field] : 1;

                        // Force to use just one way for certainly fields
                        if (isset($fields[$field]['update_milex'])) {
                            $data = (bool) $fields[$field]['update_milex'];
                            $disabled = true;
                            $forceDirection = true;
                        }
                        ...
                        $form->add(
                            ...
                            [
                                ...
                                'attr'              => [
                                    'data-toggle'   => 'tooltip',
                                    'title'         => 'milex.plugin.direction.data.update',
                                    'disabled'      => $disabled,
                    ...
                    if (!$fieldObject) {
                        $milexFields['milex.lead.report.contact_id'] = 'milexContactId';
                        $milexFields['milex.plugin.integration.contact.timeline.link'] = 'milexContactTimelineLink';
                        $milexFields['milex.plugin.integration.contact.donotcontact.email'] = 'milexContactIsContactableByEmail';
                    }
    ...
    {
        $resolver->setRequired(['integration_fields', 'milex_fields', 'integration', 'integration_object', 'page']);
        $resolver->setDefined([('lead' === $object) ? 'update_milex' : 'update_milex_company']);
