...
use Milex\UserBundle\Form\Type\RoleListType;
...
abstract class AbstractSsoServiceIntegration extends AbstractIntegration
{
    ...
    {
        return $this->router->generate('milex_sso_login_check',
    ...
    public function extractAuthKeys($data, $tokenOverride = null)
    {
        ...
        if (empty($error)) {
            $error = $this->translator->trans('milex.integration.error.genericerror', [], 'flashes');
        }
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        if ('features' == $formArea) {
            $builder->add('auto_create_user',
                ...
                [
                    'label' => 'milex.integration.sso.auto_create_user',
                    'data'  => (isset($data['auto_create_user'])) ? (bool) $data['auto_create_user'] : false,
                    'attr'  => [
                        'tooltip' => 'milex.integration.sso.auto_create_user.tooltip',
                    ],
                ]
            );
            ...
            $builder->add(
                ...
                RoleListType::class,
                [
                    'label'      => 'milex.integration.sso.new_user_role',
                    'label_attr' => ['class' => 'control-label'],
                    'attr'       => [
                        'class'   => 'form-control',
                        'tooltip' => 'milex.integration.sso.new_user_role.tooltip',
                    ],
                ]
            );
        }
