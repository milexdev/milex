...
namespace Milex\PluginBundle\Model;
...
class IntegrationEntityModel extends FormModel
{
    ...
    public function getMilexContactsById($milexContactIds, $integrationName, $internalObject)
    {
        if (!$formattedRecords = $this->formatListOfContacts($milexContactIds)) {
            return [];
        }
        $integrationEntityRepo = $this->getIntegrationEntityRepository();
