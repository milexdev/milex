...
class IntegrationHelper
{
    ...
    public function getIntegrationObjects($specificIntegrations = null, $withFeatures = null, $alphabetical = false, $pluginFilter = null, $publishedOnly = false)
    {
        ...
        if (empty($this->available)) {
            ...
            $integrationRepo = $this->em->getRepository('MilexPluginBundle:Integration');
            ...
            foreach ($plugins as $plugin) {
                ...
                if (is_dir($plugin['directory'].'/Integration')) {
                    ...
                    $this->byPlugin[$id] = [];
                    $pluginReference     = $this->em->getReference('MilexPluginBundle:Plugin', $id);
                    $pluginNamespace     = str_replace('MilexPlugin', '', $plugin['bundle']);
                    ...
                    foreach ($finder as $file) {
                        ...
                        if (!isset($integrationSettings[$integrationName])) {
                            ...
                            $integrationContainerKey               = strtolower("milex.integration.{$integrationName}");
                        ...
                        /** @var \Milex\PluginBundle\Entity\Integration $settings */
            ...
            foreach ($this->bundleHelper->getMilexBundles() as $coreBundle) {
                if (
                    // Skip plugin bundles
                    ...
                    $coreBundleNamespace = str_replace('Milex', '', $coreBundle['bundle']);
    ...
    {
        return $this->em->getRepository('MilexPluginBundle:Integration')->getIntegrations();
    }

    public function getCoreIntegrationSettings()
    {
        return $this->em->getRepository('MilexPluginBundle:Integration')->getCoreIntegrations();
    }
    ...
    public function getUserProfiles($lead, $fields = [], $refresh = false, $specificIntegration = null, $persistLead = true, $returnSettings = false)
    {
        ...
        if ($refresh) {
            ...
            /* @var \MilexPlugin\MilexSocialBundle\Integration\SocialIntegration $sn */
    ...
    public function getShareButtons()
    {
        ...
        if (empty($shareBtns)) {
            ...
            /**
             * @var string
             * @var \Milex\PluginBundle\Integration\AbstractIntegration $details
             */
