...
use Milex\PluginBundle\Form\Type\CompanyFieldsType;
use Milex\PluginBundle\Form\Type\FieldsType;
use Milex\PluginBundle\Form\Type\IntegrationCampaignsType;
use Milex\PluginBundle\Form\Type\IntegrationConfigType;
...
class AjaxController extends CommonAjaxController
{
    ...
    protected function getIntegrationFieldsAction(Request $request)
    {
        ...
        if (!empty($integration) && !empty($settings)) {
            ...
            $helper = $this->get('milex.helper.integration');
            ...
            if ($integrationObject) {
                ...
                if (!empty($integrationFields)) {
                    $session = $this->get('session');
                    $session->set('milex.plugin.'.$integration.'.'.$object.'.page', $page);

                    /** @var PluginModel $pluginModel */
                    $pluginModel = $this->getModel('plugin');

                    // Get a list of custom form fields
                    $milexFields       = ($isLead) ? $pluginModel->getLeadFields() : $pluginModel->getCompanyFields();
                    ...
                    $form               = $this->createForm(
                        ...
                        [
                            ...
                            'page'                 => $page,
                            'limit'                => $this->get('milex.helper.core_parameters')->get('default_pagelimit'),
                        ]
                    );

                    $html = $this->render(
                        'MilexCoreBundle:Helper:blank_form.html.php',
                        [
                            'form' => $this->setFormTheme(
                                $form,
                                'MilexCoreBundle:Helper:blank_form.html.php',
                                'MilexPluginBundle:FormTheme\Integration'
                            ),
                            'function' => 'row',
                        ]
                    )->getContent();
    ...
    protected function getIntegrationConfigAction(Request $request)
    {
        ...
        if (!empty($integration) && !empty($settings)) {
            ...
            /** @var \Milex\PluginBundle\Integration\AbstractIntegration $object */
            ...
            if ($object) {
                ...
                $form = $this->setFormTheme($form, 'MilexCoreBundle:Helper:blank_form.html.php', 'MilexPluginBundle:FormTheme\Integration');

                $html = $this->render('MilexCoreBundle:Helper:blank_form.html.php', [
                    'form'      => $form,
                    'function'  => 'widget',
                    'variables' => [
                        'integration' => $object,
    ...
    protected function matchFieldsAction(Request $request)
    {
        ...
        $milex_field      = $request->request->get('milexField');
        $update_milex     = $request->request->get('updateMilex');
        ...
        $doNotMatchField    = ('-1' === $milex_field || '' === $milex_field);
        if ('lead' == $object) {
            $fields       = 'leadFields';
            $updateFields = 'update_milex';
        } else {
            $fields       = 'companyFields';
            $updateFields = 'update_milex_company';
        }
        $newFeatureSettings = [];
        if ($doNotMatchField) {
            ...
            $newFeatureSettings[$integration_field] = $update_milex;
            ...
            $newFeatureSettings[$integration_field] = $milex_field;
