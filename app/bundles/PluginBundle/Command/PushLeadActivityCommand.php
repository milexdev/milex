...
class PushLeadActivityCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('milex:integration:pushleadactivity')
            ->setAliases(
                [
                    'milex:integration:pushactivity',
                ]
            )
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($integration && $startDate && $endDate) {
            ...
            if (null !== $integrationObject && method_exists($integrationObject, 'pushLeadActivity')) {
                $output->writeln('<info>'.$translator->trans('milex.plugin.command.push.leads.activity', ['%integration%' => $integration]).'</info>');
                ...
                $output->writeln('<comment>'.$translator->trans('milex.plugin.command.push.leads.events_executed', ['%events%' => $processed]).'</comment>'."\n");
            }
        }
