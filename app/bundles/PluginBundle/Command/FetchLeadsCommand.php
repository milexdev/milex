...
class FetchLeadsCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('milex:integration:fetchleads')
            ->setAliases(
                [
                    'milex:integration:synccontacts',
                ]
            )
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $integrationHelper = $container->get('milex.helper.integration');
        ...
        if (!$integrationObject->isAuthorized()) {
            $output->writeln(sprintf('<error>ERROR:</error> <info>'.$translator->trans('milex.plugin.command.notauthorized').'</info>', $integration));

            return 255;
        }

        // Tell audit log to use integration name
        define('MILEX_AUDITLOG_USER', $integration);
        ...
        defined('MILEX_CONSOLE_VERBOSITY') or define('MILEX_CONSOLE_VERBOSITY', $output->getVerbosity());
        ...
        define('MILEX_DATE_MODIFIED_OVERRIDE', time());
        ...
        if (isset($supportedFeatures) && in_array('get_leads', $supportedFeatures)) {
            if (null !== $integrationObject && method_exists($integrationObject, 'getLeads') && isset($config['objects'])) {
                $output->writeln('<info>'.$translator->trans('milex.plugin.command.fetch.leads', ['%integration%' => $integration]).'</info>');
                $output->writeln('<comment>'.$translator->trans('milex.plugin.command.fetch.leads.starting').'</comment>');
                ...
                if (in_array(strtolower($contactObjectName), array_map(function ($i) {
                    ...
                    $output->writeln('');
                    $output->writeln('<comment>'.$translator->trans('milex.plugin.command.fetch.contacts.starting').'</comment>');
                    $contactList = [];
                ...
                if ($processed) {
                    $output->writeln(
                        '<comment>'.$translator->trans('milex.plugin.command.fetch.leads.events_executed', ['%events%' => $processed])
                        .'</comment>'."\n"
                    );
                } else {
                    $output->writeln(
                        '<comment>'.$translator->trans(
                            'milex.plugin.command.fetch.leads.events_executed_breakout',
            ...
            if (null !== $integrationObject && method_exists($integrationObject, 'getCompanies') && isset($config['objects'])
                ...
                $updated = $created = $processed = 0;
                $output->writeln('<info>'.$translator->trans('milex.plugin.command.fetch.companies', ['%integration%' => $integration]).'</info>');
                $output->writeln('<comment>'.$translator->trans('milex.plugin.command.fetch.companies.starting').'</comment>');
                ...
                if ($processed) {
                    $output->writeln(
                        '<comment>'.$translator->trans('milex.plugin.command.fetch.companies.events_executed', ['%events%' => $processed])
                        .'</comment>'."\n"
                    );
                } else {
                    $output->writeln(
                        '<comment>'.$translator->trans(
                            'milex.plugin.command.fetch.companies.events_executed_breakout',
                            ['%updated%' => $updated, '%created%' => $created]
                        )
                        .'</comment>'."\n"
                    );
        ...
        if (isset($supportedFeatures) && in_array('push_leads', $supportedFeatures) && method_exists($integrationObject, 'pushLeads')) {
            $output->writeln('<info>'.$translator->trans('milex.plugin.command.pushing.leads', ['%integration%' => $integration]).'</info>');
            ...
            $output->writeln(
                '<comment>'.$translator->trans(
                    'milex.plugin.command.fetch.pushing.leads.events_executed',
                    [
                        '%updated%' => $updated,
                        '%created%' => $created,
                        '%errored%' => $errored,
            ...
            if (in_array('push_companies', $supportedFeatures) && method_exists($integrationObject, 'pushCompanies')) {
                $output->writeln('<info>'.$translator->trans('milex.plugin.command.pushing.companies', ['%integration%' => $integration]).'</info>');
                ...
                $output->writeln(
                    '<comment>'.$translator->trans(
                        'milex.plugin.command.fetch.pushing.companies.events_executed',
                        [
                            '%updated%' => $updated,
                            '%created%' => $created,
                            '%errored%' => $errored,
