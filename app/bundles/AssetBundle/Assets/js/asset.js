//AssetBundle
Milex.assetOnLoad = function (container) {
    if (typeof milexAssetUploadEndpoint !== 'undefined' && typeof Milex.assetDropzone == 'undefined' && mQuery('div#dropzone').length) {
        Milex.initializeDropzone();
    }
};

Milex.assetOnUnload = function(id) {
    if (id === '#app-content') {
        delete Milex.assetDropzone;
    }
};

Milex.updateRemoteBrowser = function(provider, path) {
...
Milex.selectRemoteFile = function(url) {
    mQuery('#asset_remotePath').val(url);
    mQuery('#RemoteFileModal').modal('hide');
};

Milex.changeAssetStorageLocation = function() {
...
Milex.initializeDropzone = function() {
    var options = {
    ...
    Milex.assetDropzone = new Dropzone("div#dropzone", options);
    var preview = mQuery('.preview div.text-center');

    Milex.assetDropzone.on("sending", function (file, request, formData) {
