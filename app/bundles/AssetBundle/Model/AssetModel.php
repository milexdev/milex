...
use Milex\LeadBundle\Tracker\Factory\DeviceDetectorFactory\DeviceDetectorFactoryInterface;
use Milex\LeadBundle\Tracker\Service\DeviceCreatorService\DeviceCreatorServiceInterface;
...
class AssetModel extends FormModel
{
        ...
        if (empty($systemEntry)) {
            ...
            if (isset($systemEntry['email'])) {
                ...
                if (!$email instanceof Email) {
                    ...
                    $email   = $this->em->getReference('MilexEmailBundle:Email', $emailId);
                }
            ...
            if (isset($systemEntry['tracking_id'])) {
                ...
                $trackingNewlyGenerated = false;
            } elseif ($this->security->isAnonymous() && !defined('IN_MILEX_CONSOLE')) {
    ...
    /**
     * @return \Milex\AssetBundle\Entity\AssetRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexAssetBundle:Asset');
    }

    /**
     * @return \Milex\AssetBundle\Entity\DownloadRepository
     */
    public function getDownloadRepository()
    {
        return $this->em->getRepository('MilexAssetBundle:Download');
    }
    ...
    public function getDownloadsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$canViewOthers) {
            $q->join('t', MILEX_TABLE_PREFIX.'assets', 'a', 'a.id = t.asset_id')
    ...
    public function getUniqueVsRepetitivePieChartData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        if (!$canViewOthers) {
            $allQ->join('t', MILEX_TABLE_PREFIX.'assets', 'a', 'a.id = t.asset_id')
                ->andWhere('a.created_by = :userId')
                ->setParameter('userId', $this->userHelper->getUser()->getId());
            $uniqueQ->join('t', MILEX_TABLE_PREFIX.'assets', 'a', 'a.id = t.asset_id')
                ->andWhere('a.created_by = :userId')
                ->setParameter('userId', $this->userHelper->getUser()->getId());
        }
    ...
    public function getPopularAssets($limit = 10, $dateFrom = null, $dateTo = null, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('COUNT(DISTINCT t.id) AS download_count, a.id, a.title')
            ->from(MILEX_TABLE_PREFIX.'asset_downloads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'assets', 'a', 'a.id = t.asset_id')
    ...
    public function getAssetList($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $options = [])
    {
        ...
        $q->select('t.id, t.title as name, t.date_added, t.date_modified')
            ->from(MILEX_TABLE_PREFIX.'assets', 't')
            ->setMaxResults($limit);
