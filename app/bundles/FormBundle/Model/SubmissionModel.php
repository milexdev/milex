...
use Milex\CoreBundle\Exception\FileUploadException;
...
use Milex\FormBundle\Crate\UploadFileCrate;
...
use Milex\FormBundle\Event\ValidationEvent;
...
use Milex\LeadBundle\Helper\CustomFieldValueHelper;
...
class SubmissionModel extends CommonFormModel
{
    ...
    {
        return $this->em->getRepository('MilexFormBundle:Submission');
    }
    ...
    public function saveSubmission($post, $server, Form $form, Request $request, $returnEvent = false)
    {
        ...
        if (!empty($post['milexpage'])) {
            $page = $this->pageModel->getEntity((int) $post['milexpage']);
            if (null != $page) {
        ...
        //clean the referer by removing milexError and milexMessage
        $referer = InputHelper::url($referer, null, null, ['milexError', 'milexMessage']);
            ...
            if ('' === $value && $f->isRequired()) {
                ...
                if (empty($msg)) {
                    $msg = $this->translator->trans(
                        'milex.form.field.generic.validationfailed',
                        [
                            '%label%' => $f->getLabel(),
                        ],
                        'validators'
            ...
            if ($f->isCustom()) {
                ...
                if (!empty($value)) {
                    if (isset($params['valueFilter'])) {
                        if (is_string($params['valueFilter']) && is_callable(['\Milex\CoreBundle\Helper\InputHelper', $params['valueFilter']])) {
        ...
        try {
            ...
            $msg                                = $this->translator->trans('milex.form.submission.error.file.uploadFailed', [], 'validators');
        ...
        switch ($format) {
            case 'csv':
                $response = new StreamedResponse(
                    function () use ($results, $form, $translator, $viewOnlyFields) {
                        ...
                        $fields = $form->getFields();
                        $header = [
                            $translator->trans('milex.core.id'),
                            $translator->trans('milex.form.result.thead.date'),
                            $translator->trans('milex.core.ipaddress'),
                            $translator->trans('milex.form.result.thead.referrer'),
                        ];
                        foreach ($fields as $f) {
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'MilexFormBundle:Result:export.html.php',
                    [
                        'form'           => $form,
                        'results'        => $results,
                        'pageTitle'      => $name,
    ...
    public function exportResultsForPage($format, $page, $queryArgs)
    {
        ...
        switch ($format) {
            case 'csv':
                $response = new StreamedResponse(
                    function () use ($results, $translator) {
                        ...
                        $header = [
                            $translator->trans('milex.core.id'),
                            $translator->trans('milex.lead.report.contact_id'),
                            $translator->trans('milex.form.report.form_id'),
                            $translator->trans('milex.form.result.thead.date'),
                            $translator->trans('milex.core.ipaddress'),
                            $translator->trans('milex.form.result.thead.referrer'),
                        ];
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'MilexPageBundle:Result:export.html.php',
                    [
                        'page'      => $page,
                        'results'   => $results,
                        'pageTitle' => $name,
    ...
    public function getSubmissionsLineChartData(
        ...
        if (!$canViewOthers) {
            $q->join('t', MILEX_TABLE_PREFIX.'forms', 'f', 'f.id = t.form_id')
                ->andWhere('f.created_by = :userId')
                ->setParameter('userId', $this->userHelper->getUser()->getId());
        }

        $data = $query->loadAndBuildTimeData($q);
        $chart->setDataset($this->translator->trans('milex.form.submission.count'), $data);
    ...
    public function getTopSubmissionReferrers($limit = 10, $dateFrom = null, $dateTo = null, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('COUNT(DISTINCT t.id) AS submissions, t.referer')
            ->from(MILEX_TABLE_PREFIX.'form_submissions', 't')
    ...
    protected function createLeadFromSubmit(Form $form, array $leadFieldMatches, $leadFields)
    {
        ...
        $leads = (!empty($uniqueFieldsWithData)) ? $this->em->getRepository('MilexLeadBundle:Lead')->getLeadsByUniqueFields(
        ...
        if (count($leads)) {
            ...
            /** @var \Milex\LeadBundle\Entity\Lead $foundLead */
            $foundLead = $leads[0];
