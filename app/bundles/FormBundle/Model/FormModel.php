...
use Milex\FormBundle\Event\FormEvent;
...
use Milex\LeadBundle\Helper\FormFieldHelper as ContactFieldHelper;
...
class FormModel extends CommonFormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\FormBundle\Entity\FormRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexFormBundle:Form');
    }
    ...
    public function deleteActions(Form $entity, $actions)
    {
        ...
        foreach ($actions as $actionId) {
            if (isset($existingActions[$actionId])) {
                $actionEntity = $this->em->getReference('MilexFormBundle:Action', (int) $actionId);
    ...
    public function generateHtml(Form $entity, $persist = true)
    {
        ...
        if ($entity->getRenderStyle()) {
            ...
            $styleTheme = $theme.'MilexFormBundle:Builder:style.html.php';
        ...
        $html               = $this->templatingHelper->getTemplating()->render(
            $theme.'MilexFormBundle:Builder:form.html.php',
            [
    ...
    public function getFormScript(Form $form)
    {
        ...
        $script = $this->templatingHelper->getTemplating()->render(
            $theme.'MilexFormBundle:Builder:script.html.php',
            [
                'form'  => $form,
                'theme' => $theme,
            ]
    ...
    public function getFormList($limit = 10, \DateTime $dateFrom = null, \DateTime $dateTo = null, $filters = [], $options = [])
    {
        ...
        $q->select('t.id, t.name, t.date_added, t.date_modified')
            ->from(MILEX_TABLE_PREFIX.'forms', 't')
            ->setMaxResults($limit);
