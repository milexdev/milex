    ...
    public function onWidgetDetailGenerate(WidgetDetailEvent $event)
    {
        ...
        if ('top.submission.referrers' == $event->getType()) {
            if (!$event->isCached()) {
                ...
                $event->setTemplateData([
                    'headItems' => [
                        'milex.form.result.thead.referrer',
                        'milex.form.graph.line.submissions',
                    ],
                    'bodyItems' => $items,
                    'raw'       => $referrers,
                ]);
            ...
            if (!$event->isCached()) {
                ...
                if ($submitters) {
                    foreach ($submitters as &$submitter) {
                        ...
                        $leadUrl = $this->router->generate('milex_contact_action', ['objectAction' => 'view', 'objectId' => $submitter['lead_id']]);
            ...
            if (!$event->isCached()) {
                ...
                if ($forms) {
                    foreach ($forms as &$form) {
                        $formUrl = $this->router->generate('milex_form_action', ['objectAction' => 'view', 'objectId' => $form['id']]);
                        $row     = [
                            [
                                'value' => $form['name'],
                                'type'  => 'link',
