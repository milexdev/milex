...
class PublicController extends CommonFormController
{
    ...
    public function submitAction()
    {
        ...
        $form          = null;
        $post          = $this->request->request->get('milexform');
        ...
        if (!empty($return)) {
            //remove milexError and milexMessage from the referer so it doesn't get sent back
            $return = InputHelper::url($return, null, null, null, ['milexError', 'milexMessage'], true);
        ...
        if (!isset($post['formId'])) {
            $error = $translator->trans('milex.form.submit.error.unavailable', [], 'flashes');
        } else {
            ...
            if (null === $form) {
                ...
                $dateTemplateHelper = $this->get('milex.helper.template.date');
                if ('pending' == $status) {
                    $error = $translator->trans(
                        'milex.form.submit.error.pending',
                        [
                    ...
                    $error = $translator->trans(
                        'milex.form.submit.error.expired',
                        [
                    ...
                    if (!empty($result['errors'])) {
                        if ($messengerMode || $isAjax) {
                            ...
                            $error = ($result['errors']) ?
                                $this->get('translator')->trans('milex.form.submission.errors').'<br /><ol><li>'.
        ...
        if ($messengerMode || $isAjax) {
            ...
            if ($isAjax) {
                ...
                return $this->render('MilexFormBundle::messenger.html.php', ['response' => $response]);
            }
        } else {
            if (!empty($error)) {
                if ($return) {
                    ...
                    return $this->redirect($return.$query.'milexError='.rawurlencode($error).$hash);
                } else {
                    $msg     = $error;
                    $msgType = 'error';
                }
                ...
                if (!empty($return)) {
                    if (!empty($postActionProperty)) {
                        $return .= $query.'milexMessage='.rawurlencode($postActionProperty);
                    }

                    return $this->redirect($return);
                } else {
                    $msg = $this->get('translator')->trans('milex.form.submission.thankyou');
                }
            } else {
            ...
            return $this->redirect($this->generateUrl('milex_form_postmessage'));
        }
    }
    ...
    {
        $session = $this->get('session');
        $message = $session->get('milex.emailbundle.message', []);
        ...
        if (null !== $form) {
            ...
            if ('published' === $status) {
                if ($this->request->get('video')) {
                    return $this->render(
                        'MilexFormBundle:Public:videoembed.html.php',
