...
class FieldController extends CommonFormController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->fieldHelper    = $this->get('milex.helper.form.field_helper');
    }
    ...
    public function newAction()
    {
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $fields          = $session->get('milex.form.'.$formId.'.fields.modified', []);
                    ...
                    $session->set('milex.form.'.$formId.'.fields.modified', $fields);

                    // Keep track of used lead fields
                    $usedLeadFields = $this->get('session')->get('milex.form.'.$formId.'.fields.leadfields', []);
                    if (!empty($formData['leadField']) && empty($formData['parent'])) {
                        $usedLeadFields[$keyId] = $formData['leadField'];
                    } else {
                        unset($usedLeadFields[$keyId]);
                    }
                    $session->set('milex.form.'.$formId.'.fields.leadfields', $usedLeadFields);
                } else {
                    $success = 0;
                }
            }
        ...
        if ($cancelled || $valid) {
            ...
            $viewParams['form']        = (isset($customParams['formTheme'])) ? $this->setFormTheme($form, 'MilexFormBundle:Builder:field.html.php', $customParams['formTheme']) : $form->createView();
            $viewParams['fieldHeader'] = (!empty($customParams)) ? $this->get('translator')->trans($customParams['label']) : $this->get('translator')->transConditional('milex.core.type.'.$fieldType, 'milex.form.field.type.'.$fieldType);
        }

        $passthroughVars = [
            'milexContent' => 'formField',
            'success'       => $success,
            'route'         => false,
        ];
        ...
        if (!empty($keyId)) {
            ...
            $template                     = (!empty($customParams)) ? $customParams['template'] : 'MilexFormBundle:Field:'.$fieldType.'.html.php';
        ...
        if ($formField) {
            ...
            if ($cancelled || $valid) {
                ...
                $viewParams['form'] = (isset($customParams['formTheme'])) ? $this->setFormTheme(
                    $form,
                    'MilexFormBundle:Builder:field.html.php',
                ...
                $viewParams['fieldHeader'] = (!empty($customParams))
                    ...
                    : $this->get(
                        'translator'
                    )->transConditional('milex.core.type.'.$fieldType, 'milex.form.field.type.'.$fieldType);
            }
    ...
    public function deleteAction($objectId)
    {
        ...
        $formId  = $this->request->query->get('formId');
        $fields  = $session->get('milex.form.'.$formId.'.fields.modified', []);
        $delete  = $session->get('milex.form.'.$formId.'.fields.deleted', []);
        ...
        if ('POST' === $this->request->getMethod() && null !== $formField) {
            $usedLeadFields = $session->get('milex.form.'.$formId.'.fields.leadfields');
            ...
            if (!in_array($objectId, $delete)) {
                $delete[] = $objectId;
                $session->set('milex.form.'.$formId.'.fields.deleted', $delete);
            }
    ...
    private function getFieldForm($formId, array $formField)
    {
        ...
        $form = $this->getModel('form.field')->createForm(
            ...
            (!empty($formField['id'])) ?
                $this->generateUrl('milex_formfield_action', ['objectAction' => 'edit', 'objectId' => $formField['id']])
                : $this->generateUrl('milex_formfield_action', ['objectAction' => 'new']),
