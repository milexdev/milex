...
namespace Milex\FormBundle\Controller\Api;
...
class FormApiController extends CommonApiController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->entityClass      = 'Milex\FormBundle\Entity\Form';
    ...
    {
        $entity->automaticJs = '<script type="text/javascript" src="'.$this->generateUrl('milex_form_generateform', ['id' => $entity->getId()], true).'"></script>';
    }
            ...
            foreach ($parameters['fields'] as &$fieldParams) {
                ...
                if (is_null($fieldEntity)) {
                    $msg = $this->translator->trans(
                        'milex.core.error.entity.not.found',
                        [
                            '%entity%' => $this->translator->trans('milex.form.field'),
                            '%id%'     => $fieldParams['id'],
                        ],
                        'flashes'
                    );
