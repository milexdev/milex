...
class ResultController extends CommonFormController
{
    public function __construct()
    {
        $this->setStandardParameters(
            ...
            'milex_form', // route base
            'milex.formresult', // session base
            'milex.form.result', // lang string base
            'MilexFormBundle:Result', // template base
            'milex_form', // activeLink
            'formresult' // milexContent
        );
    }
    ...
    public function indexAction($objectId, $page = 1)
    {
        ...
        $formPage       = $session->get('milex.form.page', 1);
        $returnUrl      = $this->generateUrl('milex_form_index', ['page' => $formPage]);
        ...
        $pageHelper        = $pageHelperFacotry->make("milex.formresult.{$objectId}", $page);
        ...
        if (!$session->get('milex.formresult.'.$objectId.'.orderbydir', null)) {
            $session->set('milex.formresult.'.$objectId.'.orderbydir', 'DESC');
        }

        $orderBy    = $session->get('milex.formresult.'.$objectId.'.orderby', 's.date_submitted');
        $orderByDir = $session->get('milex.formresult.'.$objectId.'.orderbydir', 'DESC');
        $filters    = $session->get('milex.formresult.'.$objectId.'.filters', []);
        ...
        if ($this->request->query->has('result')) {
            ...
            $session->set("milex.formresult.$objectId.filters", $filters);
        }
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('milex_form_results', ['objectId' => $objectId, 'page' => $lastPage]);

            return $this->postActionRedirect(
                [
                    'returnUrl'       => $returnUrl,
                    'viewParameters'  => ['page' => $lastPage],
                    'contentTemplate' => 'MilexFormBundle:Result:index',
                    'passthroughVars' => [
                        'activeLink'    => 'milex_form_index',
                        'milexContent' => 'formresult',
                    ],
                ]
            );
        }
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    ...
                    'canDelete'      => $this->get('milex.security')->hasEntityAccess(
                        'form:forms:editown',
                        'form:forms:editother',
                        $form->getCreatedBy()
                    ),
                ],
                'contentTemplate' => 'MilexFormBundle:Result:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => 'milex_form_index',
                    'milexContent' => 'formresult',
    ...
    public function downloadFileAction($submissionId, $field)
    {
        ...
        if (empty($fieldEntity->getProperties()['public']) && !$this->get('milex.security')->hasEntityAccess(
            'form:forms:viewown',
            'form:forms:viewother',
        ...
        $formUploader = $this->get('milex.form.helper.form_uploader');
    ...
    public function deleteAction()
    {
        ...
        $session  = $this->get('session');
        $page     = $session->get("milex.formresult.{$formId}.page", 1);
        $flashes  = [];
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            } elseif (!$this->get('milex.security')->hasEntityAccess('form:forms:editown', 'form:forms:editother', $entity->getCreatedBy())) {
                return $this->accessDenied();
            } else {
                $id = $entity->getId();
        ...
        return $this->postActionRedirect(
            [
                'returnUrl'       => $this->generateUrl('milex_form_results', $viewParameters),
    ...
    {
        return 'milex_form_results_action';
    }
    ...
    protected function generateUrl(string $route, array $parameters = [], int $referenceType = UrlGeneratorInterface::ABSOLUTE_PATH): string
    {
        ...
        switch ($route) {
            case 'milex_form_results_action':
                $parameters['formId'] = $formId;
                break;
            case 'milex_form_results':
