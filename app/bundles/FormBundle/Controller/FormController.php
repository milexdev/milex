...
class FormController extends CommonFormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        $pageHelper        = $pageHelperFacotry->make('milex.form', $page);
        $limit             = $pageHelper->getLimit();
        $start             = $pageHelper->getStart();
        $search            = $this->request->get('search', $session->get('milex.form.filter', ''));
        $filter            = ['string' => $search, 'force' => []];
        $session->set('milex.form.filter', $search);

        if (!$permissions['form:forms:viewother']) {
            $filter['force'][] = ['column' => 'f.createdBy', 'expr' => 'eq', 'value' => $this->user->getId()];
        }

        $orderBy    = $session->get('milex.form.orderby', 'f.dateModified');
        $orderByDir = $session->get('milex.form.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('milex_form_index', ['page' => $lastPage]);

            return $this->postActionRedirect(
                [
                    'returnUrl'       => $returnUrl,
                    'viewParameters'  => ['page' => $lastPage],
                    'contentTemplate' => 'MilexFormBundle:Form:index',
                    'passthroughVars' => [
                        'activeLink'    => '#milex_form_index',
                        'milexContent' => 'form',
                    ],
                ]
            );
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexFormBundle:Form:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_form_index',
                    'milexContent' => 'form',
                    'route'         => $this->generateUrl('milex_form_index', ['page' => $page]),
                ],
            ]
        );
    }
    ...
    {
        /** @var \Milex\FormBundle\Model\FormModel $model */
        $model      = $this->getModel('form');
        $activeForm = $model->getEntity($objectId);

        //set the page we came from
        $page = $this->get('session')->get('milex.form.page', 1);

        if (null === $activeForm) {
            //set the return URL
            $returnUrl = $this->generateUrl('milex_form_index', ['page' => $page]);
        ...
        $action          = $this->generateUrl('milex_form_action', ['objectAction' => 'view', 'objectId' => $objectId]);
        ...
        $activeFormFields = [];
        $fieldHelper      = $this->get('milex.helper.form.field_helper');
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexFormBundle:Form:details.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_form_index',
                    'milexContent' => 'form',
                    'route'         => $action,
    ...
    public function newAction()
    {
        ...
        if (!$this->get('milex.security')->isGranted('form:forms:create')) {
            return $this->accessDenied();
        }

        //set the page we came from
        $page       = $this->get('session')->get('milex.form.page', 1);
        $milexform = $this->request->request->get('milexform', []);
        $sessionId  = $milexform['sessionId'] ?? 'milex_'.sha1(uniqid(mt_rand(), true));

        //set added/updated fields
        $modifiedFields = $session->get('milex.form.'.$sessionId.'.fields.modified', []);
        $deletedFields  = $session->get('milex.form.'.$sessionId.'.fields.deleted', []);

        //set added/updated actions
        $modifiedActions = $session->get('milex.form.'.$sessionId.'.actions.modified', []);
        $deletedActions  = $session->get('milex.form.'.$sessionId.'.actions.deleted', []);

        $action = $this->generateUrl('milex_form_action', ['objectAction' => 'new']);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if (empty($fields)) {
                        ...
                        $form->addError(
                            new FormError(
                                $this->get('translator')->trans('milex.form.form.fields.notempty', [], 'validators')
                            )
                        );
                        $valid = false;
                    } else {
                        ...
                        try {
                            ...
                            if ($form->get('buttons')->get('save')->isClicked()) {
                                ...
                                $returnUrl = $this->generateUrl('milex_form_action', $viewParameters);
                                $template  = 'MilexFormBundle:Form:view';
                            } else {
                            ...
                            $form['name']->addError(
                                new FormError($this->get('translator')->trans('milex.form.schema.failed', [], 'validators'))
                            );
                            $valid = false;
                ...
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('milex_form_index', $viewParameters);
                $template       = 'MilexFormBundle:Form:index';
            }
            ...
            $modifiedFields[$keyId]['label']           = $this->translator->trans('milex.core.form.submit');
            ...
            $session->set('milex.form.'.$sessionId.'.fields.modified', $modifiedFields);
        }
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexFormBundle:Builder:index.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_form_index',
                    'milexContent' => 'form',
    ...
    public function editAction($objectId, $ignorePost = false, $forceTypeSelection = false)
    {
        ...
        $action = $this->generateUrl('milex_form_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($entity, $this->get('form.factory'), $action);
        ...
        if (!$ignorePost && 'POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                //set added/updated fields
                $modifiedFields = $session->get('milex.form.'.$objectId.'.fields.modified', []);
                $deletedFields  = $session->get('milex.form.'.$objectId.'.fields.deleted', []);
                $fields         = array_diff_key($modifiedFields, array_flip($deletedFields));

                //set added/updated actions
                $modifiedActions = $session->get('milex.form.'.$objectId.'.actions.modified', []);
                $deletedActions  = $session->get('milex.form.'.$objectId.'.actions.deleted', []);
                $actions         = array_diff_key($modifiedActions, array_flip($deletedActions));
            ...
            if ($cancelled || ($valid && $form->get('buttons')->get('save')->isClicked())) {
                ...
                $session->remove('milex.formresult.'.$entity->getId().'.orderby');
                $session->remove('milex.formresult.'.$entity->getId().'.orderbydir');
                $session->remove('milex.formresult.'.$entity->getId().'.filters');
                ...
                if ($valid) {
                    ...
                    $action = $this->generateUrl('milex_form_action', ['objectAction' => 'edit', 'objectId' => $entity->getId()]);
                    $form   = $model->createForm($entity, $this->get('form.factory'), $action);
                }
            }
        } else {
        ...
        if ($cleanSlate) {
            ...
            $session->set('milex.form.'.$objectId.'.fields.leadfields', $usedLeadFields);
            ...
            $session->set('milex.form.'.$objectId.'.fields.modified', $modifiedFields);
            $deletedFields = [];
            ...
            $session->set('milex.form.'.$objectId.'.actions.modified', $modifiedActions);
            $deletedActions = [];
        }
    ...
    public function cloneAction($objectId)
    {
        ...
        /** @var \Milex\FormBundle\Entity\Form $entity */
        $entity = $model->getEntity($objectId);
        ...
        if (null != $entity) {
            ...
            /** @var \Milex\FormBundle\Entity\Field $field */
            ...
            /** @var \Milex\FormBundle\Entity\Action $action */
    ...
    public function previewAction($objectId)
    {
        ...
        if (null === $form) {
            $html =
                '<h1>'.
                $this->get('translator')->trans('milex.form.error.notfound', ['%id%' => $objectId], 'flashes').
                '</h1>';
        } elseif (!$this->get('milex.security')->hasEntityAccess(
            'form:forms:editown',
            'form:forms:editother',
            ...
            $html = '<h1>'.$this->get('translator')->trans('milex.core.error.accessdenied', [], 'flashes').'</h1>';
        } else {
        ...
        if (!empty($template)) {
            $theme = $this->get('milex.helper.theme')->getTheme($template);
        ...
        if (!empty($template)) {
            $logicalName     = $this->get('milex.helper.theme')->checkForTwigTemplate(':'.$template.':form.html.php');
            $assetsHelper    = $this->get('templating.helper.assets');
            $slotsHelper     = $this->get('templating.helper.slots');
            $analyticsHelper = $this->get('milex.helper.template.analytics');
        ...
        return $this->render('MilexFormBundle::form.html.php', $viewParams);
    }
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'milex.form.notice.batch_deleted',
                    'msgVars' => [
    ...
    {
        $session = $this->get('session');
        $session->remove('milex.form.'.$sessionId.'.fields.modified');
        $session->remove('milex.form.'.$sessionId.'.fields.deleted');
        $session->remove('milex.form.'.$sessionId.'.fields.leadfields');

        $session->remove('milex.form.'.$sessionId.'.actions.modified');
        $session->remove('milex.form.'.$sessionId.'.actions.deleted');
    }
    ...
    public function batchRebuildHtmlAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            $flashes[] = [
                'type'    => 'notice',
                'msg'     => 'milex.form.notice.batch_html_generated',
                'msgVars' => [
                    '%count%'     => $count,
                ],
            ];
