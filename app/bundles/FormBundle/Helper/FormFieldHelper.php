...
namespace Milex\FormBundle\Helper;

use Milex\CoreBundle\Helper\AbstractFormFieldHelper;
...
class FormFieldHelper extends AbstractFormFieldHelper
{
    ...
    private $types = [
        'captcha' => [
            'constraints' => [
                NotBlank::class => ['message' => 'milex.form.submission.captcha.invalid'],

                EqualTo::class => ['message' => 'milex.form.submission.captcha.invalid'],

                Blank::class => ['message' => 'milex.form.submission.captcha.invalid'],
            ],
        ],
        'checkboxgrp' => [],
        'country'     => [],
        ...
        'email'       => [
            'filter'      => 'email',
            'constraints' => [
                Email::class => ['message' => 'milex.form.submission.email.invalid'],
            ],
        ],
        'freetext' => [],
        'freehtml' => [],
        ...
        'url'       => [
            'filter'      => 'url',
            'constraints' => [
                Url::class => ['message' => 'milex.form.submission.url.invalid'],
            ],
        ],
        'file' => [],
    ];
    ...
    {
        $this->translationKeyPrefix = 'milex.form.field.type.';
    }
    ...
    public function populateField($field, $value, $formName, &$formHtml)
    {
        ...
        switch ($field->getType()) {
            ...
            case 'date':
            case 'datetime':
                if (preg_match('/<input(.*?)id="milexform_input_'.$formName.'_'.$alias.'"(.*?)value="(.*?)"(.*?)\/>/i', $formHtml, $match)) {
                    $replace = '<input'.$match[1].'id="milexform_input_'.$formName.'_'.$alias.'"'.$match[2].'value="'.$this->sanitizeValue($value).'"'
                        .$match[4].'/>';
                    $formHtml = str_replace($match[0], $replace, $formHtml);
                }
                break;
            case 'textarea':
                if (preg_match('/<textarea(.*?)id="milexform_input_'.$formName.'_'.$alias.'"(.*?)>(.*?)<\/textarea>/i', $formHtml, $match)) {
                    $replace  = '<textarea'.$match[1].'id="milexform_input_'.$formName.'_'.$alias.'"'.$match[2].'>'.$this->sanitizeValue($value).'</textarea>';
                    $formHtml = str_replace($match[0], $replace, $formHtml);
                }
                break;
            case 'checkboxgrp':
                ...
                foreach ($value as $val) {
                    ...
                    if (preg_match(
                        '/<input(.*?)id="milexform_checkboxgrp_checkbox_'.$alias.'(.*?)"(.*?)value="'.$val.'"(.*?)\/>/i',
                        $formHtml,
                        $match
                    )) {
                        $replace = '<input'.$match[1].'id="milexform_checkboxgrp_checkbox_'.$alias.$match[2].'"'.$match[3].'value="'.$val.'"'
                            .$match[4].' checked />';
                        $formHtml = str_replace($match[0], $replace, $formHtml);
                    }
                }
            ...
            case 'radiogrp':
                ...
                if (preg_match('/<input(.*?)id="milexform_radiogrp_radio_'.$alias.'(.*?)"(.*?)value="'.$value.'"(.*?)\/>/i', $formHtml, $match)) {
                    $replace = '<input'.$match[1].'id="milexform_radiogrp_radio_'.$alias.$match[2].'"'.$match[3].'value="'.$value.'"'.$match[4]
                        .' checked />';
                    $formHtml = str_replace($match[0], $replace, $formHtml);
                }
                break;
            case 'select':
            case 'country':
                $regex = '/<select\s*id="milexform_input_'.$formName.'_'.$alias.'"(.*?)<\/select>/is';
