...
namespace Milex\WebhookBundle\Model;

use Doctrine\Common\Collections\Criteria;
use JMS\Serializer\SerializationContext;
use JMS\Serializer\SerializerInterface;
use Milex\ApiBundle\Serializer\Exclusion\PublishDetailsExclusionStrategy;
...
use Milex\WebhookBundle\Entity\Event;
use Milex\WebhookBundle\Entity\EventRepository;
use Milex\WebhookBundle\Entity\Log;
use Milex\WebhookBundle\Entity\LogRepository;
use Milex\WebhookBundle\Entity\Webhook;
use Milex\WebhookBundle\Entity\WebhookQueue;
use Milex\WebhookBundle\Entity\WebhookQueueRepository;
use Milex\WebhookBundle\Entity\WebhookRepository;
use Milex\WebhookBundle\Event as Events;
use Milex\WebhookBundle\Event\WebhookEvent;
use Milex\WebhookBundle\Form\Type\WebhookType;
use Milex\WebhookBundle\Http\Client;
...
class WebhookModel extends FormModel
{
    ...
    /**
     * Gets array of custom events from bundles subscribed MilexWehbhookBundle::WEBHOOK_ON_BUILD.
     *
     * @return mixed
     */
    ...
    public function queueWebhooks($webhookEvents, $payload, array $serializationGroups = [])
    {
        ...
        /** @var \Milex\WebhookBundle\Entity\Event $event */
        ...
        try {
            ...
            if ($responseStatusCode >= 300 || $responseStatusCode < 200) {
                ...
                if (410 === $responseStatusCode) {
                    $this->killWebhook($webhook, 'milex.webhook.stopped.reason.410');
                }
            ...
            if ($this->isSick($webhook)) {
                ...
                $message .= ' '.$this->translator->trans('milex.webhook.killed', ['%limit%' => $this->disableLimit]);
            }
     ...
     * @param string $reason
     */
    public function killWebhook(Webhook $webhook, $reason = 'milex.webhook.stopped.reason')
    {
