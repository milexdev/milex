...
namespace Milex\WebhookBundle\Notificator;
...
class WebhookKillNotificator
{
    ...
    {
        $subject = $this->translator->trans('milex.webhook.stopped');
        ...
        /** @var User $owner */
        $owner = $toUser = $this->entityManager->getReference('MilexUserBundle:User', $webhook->getCreatedBy());

        $ccToUser = null;

        if (null !== $webhook->getModifiedBy() && $webhook->getCreatedBy() !== $webhook->getModifiedBy()) {
            $modifiedBy = $this->entityManager->getReference('MilexUserBundle:User', $webhook->getModifiedBy());
