...
class AjaxController extends CommonAjaxController
{
    protected function sendHookTestAction(Request $request)
    {
        ...
        if ('' == $url || !$url) {
            ...
            $dataArray = [
                ...
                'html'    => '<div class="has-error"><span class="help-block">'
                    .$this->translator->trans('milex.webhook.label.no.url')
                    .'</span></div>',
            ];
        ...
        $response = $this->get('milex.webhook.http.client')->post($url, $payloads, InputHelper::string($request->request->get('secret')));

        // default to an error message
        $dataArray = [
            'success' => 1,
            'html'    => '<div class="has-error"><span class="help-block">'
                .$this->translator->trans('milex.webhook.label.warning')
                .'</span></div>',
        ];
        ...
        if (2 == substr($response->getStatusCode(), 0, 1)) {
            $dataArray['html'] =
                ...
                .$this->translator->trans('milex.webhook.label.success')
                .'</span></div>';
        }
    ...
    public function getPayloadPaths($types)
    {
        ...
        foreach ($types as $type) {
            // takes an input like milex.lead_on_something
            ...
            // input is plugin.bundlename or milex.bundlename
