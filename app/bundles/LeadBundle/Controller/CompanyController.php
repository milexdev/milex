...
use Milex\LeadBundle\Form\Type\CompanyMergeType;
...
class CompanyController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        $pageHelper        = $pageHelperFacotry->make('milex.company', $page);

        $limit      = $pageHelper->getLimit();
        $start      = $pageHelper->getStart();
        $search     = $this->request->get('search', $this->get('session')->get('milex.company.filter', ''));
        $filter     = ['string' => $search, 'force' => []];
        $orderBy    = $this->get('session')->get('milex.company.orderby', 'comp.companyname');
        $orderByDir = $this->get('session')->get('milex.company.orderbydir', 'ASC');
        ...
        $this->get('session')->set('milex.company.filter', $search);
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('milex_company_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect(
                [
                    ...
                    'contentTemplate' => 'MilexLeadBundle:Company:index',
                    'passthroughVars' => [
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Company:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_company_index',
                    'milexContent' => 'company',
                    'route'         => $this->generateUrl('milex_company_index', ['page' => $page]),
                ],
            ]
        );
    }
    ...
    public function contactsListAction($objectId, $page = 1)
    {
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Company:list_rows_contacts.html.php',
            ]
        );
    }
     ...
     *
     * @param \Milex\LeadBundle\Entity\Company $entity
     *
     * @return JsonResponse|\Symfony\Component\HttpFoundation\RedirectResponse|Response
     */
    public function newAction($entity = null)
    {
        ...
        if (!($entity instanceof Company)) {
            /** @var \Milex\LeadBundle\Entity\Company $entity */
        ...
        $page         = $this->get('session')->get('milex.company.page', 1);
        $method       = $this->request->getMethod();
        $action       = $this->generateUrl('milex_company_action', ['objectAction' => 'new']);
        ...
        $returnUrl      = $this->generateUrl('milex_company_index', $viewParameters);
        $template       = 'MilexLeadBundle:Company:index';
        ...
        sort($groups);
        $template = 'MilexLeadBundle:Company:form_'.($this->request->get('modal', false) ? 'embedded' : 'standalone').'.html.php';
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        //set the return URL
        $returnUrl = $this->generateUrl('milex_company_index', ['page' => $page]);
                ...
                array_merge(
                    ...
                    [
                        'flashes' => [
                            [
                                'type'    => 'error',
                                'msg'     => 'milex.company.error.notfound',
                                'msgVars' => ['%id%' => $objectId],
                            ],
                        ],
                    ]
        ...
        $action       = $this->generateUrl('milex_company_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $method       = $this->request->getMethod();
        $company      = $this->request->request->get('company', []);
    ...
    public function viewAction($objectId)
    {
        ...
        //set the return URL
        $returnUrl = $this->generateUrl('milex_company_index');
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Company:company.html.php',
            ]
        );
    }
    ...
    public function getCompanyContacts($companyId, $page = 0, $leadsIds = '')
    {
        ...
        //set limits
        $limit = $session->get('milex.company.'.$companyId.'.contacts.limit', $this->get('milex.helper.core_parameters')->get('default_pagelimit'));
        ...
        $orderBy    = $session->get('milex.company.'.$companyId.'.contacts.orderby', 'l.last_active');
        $orderByDir = $session->get('milex.company.'.$companyId.'.contacts.orderbydir', 'DESC');
    ...
    public function deleteAction($objectId)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            } elseif (!$this->get('milex.security')->isGranted('lead:leads:deleteother')) {
                return $this->accessDenied();
            } elseif ($model->isLocked($entity)) {
                return $this->isLocked($postActionVars, $entity, 'lead.company');
            }
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $this->addFlash(
                    'milex.company.notice.batch_deleted',
                    [
                        '%count%'     => $deleted,
                    ]
                );
    ...
    public function mergeAction($objectId)
    {
                ...
                array_merge(
                    ...
                    [
                        'flashes' => [
                            [
                                'type'    => 'error',
                                'msg'     => 'milex.lead.company.error.notfound',
                                'msgVars' => ['%id%' => $objectId],
                            ],
                        ],
                    ]
        ...
        $action = $this->generateUrl('milex_company_action', ['objectAction' => 'merge', 'objectId' => $secondaryCompany->getId()]);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            return $this->postActionRedirect(
                [
                    'returnUrl'       => $this->generateUrl('milex_company_action', $viewParameters),
                    'viewParameters'  => $viewParameters,
                    'contentTemplate' => 'MilexLeadBundle:Company:edit',
                    'passthroughVars' => [
                        'closeModal' => 1,
                    ],
                ]
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Company:merge.html.php',
                'passthroughVars' => [
                    'route'  => false,
