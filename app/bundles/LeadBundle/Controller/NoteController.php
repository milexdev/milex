...
class NoteController extends FormController
{
    ...
    public function indexAction($leadId = 0, $page = 1)
    {
        ...
        //set limits
        $limit = $session->get(
            'milex.lead.'.$lead->getId().'.note.limit',
        ...
        $search = $this->request->get('search', $session->get('milex.lead.'.$lead->getId().'.note.filter', ''));
        $session->set('milex.lead.'.$lead->getId().'.note.filter', $search);

        //do some default filtering
        $orderBy    = $session->get('milex.lead.'.$lead->getId().'.note.orderby', 'n.dateTime');
        $orderByDir = $session->get('milex.lead.'.$lead->getId().'.note.orderbydir', 'DESC');
        ...
        if (empty($noteType) && 'index' == $tmpl) {
            $noteType = $session->get('milex.lead.'.$lead->getId().'.notetype.filter', []);
        }
        $session->set('milex.lead.'.$lead->getId().'.notetype.filter', $noteType);

        $noteTypes = [
            'general' => 'milex.lead.note.type.general',
            'email'   => 'milex.lead.note.type.email',
            'call'    => 'milex.lead.note.type.call',
            'meeting' => 'milex.lead.note.type.meeting',
        ];
        ...
        return $this->delegateView(
            [
                ...
                'passthroughVars' => [
                    'route'         => false,
                    'milexContent' => 'leadNote',
                    'noteCount'     => count($items),
                ],
                'contentTemplate' => 'MilexLeadBundle:Note:list.html.php',
            ]
        );
    }
        ...
        if ($closeModal) {
            ...
            if ($valid && !$cancelled) {
                ...
                $passthroughVars['noteHtml']    = $this->renderView(
                    'MilexLeadBundle:Note:note.html.php',
                    [
                        'note'        => $note,
                        'lead'        => $lead,
            ...
            return $this->delegateView(
                [
                    ...
                    'contentTemplate' => 'MilexLeadBundle:Note:form.html.php',
                ]
            );
        }
    }
    ...
    public function editAction($leadId, $objectId)
    {
        ...
        if (null === $note || !$this->get('milex.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $lead->getPermissionUser())) {
            return $this->accessDenied();
        }
        ...
        if ($closeModal) {
            ...
            $passthroughVars['milexContent'] = 'leadNote';
    ...
    public function deleteAction($leadId, $objectId)
    {
        ...
        if (
            !$this->get('milex.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $lead->getPermissionUser())
            || $model->isLocked($note)
        ) {
            return $this->accessDenied();
        }
