...
use Milex\LeadBundle\Form\Type\LeadImportFieldType;
use Milex\LeadBundle\Form\Type\LeadImportType;
use Milex\LeadBundle\Helper\Progress;
use Milex\LeadBundle\LeadEvents;
use Milex\LeadBundle\Model\ImportModel;
...
class ImportController extends FormController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->logger      = $this->container->get('monolog.logger.milex');
    ...
    public function indexAction($page = 1)
    {
        ...
        $this->session->set('milex.import.object', $initEvent->objectSingular);
    ...
    {
        $object = $this->session->get('milex.import.object');
    ...
    public function cancelAction()
    {
        ...
        $import      = $this->importModel->getEntity($this->session->get('milex.lead.import.id', null));
    ...
    public function newAction($objectId = 0, $ignorePost = false)
    {
        ...
        $this->session->set('milex.import.object', $object);

        // Move the file to cache and rename it
        $forceStop = $this->request->get('cancel', false);
        $step      = ($forceStop) ? self::STEP_UPLOAD_CSV : $this->session->get('milex.'.$object.'.import.step', self::STEP_UPLOAD_CSV);
        ...
        if (!file_exists($fullPath) && self::STEP_UPLOAD_CSV !== $step) {
            ...
            $this->addFlash('milex.import.file.missing', ['%file%' => $this->getImportFileName($object)], FlashBag::LEVEL_ERROR);
            $step = self::STEP_UPLOAD_CSV;
            $this->session->set('milex.'.$object.'.import.step', self::STEP_UPLOAD_CSV);
        }

        $progress = (new Progress())->bindArray($this->session->get('milex.'.$object.'.import.progress', [0, 0]));
        $import   = $this->importModel->getEntity();
        $action   = $this->generateUrl('milex_import_action', ['object' => $this->request->get('object'), 'objectAction' => 'new']);
        ...
        switch ($step) {
                ...
                try {
                    $form = $this->get('form.factory')->create(
                        ...
                        [
                            ...
                            'import_fields'    => $this->session->get('milex.'.$object.'.import.importfields', []),
            ...
            case self::STEP_PROGRESS_BAR:
                ...
                $this->session->set('milex.'.$object.'.import.step', self::STEP_IMPORT_FROM_CSV);
                break;

            case self::STEP_IMPORT_FROM_CSV:
                ignore_user_abort(true);

                $inProgress = $this->session->get('milex.'.$object.'.import.inprogress', false);
                $checks     = $this->session->get('milex.'.$object.'.import.progresschecks', 1);
                if (!$inProgress || $checks > 5) {
                    $this->session->set('milex.'.$object.'.import.inprogress', true);
                    $this->session->set('milex.'.$object.'.import.progresschecks', 1);

                    $import = $this->importModel->getEntity($this->session->get('milex.'.$object.'.import.id', null));
                    ...
                    if ($progress->isFinished()) {
                        ...
                        $complete = false;
                        $this->session->set('milex.'.$object.'.import.inprogress', false);
                        $this->session->set('milex.'.$object.'.import.progress', $progress->toArray());
                    }
                    ...
                    ++$checks;
                    $this->session->set('milex.'.$object.'.import.progresschecks', $checks);
                }
        }
        ...
        if (!$ignorePost && 'POST' == $this->request->getMethod()) {
            ...
            switch ($step) {
                    ...
                    if ($valid) {
                        ...
                        if (!empty($fileData)) {
                            ...
                            try {
                                ...
                                $this->session->set('milex.'.$object.'.import.config', $config);
                                ...
                                if (false !== $file) {
                                    ...
                                    if (!empty($headers) && is_array($headers)) {
                                        ...
                                        $this->session->set('milex.'.$object.'.import.headers', $headers);
                                        $this->session->set('milex.'.$object.'.import.step', self::STEP_MATCH_FIELDS);
                                        $this->session->set('milex.'.$object.'.import.importfields', CsvHelper::convertHeadersIntoFields($headers));
                                        $this->session->set('milex.'.$object.'.import.progress', [0, $linecount]);
                                        $this->session->set('milex.'.$object.'.import.original.file', $fileData->getClientOriginalName());
                                ...
                                if (false !== strpos($e->getMessage(), 'upload_max_filesize')) {
                                    $errorMessage    = 'milex.lead.import.filetoolarge';
                                    $errorParameters = [
                                        '%upload_max_filesize%' => ini_get('upload_max_filesize'),
                                    ];
                                } else {
                                    $errorMessage = 'milex.lead.import.filenotreadable';
                                }
                            } catch (\Exception $e) {
                                $errorMessage = 'milex.lead.import.filenotreadable';
                            } finally {
                ...
                case self::STEP_MATCH_FIELDS:
                    ...
                    /** @var \Milex\LeadBundle\Entity\Import $import */
                    $import = $this->importModel->getEntity();
                    ...
                    $import->setMatchedFields($matchedFields)
                        ...
                        ->setFile($fileName)
                        ->setOriginalFile($this->session->get('milex.'.$object.'.import.original.file'))
                        ...
                        ->setHeaders($this->session->get('milex.'.$object.'.import.headers'))
                        ->setParserConfig($this->session->get('milex.'.$object.'.import.config'));
                    ...
                    if ($this->importInBrowser($form, $object)) {
                        ...
                        $this->session->set('milex.'.$object.'.import.step', self::STEP_PROGRESS_BAR);
                    }

                    $this->importModel->saveEntity($import);

                    $this->session->set('milex.'.$object.'.import.id', $import->getId());

                    // In case the user decided to queue the import
                    if ($this->importInCli($form, $object)) {
                        $this->addFlash('milex.'.$object.'.batch.import.created');
                        $this->resetImport($object);
        ...
        if (self::STEP_UPLOAD_CSV === $step || self::STEP_MATCH_FIELDS === $step) {
            $contentTemplate = 'MilexLeadBundle:Import:new.html.php';
            $viewParameters  = [
                'form'       => $form->createView(),
                'objectName' => $initEvent->objectName,
            ];
        } else {
            $contentTemplate = 'MilexLeadBundle:Import:progress.html.php';
            $viewParameters  = [
                'progress'         => $progress,
                'import'           => $import,
                'complete'         => $complete,
    ...
    {
        $progress = $this->session->get('milex.'.$object.'.import.progress', [0, 0]);
    ...
    {
        return $this->get('milex.helper.core_parameters')->get('background_import_if_more_rows_than', 0);
    }
    ...
    protected function getImportFileName($object)
    {
        ...
        if ($fileName = $this->session->get('milex.'.$object.'.import.file')) {
            return $fileName;
        }
        ...
        $this->session->set('milex.'.$object.'.import.file', $fileName);
    ...
    {
        $this->session->set('milex.'.$object.'.import.headers', []);
        $this->session->set('milex.'.$object.'.import.file', null);
        $this->session->set('milex.'.$object.'.import.step', self::STEP_UPLOAD_CSV);
        $this->session->set('milex.'.$object.'.import.progress', [0, 0]);
        $this->session->set('milex.'.$object.'.import.inprogress', false);
        $this->session->set('milex.'.$object.'.import.importfields', []);
        $this->session->set('milex.'.$object.'.import.original.file', null);
        $this->session->set('milex.'.$object.'.import.id', null);
    }
