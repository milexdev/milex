...
use Milex\LeadBundle\Controller\LeadDetailsTrait;
...
class LeadApiController extends CommonApiController
{
    ...
    {
        if (!$this->get('milex.security')->isGranted(
    ...
    {
        if (!$this->get('milex.security')->isGranted(['lead:leads:editown', 'lead:leads:editother'], 'MATCH_ONE')) {
            return $this->accessDenied();
        }
    ...
    public function getNotesAction($id)
    {
        ...
        if (!$this->get('milex.security')->hasEntityAccess('lead:leads:viewown', 'lead:leads:viewother', $entity->getPermissionUser())) {
            return $this->accessDenied();
        }
    ...
    protected function preSaveEntity(&$entity, $form, $parameters, $action = 'edit')
    {
        ...
        if (isset($this->entityRequestParameters['ipAddress'])) {
            $ipAddress = $this->get('milex.helper.ip_lookup')->getIpAddress($this->entityRequestParameters['ipAddress']);
