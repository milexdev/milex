...
use Milex\CoreBundle\Exception\SchemaException;
...
class FieldController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        $permissions = $this->get('milex.security')->isGranted(['lead:fields:view', 'lead:fields:full'], 'RETURN_ARRAY');
        ...
        $limit  = $session->get('milex.leadfield.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $search = $this->request->get('search', $session->get('milex.leadfield.filter', ''));
        $session->set('milex.leadfilter.filter', $search);

        //do some default filtering
        $orderBy    = $this->get('session')->get('milex.leadfilter.orderby', 'f.order');
        $orderByDir = $this->get('session')->get('milex.leadfilter.orderbydir', 'ASC');
        ...
        $search  = $request->get('search', $session->get('milex.lead.emailtoken.filter', ''));

        $session->set('milex.lead.emailtoken.filter', $search);
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('milex.leadfield.page', $lastPage);
            $returnUrl = $this->generateUrl('milex_contactfield_index', ['page' => $lastPage]);

            return $this->postActionRedirect([
                'returnUrl'       => $returnUrl,
                'viewParameters'  => ['page' => $lastPage],
                'contentTemplate' => 'MilexLeadBundle:Field:index',
                'passthroughVars' => [
                    'activeLink'    => '#milex_contactfield_index',
                    'milexContent' => 'leadfield',
                ],
            ]);
        }

        //set what page currently on so that we can return here after form submission/cancellation
        $session->set('milex.leadfield.page', $page);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexLeadBundle:Field:list.html.php',
            'passthroughVars' => [
                'activeLink'    => '#milex_contactfield_index',
                'route'         => $this->generateUrl('milex_contactfield_index', ['page' => $page]),
                'milexContent' => 'leadfield',
            ],
        ]);
    }
    ...
    {
        if (!$this->get('milex.security')->isGranted('lead:fields:full')) {
            return $this->accessDenied();
        }
        ...
        $returnUrl = $this->generateUrl('milex_contactfield_index');
        $action    = $this->generateUrl('milex_contactfield_action', ['objectAction' => 'new']);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if ($valid) {
                        $flashMessage = 'milex.core.notice.created';
                        try {
                            ...
                            $flashMessage = $this->get('translator')->trans('milex.lead.field.pushed_to_background');
                            ...
                            $form['alias']->addError(
                                    new FormError(
                                        $this->get('translator')->trans('milex.lead.field.failed', ['%error%' => $e->getMessage()], 'validators')
                                    )
                                );
                            $valid = false;
                        }
                        $this->addFlash(
                                ...
                                [
                                    '%name%'      => $field->getLabel(),
                                    '%menu_link%' => 'milex_contactfield_index',
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Field:form.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_contactfield_index',
                    'route'         => $this->generateUrl('milex_contactfield_action', ['objectAction' => 'new']),
                    'milexContent' => 'leadfield',
                ],
            ]
        );
    ...
    public function editAction($objectId, $ignorePost = false)
    {
            ...
            return $this->postActionRedirect(
                array_merge($postActionVars, [
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'milex.lead.field.error.notfound',
                            'msgVars' => ['%id%' => $objectId],
                        ],
                    ],
                ])
        ...
        $action = $this->generateUrl('milex_contactfield_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($field, $this->get('form.factory'), $action);
        ...
        if (!$ignorePost && 'POST' == $this->request->getMethod()) {
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if ($valid) {
                        ...
                        $this->addFlash('milex.core.notice.updated', [
                            '%name%'      => $field->getLabel(),
                            '%menu_link%' => 'milex_contactfield_index',
                            '%url%'       => $this->generateUrl('milex_contactfield_action', [
                                'objectAction' => 'edit',
                                'objectId'     => $field->getId(),
                            ]),
                        ]);
            ...
            if ($cancelled || ($valid && $form->get('buttons')->get('save')->isClicked())) {
                ...
                $action = $this->generateUrl('milex_contactfield_action', ['objectAction' => 'edit', 'objectId' => $field->getId()]);
                $form   = $model->createForm($field, $this->get('form.factory'), $action);
            } else {
                // some bug in Symfony prevents repopulating list options on errors
                $field   = $form->getData();
    ...
    public function cloneAction($objectId)
    {
        ...
        if (null != $entity) {
            ...
            $this->get('milex.helper.field.alias')->makeAliasUnique($clone);
    ...
    public function deleteAction($objectId)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (count($segments)) {
                $flashMessage = [
                    'type'    => 'error',
                    'msg'     => 'milex.core.notice.used.field',
                    'msgVars' => [
                        '%name%'     => $field->getLabel(),
                        '%id%'       => $objectId,
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                if ($filteredDeleteIds !== $deleteIds) {
                    $flashes[] = [
                        'type'    => 'error',
                        'msg'     => 'milex.core.notice.used.fields',
                        'msgVars' => [
                            '%segments%' => implode(', ', $segments),
                ...
                if (count($filteredDeleteIds)) {
                    ...
                    $flashes[] = [
                        'type'    => 'notice',
                        'msg'     => 'milex.lead.field.notice.batch_deleted',
                        'msgVars' => [
