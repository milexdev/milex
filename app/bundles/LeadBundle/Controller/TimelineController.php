...
class TimelineController extends CommonController
{
    ...
    public function indexAction(Request $request, $leadId, $page = 1)
    {
        ...
        if ('POST' == $request->getMethod() && $request->request->has('search')) {
            ...
            $session->set('milex.lead.'.$leadId.'.timeline.filters', $filters);
        } else {
            $filters = null;
        }

        $order = [
            $session->get('milex.lead.'.$leadId.'.timeline.orderby'),
            $session->get('milex.lead.'.$leadId.'.timeline.orderbydir'),
        ];
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexLeadBundle:Timeline:list.html.php',
            ]
        );
    }
    ...
    public function pluginIndexAction(Request $request, $integration, $page = 1)
    {
        ...
        if ('POST' === $request->getMethod() && $request->request->has('search')) {
            ...
            $session->set('milex.plugin.timeline.filters', $filters);
        } else {
            $filters = null;
        }
        ...
        return $this->delegateView(
            [
                ...
                'passthroughVars' => [
                    'route'         => false,
                    'milexContent' => 'pluginTimeline',
                    'timelineCount' => $events['total'],
                ],
                'contentTemplate' => sprintf('MilexLeadBundle:Timeline:plugin_%s.html.php', $tmpl),
            ]
        );
    }
    ...
    public function pluginViewAction(Request $request, $integration, $leadId, $page = 1)
    {
        ...
        if ('POST' === $request->getMethod() && $request->request->has('search')) {
            ...
            $session->set('milex.plugin.timeline.'.$leadId.'.filters', $filters);
        } else {
            $filters = null;
        }

        $order = [
            $session->get('milex.plugin.timeline.'.$leadId.'.orderby'),
            $session->get('milex.plugin.timeline.'.$leadId.'.orderbydir'),
        ];
    ...
    public function batchExportAction(Request $request, $leadId)
    {
        ...
        $resultsCallback = function ($event) {
            ...
            return [
                ...
                'eventTimestamp' => $this->get('milex.helper.template.date')->toText($event['timestamp'], 'local', 'Y-m-d H:i:s', true),
            ];
        };
