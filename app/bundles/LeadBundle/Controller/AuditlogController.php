...
class AuditlogController extends CommonController
{
    ...
    public function indexAction(Request $request, $leadId, $page = 1)
    {
        ...
        if ('POST' == $request->getMethod() && $request->request->has('search')) {
            ...
            $session->set('milex.lead.'.$leadId.'.auditlog.filters', $filters);
        } else {
            $filters = null;
        }

        $order = [
            $session->get('milex.lead.'.$leadId.'.auditlog.orderby'),
            $session->get('milex.lead.'.$leadId.'.auditlog.orderbydir'),
        ];
        ...
        return $this->delegateView(
            [
                ...
                'passthroughVars' => [
                    'route'         => false,
                    'milexContent' => 'leadAuditlog',
                    'auditLogCount' => $events['total'],
                ],
                'contentTemplate' => 'MilexLeadBundle:Auditlog:list.html.php',
            ]
        );
    }
