...
class ForeignFuncFilterQueryBuilder extends BaseFilterQueryBuilder
{
    ...
    {
        return 'milex.lead.query.builder.foreign.func';
    }
        ...
        if (!$tableAlias) {
            ...
            if ($filterAggr) {
                ...
                if ('companies' == $filter->getTable()) {
                    ...
                    $queryBuilder->leftJoin($leadsTableAlias, MILEX_TABLE_PREFIX.'companies_leads', $relTable, $relTable.'.lead_id = '.$leadsTableAlias.'.id');
                    ...
                    $queryBuilder->leftJoin(
                        ...
                        $filter->getTable(),
                        $tableAlias,
                        sprintf('%s.id = %s.lead_id', $queryBuilder->getTableAlias(MILEX_TABLE_PREFIX.'leads'), $tableAlias)
                    );
                }
            }
        }
