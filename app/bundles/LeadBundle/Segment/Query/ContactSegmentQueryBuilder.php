...
namespace Milex\LeadBundle\Segment\Query;
...
use Milex\LeadBundle\Event\LeadListFilteringEvent;
use Milex\LeadBundle\Event\LeadListQueryBuilderGeneratedEvent;
...
use Milex\LeadBundle\Segment\Exception\PluginHandledFilterException;
use Milex\LeadBundle\Segment\Exception\SegmentQueryException;
...
class ContactSegmentQueryBuilder
{
    ...
    public function assembleContactsSegmentQueryBuilder($segmentId, $segmentFilters, bool $changeAlias = false)
    {
        ...
        $queryBuilder->select($leadsTableAlias.'.id')->from(MILEX_TABLE_PREFIX.'leads', $leadsTableAlias);
    ...
    public function addNewContactsRestrictions(QueryBuilder $queryBuilder, $segmentId)
    {
        ...
        $segmentQueryBuilder = $queryBuilder->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', $tableAlias)
    ...
    public function addManuallyUnsubscribedQuery(QueryBuilder $queryBuilder, $leadListId)
    {
        ...
        $queryBuilder->leftJoin(
            $leadsTableAlias,
            MILEX_TABLE_PREFIX.'lead_lists_leads',
            $tableAlias,
    ...
    {
        $segment = $this->entityManager->getRepository('MilexLeadBundle:LeadList')->find($segmentId);
        if (null === $segment) {
            return [];
        }
