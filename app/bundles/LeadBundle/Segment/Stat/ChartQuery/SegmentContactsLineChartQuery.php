...
namespace Milex\LeadBundle\Segment\Stat\ChartQuery;
...
class SegmentContactsLineChartQuery extends ChartQuery
{
    ...
    private function getFirstDateAddedSegmentEventLog()
    {
        ...
        $subQuery->select('el.date_added - INTERVAL 10 SECOND')
            ->from(MILEX_TABLE_PREFIX.'lead_event_log', 'el')
            ->where(
                $subQuery->expr()->andX(
    ...
    private function optimizeListLeadQuery(QueryBuilder $qb): QueryBuilder
    {
        if (
            ...
            false !== ($key = array_search(MILEX_TABLE_PREFIX.ListLead::TABLE_NAME, array_column($qb->getQueryPart('from'), 'table'))) &&
            false !== ($joinKey = array_search(MILEX_TABLE_PREFIX.ListLead::TABLE_NAME, array_column($qb->getQueryPart('join')[$tableAlias = $qb->getQueryPart('from')[$key]['alias']], 'joinTable')))
        ) {
    ...
    private function optimizeSearchInLeadEventLog(QueryBuilder $qb): QueryBuilder
    {
        ...
        $fromPart[0]['alias'] = sprintf('%s USE INDEX (%s)', $fromPart[0]['alias'], MILEX_TABLE_PREFIX.LeadEventLog::INDEX_SEARCH);
