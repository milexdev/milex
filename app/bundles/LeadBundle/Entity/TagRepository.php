...
class TagRepository extends CommonRepository
{
    ...
    public function deleteOrphans()
    {
        ...
        $havingQb->select('count(x.lead_id) as the_count')
            ->from(MILEX_TABLE_PREFIX.'lead_tags_xref', 'x')
            ->where('x.tag_id = t.id');

        $qb->select('t.id')
            ->from(MILEX_TABLE_PREFIX.'lead_tags', 't')
            ->having(sprintf('(%s)', $havingQb->getSQL()).' = 0');
        $delete = $qb->execute()->fetch();

        if (count($delete)) {
            $qb->resetQueryParts();
            $qb->delete(MILEX_TABLE_PREFIX.'lead_tags')
                ->where(
    ...
    public function checkLeadByTags(Lead $lead, $tags)
    {
        ...
        $q->select('l.id')
            ...
            ->join('l', MILEX_TABLE_PREFIX.'lead_tags_xref', 'x', 'l.id = x.lead_id')
            ->join('l', MILEX_TABLE_PREFIX.'lead_tags', 't', 'x.tag_id = t.id')
            ->where(
                $q->expr()->andX(
