...
{
    use OperatorListTrait; // @deprecated to be removed in Milex 3. Not used inside this class.
    ...
    public function checkLeadSegmentsByIds(Lead $lead, $ids)
    {
        ...
        $q->join('l', MILEX_TABLE_PREFIX.'lead_lists_leads', 'x', 'l.id = x.lead_id')
            ->where(
                $q->expr()->andX(
    ...
    public function getLeadCount($listIds)
    {
        ...
        $q->select('count(l.lead_id) as thecount, l.leadlist_id')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'l');

        $expression   = null;
        $countListIds = count($listIds);

        if (1 === $countListIds) {
            $q          = $this->forceUseIndex($q, MILEX_TABLE_PREFIX.'manually_removed');
    ...
    protected function createFilterExpressionSubQuery($table, $alias, $column, $value, array &$parameters, $leadId = null, array $subQueryFilters = [])
    {
        ...
        $subQb->select('null')
            ->from(MILEX_TABLE_PREFIX.$table, $alias)
            ->where($subExpr);
    ...
    protected function addSearchCommandWhereClause($q, $filter)
    {
        ...
        switch ($command) {
            case $this->translator->trans('milex.lead.list.searchcommand.isglobal'):
            case $this->translator->trans('milex.lead.list.searchcommand.isglobal', [], null, 'en_US'):
    ...
    {
        $commands = [
            'milex.lead.list.searchcommand.isglobal',
    ...
    {
        $tableName = MILEX_TABLE_PREFIX.'lead_lists';
