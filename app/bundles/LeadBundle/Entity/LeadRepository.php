...
use Milex\CoreBundle\Helper\SearchStringHelper;
use Milex\LeadBundle\Controller\ListController;
use Milex\LeadBundle\Event\LeadBuildSearchEvent;
...
class LeadRepository extends CommonRepository implements CustomFieldRepositoryInterface
{
    ...
    protected function buildQueryForGetLeadsByFieldValue($field, $value, $ignoreId = null)
    {
        ...
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ->select('l.id')
            ->from(MILEX_TABLE_PREFIX.'leads', 'l');
    ...
    public function getContactIdsByEmails(array $emails): array
    {
        $result = $this->getEntityManager()
            ->createQuery("
                SELECT c.id 
                FROM Milex\LeadBundle\Entity\Lead c
    ...
    public function getEntitiesDbalQueryBuilder()
    {
        ...
        return $this->getEntityManager()->getConnection()->createQueryBuilder()
            ->from(MILEX_TABLE_PREFIX.'leads', $alias)
            ->leftJoin($alias, MILEX_TABLE_PREFIX.'users', 'u', 'u.id = '.$alias.'.owner_id');
    }
    ...
    public function getEntitiesOrmQueryBuilder($order)
    {
        ...
        $q->select($alias.', u, i,'.$order)
            ->from('MilexLeadBundle:Lead', $alias, $alias.'.id')
    ...
    protected function addSearchCommandWhereClause($q, $filter)
    {
        ...
        switch ($command) {
            case $this->translator->trans('milex.lead.lead.searchcommand.isanonymous'):
            case $this->translator->trans('milex.lead.lead.searchcommand.isanonymous', [], null, 'en_US'):
                $expr = $q->expr()->$nullExpr('l.date_identified');
                break;
            case $this->translator->trans('milex.core.searchcommand.ismine'):
            case $this->translator->trans('milex.core.searchcommand.ismine', [], null, 'en_US'):
                $expr = $q->expr()->$eqExpr('l.owner_id', $this->currentUser->getId());
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.isunowned'):
            case $this->translator->trans('milex.lead.lead.searchcommand.isunowned', [], null, 'en_US'):
                $expr = $q->expr()->orX(
                    $q->expr()->$eqExpr('l.owner_id', 0),
                    $q->expr()->$nullExpr('l.owner_id')
                );
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.owner'):
            case $this->translator->trans('milex.lead.lead.searchcommand.owner', [], null, 'en_US'):
                $expr = $q->expr()->orX(
            ...
            case $this->translator->trans('milex.core.searchcommand.name', [], null, 'en_US'):
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.core.searchcommand.email'):
            case $this->translator->trans('milex.core.searchcommand.email', [], null, 'en_US'):
                $expr            = $q->expr()->$likeExpr('l.email', ":$unique");
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.list'):
            case $this->translator->trans('milex.lead.lead.searchcommand.list', [], null, 'en_US'):
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.company_id'):
            case $this->translator->trans('milex.lead.lead.searchcommand.company_id', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                break;
            case $this->translator->trans('milex.core.searchcommand.ip'):
            case $this->translator->trans('milex.core.searchcommand.ip', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.duplicate'):
            case $this->translator->trans('milex.lead.lead.searchcommand.duplicate', [], null, 'en_US'):
                ...
                $sq->select('duplicate.lead_id')
                    ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'duplicate')
                    ->where(
                        $q->expr()->andX(
                ...
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.tag'):
            case $this->translator->trans('milex.lead.lead.searchcommand.tag', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.company'):
            case $this->translator->trans('milex.lead.lead.searchcommand.company', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.stage'):
            case $this->translator->trans('milex.lead.lead.searchcommand.stage', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
    ...
    {
        $commands = [
            'milex.lead.lead.searchcommand.isanonymous',
            'milex.core.searchcommand.ismine',
            'milex.lead.lead.searchcommand.isunowned',
            'milex.lead.lead.searchcommand.list',
            'milex.core.searchcommand.name',
            'milex.lead.lead.searchcommand.company',
            'milex.lead.lead.searchcommand.company_id',
            'milex.core.searchcommand.email',
            'milex.lead.lead.searchcommand.owner',
            'milex.core.searchcommand.ip',
            'milex.lead.lead.searchcommand.tag',
            'milex.lead.lead.searchcommand.stage',
            'milex.lead.lead.searchcommand.duplicate',
            'milex.lead.lead.searchcommand.email_sent',
            'milex.lead.lead.searchcommand.email_read',
            'milex.lead.lead.searchcommand.email_queued',
            'milex.lead.lead.searchcommand.email_pending',
            'milex.lead.lead.searchcommand.page_source',
            'milex.lead.lead.searchcommand.page_source_id',
            'milex.lead.lead.searchcommand.import_id',
            'milex.lead.lead.searchcommand.import_action',
            'milex.lead.lead.searchcommand.page_id',
            'milex.lead.lead.searchcommand.sms_sent',
            'milex.lead.lead.searchcommand.web_sent',
            'milex.lead.lead.searchcommand.mobile_sent',
        ];
    ...
    public function updateLastActive($leadId)
    {
        ...
        $this->getEntityManager()->getConnection()->update(MILEX_TABLE_PREFIX.'leads', $fields, ['id' => $leadId]);
    }
    ...
    public function getLeadOwner($ownerId)
    {
        ...
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'users', 'u')
            ->where('u.id = :ownerId')
    ...
    public function checkLeadOwner(Lead $lead, $ownerIds = [])
    {
        ...
        $q->select('u.id')
            ->from(MILEX_TABLE_PREFIX.'users', 'u')
            ->join('u', MILEX_TABLE_PREFIX.'leads', 'l', 'l.owner_id = u.id')
            ->where(
                $q->expr()->andX(
    ...
    public function getContacts(array $contactIds)
    {
        ...
        $qb->select('l.*')->from(MILEX_TABLE_PREFIX.'leads', 'l')
            ->where(
    ...
    public function applySearchQueryRelationship(QueryBuilder $q, array $tables, $innerJoinTables, $whereExpression = null, $having = null)
    {
        ...
        if (!preg_match('/"'.preg_quote($primaryTable['alias'], '/').'"/i', json_encode($q->getQueryPart('join')))) {
            $q->$joinType(
                ...
                MILEX_TABLE_PREFIX.$primaryTable['table'],
                $primaryTable['alias'],
        ...
        foreach ($tables as $table) {
            ...
            if (!$exists) {
                $q->$joinType(
                    $table['from_alias'],
                    MILEX_TABLE_PREFIX.$table['table'],
                    $table['alias'],
                    $table['condition']
                );
            }
    ...
    {
        $qb = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ->update(MILEX_TABLE_PREFIX.'leads')
            ->where('id = '.$id);
