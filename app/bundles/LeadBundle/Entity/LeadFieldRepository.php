...
class LeadFieldRepository extends CommonRepository
{
    ...
    public function getAliases($exludingId, $publishedOnly = false, $includeEntityFields = true, $object = 'lead')
    {
        $q = $this->_em->getConnection()->createQueryBuilder()
            ->select('l.alias')
            ->from(MILEX_TABLE_PREFIX.'lead_fields', 'l');
        ...
        if ($includeEntityFields) {
            ...
            $leadRepo = $this->_em->getRepository('MilexLeadBundle:Lead')->getBaseColumns('Milex\\LeadBundle\\Entity\\Lead', true);
    ...
    {
        $q->leftJoin('l', MILEX_TABLE_PREFIX.'companies_leads', 'companies_lead', 'l.id = companies_lead.lead_id');
        $q->leftJoin('companies_lead', MILEX_TABLE_PREFIX.'companies', 'company', 'companies_lead.company_id = company.id');
    }
    ...
    public function getPropertyByField($field, $q)
    {
        ...
        if (in_array($field, array_column($this->getFieldAliases('company'), 'alias'))) {
            ...
            $q->join('l', MILEX_TABLE_PREFIX.'lead_utmtags', 'u', 'l.id = u.lead_id');
            $columnAlias = 'u.';
        }
    ...
    public function compareValue($lead, $field, $value, $operatorExpr)
    {
        ...
        if ('tags' === $field) {
            ...
            $q->join('l', MILEX_TABLE_PREFIX.'lead_tags_xref', 'x', 'l.id = x.lead_id')
                ->join('x', MILEX_TABLE_PREFIX.'lead_tags', 't', 'x.tag_id = t.id')
                ->where(
                    $q->expr()->andX(
                        $q->expr()->eq('l.id', ':lead'),
