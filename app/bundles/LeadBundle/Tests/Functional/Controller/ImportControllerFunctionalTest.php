...
namespace Milex\LeadBundle\Tests\Functional\Controller;
...
class ImportControllerFunctionalTest extends MilexMysqlTestCase
{
    ...
    public function testScheduleImport(): void
    {
        ...
        Assert::assertStringContainsString(
            'Match the columns from the imported file to Milex\'s contact fields.',
            $html->text()
        );
    ...
    public function testImportCSVWithFileAsHeaderName(): void
    {
        ...
        $output = $this->runCommand('milex:import', ['-e' => 'dev', '--id' => $import->getId(), '--limit' => 10000]);
        Assert::assertStringContainsString(
    ...
    private function createCsvContactImport(): Import
    {
        ...
        $importModel = self::$container->get('milex.lead.model.import');
    ...
    {
        $tmpFile = tempnam(sys_get_temp_dir(), 'milex_import_test_').'.csv';
    ...
    private function createTag(string $tagName): Tag
    {
        ...
        $tagModel = self::$container->get('milex.lead.model.tag');
