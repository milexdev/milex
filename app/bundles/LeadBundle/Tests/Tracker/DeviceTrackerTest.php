...
use Milex\LeadBundle\Tracker\Factory\DeviceDetectorFactory\DeviceDetectorFactory;
use Milex\LeadBundle\Tracker\Service\DeviceCreatorService\DeviceCreatorService;
...
class DeviceTrackerTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $coreParametersHelper->expects($this->once())
          ...
          ->willReturn('milex.cache.adapter.filesystem');

        $container = $this->createMock(ContainerInterface::class);
        $container
          ->expects($this->once())
          ->method('has')
          ->with($this->equalTo('milex.cache.adapter.filesystem'))
          ->willReturn(true);
        $container
          ->expects($this->once())
          ->method('get')
