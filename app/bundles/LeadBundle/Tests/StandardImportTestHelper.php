...
namespace Milex\LeadBundle\Tests;
...
abstract class StandardImportTestHelper extends CommonMocks
{
    ...
    {
        $tmpFile = tempnam(sys_get_temp_dir(), 'milex_import_test_');
    ...
    {
        $tmpFile = tempnam(sys_get_temp_dir(), 'milex_import_large_test_');
        $file    = fopen($tmpFile, 'w');
    ...
    protected function initImportModel()
    {
        ...
        $entityManager->expects($this->any())
            ...
            ->will(
                $this->returnValueMap(
                    [
                        ['MilexLeadBundle:LeadEventLog', $logRepository],
                        ['MilexLeadBundle:Import', $importRepository],
                    ]
                )
            );
