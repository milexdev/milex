...
use Milex\LeadBundle\Entity\StagesChangeLogRepository;
...
use Milex\LeadBundle\Model\IpAddressModel;
use Milex\LeadBundle\Model\LeadModel;
use Milex\LeadBundle\Model\LegacyLeadModel;
...
class LeadModelTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testSetFieldValuesWithStage(): void
    {
        ...
        $this->entityManagerMock->expects($this->exactly(2))
            ->method('getRepository')
            ->withConsecutive(
                ['MilexLeadBundle:StagesChangeLog'],
                [Stage::class]
            )
        ...
        $translator->expects($this->once())
            ->method('trans')
            ->with('milex.stage.event.changed');
    ...
    public function testImportIsIgnoringContactWithNotFoundStage(): void
    {
        ...
        $translator->expects($this->once())
            ->method('trans')
            ->with('milex.lead.import.stage.not.exists', ['id' => $data['stage']])
