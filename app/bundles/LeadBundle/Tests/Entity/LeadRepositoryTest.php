...
use Milex\LeadBundle\Entity\CustomFieldRepositoryTrait;
...
class LeadRepositoryTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testApplySearchQueryRelationshipJoinOnlyOnce(): void
    {
        ...
        $queryBuilder->select('*')->from(MILEX_TABLE_PREFIX.'table_a');
        $tableB = [
            'alias'      => 'alias_b',
            'from_alias' => MILEX_TABLE_PREFIX.'table_a',
            'table'      => 'table_b',
        ...
        Assert::assertSame(
            'SELECT * FROM '.MILEX_TABLE_PREFIX.'table_a INNER JOIN '.MILEX_TABLE_PREFIX.'table_b alias_b ON condition_b INNER JOIN '.MILEX_TABLE_PREFIX.'table_c alias_c ON condition_c GROUP BY l.id',
            $queryBuilder->getSQL()
        );
    }
