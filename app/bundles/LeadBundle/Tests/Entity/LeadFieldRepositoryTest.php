...
class LeadFieldRepositoryTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testCompareDateValueForContactField(): void
    {
        ...
        $builderAlias->expects($this->once())
            ->method('from')
            ->with(MILEX_TABLE_PREFIX.'lead_fields', 'f')
            ->willReturnSelf();
        ...
        $builderCompare->expects($this->once())
            ->method('from')
            ->with(MILEX_TABLE_PREFIX.'leads', 'l')
            ->willReturnSelf();
    ...
    public function testCompareDateValueForCompanyField(): void
    {
        ...
        $builderCompare->expects($this->exactly(2))
            ->method('leftJoin')
            ->withConsecutive(
                ['l', MILEX_TABLE_PREFIX.'companies_leads', 'companies_lead', 'l.id = companies_lead.lead_id'],
                ['companies_lead', MILEX_TABLE_PREFIX.'companies', 'company', 'companies_lead.company_id = company.id']
            );
