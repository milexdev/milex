...
if ('index' == $tmpl):
    $view->extend('MilexLeadBundle:List:index.html.php');
endif;
$listCommand = $view['translator']->trans('milex.lead.lead.searchcommand.list');
...
<?php if (count($items)): ?>
    <div class="table-responsive">
        <table class="table table-hover table-striped table-bordered" id="leadListTable">
            ...
            <tbody>
            <?php /** @var \Milex\LeadBundle\Entity\LeadList $item */?>
                ...
                <tr>
                            ...
                            <?php if ($lastBuiltDateDifference >= $segmentRebuildWarningThreshold): ?>
                                <label class="control-label" data-toggle="tooltip"
                                       ...
                                       data-original-title="<?php echo $view['translator']->trans(
                                               'milex.lead.list.form.config.segment_rebuild_time.message',
                                               ['%count%' => $lastBuiltDateDifference]
                                       ); ?>">
                                    <i class="fa text-danger fa-exclamation-circle"></i></label>
                            <?php endif; ?>
                            <?php echo $view['content']->getCustomContent('segment.name', $milexTemplateVars); ?>
                        </div>
                    ...
                    <td class="visible-md visible-lg">
                        <a class="label label-primary col-count" data-id="<?php echo $item->getId(); ?>" href="<?php echo $view['router']->path(
                            'milex_contact_index',
                            ['search' => $view['translator']->trans('milex.lead.lead.searchcommand.list').':'.$item->getAlias()]
                        ); ?>" data-toggle="ajax">
        ...
        <div class="panel-footer">
            <?php echo $view->render(
                ...
                [
                    ...
                    'page'       => $page,
                    'limit'      => $limit,
                    'baseUrl'    => $view['router']->path('milex_segment_index'),
                    'sessionVar' => 'lead.list',
                ]
            ); ?>
        </div>
