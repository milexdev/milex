...
/** @var \Milex\LeadBundle\Entity\Import $item */
?>
<?php foreach ($items as $item): ?>
    <?php
        ...
        switch ($item->getObject()) {
            case 'lead':
                $objectRoute = 'milex_contact_index';
                break;
        }
    ?>
    <tr>
        <td class="col-actions text-center">
            <span class="label label-<?php echo $item->getSatusLabelClass(); ?>">
                <?php echo $view['translator']->trans('milex.lead.import.status.'.$item->getStatus()); ?>
            </span>
        </td>
        <td>
            <div>
        ...
        <td class="visible-md visible-lg">
            <?php if (null !== $objectRoute) : ?>
                <span class="mt-xs label label-primary has-click-event clickable-stat">
                    <a href="<?php echo $view['router']->path(
                        $objectRoute,
                        ['search' => $view['translator']->trans('milex.lead.lead.searchcommand.import_id').':'.$item->getId()]
        ...
        <td class="visible-md visible-lg">
            <?php if (null !== $objectRoute) : ?>
                <span class="mt-xs label label-primary has-click-event clickable-stat">
                    <a href="<?php echo $view['router']->path(
                        $objectRoute,
                        ['search' => $view['translator']->trans('milex.lead.lead.searchcommand.import_id').':'.$item->getId().' '.$view['translator']->trans('milex.lead.lead.searchcommand.import_action').':inserted']
        ...
        <td class="visible-md visible-lg">
            <?php if (null !== $objectRoute) : ?>
                <span class="mt-xs label label-primary has-click-event clickable-stat">
                    <a href="<?php echo $view['router']->path(
                        $objectRoute,
                        ['search' => $view['translator']->trans('milex.lead.lead.searchcommand.import_id').':'.$item->getId().' '.$view['translator']->trans('milex.lead.lead.searchcommand.import_action').':updated']
