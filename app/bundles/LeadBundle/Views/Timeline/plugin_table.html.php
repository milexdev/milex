...
if (!$this->request->isXmlHttpRequest()) {
    $view->extend('MilexLeadBundle:Timeline:plugin_index.html.php');
}
$baseUrl = isset($lead) ? $view['router']->path(
    'milex_plugin_timeline_view',
    ['leadId' => $lead->getId(), 'integration' => $integration]
) :
    $view['router']->path('milex_plugin_timeline_index', ['integration' => $integration]);
?>
<div class="table-responsive">
    <div class="tl-header">
        <?php echo $view['translator']->trans('milex.lead.timeline.displaying_events', ['%total%' => $events['total']]); ?>
        <?php if (isset($lead)) {
    echo $view['translator']->trans('milex.lead.timeline.displaying_events_for_contact', ['%contact%' => $lead->getName(), '%id%' => $lead->getId()]);
} ?>
        (<span class="tl-new"><?php echo $newCount; ?></span> <?php echo $view['translator']->trans(
            'milex.lead.timeline.events_new'
        ); ?>)
    </div>
    <table class="table table-hover table-bordered" id="contact-timeline">
        ...
        <tr>
            ...
            echo $view->render('MilexCoreBundle:Helper:tableheader.html.php', [
                'orderBy'    => 'eventLabel',
                'text'       => 'milex.lead.timeline.event_name',
                'class'      => 'timeline-name',
        ...
        <?php foreach ($events['events'] as $counter => $event): ?>
            ...
            <tr class="timeline-row<?php echo $rowStripe; ?><?php if (!empty($event['featured'])) {
                ...
                echo ' disabled';
            } ?>" data-toggle="tooltip" title="<?php echo $view['translator']->trans('milex.lead.timeline.toggle_details'); ?>" data-target="timeline-details-<?php echo $counter; ?>">
