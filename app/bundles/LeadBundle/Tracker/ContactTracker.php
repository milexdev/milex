...
namespace Milex\LeadBundle\Tracker;
...
use Milex\LeadBundle\Model\DefaultValueTrait;
...
class ContactTracker
{
    ...
    public function getContact()
    {
        ...
        if (!defined('MILEX_LEAD_LASTACTIVE_LOGGED')) {
            $this->leadRepository->updateLastActive($this->trackedContact->getId());
            define('MILEX_LEAD_LASTACTIVE_LOGGED', 1);
        }
    ...
    private function getSystemContact()
    {
        ...
        if ($this->isUserSession()) {
            $this->logger->addDebug('CONTACT: In a Milex user session');
        }
    ...
    {
        return $this->isUserSession() || $this->systemContact || defined('IN_MILEX_CONSOLE') || null === $this->requestStack->getCurrentRequest();
    }
