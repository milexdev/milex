...
class SearchSubscriber implements EventSubscriberInterface
{
    ...
    public function onGlobalSearch(MilexEvents\GlobalSearchEvent $event)
    {
        ...
        $anonymous = $this->translator->trans('milex.lead.lead.searchcommand.isanonymous');
        $mine      = $this->translator->trans('milex.core.searchcommand.ismine');
            ...
            if ($count > 0) {
                ...
                foreach ($leads as $lead) {
                    $leadResults[] = $this->templating->getTemplating()->renderResponse(
                        'MilexLeadBundle:SubscribedEvents\Search:global.html.php',
                        ['lead' => $lead]
                    )->getContent();
                }
                ...
                $event->addResults('milex.lead.leads', $leadResults);
            }
        }
    }
    ...
    public function onBuildSearchCommands(LeadBuildSearchEvent $event)
    {
        switch ($event->getCommand()) {
            case $this->translator->trans('milex.lead.lead.searchcommand.email_read'):
            case $this->translator->trans('milex.lead.lead.searchcommand.email_read', [], null, 'en_US'):
                $this->buildEmailReadQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.email_sent'):
            case $this->translator->trans('milex.lead.lead.searchcommand.email_sent', [], null, 'en_US'):
                $this->buildEmailSentQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.email_queued'):
            case $this->translator->trans('milex.lead.lead.searchcommand.email_queued', [], null, 'en_US'):
                $this->buildEmailQueuedQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.email_pending'):
            case $this->translator->trans('milex.lead.lead.searchcommand.email_pending', [], null, 'en_US'):
                $this->buildEmailPendingQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.page_source'):
            case $this->translator->trans('milex.lead.lead.searchcommand.page_source', [], null, 'en_US'):
                $this->buildPageHitSourceQuery($event);
                break;

            case $this->translator->trans('milex.lead.lead.searchcommand.page_source_id'):
            case $this->translator->trans('milex.lead.lead.searchcommand.page_source_id', [], null, 'en_US'):
                $this->buildPageHitSourceIdQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.import_id'):
            case $this->translator->trans('milex.lead.lead.searchcommand.import_id', [], null, 'en_US'):
                $this->buildImportIdQuery($event);
                break;

            case $this->translator->trans('milex.lead.lead.searchcommand.import_action'):
            case $this->translator->trans('milex.lead.lead.searchcommand.import_action', [], null, 'en_US'):
                $this->buildImportActionQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.page_id'):
            case $this->translator->trans('milex.lead.lead.searchcommand.page_id', [], null, 'en_US'):
                $this->buildPageHitIdQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.sms_sent'):
            case $this->translator->trans('milex.lead.lead.searchcommand.sms_sent', [], null, 'en_US'):
                $this->buildSmsSentQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.web_sent'):
            case $this->translator->trans('milex.lead.lead.searchcommand.web_sent', [], null, 'en_US'):
                $this->buildWebSentQuery($event);
                break;
            case $this->translator->trans('milex.lead.lead.searchcommand.mobile_sent'):
            case $this->translator->trans('milex.lead.lead.searchcommand.mobile_sent', [], null, 'en_US'):
