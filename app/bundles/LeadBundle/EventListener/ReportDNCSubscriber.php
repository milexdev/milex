...
class ReportDNCSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $dncColumns = [
            'dnc.reason' => [
                'label' => 'milex.lead.report.dnc_reason',
                'type'  => 'select',
                'list'  => $this->getDncReasons(),
            ],
            'dnc.comments' => [
                'label' => 'milex.lead.report.dnc_comment',
                'type'  => 'text',
            ],
            'dnc.date_added' => [
                'label' => 'milex.lead.report.dnc_date_added',
                'type'  => 'datetime',
            ],
            'dnc.channel' => [
                'label' => 'milex.lead.report.dnc_channel',
                'type'  => 'html',
            ],
            'dnc.channel_id' => [
                'label' => 'milex.lead.report.dnc_channel_id',
                'type'  => 'html',
            ],
        ];

        $data = [
            'display_name' => 'milex.lead.report.dnc',
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'lead_donotcontact', 'dnc');
        $qb->leftJoin('dnc', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = dnc.lead_id');
    ...
    {
        return [
            0 => $this->translator->trans('milex.lead.report.dnc_contactable'),
            1 => $this->translator->trans('milex.lead.report.dnc_unsubscribed'),
            2 => $this->translator->trans('milex.lead.report.dnc_bounced'),
            3 => $this->translator->trans('milex.lead.report.dnc_manual'),
        ];
    }
