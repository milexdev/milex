...
class ReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        if ($event->checkContext($this->leadContexts)) {
            ...
            if ($event->checkContext([self::CONTEXT_LEADS])) {
                $stageColumns = [
                    'l.stage_id'           => [
                        'label' => 'milex.lead.report.attribution.stage_id',
                        'type'  => 'int',
                        'link'  => 'milex_stage_action',
                    ],
                    's.name'               => [
                        'alias' => 'stage_name',
                        'label' => 'milex.lead.report.attribution.stage_name',
                        'type'  => 'string',
                    ],
                    's.date_added' => [
                        'alias'   => 'stage_date_added',
                        'label'   => 'milex.lead.report.attribution.stage_date_added',
                        'type'    => 'string',
            ...
            if ($event->checkContext([self::CONTEXT_LEADS, self::CONTEXT_LEAD_POINT_LOG])) {
                // Add shared graphs
                $event->addGraph(self::CONTEXT_LEADS, 'line', 'milex.lead.graph.line.leads');
                $event->addGraph(self::CONTEXT_LEAD_POINT_LOG, 'line', 'milex.lead.graph.line.leads');
        ...
        if ($event->checkContext($this->companyContexts)) {
            ...
            foreach ($this->companyContexts as $context) {
                ...
                $event->addGraph($context, 'line', 'milex.lead.graph.line.companies');
                $event->addGraph($context, 'pie', 'milex.lead.graph.pie.companies.industry');
                $event->addGraph($context, 'pie', 'milex.lead.table.pie.company.country');
                $event->addGraph($context, 'table', 'milex.lead.company.table.top.cities');
            }
        }
    }
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        switch ($context) {
            case self::CONTEXT_LEADS:
                $qb->from(MILEX_TABLE_PREFIX.'leads', 'l');
                ...
                if ($event->hasColumn(['s.name']) || $event->hasFilter(['s.name'])) {
                    $qb->leftJoin('l', MILEX_TABLE_PREFIX.'stages', 's', 's.id = l.stage_id');
                }

                if ($event->hasFilter('s.leadlist_id')) {
                    $qb->join('l', MILEX_TABLE_PREFIX.'lead_lists_leads', 's', 's.lead_id = l.id AND s.manually_removed = 0');
            ...
            case self::CONTEXT_LEAD_POINT_LOG:
                ...
                $qb->from(MILEX_TABLE_PREFIX.'lead_points_change_log', 'lp')
                    ->leftJoin('lp', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = lp.lead_id');
            ...
            case self::CONTEXT_CONTACT_FREQUENCYRULES:
                ...
                $qb->from(MILEX_TABLE_PREFIX.'lead_frequencyrules', 'lf')
                    ->leftJoin('lf', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = lf.lead_id');
            ...
            case self::CONTEXT_CONTACT_ATTRIBUTION_LAST:
                ...
                $qb->from(MILEX_TABLE_PREFIX.'leads', 'l')
                    ->join('l', MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'log', 'l.id = log.lead_id')
                    ->leftJoin('l', MILEX_TABLE_PREFIX.'stages', 's', 'l.stage_id = s.id')
                    ->join('log', MILEX_TABLE_PREFIX.'campaign_events', 'e', 'log.event_id = e.id')
                    ->join('log', MILEX_TABLE_PREFIX.'campaigns', 'c', 'log.campaign_id = c.id')
                    ->andWhere(
                        $qb->expr()->andX(
                ...
                $subQ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', "{$alias}log")
                    ->join("{$alias}log", MILEX_TABLE_PREFIX.'campaign_events', "{$alias}e", "{$alias}log.event_id = {$alias}e.id")
                    ->join("{$alias}e", MILEX_TABLE_PREFIX.'campaigns', "{$alias}c", "{$alias}e.campaign_id = {$alias}c.id")
                    ->where($expr);
            ...
            case self::CONTEXT_COMPANIES:
                ...
                $qb->from(MILEX_TABLE_PREFIX.'companies', 'comp');

                if ($event->usesColumn(['u.first_name', 'u.last_name'])) {
                    $qb->leftJoin('comp', MILEX_TABLE_PREFIX.'users', 'u', 'u.id = comp.owner_id');
                }
    ...
    public function onReportGraphGenerate(ReportGraphEvent $event)
    {
        ...
        foreach ($graphs as $g) {
            ...
            if ('lp' === $queryBuilder->getQueryPart('from')[0]['alias']) {
                ...
                $queryBuilder->leftJoin('lp', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = lp.lead_id');
            }

            switch ($g) {
                case 'milex.lead.graph.pie.attribution_stages':
                case 'milex.lead.graph.pie.attribution_campaigns':
                case 'milex.lead.graph.pie.attribution_actions':
                case 'milex.lead.graph.pie.attribution_channels':
                    ...
                    $groupBy = str_replace('milex.lead.graph.pie.attribution_', '', $g);
                    switch ($groupBy) {
                        case 'stages':
                    ...
                    foreach ($data as $row) {
                        switch ($groupBy) {
                            ...
                            default:
                                $label = (empty($row['slice'])) ? $this->translator->trans('milex.core.none') : $row['slice'];
                        }
                ...
                case 'milex.lead.graph.line.leads':
                    ...
                    $chart->setDataset($options['translator']->trans('milex.lead.all.leads'), $leads);
                    $queryBuilder->andwhere($qb->expr()->isNotNull('l.date_identified'));
                    $identified = $chartQuery->loadAndBuildTimeData($queryBuilder);
                    $chart->setDataset($options['translator']->trans('milex.lead.identified'), $identified);
                    $data         = $chart->render();
                    $data['name'] = $g;
                    $event->setGraph($g, $data);
                    break;

                case 'milex.lead.graph.line.points':
                    $chart = new LineChart(null, $options['dateFrom'], $options['dateTo']);
                    $chartQuery->modifyTimeDataQuery($queryBuilder, 'date_added', 'lp');
                    $leads = $chartQuery->loadAndBuildTimeData($queryBuilder);
                    $chart->setDataset($options['translator']->trans('milex.lead.graph.line.points'), $leads);
                    $data         = $chart->render();
                    $data['name'] = $g;
                    $event->setGraph($g, $data);
                    break;

                case 'milex.lead.table.most.points':
                    ...
                    $graphData['link']      = 'milex_contact_action';
                    $event->setGraph($g, $graphData);
                    break;

                case 'milex.lead.table.top.countries':
                ...
                case 'milex.lead.table.top.cities':
                ...
                case 'milex.lead.table.top.events':
                ...
                case 'milex.lead.table.top.actions':
                ...
                case 'milex.lead.table.pie.company.country':
                    ...
                    $chart->setDataset($options['translator']->trans('milex.lead.all.companies'), $companyCount);
                    $event->setGraph(
                        $g,
                        [
                            'data'      => $chart->render(),
                    ...
                    break;
                case 'milex.lead.graph.line.companies':
                    $chart = new LineChart(null, $options['dateFrom'], $options['dateTo']);
                    $chartQuery->modifyTimeDataQuery($queryBuilder, 'date_added', 'comp');
                    $companies = $chartQuery->loadAndBuildTimeData($queryBuilder);
                    $chart->setDataset($options['translator']->trans('milex.lead.all.companies'), $companies);
                    $data         = $chart->render();
                    $data['name'] = $g;
                    $event->setGraph($g, $data);
                    break;
                case 'milex.lead.graph.pie.companies.industry':
                    ...
                    break;
                case 'milex.lead.company.table.top.cities':
    ...
    private function injectPointsReportData(ReportBuilderEvent $event, array $columns, array $filters)
    {
        $pointColumns = [
            'lp.id' => [
                'label' => 'milex.lead.report.points.id',
                'type'  => 'int',
            ],
            'lp.type' => [
                'label' => 'milex.lead.report.points.type',
                'type'  => 'string',
            ],
            'lp.event_name' => [
                'label' => 'milex.lead.report.points.event_name',
                'type'  => 'string',
            ],
            'lp.action_name' => [
                'label' => 'milex.lead.report.points.action_name',
                'type'  => 'string',
            ],
            'lp.delta' => [
                'label' => 'milex.lead.report.points.delta',
                'type'  => 'int',
            ],
            'lp.date_added' => [
                'label'          => 'milex.lead.report.points.date_added',
                'type'           => 'datetime',
        ...
        $event->addGraph($context, 'line', 'milex.lead.graph.line.points')
            ->addGraph($context, 'table', 'milex.lead.table.most.points')
            ->addGraph($context, 'table', 'milex.lead.table.top.countries')
            ->addGraph($context, 'table', 'milex.lead.table.top.cities')
            ->addGraph($context, 'table', 'milex.lead.table.top.events')
            ->addGraph($context, 'table', 'milex.lead.table.top.actions');
    }
    ...
    private function injectFrequencyReportData(ReportBuilderEvent $event, array $columns, array $filters)
    {
        $frequencyColumns = [
            'lf.frequency_number' => [
                'label' => 'milex.lead.report.frequency.frequency_number',
                'type'  => 'int',
            ],
            'lf.frequency_time' => [
                'label' => 'milex.lead.report.frequency.frequency_time',
                'type'  => 'string',
            ],
            'lf.channel' => [
                'label' => 'milex.lead.report.frequency.channel',
                'type'  => 'string',
            ],
            'lf.preferred_channel' => [
                'label' => 'milex.lead.report.frequency.preferred_channel',
                'type'  => 'boolean',
            ],
            'lf.pause_from_date' => [
                'label' => 'milex.lead.report.frequency.pause_from_date',
                'type'  => 'datetime',
            ],
            'lf.pause_to_date' => [
                'label' => 'milex.lead.report.frequency.pause_to_date',
                'type'  => 'datetime',
            ],
            'lf.date_added' => [
                'label'          => 'milex.lead.report.frequency.date_added',
                'type'           => 'datetime',
    ...
    private function injectAttributionReportData(ReportBuilderEvent $event, array $columns, array $filters, $type)
    {
        $attributionColumns = [
            'log.campaign_id' => [
                'label' => 'milex.lead.report.attribution.campaign_id',
                'type'  => 'int',
                'link'  => 'milex_campaign_action',
            ],
            'log.date_triggered' => [
                'label'          => 'milex.lead.report.attribution.action_date',
                'type'           => 'datetime',
                'groupByFormula' => 'DATE(log.date_triggered)',
            ],
            'c.name' => [
                'alias' => 'campaign_name',
                'label' => 'milex.lead.report.attribution.campaign_name',
                'type'  => 'string',
            ],
            'l.stage_id' => [
                'label' => 'milex.lead.report.attribution.stage_id',
            ...
            'channel' => [
                ...
                'label'   => 'milex.lead.report.attribution.channel',
                'type'    => 'string',
            ],
            'channel_action' => [
                'alias'   => 'channel_action',
                'formula' => 'SUBSTRING_INDEX(e.type, \'.\', -1)',
                'label'   => 'milex.lead.report.attribution.channel_action',
                'type'    => 'string',
            ],
            'e.name' => [
                'alias' => 'action_name',
                'label' => 'milex.lead.report.attribution.action_name',
                'type'  => 'string',
            ],
        ];
        ...
        foreach ($availableChannels['decision'] as $channel => $decision) {
            ...
            $channelName            = $parts[0];
            $channels[$channelName] = $this->translator->hasId('milex.channel.'.$channelName) ? $this->translator->trans(
                'milex.channel.'.$channelName
            ) : ucfirst($channelName);
            unset($parts[0]);
            $actionValue = implode('.', $parts);

            if ($this->translator->hasId('milex.channel.action.'.$channel)) {
                $actionName = $this->translator->trans('milex.channel.action.'.$channel);
            } elseif ($this->translator->hasId('milex.campaign.'.$channel)) {
                $actionName = $this->translator->trans('milex.campaign.'.$channel);
            } else {
        ...
        $filters['log.campaign_id'] = [
            'label' => 'milex.lead.report.attribution.filter.campaign',
            'type'  => 'select',
            'list'  => $campaigns,
        ];
        unset($campaigns);
        ...
        $filters['l.stage_id'] = [
            'label' => 'milex.lead.report.attribution.filter.stage',
            'type'  => 'select',
            'list'  => $stages,
        ];
        unset($stages);
        ...
        $event
            ->addGraph($context, 'pie', 'milex.lead.graph.pie.attribution_stages')
            ->addGraph($context, 'pie', 'milex.lead.graph.pie.attribution_campaigns')
            ->addGraph($context, 'pie', 'milex.lead.graph.pie.attribution_actions')
            ->addGraph($context, 'pie', 'milex.lead.graph.pie.attribution_channels');

        $data = [
            'display_name' => 'milex.lead.report.attribution.'.$type,
            'columns'      => $columns,
            'filters'      => $filters,
        ];
