...
class SegmentReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $segmentColumns = [
            'lll.manually_removed' => [
                'label' => 'milex.lead.report.segment.manually_removed',
                'type'  => 'bool',
            ],
            'lll.manually_added' => [
                'label' => 'milex.lead.report.segment.manually_added',
                'type'  => 'bool',
            ],
        ];
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'lll')
            ->leftJoin('lll', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = lll.lead_id')
            ->leftJoin('lll', MILEX_TABLE_PREFIX.'lead_lists', 's', 's.id = lll.leadlist_id')
