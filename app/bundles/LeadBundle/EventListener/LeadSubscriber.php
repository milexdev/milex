...
use Milex\CoreBundle\EventListener\ChannelTrait;
...
use Milex\LeadBundle\Helper\LeadChangeEventDispatcher;
use Milex\LeadBundle\LeadEvents;
use Milex\LeadBundle\Model\ChannelTimelineInterface;
...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    {
        $eventTypes = [
            'lead.utmtagsadded' => 'milex.lead.event.utmtagsadded',
            'lead.donotcontact' => 'milex.lead.event.donotcontact',
            'lead.imported'     => 'milex.lead.event.imported',
        ];
        ...
        if ($event->isForTimeline()) {
            $eventTypes['lead.create']     = 'milex.lead.event.create';
            $eventTypes['lead.identified'] = 'milex.lead.event.identified';
            $eventTypes['lead.ipadded']    = 'milex.lead.event.ipadded';
            $eventTypes['lead.apiadded']   = 'milex.lead.event.apiadded';
        }
        ...
        if ($this->isTest) {
            $eventTypes = [
                'lead.apiadded' => 'milex.lead.event.apiadded',
            ];
        }
            ...
            foreach ($rows['results'] as $row) {
                ...
                $event->addEvent(
                    [
                        ...
                        'contentTemplate' => 'MilexLeadBundle:SubscribedEvents\Timeline:ipadded.html.php',
            ...
            foreach ($utmTags['results'] as $utmTag) {
                ...
                $event->addEvent(
                        [
                            ...
                            'contentTemplate' => 'MilexLeadBundle:SubscribedEvents\Timeline:utmadded.html.php',
    ...
    private function addTimelineDoNotContactEntries(Events\LeadTimelineEvent $event, $eventTypeKey, $eventTypeName)
    {
        ...
        if (!$event->isEngagementCount()) {
            foreach ($rows['results'] as $row) {
                ...
                $template = 'MilexLeadBundle:SubscribedEvents\Timeline:donotcontact.html.php';
                $icon     = 'fa-ban';
    ...
    private function addTimelineImportedEntries(Events\LeadTimelineEvent $event, $eventTypeKey, $eventTypeName)
    {
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($imports['results'] as $import) {
                ...
                $eventLabel = $this->translator->trans('milex.lead.import.contact.action.'.$import['action'], ['%name%' => $eventLabel]);
                $event->addEvent(
                        [
                            'event'      => $eventTypeKey,
                            ...
                            'icon'            => 'fa-download',
                            'extra'           => $import,
                            'contentTemplate' => 'MilexLeadBundle:SubscribedEvents\Timeline:import.html.php',
