...
class SegmentLogReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $segmentColumns = [
            'log_added.object_id' => [
                'label' => 'milex.lead.report.segment.id',
                'type'  => 'int',
            ],
            'log_added.date_added' => [
                'label' => 'milex.lead.report.segment.date_added',
                'type'  => 'datetime',
            ],
            'log_removed.date_added' => [
                'label' => 'milex.lead.report.segment.date_removed',
                'type'  => 'datetime',
            ],
        ];

        $data = [
            'display_name' => 'milex.lead.report.segment.log',
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'leads', 'l')
            ->leftJoin('l', MILEX_TABLE_PREFIX.'lead_event_log', 'log_added', $this->generateLeftJoinCondition('log_added', 'added'))
            ->leftJoin('l', MILEX_TABLE_PREFIX.'lead_event_log', 'log_removed', $this->generateLeftJoinCondition('log_removed', 'removed').' AND log_removed.object_id = log_added.object_id ');
