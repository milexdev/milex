...
class DoctrineSubscriber implements \Doctrine\Common\EventSubscriber
{
    ...
    public function postGenerateSchema(GenerateSchemaEventArgs $args)
    {
        ...
        try {
            if (!$schema->hasTable(MILEX_TABLE_PREFIX.'lead_fields')) {
                return;
            }
            ...
            foreach ($objects as $object => $tableName) {
                $table = $schema->getTable(MILEX_TABLE_PREFIX.$tableName);
                ...
                $fields = $args->getEntityManager()->getConnection()->createQueryBuilder()
                    ...
                    ->from(MILEX_TABLE_PREFIX.'lead_fields', 'f')
                ...
                foreach ($fields as $f) {
                    ...
                    if ('text' != $columnDef['type']) {
                        $table->addIndex([$columnDef['name']], MILEX_TABLE_PREFIX.$f['alias'].'_search');
                    }
                }
                ...
                if (count($uniqueFields) > 1) {
                    ...
                    $table->addIndex($uniqueFields, MILEX_TABLE_PREFIX.'unique_identifier_search');
                }

                switch ($object) {
                    case 'lead':
                        $table->addIndex(['attribution', 'attribution_date'], MILEX_TABLE_PREFIX.'contact_attribution');
                        $table->addIndex(['date_added', 'country'], MILEX_TABLE_PREFIX.'date_added_country_index');
                        break;
                    case 'company':
                        $table->addIndex(['companyname', 'companyemail'], MILEX_TABLE_PREFIX.'company_filter');
                        $table->addIndex(['companyname', 'companycity', 'companycountry', 'companystate'], MILEX_TABLE_PREFIX.'company_match');
                        break;
                }
            }
        } catch (\Exception $e) {
