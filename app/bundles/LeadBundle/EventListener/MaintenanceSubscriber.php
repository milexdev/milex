...
class MaintenanceSubscriber implements EventSubscriberInterface
{
    ...
    public function onDataCleanup(MaintenanceEvent $event)
    {
        ...
        if ($event->isDryRun()) {
            ...
            $qb->select('l.id')->from(MILEX_TABLE_PREFIX.'leads', 'l')
              ->where($qb->expr()->lte('l.last_active', ':date'));
            ...
            while (true) {
                ...
                foreach ($leadsIds as $leadId) {
                    $rows += $qb2->delete(MILEX_TABLE_PREFIX.'leads')
                      ->where(
                        $qb2->expr()->eq(
                          'id', $leadId
                        )
        ...
        $event->setStat($this->translator->trans('milex.maintenance.visitors'), $rows, $qb->getSQL(), $qb->getParameters());
    }
}
