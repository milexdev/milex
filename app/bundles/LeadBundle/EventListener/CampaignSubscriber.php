...
use Milex\LeadBundle\Form\Type\AddToCompanyActionType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadCampaignsType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadDeviceType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadFieldValueType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadOwnerType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadSegmentsType;
use Milex\LeadBundle\Form\Type\CampaignEventLeadTagsType;
use Milex\LeadBundle\Form\Type\ChangeOwnerType;
...
use Milex\LeadBundle\Form\Type\PointActionType;
use Milex\LeadBundle\Form\Type\UpdateCompanyActionType;
use Milex\LeadBundle\Form\Type\UpdateLeadActionType;
use Milex\LeadBundle\Helper\CustomFieldHelper;
...
class CampaignSubscriber implements EventSubscriberInterface
{
    ...
    public function onCampaignBuild(CampaignBuilderEvent $event)
    {
        //Add actions
        $action = [
            'label'       => 'milex.lead.lead.events.changepoints',
            'description' => 'milex.lead.lead.events.changepoints_descr',
        ...
        $action = [
            'label'       => 'milex.lead.lead.events.updatelead',
            'description' => 'milex.lead.lead.events.updatelead_descr',
            'formType'    => UpdateLeadActionType::class,
            'formTheme'   => 'MilexLeadBundle:FormTheme\ActionUpdateLead',
            'eventName'   => LeadEvents::ON_CAMPAIGN_TRIGGER_ACTION,
        ];
        $event->addAction('lead.updatelead', $action);

        $action = [
            'label'       => 'milex.lead.lead.events.updatecompany',
            'description' => 'milex.lead.lead.events.updatecompany_descr',
            'formType'    => UpdateCompanyActionType::class,
            'formTheme'   => 'MilexLeadBundle:FormTheme\ActionUpdateCompany',
            'eventName'   => LeadEvents::ON_CAMPAIGN_TRIGGER_ACTION,
        ];
        ...
        $action = [
            'label'       => 'milex.lead.lead.events.addtocompany',
            'description' => 'milex.lead.lead.events.addtocompany_descr',
        ...
        $action = [
            'label'       => 'milex.lead.lead.events.changeowner',
            'description' => 'milex.lead.lead.events.changeowner_descr',
        ...
        $trigger = [
            'label'       => 'milex.lead.lead.events.field_value',
            'description' => 'milex.lead.lead.events.field_value_descr',
            'formType'    => CampaignEventLeadFieldValueType::class,
            'formTheme'   => 'MilexLeadBundle:FormTheme\FieldValueCondition',
            'eventName'   => LeadEvents::ON_CAMPAIGN_TRIGGER_CONDITION,
        ];
        $event->addCondition('lead.field_value', $trigger);

        $trigger = [
            'label'       => 'milex.lead.lead.events.device',
            'description' => 'milex.lead.lead.events.device_descr',
        ...
        $trigger = [
            'label'       => 'milex.lead.lead.events.tags',
            'description' => 'milex.lead.lead.events.tags_descr',
        ...
        $trigger = [
            'label'       => 'milex.lead.lead.events.segments',
            'description' => 'milex.lead.lead.events.segments_descr',
        ...
        $trigger = [
            'label'       => 'milex.lead.lead.events.owner',
            'description' => 'milex.lead.lead.events.owner_descr',
        ...
        $trigger = [
            'label'       => 'milex.lead.lead.events.campaigns',
            'description' => 'milex.lead.lead.events.campaigns_descr',
    ...
    public function onCampaignTriggerActionChangeCompanyScore(CampaignExecutionEvent $event)
    {
        ...
        if (!$this->leadModel->scoreContactsCompany($lead, $score)) {
            return $event->setFailed('milex.lead.no_company');
        } else {
            return $event->setResult(true);
        }
    }
        ...
        if ($event->checkContext('lead.device')) {
            ...
            if ('date' === $event->getConfig()['operator']) {
                ...
                if (false !== strpos($event->getConfig()['value'], '+P')) { //add date
                    ...
                    /**
                     * note: currently milex campaign only one time execution
