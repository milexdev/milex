...
use Milex\LeadBundle\Form\Type\DashboardLeadsInTimeWidgetType;
use Milex\LeadBundle\Form\Type\DashboardLeadsLifetimeWidgetType;
...
class DashboardSubscriber extends MainDashboardSubscriber
{
    ...
    public function onWidgetDetailGenerate(WidgetDetailEvent $event)
    {
        ...
        if ('map.of.leads' == $event->getType()) {
            ...
            $event->setTemplate('MilexCoreBundle:Helper:map.html.php');
            $event->stopPropagation();
            ...
            if (!$event->isCached()) {
                ...
                if ($lists) {
                    foreach ($lists as &$list) {
                        $listUrl    = $this->router->generate('milex_segment_action', ['objectAction' => 'edit', 'objectId' => $list['id']]);
                        $contactUrl = $this->router->generate('milex_contact_index', ['search' => 'segment:'.$list['alias']]);
                        $row        = [
                            [
                                'value' => $list['name'],
                                'type'  => 'link',
        ...
        if ('lead.lifetime' == $event->getType()) {
            ...
            if (empty($lists)) {
                $lists[] = [
                    'leads' => 0,
                    'id'    => 0,
                    'name'  => $event->getTranslator()->trans('milex.lead.all.leads'),
                    'alias' => '',
                ];
            }
            ...
            if ($lists) {
                ...
                foreach ($lists as &$list) {
                    if ('' != $list['alias']) {
                        $listUrl = $this->router->generate('milex_contact_index', ['search' => 'segment:'.$list['alias']]);
                    } else {
                        $listUrl = $this->router->generate('milex_contact_index', []);
                    }
                    if ($list['id']) {
                ...
                $event->setTemplate('MilexCoreBundle:Helper:lifecycle.html.php');
                $event->stopPropagation();
            }
        ...
        if ('top.owners' == $event->getType()) {
            if (!$canViewOthers) {
                $event->setErrorMessage($this->translator->trans('milex.dashboard.missing.permission', ['%section%' => $this->bundle]));
                $event->stopPropagation();
            ...
            if (!$event->isCached()) {
                ...
                if ($owners) {
                    foreach ($owners as &$owner) {
                        $ownerUrl = $this->router->generate('milex_user_action', ['objectAction' => 'edit', 'objectId' => $owner['owner_id']]);
                        $row      = [
                            [
                ...
                $event->setTemplateData([
                    'headItems' => [
                        'milex.user.account.permissions.editname',
                        'milex.lead.leads',
                    ],
                    'bodyItems' => $items,
                    'raw'       => $owners,
            ...
            if (!$event->isCached()) {
                ...
                if ($creators) {
                    foreach ($creators as &$creator) {
                        $creatorUrl = $this->router->generate('milex_user_action', ['objectAction' => 'edit', 'objectId' => $creator['created_by']]);
                        $row        = [
                            [
        ...
        if ('created.leads' == $event->getType()) {
            if (!$event->isCached()) {
                ...
                if (empty($leads)) {
                    $leads[] = [
                        'name' => $this->translator->trans('milex.report.report.noresults'),
                    ];
                }
                ...
                if ($leads) {
                    foreach ($leads as &$lead) {
                        $leadUrl = isset($lead['id']) ? $this->router->generate('milex_contact_action', ['objectAction' => 'view', 'objectId' => $lead['id']]) : '';
