...
use Milex\CoreBundle\Helper\ProgressBarHelper;
...
use Milex\LeadBundle\Entity\ListLeadRepository;
use Milex\LeadBundle\Entity\OperatorListTrait;
use Milex\LeadBundle\Event\LeadListEvent;
...
use Milex\LeadBundle\Form\Type\ListType;
use Milex\LeadBundle\Helper\SegmentCountCacheHelper;
...
use Milex\LeadBundle\Segment\Stat\SegmentChartQueryFactory;
...
class ListModel extends FormModel
{
    ...
    {
        defined('MILEX_REBUILDING_LEAD_LISTS') or define('MILEX_REBUILDING_LEAD_LISTS', 1);
        ...
        if ($output) {
            $output->writeln($this->translator->trans('milex.lead.list.rebuild.to_be_added', ['%leads%' => $leadCount, '%batch%' => $limit]));
        }
        ...
        if ($output) {
            $output->writeln($this->translator->trans('milex.lead.list.rebuild.to_be_removed', ['%leads%' => $leadCount, '%batch%' => $limit]));
        }
    ...
    public function getTopLists($limit = 10, $dateFrom = null, $dateTo = null, $canViewOthers = true)
    {
        ...
        $q->select('COUNT(t.date_added) AS leads, ll.id, ll.name, ll.alias')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'lead_lists', 'll', 'll.id = t.leadlist_id')
            ->orderBy('leads', 'DESC')
    ...
    public function getLifeCycleSegments($limit, $dateFrom, $dateTo, $canViewOthers, $segments)
    {
        ...
        $q->select('COUNT(t.date_added) AS leads, ll.id, ll.name as name,ll.alias as alias')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'lead_lists', 'll', 'll.id = t.leadlist_id')
            ->join('t', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = t.lead_id')
            ->orderBy('leads', 'DESC')
            ->where($q->expr()->eq('ll.is_published', ':published'))
            ->setParameter('published', true)
            ->groupBy('ll.id');
    ...
    public function getLifeCycleSegmentChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat, $filter, $canViewOthers, $listName)
    {
        ...
        if (isset($filter['leadlist_id']['value'])) {
            $chart->setDataset(
                $this->translator->trans('milex.lead.lifecycle.graph.pie.all.lists'),
                $all
            );
        }
    ...
    public function getStagesBarChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        $q->select('count(l.id) as leads, s.name as stage')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = t.lead_id')
            ->join('t', MILEX_TABLE_PREFIX.'stages', 's', 's.id=l.stage_id')
            ->orderBy('leads', 'DESC')
        ...
        $baseData = [
            'label' => $this->translator->trans('milex.lead.leads'),
            'data'  => $data['values'],
        ];
    ...
    public function getDeviceGranularityData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        $q->select('count(l.id) as leads, ds.device')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = t.lead_id')
            ->join('t', MILEX_TABLE_PREFIX.'page_hits', 'h', 'h.lead_id=l.id')
            ->join('h', MILEX_TABLE_PREFIX.'lead_devices', 'ds', 'ds.id = h.device_id')
        ...
        foreach ($results as $result) {
            $data['labels'][] = substr(empty($result['device']) ? $this->translator->trans('milex.core.no.info') : $result['device'], 0, 12);
            $data['values'][] = $result['leads'];
        }
        ...
        $baseData = [
            'label' => $this->translator->trans('milex.core.device'),
            'data'  => $data['values'],
        ];
    ...
    public function getSegmentContactsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [])
    {
        ...
        $chart->setDataset($this->translator->trans('milex.lead.segments.contacts.added'), $this->segmentChartQueryFactory->getContactsAdded($query));
        ...
        if ($query->isStatsFromEventLog()) {
            $chart->setDataset($this->translator->trans('milex.lead.segments.contacts.removed'), $this->segmentChartQueryFactory->getContactsRemoved($query));
            $chart->setDataset($this->translator->trans('milex.lead.segments.contacts.total'), $this->segmentChartQueryFactory->getContactsTotal($query, $this));
        }
