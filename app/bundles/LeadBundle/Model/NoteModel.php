...
use Milex\LeadBundle\Event\LeadNoteEvent;
use Milex\LeadBundle\Form\Type\NoteType;
...
class NoteModel extends FormModel
{
    ...
    {
        return $this->em->getRepository('MilexLeadBundle:LeadNote');
    }
    ...
    {
        $filter   = ($useFilters) ? $this->session->get('milex.lead.'.$lead->getId().'.note.filter', '') : null;
        $noteType = ($useFilters) ? $this->session->get('milex.lead.'.$lead->getId().'.notetype.filter', []) : null;
