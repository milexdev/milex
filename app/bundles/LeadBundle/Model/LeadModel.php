...
use Milex\LeadBundle\Entity\DoNotContact as DNC;
...
use Milex\LeadBundle\Entity\OperatorListTrait;
...
use Milex\LeadBundle\Event\CategoryChangeEvent;
...
use Milex\LeadBundle\Form\Type\LeadType;
use Milex\LeadBundle\Helper\ContactRequestHelper;
...
use Milex\UserBundle\Security\Provider\UserProvider;
...
class LeadModel extends FormModel
{
     ...
     *
     * @return \Milex\LeadBundle\Entity\TagRepository
     */
    public function getTagRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:Tag');
    }

    /**
     * @return \Milex\LeadBundle\Entity\PointsChangeLogRepository
     */
    public function getPointLogRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:PointsChangeLog');
    }

    /**
     * Get the tags repository.
     *
     * @return \Milex\LeadBundle\Entity\UtmTagRepository
     */
    public function getUtmTagRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:UtmTag');
    }

    /**
     * Get the tags repository.
     *
     * @return \Milex\LeadBundle\Entity\LeadDeviceRepository
     */
    public function getDeviceRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:LeadDevice');
    }

    /**
     * Get the lead event log repository.
     *
     * @return \Milex\LeadBundle\Entity\LeadEventLogRepository
     */
    public function getEventLogRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:LeadEventLog');
    }

    /**
     * Get the frequency rules repository.
     *
     * @return \Milex\LeadBundle\Entity\FrequencyRuleRepository
     */
    public function getFrequencyRuleRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:FrequencyRule');
    }

    /**
     * Get the Stages change log repository.
     *
     * @return \Milex\LeadBundle\Entity\StagesChangeLogRepository
     */
    public function getStagesChangeLogRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:StagesChangeLog');
    }

    /**
     * Get the lead categories repository.
     *
     * @return \Milex\LeadBundle\Entity\LeadCategoryRepository
     */
    public function getLeadCategoryRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:LeadCategory');
    }

    /**
     * @return \Milex\LeadBundle\Entity\MergeRecordRepository
     */
    public function getMergeRecordRepository()
    {
        return $this->em->getRepository('MilexLeadBundle:MergeRecord');
    }
    ...
    {
        return $this->em->getRepository('MilexLeadBundle:LeadList');
    }
            ...
            if ((int) $newLeadStageIdOrName !== $currentLeadStageId && $newLeadStageIdOrName !== $currentLeadStageName) {
                ...
                if ($newStage) {
                    $lead->stageChangeLogEntry(
                        ...
                        $this->translator->trans('milex.stage.event.changed')
                    );
                } else {
                    throw new ImportFailedException($this->translator->trans('milex.lead.import.stage.not.exists', ['id' => $newLeadStageIdOrName]));
                }
            }
        }
    ...
    public function getLookupResults($type, $filter = '', $limit = 10, $start = 0)
    {
        ...
        switch ($type) {
            case 'user':
                $results = $this->em->getRepository('MilexUserBundle:User')->getUserList($filter, $limit, $start, ['lead' => 'leads']);
                break;
        }
    ...
    {
        return $this->em->getRepository('MilexUserBundle:User')->getUserList('', 0);
    }
    ...
    {
        $repo = $this->em->getRepository('MilexLeadBundle:CompanyLead');
    ...
    public function addToStages($lead, $stage, $manuallyAdded = true)
    {
        ...
        $lead->stageChangeLogEntry(
            ...
            $this->translator->trans('milex.stage.event.added.batch')
        );
    ...
    public function removeFromStages($lead, $stage, $manuallyRemoved = true)
    {
        ...
        $lead->stageChangeLogEntry(
            $stage,
            $stage->getId().': '.$stage->getName(),
            $this->translator->trans('milex.stage.event.removed.batch')
        );
    ...
    public function getFrequencyRules(Lead $lead, $channel = null)
    {
        ...
        /** @var \Milex\LeadBundle\Entity\FrequencyRuleRepository $frequencyRuleRepo */
        $frequencyRuleRepo = $this->em->getRepository('MilexLeadBundle:FrequencyRule');
    ...
    public function setFrequencyRules(Lead $lead, $data, $leadLists, $persist = true)
    {
        ...
        if (!empty($entities)) {
            $this->em->getRepository('MilexLeadBundle:FrequencyRule')->saveEntities($entities);
        }
        ...
        if (!empty($deleted)) {
            $this->em->getRepository('MilexLeadBundle:FrequencyRule')->deleteEntities($deleted);
        }
    ...
    public function import($fields, $data, $owner = null, $list = null, $tags = null, $persist = true, LeadEventLog $eventLog = null, $importId = null, $skipIfExists = false)
    {
        ...
        if (!empty($fields['points']) && !empty($data[$fields['points']]) && null === $lead->getId()) {
            ...
            $log->setLead($lead);
            $log->setType('lead');
            $log->setEventName($this->translator->trans('milex.lead.import.event.name'));
            $log->setActionName($this->translator->trans('milex.lead.import.action.name', [
        ...
        if (!empty($fields['stage']) && !empty($data[$fields['stage']])) {
            ...
            if (!array_key_exists($stageName, $stages)) {
                ...
                $stage = $this->em->getRepository('MilexStageBundle:Stage')->getStageByName($stageName);
            ...
            $log->setActionName(
                $this->translator->trans(
                    'milex.stage.import.action.name',
                    [
                        '%name%' => $this->userHelper->getUser()->getUsername(),
                    ]
                )
        ...
        if (!empty($fields['doNotEmail']) && isset($data[$fields['doNotEmail']]) && (!empty($fields['email']) && !empty($data[$fields['email']]))) {
            ...
            if (null !== $doNotEmail) {
                $reason = $this->translator->trans('milex.lead.import.by.user', [
        ...
        if (null !== $owner) {
            $lead->setOwner($this->em->getReference('MilexUserBundle:User', $owner));
        }
        ...
        if (!empty($tags)) {
            foreach ($tags as $tag) {
                if (is_numeric($tag)) {
                    ...
                    $lead->addTag(
                        $this->em->getReference('MilexLeadBundle:Tag', $tag)
                    );
                } else {
                    $lead->addTag(
    ...
    public function getLeadsLineChartData($unit, $dateFrom, $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        $flag        = null;
        $topLists    = null;
        $allLeadsT   = $this->translator->trans('milex.lead.all.leads');
        $identifiedT = $this->translator->trans('milex.lead.identified');
        $anonymousT  = $this->translator->trans('milex.lead.lead.anonymous');
    ...
    public function getAnonymousVsIdentifiedPieChartData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $chart->setDataset($this->translator->trans('milex.lead.identified'), $identified);
        $chart->setDataset($this->translator->trans('milex.lead.lead.anonymous'), ($all - $identified));
    ...
    public function getLeadMapData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $q->select('COUNT(t.id) as quantity, t.country')
            ->from(MILEX_TABLE_PREFIX.'leads', 't')
            ->groupBy('t.country')
    ...
    public function getTopOwners($limit = 10, $dateFrom = null, $dateTo = null, $filters = [])
    {
        ...
        $q->select('COUNT(t.id) AS leads, t.owner_id, u.first_name, u.last_name')
            ->from(MILEX_TABLE_PREFIX.'leads', 't')
            ->join('t', MILEX_TABLE_PREFIX.'users', 'u', 'u.id = t.owner_id')
    ...
    protected function createNewContact(IpAddress $ip, $persist = true)
    {
        ...
        if ($persist && !defined('MILEX_NON_TRACKABLE_REQUEST')) {
    ...
    /**
     * @deprecated 2.12.0 to be removed in 3.0; use Milex\LeadBundle\Model\DoNotContact instead
     *
     * @param string $channel
     *
     * @return int
     *
     * @see \Milex\LeadBundle\Entity\DoNotContact This method can return boolean false, so be
    ...
    public function isContactable(Lead $lead, $channel)
    {
        ...
        /** @var \Milex\LeadBundle\Entity\DoNotContact[] $entries */
     ...
     *
     * @deprecated 2.13.0; to be removed in 3.0. Use \Milex\LeadBundle\Deduplicate\ContactMerger instead
     *
