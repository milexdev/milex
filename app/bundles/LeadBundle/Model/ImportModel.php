...
class ImportModel extends FormModel
{
    ...
    public function setGhostImportsAsFailed()
    {
        ...
        foreach ($imports as $import) {
            $import->setStatus($import::FAILED)
                ->setStatusInfo($this->translator->trans('milex.lead.import.ghost.limit.hit', ['%limit%' => $ghostDelay]))
                ->removeFile();

            if ($import->getCreatedBy()) {
                $this->notificationModel->addNotification(
                    $this->translator->trans(
                        'milex.lead.import.result.info',
                        ['%import%' => $this->generateLink($import)]
                    ),
                    'info',
                    false,
                    $this->translator->trans('milex.lead.import.failed'),
                    'fa-download',
                    null,
                    $this->em->getReference('MilexUserBundle:User', $import->getCreatedBy())
                );
            }
        }
    ...
    public function beginImport(Import $import, Progress $progress, $limit = 0)
    {
        ...
        if (!$this->checkParallelImportLimit()) {
            $info = $this->translator->trans(
                'milex.lead.import.parallel.limit.hit',
        ...
        try {
            ...
            $info = $this->translator->trans(
                'milex.lead.import.database.exception',
        ...
        if ($import->getCreatedBy()) {
            $this->notificationModel->addNotification(
                ...
                'info',
                false,
                $this->translator->trans('milex.lead.import.completed'),
                'fa-download',
                null,
                $this->em->getReference('MilexUserBundle:User', $import->getCreatedBy())
            );
    ...
    public function process(Import $import, Progress $progress, $limit = 0)
    {
        ...
        while ($batchSize && !$file->eof()) {
            ...
            if ($this->isEmptyCsvRow($data)) {
                $errorMessage = 'milex.lead.import.error.line_empty';
            }

            if ($this->hasMoreValuesThanColumns($data, $headerCount)) {
                $errorMessage = 'milex.lead.import.error.header_mismatch';
            }
    ...
    public function getImportedRowsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [])
    {
        ...
        $chart->setDataset($this->translator->trans('milex.lead.import.processed.rows'), $data);
    ...
    {
        return $this->em->getRepository('MilexLeadBundle:Import');
    }
