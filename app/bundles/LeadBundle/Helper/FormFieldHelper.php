...
namespace Milex\LeadBundle\Helper;
...
class FormFieldHelper extends AbstractFormFieldHelper
{
    ...
    private static $types = [
        ...
        'multiselect' => [
            'properties' => [
                'list' => [
                    'required'  => true,
                    'error_msg' => 'milex.lead.field.select.listmissing',
                ],
            ],
        ],
        'select' => [
        ...
        'boolean' => [
            'properties' => [
                'yes' => [
                    'required'  => true,
                    'error_msg' => 'milex.lead.field.boolean.yesmissing',
                ],
                'no' => [
                    'required'  => true,
                    'error_msg' => 'milex.lead.field.boolean.nomissing',
                ],
            ],
        ],
        'lookup' => [
    ...
    {
        $this->translationKeyPrefix = 'milex.lead.field.type.';
    }
    ...
    public static function validateProperties($type, &$properties)
    {
        if (!array_key_exists($type, self::$types)) {
            ...
            return [false, 'milex.lead.field.typenotrecognized'];
        }
    ...
    {
        return [
            'anniversary' => $this->translator->trans('milex.campaign.event.timed.choice.anniversary'),
            '+P0D'        => $this->translator->trans('milex.campaign.event.timed.choice.today'),
            '-P1D'        => $this->translator->trans('milex.campaign.event.timed.choice.yesterday'),
            '+P1D'        => $this->translator->trans('milex.campaign.event.timed.choice.tomorrow'),
        ];
    }
}
