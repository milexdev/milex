...
namespace Milex\CampaignBundle\Entity;

use Doctrine\DBAL\Cache\QueryCacheProfile;
use Doctrine\DBAL\Types\Types;
use Doctrine\ORM\Query\Expr;
use Milex\CampaignBundle\Entity\Result\CountResult;
...
class CampaignRepository extends CommonRepository
{
    ...
    public function getEntities(array $args = [])
    {
        $q = $this->getEntityManager()
            ...
            ->from('MilexCampaignBundle:Campaign', $this->getTableAlias(), $this->getTableAlias().'.id')
    ...
    public function deleteEntity($entity, $flush = true)
    {
        ...
        $q->update(MILEX_TABLE_PREFIX.'campaign_events')
            ->set('parent_id', ':null')
        ...
        // Delete events
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder();
        $q->delete(MILEX_TABLE_PREFIX.'campaign_events')
            ->where('campaign_id = '.$entity->getId())
            ->execute();
    ...
    public function getPublishedCampaigns($specificId = null, $leadId = null, $forList = false, $viewOther = false)
    {
        $q = $this->getEntityManager()->createQueryBuilder()
            ->from('MilexCampaignBundle:Campaign', 'c', 'c.id');
    ...
    public function getPublishedCampaignsByLeadLists($leadLists)
    {
        ...
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'campaigns', 'c');

        $q->join('c', MILEX_TABLE_PREFIX.'campaign_leadlist_xref', 'll', 'c.id = ll.campaign_id')
            ->where($this->getPublishedByDateExpression($q));
    ...
    {
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ->from(MILEX_TABLE_PREFIX.'campaign_leadlist_xref', 'cl');
    ...
    public function getCampaignListSources($id)
    {
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'campaign_leadlist_xref', 'cl')
            ->join('cl', MILEX_TABLE_PREFIX.'lead_lists', 'l', 'l.id = cl.leadlist_id');
        $q->where(
            $q->expr()->eq('cl.campaign_id', $id)
        );
    ...
    public function getCampaignFormSources($id)
    {
        $q = $this->getEntityManager()->getConnection()->createQueryBuilder()
            ...
            ->from(MILEX_TABLE_PREFIX.'campaign_form_xref', 'cf')
            ->join('cf', MILEX_TABLE_PREFIX.'forms', 'f', 'f.id = cf.form_id');
        $q->where(
    ...
    public function getPopularCampaigns($limit = 10)
    {
        ...
        $q->select('count(cl.ip_id) as hits, c.id AS campaign_id, c.name')
            ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'cl')
            ->leftJoin('cl', MILEX_TABLE_PREFIX.'campaigns', 'c', 'cl.campaign_id = c.id')
            ->orderBy('hits', 'DESC')
            ->groupBy('c.id, c.name')
            ->setMaxResults($limit);
    ...
    public function getCountsForPendingContacts($campaignId, array $pendingEvents, ContactLimiter $limiter)
    {
        ...
        $q->select('min(cl.lead_id) as min_id, max(cl.lead_id) as max_id, count(cl.lead_id) as the_count')
            ->from(MILEX_TABLE_PREFIX.'campaign_leads', 'cl')
            ->where(
                $q->expr()->andX(
        ...
        if (count($pendingEvents) > 0) {
            ...
            $sq->select('null')
                ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'e')
                ->where(
                    $sq->expr()->andX(
    ...
    public function getContactSingleSegmentByCampaign($contactId, $campaignId)
    {
        ...
        return $q->select('ll.id, ll.name')
            ->from(MILEX_TABLE_PREFIX.'lead_lists', 'll')
            ->join('ll', MILEX_TABLE_PREFIX.'lead_lists_leads', 'lll', 'lll.leadlist_id = ll.id and lll.lead_id = :contactId and lll.manually_removed = 0')
            ->join('ll', MILEX_TABLE_PREFIX.'campaign_leadlist_xref', 'clx', 'clx.leadlist_id = ll.id and clx.campaign_id = :campaignId')
    ...
    public function getCampaignsSegmentShare($segmentId, $campaignIds = [])
    {
        ...
        $q->from(MILEX_TABLE_PREFIX.'campaigns', 'c')
            ->leftJoin('c', MILEX_TABLE_PREFIX.'campaign_leads', 'cl', 'cl.campaign_id = c.id AND cl.manually_removed = 0')
            ->leftJoin('cl',
                '(SELECT lll.lead_id AS ll, lll.lead_id FROM '.MILEX_TABLE_PREFIX.'lead_lists_leads lll WHERE lll.leadlist_id = '.$segmentId
