...
class LeadEventLogRepository extends CommonRepository
{
    ...
    public function getLeadLogs($leadId = null, array $options = [])
    {
        $query = $this->getEntityManager()
                    ...
                    '
                        ...
                        ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'll')
                        ->join('ll', MILEX_TABLE_PREFIX.'campaign_events', 'e', 'll.event_id = e.id')
                        ->join('ll', MILEX_TABLE_PREFIX.'campaigns', 'c', 'll.campaign_id = c.id')
                        ->leftJoin('ll', MILEX_TABLE_PREFIX.'campaign_lead_event_failed_log', 'fl', 'fl.log_id = ll.id')
    ...
    public function getUpcomingEvents(array $options = null)
    {
        ...
        $query->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'll')
            ->select('ll.event_id,
                    ll.campaign_id,
                    ll.trigger_date,
                    ll.lead_id,
            ...
            ->leftJoin('ll', MILEX_TABLE_PREFIX.'campaign_events', 'e', 'e.id = ll.event_id')
            ->leftJoin('ll', MILEX_TABLE_PREFIX.'campaigns', 'c', 'c.id = e.campaign_id')
            ->leftJoin('ll', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = ll.lead_id')
            ->where($query->expr()->eq('ll.is_scheduled', 1));

        if (isset($options['lead'])) {
            /** @var \Milex\CoreBundle\Entity\IpAddress $ip */
    ...
    public function getCampaignLogCounts(
        ...
        $q->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'o');
        $q->$join(
            'o',
            MILEX_TABLE_PREFIX.'campaign_leads',
            'l',
        ...
        $failedSq->select('null')
            ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_failed_log', 'fe')
            ->where(
    ...
    public function updateLead($fromLeadId, $toLeadId)
    {
        ...
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MILEX_TABLE_PREFIX.'campaign_lead_event_log')
        ...
        if (!empty($exists)) {
            ...
            $this->_em->getConnection()->createQueryBuilder()
                ->delete(MILEX_TABLE_PREFIX.'campaign_lead_event_log')
                ->where('lead_id = '.(int) $fromLeadId)
                ->execute();
        } else {
            $q->execute();
    ...
    public function getChartQuery($options)
    {
        ...
        $query->select('ll.id, ll.date_triggered')
            ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'll')
            ->join('ll', MILEX_TABLE_PREFIX.'campaign_events', 'e', 'e.id = ll.event_id');
    ...
    public function getScheduledCounts($campaignId, \DateTime $date, ContactLimiter $limiter)
    {
        ...
        $results = $q->select('COUNT(*) as event_count, l.event_id')
            ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'l')
            ->join('l', MILEX_TABLE_PREFIX.'campaigns', 'c', 'l.campaign_id = c.id')
            ->where($expr)
    ...
    public function getDatesExecuted($eventId, array $contactIds)
    {
        ...
        $qb->select('log.lead_id, log.date_triggered, log.is_scheduled')
            ->from(MILEX_TABLE_PREFIX.'campaign_lead_event_log', 'log')
            ->where(
                $qb->expr()->andX(
...
SQL;
        ...
        $qb->update(MILEX_TABLE_PREFIX.'campaign_lead_event_log')
            ->set('is_scheduled', 0)
            ->where(
                $qb->expr()->andX(
    ...
    public function removeEventLogs($eventId)
    {
        ...
        $conn->delete(MILEX_TABLE_PREFIX.'campaign_lead_event_log', [
