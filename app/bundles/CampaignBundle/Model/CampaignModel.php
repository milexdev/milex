...
namespace Milex\CampaignBundle\Model;
...
use Milex\CampaignBundle\Entity\Lead as CampaignLead;
...
use Milex\CampaignBundle\Form\Type\CampaignType;
use Milex\CampaignBundle\Helper\ChannelExtractor;
...
class CampaignModel extends CommonFormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \Milex\CampaignBundle\Entity\CampaignRepository
     */
    public function getRepository()
    {
        $repo = $this->em->getRepository('MilexCampaignBundle:Campaign');
    ...
    /**
     * @return \Milex\CampaignBundle\Entity\EventRepository
     */
    public function getEventRepository()
    {
        return $this->em->getRepository('MilexCampaignBundle:Event');
    }

    /**
     * @return \Milex\CampaignBundle\Entity\LeadRepository
     */
    public function getCampaignLeadRepository()
    {
        return $this->em->getRepository('MilexCampaignBundle:Lead');
    }

    /**
     * @return \Milex\CampaignBundle\Entity\LeadEventLogRepository
     */
    public function getCampaignLeadEventLogRepository()
    {
        return $this->em->getRepository('MilexCampaignBundle:LeadEventLog');
    }
    ...
    {
        if ($entity instanceof \Milex\CampaignBundle\Entity\Lead) {
            return;
        }
    ...
    public function setLeadSources(Campaign $entity, $addedSources, $deletedSources)
    {
        foreach ($addedSources as $type => $sources) {
            foreach ($sources as $id => $label) {
                switch ($type) {
                    case 'lists':
                        $entity->addList($this->em->getReference('MilexLeadBundle:LeadList', $id));
                        break;
                    case 'forms':
                        $entity->addForm($this->em->getReference('MilexFormBundle:Form', $id));
                        break;
                    default:
                        break;
                }
        ...
        foreach ($deletedSources as $type => $sources) {
            foreach ($sources as $id => $label) {
                switch ($type) {
                    case 'lists':
                        $entity->removeList($this->em->getReference('MilexLeadBundle:LeadList', $id));
                        break;
                    case 'forms':
                        $entity->removeForm($this->em->getReference('MilexFormBundle:Form', $id));
                        break;
                    default:
                        break;
                }
    ...
    public function getLeadDetails($campaign, $leads = null)
    {
        ...
        return $this->em->getRepository('MilexCampaignBundle:Lead')->getLeadDetails($campaignId, $leads);
    }
    ...
    public function getCampaignLeads($campaign, $event = null)
    {
        ...
        return $this->em->getRepository('MilexCampaignBundle:Lead')->getLeads($campaignId, $eventId);
    }
    ...
    public function getLeadsAddedLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$canViewOthers) {
            $q->join('t', MILEX_TABLE_PREFIX.'campaigns', 'c', 'c.id = c.campaign_id')
