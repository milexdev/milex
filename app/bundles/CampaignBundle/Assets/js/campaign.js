 ...
 * @param container
 */
Milex.campaignOnLoad = function (container, response) {
    Milex.lazyLoadContactListOnCampaignDetail();

    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'campaign');
    }
    ...
    if (mQuery('#CampaignEventPanel').length) {
        ...
        mQuery('#CampaignEventPanelGroups button').on('click', function() {
            ...
            Milex.campaignBuilderUpdateEventList([eventType], false, 'lists', true);
        });

        mQuery('#CampaignEventPanelLists button').on('click', function() {
            Milex.campaignBuilderUpdateEventList(Milex.campaignBuilderAnchorClickedAllowedEvents, true, 'groups', true);
        });
        ...
        mQuery('.campaign-event-selector').on('chosen:showing_dropdown', function (event) {
            ...
            Milex.campaignBuilderUpdateEventListTooltips(thisSelect, false);

            mQuery('#'+thisSelect+'_chosen .chosen-search input').on('keydown.tooltip', function () {
                // Destroy tooltips that are filtered out
                Milex.campaignBuilderUpdateEventListTooltips(thisSelect, true);
        ...
        mQuery('.campaign-event-selector').on('change', function() {
            ...
            if (option.attr('data-href') && Milex.campaignBuilderAnchorNameClicked) {
                var updatedUrl = option.attr('data-href').replace(/anchor=(.*?)$/, "anchor=" + Milex.campaignBuilderAnchorNameClicked + "&anchorEventType=" + Milex.campaignBuilderAnchorEventTypeClicked);
...
Milex.lazyLoadContactListOnCampaignDetail = function() {
 ...
 * @param onlyDestroy
 */
Milex.campaignBuilderUpdateEventListTooltips = function(theSelect, onlyDestroy) {
 ...
 */
Milex.campaignOnUnload = function(container) {
    delete Milex.campaignBuilderInstance;
    delete Milex.campaignBuilderLabels;
}
 ...
 * @param response
 */
Milex.campaignEventOnLoad = function (container, response) {
    if (mQuery('#campaignevent_triggerHour').length) {
        Milex.campaignEventShowHideIntervalSettings();
        Milex.campaignEventUpdateIntervalHours();
        mQuery('#campaignevent_triggerHour').on('change', Milex.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerRestrictedStartHour').on('change', Milex.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerRestrictedStopHour').on('change', Milex.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerIntervalUnit').on('change', Milex.campaignEventShowHideIntervalSettings);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_0').on('change', Milex.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_1').on('change', Milex.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_2').on('change', Milex.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_3').on('change', Milex.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_4').on('change', Milex.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_7').on('change', Milex.campaignEventSelectDOW);
    }
    ...
    Milex.campaignBuilderLabels[domEventId] = (response.label) ? response.label : '';

    if (!response.success && Milex.campaignBuilderConnectionRequiresUpdate) {
        // Modal exited - check to see if a connection needs to be removed
        Milex.campaignBuilderInstance.deleteConnection(Milex.campaignBuilderLastConnection);
    }
    Milex.campaignBuilderConnectionRequiresUpdate = false;
    Milex.campaignBuilderUpdateLabel(domEventId);
    Milex.campaignBuilderCanvasEvents[response.event.id] = response.event;

    if (response.deleted) {
        Milex.campaignBuilderInstance.remove(document.getElementById(domEventId));
        delete Milex.campaignBuilderEventPositions[domEventId];
        delete Milex.campaignBuilderCanvasEvents[response.event.id];
        ...
        Milex.campaignBuilderEventPositions[domEventId] = {
            'left': x,
            'top': y
        };
        ...
        Milex.campaignBuilderRegisterAnchors(Milex.getAnchorsForEvent(response.event), eventId);
        Milex.campaignBuilderInstance.draggable(domEventId, Milex.campaignDragOptions);
        ...
        Milex.campaignBuilderInstance.connect({
            uuids: [
                Milex.campaignBuilderAnchorClicked,
                domEventId+'_top'
            ]
        });
    }

    Milex.campaignBuilderInstance.repaintEverything();
};

/**
 * Update the trigger hour based on the interval unit selected
 */
Milex.campaignEventUpdateIntervalHours = function () {
 ...
 */
Milex.campaignEventShowHideIntervalSettings = function() {
 ...
 */
Milex.campaignEventSelectDOW = function() {
 ...
 * @param event
 */
Milex.getAnchorsForEvent = function (event) {
    var restrictions = Milex.campaignBuilderConnectionRestrictions[event.type].target;
 ...
 * @param response
 */
Milex.campaignSourceOnLoad = function (container, response) {
    //new action created so append it to the form
    ...
    if (response.deleted) {
        ...
        if (!mQuery('.list-campaign-source:not(#CampaignEvent_newsource_hide)').length) {
            ...
            Milex.campaignBuilderPrepareNewSource();
        }
        ...
        Milex.campaignBuilderRegisterAnchors(['leadSource', 'leadSourceLeft', 'leadSourceRight'], eventId);
        Milex.campaignBuilderInstance.draggable(domEventId, Milex.campaignDragOptions);
        ...
        if (autoConnect) {
            // Connect into last anchor clicked
            if (Milex.campaignBuilderAnchorClicked.search('left') !== -1) {
                var source = domEventId + '_leadsourceright';
                var target = Milex.campaignBuilderAnchorClicked;
            } else {
                var source = Milex.campaignBuilderAnchorClicked;
 ...
 * @param domEventId
 */
Milex.campaignBuilderUpdateLabel = function (domEventId) {
    var theLabel = typeof Milex.campaignBuilderLabels[domEventId] == 'undefined' ? '' : Milex.campaignBuilderLabels[domEventId];
    var currentConnections = Milex.campaignBuilderInstance.select({
        target: domEventId
    });
 ...
 */
Milex.launchCampaignEditor = function() {
    ...
    if (Milex.campaignBuilderCanvasSettings) {
        Milex.campaignBuilderInstance.setSuspendDrawing(true);
        Milex.campaignBuilderReconnectEndpoints();
        Milex.campaignBuilderInstance.setSuspendDrawing(false, true);
    }
    Milex.campaignBuilderInstance.repaintEverything();
};
 ...
 */
Milex.launchCampaignPreview = function() {
    Milex.stopIconSpinPostEvent();
 ...
 */
Milex.campaignBuilderConnectionsMap = {
    // source
    'source': {
        // source anchors
        'leadsource': {
...
Milex.campaignBuilderAnchorDefaultColor = '#d5d4d4';

Milex.campaignEndpointDefinitions = {
    'top': {
 ...
 */
Milex.campaignConnectionCallbacks = {
...
Milex.campaignBuilderAnchorClicked = false;
Milex.campaignBuilderEventPositions = {};

Milex.prepareCampaignCanvas = function() {
    if (typeof Milex.campaignBuilderInstance == 'undefined') {
        Milex.campaignBuilderInstance = jsPlumb.getInstance({
            Container: document.querySelector("#CampaignCanvas")
        });

        Milex.campaignEndpoints = {};

        var startingPosition;
        Milex.campaignDragOptions = {
            ...
            stop: function (params) {
                ...
                if (startingPosition.left !== endingPosition.left || startingPosition.top !== endingPosition.top) {
                    //update coordinates
                    Milex.campaignBuilderEventPositions[mQuery(params.el).attr('id')] = {
        ...
        Milex.campaignBuilderEventDimensions = {
            'width': 200,
            'height': 45,
            'anchor': 10,
            'wiggleWidth': 30,
        ...
        // Store labels
        Milex.campaignBuilderLabels = {};

        // Update the labels on connection/disconnection
        Milex.campaignBuilderInstance.bind("connection", function (info, originalEvent) {
            // Mark the connection so it can be removed if the form is cancelled
            Milex.campaignBuilderConnectionRequiresUpdate = false;
            Milex.campaignBuilderLastConnection           = info.connection;

            // If there is a switch between active/inactive anchors, reload the form
            var epDetails          = Milex.campaignBuilderGetEndpointDetails(info.sourceEndpoint);
            ...
            if (editUrl) {
                ...
                if (previousConnection && previousConnection != epDetails.anchorName && (previousConnection == 'no' || epDetails.anchorName == 'no')) {
                    ...
                    Milex.campaignBuilderConnectionRequiresUpdate = true;
            ...
            Milex.campaignBuilderUpdateLabel(info.connection.targetId);
        ...
        Milex.campaignBuilderInstance.bind("connectionDetached", function (info, originalEvent) {
            Milex.campaignBuilderUpdateLabel(info.connection.targetId);
        ...
        Milex.campaignBuilderInstance.bind("connectionMoved", function (info, originalEvent) {
            Milex.campaignBuilderUpdateLabel(info.connection.originalTargetId);
            ...
            Milex.campaignBuilderUpdateLabel(info.connection.newTargetId);
        ...
        mQuery.each(Milex.campaignConnectionCallbacks.beforeEndpointsRegistered, function (index, callback) {
            callback();
        });
        mQuery.each(Milex.campaignEndpointDefinitions, function (ep, definition) {
            Milex.campaignBuilderRegisterEndpoint(ep, definition);
        });

        //manually loop through each so a UUID can be set for reconnecting connections
        mQuery.each(Milex.campaignConnectionCallbacks.beforeAnchorsRegistered, function (index, callback) {
            callback();
        });

        mQuery("#CampaignCanvas div[data-event-id]").each(function () {
            var event = Milex.campaignBuilderCanvasEvents[mQuery(this).data('eventId')];

            Milex.campaignBuilderRegisterAnchors(Milex.getAnchorsForEvent(event), this);
        });
        ...
        mQuery("#CampaignCanvas div.list-campaign-event.list-campaign-source").not('#CampaignEvent_newsource').not('#CampaignEvent_newsource_hide').each(function () {
            Milex.campaignBuilderRegisterAnchors(['bottom'], this);
        });
        ...
        mQuery("#CampaignCanvas div.list-campaign-leadsource").not('#CampaignEvent_newsource').not('#CampaignEvent_newsource_hide').each(function () {
            Milex.campaignBuilderRegisterAnchors(['leadSource', 'leadSourceLeft', 'leadSourceRight'], this);
        });

        mQuery.each(Milex.campaignConnectionCallbacks.afterAnchorsRegistered, function (index, callback) {
            callback();
        });

        if (mQuery('.preview').length) {
            Milex.launchCampaignPreview();
        } else {
            //enable drag and drop
            Milex.campaignBuilderInstance.draggable(
 ...
 * @returns {boolean}
 */
Milex.campaignBeforeDropCallback = function(params) {
    var sourceEndpoint = Milex.campaignBuilderGetEndpointDetails(params.connection.endpoints[0]);
    var targetEndpoint = Milex.campaignBuilderGetEndpointDetails(params.dropEndpoint);

    var callbackAllowed = null;
    mQuery.each(Milex.campaignConnectionCallbacks.beforeDrop, function(index, callback) {
    ...
    if (!Milex.campaignBuilderValidateConnection(sourceEndpoint, targetEndpoint.eventType, targetEndpoint.event)){
    ...
    if (mQuery.inArray(targetEndpoint.anchorName, ['top', 'leadsourceleft', 'leadsourceright'])) {
        ...
        var sourceConnections = Milex.campaignBuilderInstance.select({
    ...
    var allowedConnections = Milex.campaignBuilderConnectionsMap[sourceEndpoint.eventType][sourceEndpoint.anchorName][targetEndpoint.eventType];
    ...
    if (allowed) {
        ...
        if (params.dropEndpoint.connections.length > 0) {
            ...
            mQuery.each(params.dropEndpoint.connections, function(key, conn) {
                Milex.campaignBuilderInstance.deleteConnection(conn);
            });
        }
    }
 ...
 * @returns {*}
 */
Milex.campaignBeforeDetachCallback = function(connection) {
    var sourceEndpoint = Milex.campaignBuilderGetEndpointDetails(connection.sourceId);
    var targetEndpoint = Milex.campaignBuilderGetEndpointDetails(connection.targetId);

    var callbackAllowed = null;
    mQuery.each(Milex.campaignConnectionCallbacks.beforeDetach, function (index, callback) {
 ...
 * @param connection
 */
Milex.campaignBeforeDragCallback = function(endpoint, source, sourceId) {
    var sourceEndpoint = Milex.campaignBuilderGetEndpointDetails(sourceId);
    var targetEndpoint = Milex.campaignBuilderGetEndpointDetails(endpoint);

    var callbackAllowed = null;
    mQuery.each(Milex.campaignConnectionCallbacks.beforeDrag, function (index, callback) {
 ...
 * @returns {*}
 */
Milex.campaignBeforeStartDetachCallback = function(endpoint, source, sourceId, connection) {
    var sourceEndpoint = Milex.campaignBuilderGetEndpointDetails(sourceId);
    var targetEndpoint = Milex.campaignBuilderGetEndpointDetails(endpoint);

    var callbackAllowed = null;
    mQuery.each(Milex.campaignConnectionCallbacks.beforeStartDetach, function (index, callback) {
 ...
 * @param connection
 */
Milex.campaignHoverCallback = function(sourceEndpoint, endpoint, event) {
    var callbackAllowed = null;
    mQuery.each(Milex.campaignConnectionCallbacks.onHover, function (index, callback) {
 ...
 */
Milex.campaignToggleTimeframes = function() {
 ...
 */
Milex.closeCampaignBuilder = function() {
    var builderCss = {
        margin: "0",
        padding: "0",
        border: "none",
    ...
    Milex.removeButtonLoadingIndicator(mQuery('.btn-apply-builder'));
    mQuery('#builder-errors').hide('fast').text('');

    Milex.updateConnections(function(err, response) {
...
Milex.saveCampaignFromBuilder = function() {
    Milex.activateButtonLoadingIndicator(mQuery('.btn-apply-builder'));
    Milex.updateConnections(function(err) {
        if (!err) {
            var applyBtn = mQuery('.btn-apply');
            Milex.inBuilderSubmissionOn(applyBtn.closest('form'));
            applyBtn.trigger('click');
            Milex.inBuilderSubmissionOff();
        }
    });
};

Milex.updateConnections = function(callback) {
    var nodes = [];
    ...
    var connections = [];
    mQuery.each(Milex.campaignBuilderInstance.getConnections(), function (idx, connection) {
        connections.push({
            ...
            anchors:      mQuery.map(connection.endpoints, function (endpoint) {
                var anchor = Milex.campaignBuilderGetEndpointDetails(endpoint);
                return {
    ...
    mQuery.ajax({
        ...
        error: function (response, textStatus, errorThrown) {
            Milex.processAjaxError(response, textStatus, errorThrown);
 ...
 * @param e
 */
Milex.submitCampaignEvent = function(e) {
    e.preventDefault();
 ...
 * @param e
 */
Milex.submitCampaignSource = function(e) {
    e.preventDefault();
 ...
 */
Milex.campaignBuilderReconnectEndpoints = function () {

    mQuery.each(Milex.campaignConnectionCallbacks.beforeEndpointsReconnected, function (index, callback) {
        callback();
    });

    if (typeof Milex.campaignBuilderCanvasSettings == 'undefined') {
        return;
    }

    if (typeof Milex.campaignBuilderCanvasSettings.nodes !== 'undefined') {
        // Reposition events
        var sourceFound = false;
        mQuery.each(Milex.campaignBuilderCanvasSettings.nodes, function (key, node) {
            if (typeof Milex.campaignBuilderCanvasSources[node.id] !== 'undefined') {
                sourceFound = true;
            }
            ...
            Milex.campaignBuilderEventPositions['CampaignEvent_' + node.id] = {
    ...
    if (typeof Milex.campaignBuilderCanvasSettings.connections !== 'undefined') {

        // Recreate jsPlumb connections and labels
        mQuery.each(Milex.campaignBuilderCanvasSettings.connections, function (key, connection) {
            if (typeof Milex.campaignBuilderCanvasEvents[connection.targetId] !== 'undefined') {
                var targetEvent = Milex.campaignBuilderCanvasEvents[connection.targetId];
            } else if (typeof Milex.campaignBuilderCanvasSources[connection.targetId] !== 'undefined') {
                var targetEvent = Milex.campaignBuilderCanvasSources[connection.targetId];
            }

            if (targetEvent && targetEvent.label) {
                Milex.campaignBuilderLabels["CampaignEvent_" + connection.targetId] = targetEvent.label;
            }
    ...
    if (!sourceFound) {
        var topOffset = 25;
        mQuery.each(Milex.campaignBuilderCanvasSources, function (type, source) {
    ...
    mQuery.each(Milex.campaignConnectionCallbacks.afterEndpointsReconnected, function (index, callback) {
        callback();
    });

    delete Milex.campaignBuilderCanvasSettings;
};
 ...
 * @param params
 */
Milex.campaignBuilderRegisterEndpoint = function (name, params) {
    var isTarget, isSource, color, connectorColor, connectorStyle;
    if (params.color) {
        color = params.color;
    } else {
        color = Milex.campaignBuilderAnchorDefaultColor;
    }
    ...
    Milex.campaignEndpoints[name] = {
        ...
        isTarget: isTarget,
        isSource: isSource,
        beforeDrop: Milex.campaignBeforeDropCallback,
        beforeDetach: Milex.campaignBeforeDetachCallback,
        beforeStartDetach: Milex.campaignBeforeStartDetachCallback,
        beforeDrag: Milex.campaignBeforeDragCallback
    }
};
 ...
 * @param el
 */
Milex.campaignBuilderRegisterAnchors = function(names, el) {
    var id = mQuery(el).attr('id');

    mQuery(names).each(function(key, anchorName) {
        var theAnchor = Milex.campaignEndpointDefinitions[anchorName]['anchors'];
        theAnchor[6] = anchorName.toLowerCase() + ' ' + id;
        var ep = Milex.campaignBuilderInstance.addEndpoint(
            id,
            {
                anchor: theAnchor,
                uuid: id + "_" + anchorName.toLowerCase()
            },
            Milex.campaignEndpoints[anchorName]
        );

        ep.bind("mouseover", function (endpoint, event) {
            var epDetails = Milex.campaignBuilderGetEndpointDetails(endpoint);

            if (!Milex.campaignHoverCallback(epDetails, endpoint, event)) {
                return;
            }
        ...
        ep.bind("mouseout", function (endpoint) {
            ...
            if (!endpoint.connections.length) {
                ...
                endpoint.setPaintStyle(
                    {
                        fill: Milex.campaignBuilderAnchorDefaultColor
                    }
                );
        ...
        ep.bind("click", function (endpoint, event) {
            ...
            Milex.campaignBuilderAnchorClicked = endpoint.elementId+'_'+clickedAnchorName;
            Milex.campaignBuilderAnchorNameClicked = clickedAnchorName;
            Milex.campaignBuilderAnchorEventTypeClicked = epDetails.eventType;

            // Get the position of the event
            var elPos = Milex.campaignBuilderGetEventPosition(endpoint.element);
            var spotFound = false,
                putLeft = elPos.left,
                putTop = elPos.top,
                direction = '', // xl, xr, yu, yd
                fullWidth = Milex.campaignBuilderEventDimensions.width + Milex.campaignBuilderEventDimensions.anchor,
                wiggleWidth = fullWidth + Milex.campaignBuilderEventDimensions.wiggleWidth,
                fullHeight = Milex.campaignBuilderEventDimensions.height + Milex.campaignBuilderEventDimensions.anchor,
                wiggleHeight = fullHeight + Milex.campaignBuilderEventDimensions.wiggleHeight,
                debug = false;

            if (debug) {
                console.log(Milex.campaignBuilderEventPositions);
            ...
            switch (clickedAnchorName) {
                ...
                case 'leadsource':
                    ...
                    putLeft -= Milex.campaignBuilderEventDimensions.width/2;
                    putTop  += wiggleHeight;
                    direction = 'xl';
                    break;
                case 'no':
            ...
            while (!spotFound) {
                ...
                mQuery.each(Milex.campaignBuilderEventPositions, function (id, pos) {
                    ...
                    if (h > 0 && w > 0) {
                        ...
                        switch (direction) {
                            case 'xl':
                                putLeft -= (w + Milex.campaignBuilderEventDimensions.wiggleWidth);
                                if (putLeft <= 0) {
                                    putLeft = 0;
                                    // Ran out of room so go down
                                    direction = 'yd';
                                    putTop += fullHeight + Milex.campaignBuilderEventDimensions.wiggleHeight;
                                }
                                break;
                            case 'xr':
                                if (putLeft + w + Milex.campaignBuilderEventDimensions.wiggleWidth > windowWidth) {
                                    // Hit right canvas so start going down by default
                                    direction = 'yd';
                                    putLeft -= Milex.campaignBuilderEventDimensions.wiggleWidth;
                                    putTop += fullHeight + Milex.campaignBuilderEventDimensions.wiggleHeight;
                                } else {
                                    putLeft += (w + Milex.campaignBuilderEventDimensions.wiggleWidth);
                                }
                                break;
                            case 'yu':
                                putTop -= (h - Milex.campaignBuilderEventDimensions.wiggleHeight);
                                if (putTop <= 0) {
                                    putTop = 0;
            ...
            var allowedEvents = [];
            mQuery.each(Milex.campaignBuilderConnectionsMap[epDetails.eventType][epDetails.anchorName], function (group, eventTypes) {
                if (eventTypes.length) {
                    allowedEvents[allowedEvents.length] = group.charAt(0).toUpperCase() + group.substr(1);
                }
            });
            Milex.campaignBuilderAnchorClickedAllowedEvents = allowedEvents;

            if (!(mQuery('.preview').length)) {
                var el = (mQuery(event.target).hasClass('jtk-endpoint')) ? event.target : mQuery(event.target).parents('.jtk-endpoint')[0];
                Milex.campaignBuilderAnchorClickedPosition = Milex.campaignBuilderGetEventPosition(el);
                Milex.campaignBuilderUpdateEventList(allowedEvents, false, 'groups');
            }
            ...
            mQuery.each(checkSelects, function(key, targetType) {
                ...
                mQuery(selectId + ' option').each(function () {
                    ...
                    if (optionVal) {
                        if (!Milex.campaignBuilderValidateConnection(epDetails, targetType, optionVal)) {
 ...
 */
Milex.campaignBuilderGetEventPosition = function(el) {
    return {
 ...
 * @param forcePosition
 */
Milex.campaignBuilderUpdateEventList = function (groups, hidden, view, active, forcePosition) {
    var groupsEnabled = 0;
    ...
    if (inGroupsView) {
        ...
        var leftPos = (forcePosition) ? forcePosition.left : Milex.campaignBuilderAnchorClickedPosition.left - (newWidth / 2 - 10);
        var topPos  = (forcePosition) ? forcePosition.top : Milex.campaignBuilderAnchorClickedPosition.top + 25;
        ...
        var leftPos = (forcePosition) ? forcePosition.left : Milex.campaignBuilderAnchorClickedPosition.left - 125;
        var topPos  = (forcePosition) ? forcePosition.top : Milex.campaignBuilderAnchorClickedPosition.top + 25;
        mQuery('#CampaignEventPanel').css({
            left: (leftPos >= 0) ? leftPos : 10,
            top: topPos,
 ...
 */
Milex.campaignBuilderGetEndpointDetails = function(endpoint) {
 ...
 */
Milex.campaignBuilderPrepareNewSource = function () {
    var newSourcePos = {
    ...
    Milex.campaignBuilderUpdateEventList(['Source'], false, 'list', false, {
 ...
 * @returns {boolean}
 */
Milex.campaignBuilderValidateConnection = function (epDetails, targetType, targetEvent) {
    var valid = true;
    var sourceType  = epDetails.eventType;
    var sourceEvent = 'source' === sourceType ? sourceType : epDetails.event;

    if (typeof Milex.campaignBuilderConnectionRestrictions[targetEvent] !== 'undefined') {
        if ('source' === sourceEvent) {
            // If there are any restrictions, then don't allow it to be the target of the campaign source
            mQuery.each(Milex.campaignBuilderConnectionRestrictions[targetEvent]['source'], function(eventType, events) {
                if (events.length) {
                    valid = false;
        ...
        if (
            typeof Milex.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType] !== 'undefined' &&
            Milex.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType].length &&
            mQuery.inArray(sourceEvent, Milex.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType]) === -1
        ) {
    ...
    if (
        typeof Milex.campaignBuilderConnectionRestrictions[sourceEvent] !== 'undefined' &&
        typeof Milex.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType] !== 'undefined' &&
        Milex.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType].length
    ) {
        // If the target event is defined in the target list of the source event, then allow it; otherwise don't allow it
        valid = (mQuery.inArray(targetEvent, Milex.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType]) !== -1);
    }

    if (
        typeof Milex.campaignBuilderConnectionRestrictions['anchor'][sourceType] !== 'undefined' &&
        typeof Milex.campaignBuilderConnectionRestrictions['anchor'][sourceType][targetEvent] !== 'undefined'
    ) {
        mQuery(Milex.campaignBuilderConnectionRestrictions['anchor'][sourceType][targetEvent]).each(
            function(key, anchor) {
                switch (anchor) {
                    case 'inaction':
                        anchor = 'no';
 ...
 * @param contactId
 */
Milex.updateScheduledCampaignEvent = function(eventId, contactId) {
    ...
    var input = mQuery('<input type="text" id="timeline-reschedule"/>')
        ...
        .on('keyup', function(e) {
            ...
            if (code == 13) {
                ...
                mQuery(input).datetimepicker('destroy');
                Milex.ajaxActionRequest('campaign:updateScheduledCampaignEvent',
                    {
                        eventId: eventId,
                        contactId: contactId,
    ...
    Milex.activateDateTimeInputs('#timeline-reschedule');
 ...
 * @param contactId
 */
Milex.saveScheduledCampaignEvent = function (eventId, contactId) {
    var saveButton = '#timeline-campaign-event-save-' + eventId;
 ...
 * @param contactId
 */
Milex.cancelScheduledCampaignEvent = function(eventId, contactId) {
    mQuery('#timeline-campaign-event-'+eventId+' .btn').prop('disabled', true).addClass('disabled');
    var eventWrapper = '#timeline-campaign-event-'+eventId;
    var eventSpan = '.timeline-campaign-event-date-' + eventId;
    var eventText = '#timeline-campaign-event-text-' + eventId;
    Milex.ajaxActionRequest('campaign:cancelScheduledCampaignEvent',
        {
            eventId: eventId,
            contactId: contactId,
        }, function (response) {
 ...
 */
Milex.updateJumpToEventOptions = function() {
    var jumpToEventSelectNode = mQuery("#campaignevent_properties_jumpToEvent");

    jumpToEventSelectNode.children().remove();

    for (var eventId in Milex.campaignBuilderCanvasEvents) {
        var event = Milex.campaignBuilderCanvasEvents[eventId];
...
Milex.highlightJumpTarget = function(event, el) {
 ...
 */
Milex.showCampaignConfirmation = function (el) {
    let element = mQuery(el);
    if (element.prop('checked') && element.val() !== "1") {
        Milex.showConfirmation(element);
    }
};
 ...
 */
Milex.setPublishedButtonToYes = function (el) {
 ...
 */
Milex.confirmationCampaignPublishStatus = function (el) {
    let element = mQuery(el);
    ...
    if (element.data('status') === 'published') {
        ...
        Milex.confirmCallbackCampaignPublishStatus('', el);
    }
}
 ...
 */
Milex.confirmCallbackCampaignPublishStatus = function (action, el) {
    let element = mQuery(el);
    ...
    Milex.togglePublishStatus(event, idClass, model, itemId, query, backdrop);
