...
class ReportSubscriber implements EventSubscriberInterface
{
    ...
    const DNC_COLUMNS = [
        'unsubscribed' => [
            'alias'   => 'unsubscribed',
            'label'   => 'milex.email.report.unsubscribed',
            'type'    => 'string',
            'formula' => 'IFNULL((SELECT ROUND(SUM(IF('.self::DNC_PREFIX.'.id IS NOT NULL AND '.self::DNC_PREFIX.'.channel_id='.self::EMAILS_PREFIX.'.id AND dnc.reason='.DoNotContact::UNSUBSCRIBED.' , 1, 0)), 1) FROM '.MILEX_TABLE_PREFIX.'lead_donotcontact dnc), 0)',
        ],
        'unsubscribed_ratio' => [
            'alias'   => 'unsubscribed_ratio',
            'label'   => 'milex.email.report.unsubscribed_ratio',
            'type'    => 'string',
            'formula' => 'IFNULL((SELECT ROUND((SUM(IF('.self::DNC_PREFIX.'.id IS NOT NULL AND '.self::DNC_PREFIX.'.channel_id='.self::EMAILS_PREFIX.'.id AND dnc.reason='.DoNotContact::UNSUBSCRIBED.' , 1, 0))/'.self::EMAILS_PREFIX.'.sent_count)*100, 1) FROM '.MILEX_TABLE_PREFIX.'lead_donotcontact dnc), \'0.0\')',
            'suffix'  => '%',
        ],
        'bounced' => [
            'alias'   => 'bounced',
            'label'   => 'milex.email.report.bounced',
            'type'    => 'string',
            'formula' => 'IFNULL((SELECT ROUND(SUM(IF('.self::DNC_PREFIX.'.id IS NOT NULL AND '.self::DNC_PREFIX.'.channel_id='.self::EMAILS_PREFIX.'.id AND dnc.reason='.DoNotContact::BOUNCED.' , 1, 0)), 1) FROM '.MILEX_TABLE_PREFIX.'lead_donotcontact dnc), 0)',
        ],
        'bounced_ratio' => [
            'alias'   => 'bounced_ratio',
            'label'   => 'milex.email.report.bounced_ratio',
            'type'    => 'string',
            'formula' => 'IFNULL((SELECT ROUND((SUM(IF('.self::DNC_PREFIX.'.id IS NOT NULL AND '.self::DNC_PREFIX.'.channel_id='.self::EMAILS_PREFIX.'.id AND dnc.reason='.DoNotContact::BOUNCED.' , 1, 0))/'.self::EMAILS_PREFIX.'.sent_count)*100, 1) FROM '.MILEX_TABLE_PREFIX.'lead_donotcontact dnc), \'0.0\')',
            'suffix'  => '%',
        ],
    ];
    ...
    const EMAIL_VARIANT_COLUMNS = [
        self::EMAIL_VARIANT_PREFIX.'.id' => [
            'label' => 'milex.email.report.variant_parent_id',
            'type'  => 'int',
        ],
        self::EMAIL_VARIANT_PREFIX.'.subject' => [
            'label' => 'milex.email.report.variant_parent_subject',
            'type'  => 'string',
        ],
    ];
    ...
    const CLICK_COLUMNS = [
        'hits' => [
            'alias'   => 'hits',
            'label'   => 'milex.email.report.hits_count',
            'type'    => 'string',
            'formula' => 'IFNULL('.self::CLICK_PREFIX.'.hits, 0)',
        ],
        'unique_hits' => [
            'alias'   => 'unique_hits',
            'label'   => 'milex.email.report.unique_hits_count',
            'type'    => 'string',
            'formula' => 'IFNULL('.self::CLICK_PREFIX.'.unique_hits, 0)',
        ],
        'hits_ratio' => [
            'alias'   => 'hits_ratio',
            'label'   => 'milex.email.report.hits_ratio',
            'type'    => 'string',
        ...
        'unique_ratio' => [
            'alias'   => 'unique_ratio',
            'label'   => 'milex.email.report.unique_ratio',
            'type'    => 'string',
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $columns = [
            ...
            $prefix.'read_count' => [
                'label' => 'milex.email.report.read_count',
                'type'  => 'int',
            ],
            'read_ratio' => [
                'alias'   => 'read_ratio',
                'label'   => 'milex.email.report.read_ratio',
                'type'    => 'string',
                'formula' => 'IFNULL(ROUND(('.$prefix.'read_count/'.$prefix.'sent_count)*100, 1), \'0.0\')',
                'suffix'  => '%',
            ],
            $prefix.'sent_count' => [
                'label' => 'milex.email.report.sent_count',
                'type'  => 'int',
            ],
            $prefix.'revision' => [
                'label' => 'milex.email.report.revision',
                'type'  => 'int',
            ],
            $prefix.'variant_start_date' => [
                'label'          => 'milex.email.report.variant_start_date',
                'type'           => 'datetime',
                'groupByFormula' => 'DATE('.$prefix.'variant_start_date)',
            ],
            $prefix.'variant_sent_count' => [
                'label' => 'milex.email.report.variant_sent_count',
                'type'  => 'int',
            ],
            $prefix.'variant_read_count' => [
                'label' => 'milex.email.report.variant_read_count',
                'type'  => 'int',
            ],
        ];
        ...
        $columns = array_merge(
            $columns,
            $event->getStandardColumns($prefix, [], 'milex_email_action'),
        ...
        $data = [
            'display_name' => 'milex.email.emails',
            'columns'      => $columns,
        ];
        $event->addTable(self::CONTEXT_EMAILS, $data);
        $context = self::CONTEXT_EMAILS;
        $event->addGraph($context, 'pie', 'milex.email.graph.pie.read.ingored.unsubscribed.bounced');
        ...
        if ($event->checkContext(self::CONTEXT_EMAIL_STATS)) {
            ...
            $columns['is_hit'] = [
                'alias'   => 'is_hit',
                'label'   => 'milex.email.report.is_hit',
                'type'    => 'bool',
            ...
            $columns['read_delay'] = [
                'alias'   => 'read_delay',
                'label'   => 'milex.email.report.read.delay',
                'type'    => 'string',
                'formula' => 'IF(es.date_read IS NOT NULL, TIMEDIFF(es.date_read, es.date_sent), \'-\')',
            ];

            $data = [
                'display_name' => 'milex.email.stats.report.table',
                'columns'      => array_merge(
                    $columns,
            ...
            $event->addGraph($context, 'line', 'milex.email.graph.line.stats');
            $event->addGraph($context, 'pie', 'milex.email.graph.pie.ignored.read.failed');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.sent');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.read');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.read.percent');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.unsubscribed');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.bounced');
            $event->addGraph($context, 'table', 'milex.email.table.most.emails.failed');
        }
    }
    ...
    public function onReportGenerate(ReportGeneratorEvent $event)
    {
        ...
        switch ($context) {
            case self::CONTEXT_EMAILS:
                $qb->from(MILEX_TABLE_PREFIX.'emails', self::EMAILS_PREFIX)
                    ->leftJoin(self::EMAILS_PREFIX, MILEX_TABLE_PREFIX.'emails', self::EMAIL_VARIANT_PREFIX, 'vp.id = e.variant_parent_id');
                ...
                if ($useClickColumns) {
                    $qbcut->select(
                        ...
                        ->from(MILEX_TABLE_PREFIX.'channel_url_trackables', 'cut2')
            ...
            case self::CONTEXT_EMAIL_STATS:
                $qb->from(MILEX_TABLE_PREFIX.'email_stats', self::EMAIL_STATS_PREFIX);

                if ($event->usesColumnWithPrefix(self::EMAILS_PREFIX)
                    || $event->usesColumnWithPrefix(ReportGeneratorEvent::CATEGORY_PREFIX)
                    || $useVariantColumns
                ) {
                    $qb->leftJoin(self::EMAIL_STATS_PREFIX, MILEX_TABLE_PREFIX.'emails', self::EMAILS_PREFIX, 'e.id = es.email_id');
                }

                if ($useVariantColumns) {
                    $qb->leftJoin(self::EMAILS_PREFIX, MILEX_TABLE_PREFIX.'emails', self::EMAIL_VARIANT_PREFIX, 'vp.id = e.variant_parent_id');
                }
    ...
    public function onReportGraphGenerate(ReportGraphEvent $event)
    {
        ...
        if ($event->checkContext(self::CONTEXT_EMAILS) && !in_array('milex.email.graph.pie.read.ingored.unsubscribed.bounced', $graphs)) {
            return;
        }
        ...
        foreach ($graphs as $g) {
            ...
            switch ($g) {
                case 'milex.email.graph.line.stats':
                    ...
                    $chart->setDataset($options['translator']->trans('milex.email.sent.emails'), $sends);
                    $chart->setDataset($options['translator']->trans('milex.email.read.emails'), $reads);
                    $chart->setDataset($options['translator']->trans('milex.email.failed.emails'), $failes);
                    $data         = $chart->render();
                    $data['name'] = $g;
                ...
                case 'milex.email.graph.pie.ignored.read.failed':
                    $counts = $this->statRepository->getIgnoredReadFailed($queryBuilder);
                    $chart  = new PieChart();
                    $chart->setDataset($options['translator']->trans('milex.email.read.emails'), $counts['read']);
                    $chart->setDataset($options['translator']->trans('milex.email.failed.emails'), $counts['failed']);
                    $chart->setDataset(
                        $options['translator']->trans('milex.email.ignored.emails'),
                        $counts['ignored']
                    );
                    $event->setGraph(
                        $g,
                ...
                case 'milex.email.graph.pie.read.ingored.unsubscribed.bounced':
                    ...
                    $chart  = new PieChart();
                    $chart->setDataset(
                        $options['translator']->trans('milex.email.stat.read'),
                        $counts['read_count'] ?? 0
                    );
                    $chart->setDataset(
                        $options['translator']->trans('milex.email.graph.pie.ignored.read.failed.ignored'),
                        (($counts['sent_count'] ?? 0) - ($counts['read_count'] ?? 0))
                    );
                    $chart->setDataset(
                        $options['translator']->trans('milex.email.unsubscribed'),
                        $counts['unsubscribed'] ?? 0
                    );
                    $chart->setDataset(
                        $options['translator']->trans('milex.email.bounced'),
                        $counts['bounced'] ?? 0
                    );
                ...
                case 'milex.email.table.most.emails.sent':
                    ...
                    $graphData['link']      = 'milex_email_action';
                    $event->setGraph($g, $graphData);
                    break;

                case 'milex.email.table.most.emails.read':
                    $this->joinEmailsTableIfMissing($queryBuilder, $event);
                ...
                case 'milex.email.table.most.emails.failed':
                    $this->joinEmailsTableIfMissing($queryBuilder, $event);
                    $queryBuilder->select(
                ...
                case 'milex.email.table.most.emails.unsubscribed':
                    $this->joinEmailsTableIfMissing($queryBuilder, $event);
                ...
                case 'milex.email.table.most.emails.bounced':
                    $this->joinEmailsTableIfMissing($queryBuilder, $event);
                    $this->addDNCTableForEmailStats($queryBuilder);
                    $queryBuilder->select(
                ...
                case 'milex.email.table.most.emails.read.percent':
                    $this->joinEmailsTableIfMissing($queryBuilder, $event);
    ...
    {
        if ($event->checkContext(self::CONTEXT_EMAIL_STATS) && !$this->isJoined($queryBuilder, MILEX_TABLE_PREFIX.'emails', self::EMAIL_STATS_PREFIX, self::EMAILS_PREFIX)) {
            $queryBuilder->leftJoin(self::EMAIL_STATS_PREFIX, MILEX_TABLE_PREFIX.'emails', self::EMAILS_PREFIX, 'e.id = es.email_id');
        }
    }
    ...
    {
        $table = MILEX_TABLE_PREFIX.'lead_donotcontact';
