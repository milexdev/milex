...
use Milex\EmailBundle\Entity\EmailReplyRepositoryInterface;
...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    private function addEmailEvents(LeadTimelineEvent $event, $state)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.email.'.$state);
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($stats['results'] as $stat) {
                if (!empty($stat['email_name'])) {
                    ...
                    $label = $this->translator->trans('milex.email.timeline.event.custom_email').': '.$stat['storedSubject'];
                } else {
                    $label = $this->translator->trans('milex.email.timeline.event.custom_email');
                }

                if (!empty($stat['idHash'])) {
                    $eventName = [
                        'label'      => $label,
                        'href'       => $this->router->generate('milex_email_webview', ['idHash' => $stat['idHash']]),
                        'isExternal' => true,
                    ];
                } else {
                    $eventName = $label;
                ...
                $event->addEvent(
                    [
                        ...
                        'contentTemplate' => 'MilexEmailBundle:SubscribedEvents\Timeline:index.html.php',
    ...
    private function addEmailReplies(LeadTimelineEvent $event)
    {
        ...
        $eventTypeName = $this->translator->trans('milex.email.replied');
        $event->addEventType($eventTypeKey, $eventTypeName);
        $event->addSerializerGroup('emailList');
        ...
        if (!$event->isEngagementCount()) {
            foreach ($replies['results'] as $reply) {
                $label = $this->translator->trans('milex.email.timeline.event.email_reply');
