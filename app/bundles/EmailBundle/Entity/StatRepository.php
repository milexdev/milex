...
class StatRepository extends CommonRepository
{
    ...
    public function getUniqueClickedLinksPerContactAndEmail($contactId, $emailId)
    {
        ...
        $q->select('distinct ph.url, ph.date_hit')
            ->from(MILEX_TABLE_PREFIX.'page_hits', 'ph')
    ...
    public function getSentEmailToContactData(
        ...
        $q->select('s.id, s.lead_id, s.email_address, s.is_read, s.email_id, s.date_sent, s.date_read')
            ->from(MILEX_TABLE_PREFIX.'email_stats', 's')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'emails', 'e', 's.email_id = e.id')
            ->addSelect('e.name AS email_name')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'page_hits', 'ph', 'ph.source = "email" and ph.source_id = s.email_id and ph.lead_id = s.lead_id')
        ...
        $q->leftJoin('s', MILEX_TABLE_PREFIX.'companies_leads', 'cl', $companyJoinOnExpr)
            ->leftJoin('s', MILEX_TABLE_PREFIX.'companies', 'c', 'cl.company_id = c.id')
        ...
        $q->leftJoin('s', MILEX_TABLE_PREFIX.'campaign_events', 'ce', 's.source = "campaign.event" and s.source_id = ce.id')
            ->leftJoin('ce', MILEX_TABLE_PREFIX.'campaigns', 'campaign', 'ce.campaign_id = campaign.id')
        ...
        $q->leftJoin('s', MILEX_TABLE_PREFIX.'lead_lists', 'll', 's.list_id = ll.id')
        ...
        if (null !== $segmentId) {
            ...
            $sb->select('null')
                ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'lll')
                ->where(
                    $sb->expr()->andX(
    ...
    public function getStatusCount($column, $emailIds = null, $listId = null, ChartQuery $chartQuery = null, $combined = false)
    {
        ...
        if ($listId) {
            if (!$combined) {
                ...
                $subQ->select('null')
                    ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'list')
                    ->andWhere(
                        $q->expr()->andX(
    ...
    public function getOpenedRates($emailIds, \DateTime $fromDate = null)
    {
        ...
        $sq->select('e.email_id, count(e.id) as the_count')
            ->from(MILEX_TABLE_PREFIX.'email_stats', 'e')
            ->where(
                $sq->expr()->andX(
    ...
    public function getLeadStats($leadId, array $options = [])
    {
        ...
        $query->from(MILEX_TABLE_PREFIX.'email_stats', 's')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'emails', 'e', 's.email_id = e.id')
            ->leftJoin('s', MILEX_TABLE_PREFIX.'email_copies', 'ec', 's.copy_id = ec.id');
        ...
        if (!empty($options['basic_select'])) {
            ...
            $query->select(
                ...
                ->leftJoin('s', MILEX_TABLE_PREFIX.'lead_lists', 'l', 's.list_id = l.id');
        }
    ...
    {
        $q = $this->_em->getConnection()->createQueryBuilder();
        $q->update(MILEX_TABLE_PREFIX.'email_stats')
    ...
    {
        $this->getEntityManager()->getConnection()->delete(MILEX_TABLE_PREFIX.'email_stats', ['id' => (int) $id]);
    }
    ...
    public function deleteStats(array $ids)
    {
        ...
        $qb->delete(MILEX_TABLE_PREFIX.'email_stats')
            ->where(
                $qb->expr()->in('id', $ids)
            )
            ->execute();
