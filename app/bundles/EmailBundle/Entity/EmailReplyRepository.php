...
final class EmailReplyRepository extends CommonRepository implements EmailReplyRepositoryInterface
{
    ...
    public function getByLeadIdForTimeline($leadId, $options)
    {
        ...
        $qb->from(MILEX_TABLE_PREFIX.'email_stat_replies', 'reply')
            ->innerJoin('reply', MILEX_TABLE_PREFIX.'email_stats', 'stat', 'reply.stat_id = stat.id')
            ->leftJoin('stat', MILEX_TABLE_PREFIX.'emails', 'email', 'stat.email_id = email.id')
            ->leftJoin('stat', MILEX_TABLE_PREFIX.'email_copies', 'email_copy', 'stat.copy_id = email_copy.id')
