...
class EmailRepository extends CommonRepository
{
    ...
    public function getDoNotEmailList($leadIds = [])
    {
        ...
        $q->select('l.id, l.email')
            ->from(MILEX_TABLE_PREFIX.'lead_donotcontact', 'dnc')
            ->leftJoin('dnc', MILEX_TABLE_PREFIX.'leads', 'l', 'l.id = dnc.lead_id')
    ...
    {
        $this->getEntityManager()->getConnection()->delete(MILEX_TABLE_PREFIX.'lead_donotcontact', ['id' => (int) $id]);
    }
    ...
    public function getEntities(array $args = [])
    {
        $q = $this->getEntityManager()
            ->createQueryBuilder()
            ->select('e')
            ->from('MilexEmailBundle:Email', 'e', 'e.id');
    ...
    public function getEmailPendingQuery(
        ...
        $mqQb->select('mq.lead_id')
            ->from(MILEX_TABLE_PREFIX.'message_queue', 'mq')
            ->where(
                $mqQb->expr()->andX(
        ...
        $statQb->select('stat.lead_id')
            ->from(MILEX_TABLE_PREFIX.'email_stats', 'stat');
        ...
        if (is_null($listIds)) {
            ...
            $lists = $this->getEntityManager()->getConnection()->createQueryBuilder()
                ...
                ->from(MILEX_TABLE_PREFIX.'email_list_xref', 'el')
        ...
        $segmentQb->select('ll.lead_id')
            ->from(MILEX_TABLE_PREFIX.'lead_lists_leads', 'll')
            ->where(
                $segmentQb->expr()->andX(
        ...
        $q->from(MILEX_TABLE_PREFIX.'leads', 'l')
    ...
    protected function addSearchCommandWhereClause($q, $filter)
    {
        ...
        switch ($command) {
            case $this->translator->trans('milex.core.searchcommand.lang'):
    ...
    public function resetVariants($relatedIds, $date)
    {
        ...
        $qb->update(MILEX_TABLE_PREFIX.'emails')
    ...
    public function upCount($id, $type = 'sent', $increaseBy = 1, $variant = false)
    {
        ...
        $q->update(MILEX_TABLE_PREFIX.'emails');
    ...
    public function isOneUnpublished(array $ids)
    {
        $result = $this->getEntityManager()
            ...
            ->from('MilexEmailBundle:Email', $this->getTableAlias(), $this->getTableAlias().'.id')
    ...
    private function getCategoryUnsubscribedLeadsQuery(int $emailId): QueryBuilder
    {
        ...
        return $qb->select('lc.lead_id')
            ->from(MILEX_TABLE_PREFIX.'lead_categories', 'lc')
            ->innerJoin('lc', MILEX_TABLE_PREFIX.'emails', 'e', 'e.category_id = lc.category_id')
