...
use Milex\EmailBundle\Swiftmailer\Sparkpost\SparkpostFactoryInterface;
...
class SparkpostTransport extends AbstractTokenArrayTransport implements \Swift_Transport, TokenTransportInterface, CallbackTransportInterface
{
    ...
    public function start()
    {
        if (empty($this->apiKey)) {
            $this->throwException($this->translator->trans('milex.email.api_key_required', [], 'validators'));
        }
    ...
    public function getSparkPostMessage(\Swift_Mime_SimpleMessage $message)
    {
        ...
        $milexTokens  = $mergeVars = $mergeVarPlaceholders = [];
        $campaignId    = '';

        // Sparkpost uses {{ name }} for tokens so Milex's need to be converted; although using their {{{ }}} syntax to prevent HTML escaping
        if (!empty($metadata)) {
        ...
        $message = $this->messageToArray($milexTokens, $mergeVarPlaceholders, true);

        // Sparkpost requires a subject
        if (empty($message['subject'])) {
            throw new \Exception($this->translator->trans('milex.email.subject.notblank', [], 'validators'));
        }
    ...
    protected function checkTemplateIsValid(Sparkpost $sparkPostClient, array $sparkPostMessage)
    {
        ...
        if (403 === $response->getStatusCode()) {
            ...
            $this->logger->warning("The permission 'Templates: Preview' is not enabled. Enable it to let Milex check email template validity before send.");
