...
namespace Milex\EmailBundle\Swiftmailer\Momentum\Metadata;
...
class MetadataProcessor
{
    ...
    public function __construct(\Swift_Message $message)
    {
        ...
        $metadata       = ($message instanceof MilexMessage) ? $message->getMetadata() : [];
        $this->metadata = $metadata;

        // Build the substitution merge vars
        $this->buildSubstitutionData();

        if (count($this->milexTokens)) {
            // Update the content with the substitution merge vars
            MailHelper::searchReplaceTokens($this->milexTokens, $this->substitutionMergeVars, $this->message);
        }
    }
    ...
    private function buildSubstitutionData()
    {
        ...
        $this->milexTokens = array_keys($tokens);

        foreach ($this->milexTokens as $token) {
