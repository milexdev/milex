...
namespace Milex\EmailBundle\Swiftmailer\Amazon;
...
use Milex\EmailBundle\MonitoredEmail\Processor\Bounce\Definition\Category;
use Milex\EmailBundle\MonitoredEmail\Processor\Bounce\Definition\Type;
                ...
                foreach ($payload['complaint']['complainedRecipients'] as $complainedRecipient) {
                    ...
                    if (isset($payload['complaint']['complaintFeedbackType'])) {
                        ...
                        switch ($payload['complaint']['complaintFeedbackType']) {
                            case 'abuse':
                                $reason = $this->translator->trans('milex.email.complaint.reason.abuse');
                                break;
                            case 'fraud':
                                $reason = $this->translator->trans('milex.email.complaint.reason.fraud');
                                break;
                            case 'virus':
                                $reason = $this->translator->trans('milex.email.complaint.reason.virus');
                                break;
                        }
                    }

                    if (null == $reason) {
                        $reason = $this->translator->trans('milex.email.complaint.reason.unknown');
                    }
