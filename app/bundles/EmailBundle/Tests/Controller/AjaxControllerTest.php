...
namespace Milex\EmailBundle\Tests\Controller;

use Milex\CoreBundle\Factory\ModelFactory;
use Milex\EmailBundle\Controller\AjaxController;
...
class AjaxControllerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testSendBatchActionWhenIdProvidedButEmailNotPublished(): void
    {
        $this->containerMock->expects($this->exactly(2))
            ->method('get')
            ->withConsecutive(['milex.model.factory'], ['session'])
        ...
        $this->sessionMock->expects($this->exactly(3))
            ->method('get')
            ->withConsecutive(
                ['milex.email.send.progress'],
                ['milex.email.send.stats'],
                ['milex.email.send.active']
            )
