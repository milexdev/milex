...
use Milex\EmailBundle\EventListener\ReportSubscriber;
...
class ReportSubscriberTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testOnReportGenerateForEmailStatsWhenDncIsUsed(): void
    {
        ...
        $this->assertSame(
            'SELECT  FROM '.MILEX_TABLE_PREFIX.'email_stats es LEFT JOIN '.MILEX_TABLE_PREFIX."lead_donotcontact dnc ON es.email_id = dnc.channel_id AND dnc.channel='email' AND es.lead_id = dnc.lead_id WHERE es.date_sent IS NULL OR (es.date_sent BETWEEN :dateFrom AND :dateTo) GROUP BY es.id",
    ...
    public function testOnReportGenerateForEmailStatsWhenVariantIsUsed(): void
    {
        ...
        $this->assertSame(
            'SELECT  FROM '.MILEX_TABLE_PREFIX.'email_stats es LEFT JOIN '.MILEX_TABLE_PREFIX.'emails e ON e.id = es.email_id LEFT JOIN '.MILEX_TABLE_PREFIX.'emails vp ON vp.id = e.variant_parent_id WHERE es.date_sent IS NULL OR (es.date_sent BETWEEN :dateFrom AND :dateTo) GROUP BY es.id',
            $this->queryBuilder->getSQL()
        );
    }
    ...
    public function testOnReportGenerateForEmailStatsWhenClickIsUsed(): void
    {
        ...
        $this->assertSame(
            'SELECT  FROM '.MILEX_TABLE_PREFIX.'email_stats es LEFT JOIN (SELECT COUNT(ph.id) AS hits, COUNT(DISTINCT(ph.redirect_id)) AS unique_hits, cut2.channel_id, ph.lead_id FROM '.MILEX_TABLE_PREFIX.'channel_url_trackables cut2 INNER JOIN '.MILEX_TABLE_PREFIX."page_hits ph ON cut2.redirect_id = ph.redirect_id AND cut2.channel_id = ph.source_id WHERE cut2.channel = 'email' AND ph.source = 'email' GROUP BY cut2.channel_id, ph.lead_id) cut ON es.email_id = cut.channel_id AND es.lead_id = cut.lead_id WHERE es.date_sent IS NULL OR (es.date_sent BETWEEN :dateFrom AND :dateTo) GROUP BY es.id",
            $this->queryBuilder->getSQL()
        );
    }
    ...
    public function testOnReportGenerateForEmailStatsWhenCampaignIsUsed(): void
    {
        ...
        $this->assertSame(
            'SELECT  FROM '.MILEX_TABLE_PREFIX.'email_stats es LEFT JOIN '.MILEX_TABLE_PREFIX.'leads l ON l.id = es.lead_id LEFT JOIN '.MILEX_TABLE_PREFIX."campaign_lead_event_log clel ON clel.channel='email' AND es.email_id = clel.channel_id AND clel.lead_id = l.id LEFT JOIN ".MILEX_TABLE_PREFIX.'campaigns cmp ON cmp.id = clel.campaign_id WHERE es.date_sent IS NULL OR (es.date_sent BETWEEN :dateFrom AND :dateTo) GROUP BY es.id',
            $this->queryBuilder->getSQL()
        );
    }
    ...
    public function testOnReportGraphGenerateForEmailContextWithEmailGraph(): void
    {
        ...
        $eventMock->expects($this->once())
            ...
            ->willReturn(['milex.email.graph.pie.read.ingored.unsubscribed.bounced']);
        ...
        $queryBuilderMock->expects($this->once())
            ...
            ->with(
                ...
                MILEX_TABLE_PREFIX.'lead_donotcontact',
