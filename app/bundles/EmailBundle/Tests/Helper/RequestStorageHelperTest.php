...
class RequestStorageHelperTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->assertEquals(MomentumTransport::class, $this->helper->getTransportNameFromKey('Milex\EmailBundle\Swiftmailer\Transport\MomentumTransport:webhook_request:5b43832134cfb0.36545510'));
    }

    /**
     * The StorageHelper will add '%milex.db_table_prefix%' as a prefix to each cache key.
     */
    public function testGetTransportNameFromKeyWithGlobalPrefix()
    {
        $this->assertEquals(MomentumTransport::class, $this->helper->getTransportNameFromKey('milex:Milex|EmailBundle|Swiftmailer|Transport|MomentumTransport:webhook_request:5bfbe8ce671198.00044461'));
    }
}
