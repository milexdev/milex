    ...
    <div class="col-sm-offset-3 col-sm-6 text-center">
        <div class="panel panel-<?php echo ('inprogress' != $status) ? 'success' : 'danger'; ?>">
            <div class="panel-heading">
                <h4 class="panel-title"><?php echo $view['translator']->trans('milex.email.send.'.$status, ['%subject%' => \Milex\CoreBundle\Helper\EmojiHelper::toHtml($email->getSubject(), 'short')]); ?></h4>
            </div>
            <div class="panel-body">
                <?php if ('inprogress' != $status): ?>
                <h4><?php echo $view['translator']->trans('milex.email.send.stats', ['%sent%' => $stats['sent'], '%failed%' => $stats['failed']]); ?></h4>
                <?php endif; ?>
    ...
    echo ' active';
            ...
            <ul class="list-group">
                ...
                <li class="list-group-item">
                    <a target="_new" class="text-danger" href="<?php echo $view['router']->path('milex_contact_action', ['objectAction' => 'view', 'objectId' => $leadId]); ?>">
            ...
            <div class="panel-footer">
                ...
                <?php if ('inprogress' == $status): ?>
                <div>
                    <a class="text-danger mt-md" href="<?php echo $view['router']->path('milex_email_action', ['objectAction' => 'send', 'objectId' => $email->getId()]); ?>" data-toggle="ajax"><?php echo $view['translator']->trans('milex.core.form.cancel'); ?></a>
                </div>
                <?php else: ?>
                <div>
                    <a class="text-success mt-md" href="<?php echo $view['router']->path('milex_email_action', ['objectAction' => 'view', 'objectId' => $email->getId()]); ?>" data-toggle="ajax"><?php echo $view['translator']->trans('milex.core.form.done'); ?></a>
                </div>
                <?php endif; ?>
            </div>
        </div>
