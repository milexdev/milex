...
$variantContent = $view->render(
    'MilexCoreBundle:Variant:index.html.php',
    [
        'activeEntity'  => $email,
        'variants'      => $variants,
        'abTestResults' => $abTestResults,
        'model'         => 'email',
        'actionRoute'   => 'milex_email_action',
    ]
);
...
$translationContent = $view->render(
    'MilexCoreBundle:Translation:index.html.php',
    [
        'activeEntity' => $email,
...
if (!$isEmbedded) {
    ...
    $customButtons[] = [
        'attr' => [
            ...
            'href'        => $view['router']->path('milex_email_action', ['objectAction' => 'sendExample', 'objectId' => $email->getId()]),
            'data-header' => $view['translator']->trans('milex.email.send.example'),
        ],
        'iconClass' => 'fa fa-send',
        'btnText'   => 'milex.email.send.example',
        'primary'   => true,
    ];
}
...
if (!$isEmbedded) {
    ...
    $view['slots']->set(
        'publishStatus',
        $view->render('MilexCoreBundle:Helper:publishstatus_badge.html.php', ['entity' => $email])
    );
}
?>
...
<div class="box-layout">
    ...
    <div class="col-md-9 bg-white height-auto">
        <div class="bg-auto">
            ...
            <div class="pr-md pl-md pt-lg pb-lg">
                <div class="box-layout">
                    <div class="col-xs-10">
                        <div>
                            <?php echo \Milex\CoreBundle\Helper\EmojiHelper::toHtml($email->getSubject(), 'short'); ?>
                        </div>
                        <?php if ($email->isVariant(true)): ?>
                        <div class="small">
                            <a href="<?php echo $view['router']->path('milex_email_action', ['objectAction' => 'view', 'objectId' => $variants['parent']->getId()]); ?>" data-toggle="ajax">
                                <?php echo $view['translator']->trans('milex.core.variant_of', ['%parent%' => $variants['parent']->getName()]); ?>
                            </a>
                        </div>
                        <?php endif; ?>
                        <?php if ($email->isTranslation(true)): ?>
                        <div class="small">
                            <a href="<?php echo $view['router']->path('milex_email_action', ['objectAction' => 'view', 'objectId' => $translations['parent']->getId()]); ?>" data-toggle="ajax">
                                <?php echo $view['translator']->trans('milex.core.translation_of', ['%parent%' => $translations['parent']->getName()]); ?>
                            </a>
                        </div>
                        <?php endif; ?>
                    </div>
                ...
                <div class="pr-md pl-md pb-md">
                    <div class="panel shd-none mb-0">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tr>
                                <td width="20%">
                                    <span class="fw-b"><?php echo $view['translator']->trans('milex.core.form.theme'); ?></span>
                                </td>
                            ...
                            <?php if ($fromName = $email->getFromName()): ?>
                                <tr>
                                    <td width="20%">
                                        <span class="fw-b"><?php echo $view['translator']->trans('milex.email.from_name'); ?></span>
                                    </td>
                            ...
                            <?php if ($fromEmail = $email->getFromAddress()): ?>
                                <tr>
                                    <td width="20%">
                                        <span class="fw-b"><?php echo $view['translator']->trans('milex.email.from_email'); ?></span>
                                    </td>
                            ...
                            <?php if ($replyTo = $email->getReplyToAddress()): ?>
                                <tr>
                                    <td width="20%">
                                        <span class="fw-b"><?php echo $view['translator']->trans('milex.email.reply_to_email'); ?></span>
                                    </td>
                            ...
                            <?php if ($bccAddress = $email->getBccAddress()): ?>
                                <tr>
                                    <td width="20%">
                                        <span class="fw-b"><?php echo $view['translator']->trans('milex.email.bcc'); ?></span>
                                    </td>
                            ...
                            <?php if ($headers = $email->getHeaders()): ?>
                                <tr>
                                    <td width="20%">
                                        <span class="fw-b"><?php echo $view['translator']->trans('milex.email.custom_headers'); ?></span>
                                    </td>
                            ...
                            <tr>
                                <td width="20%">
                                    <span class="fw-b"><?php echo $view['translator']->trans('milex.email.stat.sent'); ?></span>
                                </td>
                                <td><?php echo $email->getSentCount(); ?></td>
                            </tr>
                            <tr>
                                <td width="20%">
                                    <span class="fw-b"><?php echo $view['translator']->trans('milex.email.stat.read'); ?></span>
                                </td>
        ...
        <div class="bg-auto bg-dark-xs">
            ...
            ?>
            <div id="emailGraphStats" data-graph-url="<?php echo $view['router']->path('milex_email_graph_stats', ['objectId' => $email->getId(), 'isVariant' => $isVariant, 'dateFrom' => $dateFrom->format('Y-m-d'), 'dateTo' => $dateTo->format('Y-m-d')]); ?>">
                <div class="spinner">
            ...
            <ul class="nav nav-tabs pr-md pl-md">
                <li class="active">
                    <a href="#clicks-container" role="tab" data-toggle="tab">
                        <?php echo $view['translator']->trans('milex.trackable.click_counts'); ?>
                    </a>
                </li>
                <li>
                    <a href="#contacts-container" role="tab" data-toggle="tab">
                        <?php echo $view['translator']->trans('milex.email.associated.contacts'); ?>
                    </a>
                </li>
                <?php if ($showVariants): ?>
                    <li>
                        <a href="#variants-container" role="tab" data-toggle="tab">
                            <?php echo $view['translator']->trans('milex.core.variants'); ?>
                        </a>
                    </li>
                <?php endif; ?>
                <?php if ($showTranslations): ?>
                    <li>
                        <a href="#translation-container" role="tab" data-toggle="tab">
                            <?php echo $view['translator']->trans('milex.core.translations'); ?>
                        </a>
                    </li>
                <?php endif; ?>
            </ul>
        ...
        <div class="tab-content pa-md">
            <div class="tab-pane active bdr-w-0" id="clicks-container">
                <?php echo $view->render('MilexPageBundle:Trackable:click_counts.html.php', [
    ...
    <div class="col-md-3 bg-white bdr-l height-auto">
        ...
        <div class="panel bg-transparent shd-none bdr-rds-0 bdr-w-0 mt-sm mb-0">
            <div class="panel-heading">
                <div class="panel-title"><?php echo $view['translator']->trans('milex.email.preview.url'); ?></div>
            </div>
            <div class="panel-body pt-xs">
                <div class="input-group">
                    <div class="input-group-addon"><?php echo $view->render('MilexCoreBundle:Helper:publishstatus_icon.html.php', ['item' => $email, 'model' => 'email', 'query' => 'customToggle=publicPreview']); ?></div>
