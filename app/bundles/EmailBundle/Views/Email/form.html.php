...
$subheader = ($variantParent) ? '<div><span class="small">'.$view['translator']->trans('milex.core.variant_of', [
    '%name%'   => $email->getName(),
    '%parent%' => $variantParent->getName(),
]).'</span></div>' : '';

$header = $isExisting ?
    $view['translator']->trans('milex.email.header.edit',
        ['%name%' => $email->getName()]) :
    $view['translator']->trans('milex.email.header.new');
...
$isCodeMode = ('milex_code_mode' === $email->getTemplate());
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <ul class="bg-auto nav nav-tabs pr-md pl-md">
                    ...
                    <li id="dynamic-content-tab" <?php echo (!$isCodeMode) ? 'class="hidden"' : ''; ?>>
                        <a href="#dynamic-content-container" role="tab" data-toggle="tab">
                            <?php echo $view['translator']->trans('milex.core.dynamicContent'); ?>
                        </a>
                    </li>
                    <?php echo $view['content']->getCustomContent('email.tabs', $milexTemplateVars); ?>
                </ul>
                <!--/ tabs controls -->
                <div class="tab-content pa-md">
                    ...
                    <div class="tab-pane fade bdr-w-0" id="advanced-container">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <?php echo $view['content']->getCustomContent('email.settings.advanced', $milexTemplateVars); ?>
                                <div>
                                    <div class="pull-left">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="text-right pr-10">
                                    ...
                                    <a class="small" onclick="Milex.autoGeneratePlaintext();"><?php echo $view['translator']->trans('milex.email.plaintext.generate'); ?></a>
                                </div>
                                <div class="clearfix"></div>
                    ...
                    <div class="tab-pane fade bdr-w-0" id="dynamic-content-container">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                $tabHtml .= '<li><a href="javascript:void(0);" role="tab" class="btn btn-primary" id="addNewDynamicContent"><i class="fa fa-plus text-success"></i> '.$view['translator']->trans('milex.core.form.new').'</a></li>';
                                ...
                                foreach ($form['dynamicContent'] as $i => $dynamicContent) {
                                    $linkText = $dynamicContent['tokenName']->vars['value'] ?: $view['translator']->trans('milex.core.dynamicContent').' '.($i + 1);
                    ...
                    <?php echo $view['content']->getCustomContent('email.tabs.content', $milexTemplateVars); ?>
                </div>
            </div>
        </div>
    </div>
...
<div class="hide" id="templates">
    <?php foreach ($templates as $dataKey => $template): ?>
        <?php $attr = ('tags' == $dataKey) ? ' data-placeholder="'.$view['translator']->trans('milex.lead.tags.select_or_create').'" data-no-results-text="'.$view['translator']->trans('milex.lead.tags.enter_to_create').'" data-allow-add="true" onchange="Milex.createLeadTag(this)"' : ''; ?>
...
if ((empty($updateSelect) && !$isExisting && !$view['form']->containsErrors($form) && !$variantParent) || empty($type) || !empty($forceTypeSelection)):
    echo $view->render('MilexCoreBundle:Helper:form_selecttype.html.php',
        [
            'item'       => $email,
            'milexLang' => [
                'newListEmail'     => 'milex.email.type.list.header',
                'newTemplateEmail' => 'milex.email.type.template.header',
            ],
            'typePrefix'         => 'email',
            'cancelUrl'          => 'milex_email_index',
            'header'             => 'milex.email.type.header',
            'typeOneHeader'      => 'milex.email.type.template.header',
            'typeOneIconClass'   => 'fa-cube',
            'typeOneDescription' => 'milex.email.type.template.description',
            'typeOneOnClick'     => "Milex.selectEmailType('template');",
            'typeTwoHeader'      => 'milex.email.type.list.header',
            'typeTwoIconClass'   => 'fa-pie-chart',
            'typeTwoDescription' => 'milex.email.type.list.description',
            'typeTwoOnClick'     => "Milex.selectEmailType('list');",
        ]);
endif;
