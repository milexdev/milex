...
<div class="tab-pane dynamic-content bdr-w-0<?php echo '0' === $form->vars['name'] ? ' active' : ' fade'; ?>" id="<?php echo $form->vars['id']; ?>">
    <div class="panel">
        ...
        $tabHtml .= '<li><a role="tab" class="addNewDynamicContentFilter" data-index="'.$form->vars['name']
            .'"><i class="fa fa-plus text-success"></i> '.$view['translator']->trans('milex.core.form.new').'</a></li>';
        ...
        foreach ($form['filters'] as $i => $filter) {
            $isFirst = 0 === $i ? ' active' : '';
            $tabHtml .= '<li><a role="tab" data-toggle="tab" href="#'.$filter->vars['id'].'">'.$view['translator']->trans('milex.core.dynamicContent.tab').' '.($i + 1).'</a></li>';
