...
use Milex\EmailBundle\Swiftmailer\Transport\CallbackTransportInterface;
use Milex\LeadBundle\Controller\FrequencyRuleTrait;
use Milex\LeadBundle\Entity\DoNotContact;
use Milex\PageBundle\Entity\Page;
use Milex\PageBundle\Event\PageDisplayEvent;
use Milex\PageBundle\EventListener\BuilderSubscriber;
...
class PublicController extends CommonFormController
{
    ...
    public function indexAction($idHash)
    {
        ...
        if (!empty($stat)) {
            ...
            $content = $this->get('milex.helper.template.analytics')->addCode($content);
    ...
    {
        $queueService = $this->get('milex.queue.service');
    ...
    public function unsubscribeAction($idHash)
    {
        ...
        if (!empty($stat)) {
            if ($email = $stat->getEmail()) {
                ...
                if ('milex_code_mode' === $template) {
                    // Use system default
                    $template = null;
                }

                /** @var \Milex\FormBundle\Entity\Form $unsubscribeForm */
                ...
                if (null != $unsubscribeForm && $unsubscribeForm->isPublished()) {
                    ...
                    $formContent  = '<div class="milex-unsubscribeform">'.$formModel->getContent($unsubscribeForm).'</div>';
                }
            }
        }
        ...
        if (!empty($stat)) {
            $successSessionName = 'milex.email.prefscenter.success';

            if ($lead = $stat->getLead()) {
                // Set the lead as current lead
                $this->get('milex.tracker.contact')->setTrackedContact($lead);
            ...
            if (!$this->get('milex.helper.core_parameters')->get('show_contact_preferences')) {
                $message = $this->getUnsubscribeMessage($idHash, $model, $stat, $translator);
            } elseif ($lead) {
                $action = $this->generateUrl('milex_email_unsubscribe', ['idHash' => $idHash]);

                $viewParameters = [
                    'lead'                         => $lead,
                    'idHash'                       => $idHash,
                    'showContactFrequency'         => $this->get('milex.helper.core_parameters')->get('show_contact_frequency'),
                    'showContactPauseDates'        => $this->get('milex.helper.core_parameters')->get('show_contact_pause_dates'),
                    'showContactPreferredChannels' => $this->get('milex.helper.core_parameters')->get('show_contact_preferred_channels'),
                    'showContactCategories'        => $this->get('milex.helper.core_parameters')->get('show_contact_categories'),
                    'showContactSegments'          => $this->get('milex.helper.core_parameters')->get('show_contact_segments'),
                ];
                ...
                if (true === $form) {
                    ...
                    return $this->postActionRedirect(
                        [
                            'returnUrl'       => $this->generateUrl('milex_email_unsubscribe', ['idHash' => $idHash]),
                ...
                if ($email && ($prefCenter = $email->getPreferenceCenter()) && ($prefCenter->getIsPreferenceCenter())) {
                    ...
                    if ($savePrefsPresent) {
                        ...
                        /** @var \Milex\CoreBundle\Templating\Helper\FormHelper $formHelper */
                ...
                if (empty($html)) {
                    $html = $this->get('milex.helper.templating')->getTemplating()->render(
                        'MilexEmailBundle:Lead:preference_options.html.php',
                        array_merge(
                            $viewParameters,
                            [
                                'form'         => $formView,
            ...
            $message = $translator->trans('milex.email.stat_record.not_found');
        }
    ...
    public function resubscribeAction($idHash)
    {
        ...
        if (!empty($stat)) {
            ...
            $message = str_replace(
                ...
                [
                    $this->generateUrl('milex_email_unsubscribe', ['idHash' => $idHash]),
            ...
            $email   = $lead   = false;
            $message = $this->translator->trans('milex.email.stat_record.not_found');
        }

        $template = (null !== $email && 'milex_code_mode' !== $email->getTemplate()) ? $email->getTemplate() : $this->coreParametersHelper->get('theme');
    ...
    public function previewAction($objectId)
    {
        ...
        if (
            ($this->get('milex.security')->isAnonymous() && (!$emailEntity->getIsPublished() || !$emailEntity->isPublicPreview()))
            || (!$this->get('milex.security')->isAnonymous()
    ...
    {
        $logger = $this->get('monolog.logger.milex');
    ...
    {
        if (null !== $lead) {
            /** @var \Milex\EmailBundle\Helper\MailHelper $mailer */
    ...
    {
        $model->setDoNotContact($stat, $translator->trans('milex.email.dnc.unsubscribed'), DoNotContact::UNSUBSCRIBED);
        ...
        return str_replace(
            ...
            [
                $this->generateUrl('milex_email_resubscribe', ['idHash' => $idHash]),
                $stat->getEmailAddress(),
            ],
            $message
        );
