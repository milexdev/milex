...
namespace Milex\InstallBundle\Controller;
...
class InstallController extends CommonController
{
    ...
    {
        $this->configurator = $this->container->get('milex.configurator');
        $this->installer    = $this->container->get('milex.install.service');
    }
    ...
    public function stepAction(float $index = 0)
    {
        ...
        $completedSteps = $session->get('milex.installer.completedsteps', []);

        // Check to ensure the installer is in the right place
        if ((empty($params) || empty($params['db_driver'])) && $index > 1) {
            $session->set('milex.installer.completedsteps', [0]);

            return $this->redirect($this->generateUrl('milex_installer_step', ['index' => 1]));
        }

        /** @var \Milex\CoreBundle\Configurator\Step\StepInterface $step */
        $step   = $this->configurator->getStep($index)[0];
        $action = $this->generateUrl('milex_installer_step', ['index' => $index]);
            ...
            if ($form->isValid()) {
                ...
                switch ($index) {
                    ...
                    case InstallService::DOCTRINE_STEP:
                        ...
                        return $this->redirect($this->generateUrl('milex_installer_step', ['index' => 1.1]));
                    case InstallService::USER_STEP:
                        ...
                        $session->set('milex.installer.user', $formData);
                ...
                case InstallService::DOCTRINE_STEP:
                    ...
                    switch ($subIndex) {
                        case 1:
                            ...
                            return $this->redirect($this->generateUrl('milex_installer_step', ['index' => 1.2]));
                        case 2:
        ...
        if ($complete) {
            ...
            $session->set('milex.installer.completedsteps', $completedSteps);
            ++$index;

            if ($index < $this->configurator->getStepCount()) {
                // On to the next step

                return $this->redirect($this->generateUrl('milex_installer_step', ['index' => (int) $index]));
            } else {
                ...
                return $this->postActionRedirect(
                    [
                        'viewParameters'    => [
                            'welcome_url' => $this->generateUrl('milex_dashboard_index'),
                            'parameters'  => $this->configurator->render(),
                            'version'     => MILEX_VERSION,
                            'tmpl'        => $tmpl,
                        ],
                        'returnUrl'         => $this->generateUrl('milex_installer_final'),
                        'contentTemplate'   => 'MilexInstallBundle:Install:final.html.php',
            ...
            if ($index && $index > $last) {
                return $this->redirect($this->generateUrl('milex_installer_step', ['index' => $last]));
            }
        }
        ...
        return $this->delegateView(
            [
                'viewParameters'  => [
                    ...
                    'configFile'     => $this->get('milex.helper.paths')->getSystemPath('local_config'),
                    'completedSteps' => $completedSteps,
                ],
                'contentTemplate' => $step->getTemplate(),
                'passthroughVars' => [
                    'route' => $this->generateUrl('milex_installer_step', ['index' => $index]),
                ],
            ]
        );
    }
    ...
    public function finalAction()
    {
        ...
        if ($this->installer->checkIfInstalled()) {
            if (!$session->has('milex.installer.completedsteps')) {
            ...
            return $this->redirect($this->generateUrl('milex_installer_home'));
        }

        // Remove installer session variables
        $session->remove('milex.installer.completedsteps');
        $session->remove('milex.installer.user');

        $this->installer->finalMigrationStep();

        $welcomeUrl = $this->generateUrl('milex_dashboard_index');
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    ...
                    'parameters'  => $this->configurator->render(),
                    'config_path' => $this->get('milex.helper.paths')->getSystemPath('local_config'),
                ...
                'contentTemplate' => 'MilexInstallBundle:Install:final.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_installer_index',
                    'milexContent' => 'installer',
                    'route'         => $this->generateUrl('milex_installer_final'),
                ],
            ]
        );
    }
