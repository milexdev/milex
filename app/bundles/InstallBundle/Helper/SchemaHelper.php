...
namespace Milex\InstallBundle\Helper;
...
class SchemaHelper
{
    ...
    public function installSchema()
    {
        ...
        foreach ($installSchema->getTables() as $m) {
            $tableName                = $m->getName();
            $milexTables[$tableName] = $this->generateBackupName($this->dbParams['table_prefix'], $backupPrefix, $tableName);
        }

        $sql = 'sqlite' === $this->em->getConnection()->getDatabasePlatform()->getName() ? [] : ['SET foreign_key_checks = 0;'];
        if ($this->dbParams['backup_tables']) {
            $sql = array_merge($sql, $this->backupExistingSchema($tables, $milexTables, $backupPrefix));
        } else {
            $sql = array_merge($sql, $this->dropExistingSchema($tables, $milexTables));
        }
    ...
    public function validateDatabaseVersion(): void
    {
        ...
        if (false !== strpos($platform, 'mariadb') || false !== strpos(strtolower($version), 'mariadb')) {
            ...
            throw new \Exception('Invalid database platform '.$platform.'. Milex only supports MySQL and MariaDB!');
        }
     ...
     * @throws DBALException
     */
    protected function backupExistingSchema($tables, $milexTables, $backupPrefix)
    {
        $sql = [];
        $sm  = $this->db->getSchemaManager();
        ...
        foreach ($tables as $t) {
            if (!isset($milexTables[$t]) && !in_array($t, $milexTables)) {
            ...
            if (isset($milexTables[$t])) {
                //to be backed up
                $backupRestraints[$milexTables[$t]] = $restraints;
                $backupTables[$t]                    = $milexTables[$t];
     ...
     * @return array
     */
    protected function dropExistingSchema($tables, $milexTables)
    {
        $sql = [];
