...
namespace Milex\InstallBundle\Tests\Command;
...
class InstallCommandTest extends TestCase
{
    ...
    public function testCommandWhenSiteInstalled(): void
    {
        ...
        $this->container->method('get')->with('milex.install.service')->willReturn($this->installer);

        $input  = new ArrayInput(['site_url' => 'localhost']);
        $output = new BufferedOutput();
        $this->command->run($input, $output);

        $this->assertSame('Milex already installed'.PHP_EOL, $output->fetch());
    }
    ...
    public function testCommandWhenSiteNotInstalled(): void
    {
        ...
        $this->container->method('get')
            ->withConsecutive(['milex.install.service'], ['doctrine'])
