...
namespace Milex\InstallBundle\Tests\EventListener;
...
use Milex\InstallBundle\EventListener\DoctrineEventSubscriber;
...
class DoctrineEventSubscriberTest extends TestCase
{
    ...
    public function testSubscriberWillAddCorrectIndexes(): void
    {
        ...
        $table      = new Table(MILEX_TABLE_PREFIX.'leads', [$idColumn, $textColumn, $dateColumn]);
        ...
        Assert::assertTrue($schema->hasTable(MILEX_TABLE_PREFIX.'leads'));
        $contactsTable = $schema->getTable(MILEX_TABLE_PREFIX.'leads');
    ...
    {
        $table      = new Table(MILEX_TABLE_PREFIX.'some_plugin_table', [new Column('id', new BigIntType())]);
        $schema     = new Schema([$table]);
        $args       = new GenerateSchemaEventArgs($this->entityManager, $schema);
        $subscriber = new DoctrineEventSubscriber();
        $subscriber->postGenerateSchema($args);

        Assert::assertTrue($schema->hasTable(MILEX_TABLE_PREFIX.'some_plugin_table'));
        Assert::assertFalse($schema->hasTable(MILEX_TABLE_PREFIX.'leads'));
    }
}
