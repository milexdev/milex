...
namespace Milex\InstallBundle\Install;
...
use Milex\InstallBundle\Configurator\Step\DoctrineStep;
use Milex\InstallBundle\Exception\AlreadyInstalledException;
use Milex\InstallBundle\Exception\DatabaseVersionTooOldException;
...
class InstallService
{
        ...
        try {
            ...
            $messages = [
                'error' => $this->translator->trans(
                    'milex.installer.error.writing.configuration',
                    [],
                    'flashes'
                ),
            ];
    ...
    public function validateDatabaseParams(array $dbParams): array
    {
        ...
        if (!isset($dbParams['port']) || (int) $dbParams['port'] <= 0) {
            $messages['port'] = $this->translator->trans(
                'milex.install.database.port.invalid',
                [],
                'validators'
            );
        }
        ...
        if (!empty($dbParams['driver']) && !in_array($dbParams['driver'], DoctrineStep::getDriverKeys())) {
            $messages['driver'] = $this->translator->trans(
                'milex.install.database.driver.invalid',
    ...
    public function createDatabaseStep(StepInterface $step, array $dbParams): array
    {
        ...
        try {
            ...
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.creating.database',
                ['%name%' => $dbParams['name']],
                'flashes'
            );
            ...
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.database.version',
                [
            ...
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.connecting.database',
    ...
    public function createSchemaStep(array $dbParams): array
    {
        ...
        try {
            if (!$schemaHelper->installSchema()) {
                $messages['error'] = $this->translator->trans(
                    'milex.installer.error.no.metadata',
                    [],
                    'flashes');
            }
        } catch (\Exception $exception) {
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.installing.data',
                ['%exception%' => $exception->getMessage()],
                'flashes');
        }
    ...
    public function createFixturesStep(ContainerInterface $container): array
    {
        ...
        try {
            ...
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.adding.fixtures',
                ['%exception%' => $exception->getMessage()],
                'flashes'
            );
        }
    ...
    public function createAdminUserStep(array $data): array
    {
        ...
        try {
            $existingUser = $entityManager->getRepository('MilexUserBundle:User')->find(1);
        } catch (\Exception $e) {
            $existingUser = null;
        }
        ...
        $emailConstraint->message = $this->translator->trans('milex.core.email.required', [], 'validators');

        $passwordConstraint             = new Assert\Length(['min' => 6]);
        $passwordConstraint->minMessage = $this->translator->trans('milex.install.password.minlength', [], 'validators');
        ...
        $adminRole = null;
        try {
            $adminRole = $entityManager->getReference('MilexUserBundle:Role', 1);
        } catch (\Exception $exception) {
            $messages['error'] = $this->translator->trans(
                'milex.installer.error.getting.role',
                ['%exception%' => $exception->getMessage()],
                'flashes'
            );
        }
        ...
        if (!empty($adminRole)) {
            ...
            try {
                ...
                $messages['error'] = $this->translator->trans(
                    'milex.installer.error.creating.user',
                    ['%exception%' => $exception->getMessage()],
                    'flashes'
                );
            }
    ...
    public function setupEmailStep(StepInterface $step, array $data): array
    {
        ...
        $emailConstraint          = new Assert\Email();
        $emailConstraint->message = $this->translator->trans('milex.core.email.required',
            [],
            'validators'
        );
