...
use Milex\ReportBundle\Form\Type\DynamicFiltersType;
...
class ReportController extends FormController
{
    ...
    {
        /* @type \Milex\ReportBundle\Model\ReportModel $model */
        $model = $this->getModel('report');

        //set some permissions
        $permissions = $this->container->get('milex.security')->isGranted(
            [
        ...
        $pageHelper        = $pageHelperFacotry->make('milex.report', $page);

        $limit  = $pageHelper->getLimit();
        $start  = $pageHelper->getStart();
        $search = $this->request->get('search', $this->container->get('session')->get('milex.report.filter', ''));
        $filter = ['string' => $search, 'force' => []];
        $this->container->get('session')->set('milex.report.filter', $search);

        if (!$permissions['report:reports:viewother']) {
            $filter['force'][] = ['column' => 'r.createdBy', 'expr' => 'eq', 'value' => $this->user->getId()];
        }

        $orderBy    = $this->container->get('session')->get('milex.report.orderby', 'r.name');
        $orderByDir = $this->container->get('session')->get('milex.report.orderbydir', 'DESC');
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('milex_report_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect(
                [
                    ...
                    'contentTemplate' => 'MilexReportBundle:Report:index',
                    'passthroughVars' => [
                        'activeLink'    => '#milex_report_index',
                        'milexContent' => 'report',
                    ],
                ]
            );
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MilexReportBundle:Report:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_report_index',
                    'milexContent' => 'report',
                    'route'         => $this->generateUrl('milex_report_index', ['page' => $page]),
                ],
            ]
        );
    }
    ...
    public function cloneAction($objectId)
    {
        ...
        if (null != $entity) {
            if (!$this->container->get('milex.security')->isGranted('report:reports:create')
                || !$this->container->get('milex.security')->hasEntityAccess(
                    'report:reports:viewown',
                    'report:reports:viewother',
                    $entity->getCreatedBy()
                )
    ...
    {
        $page      = $this->container->get('session')->get('milex.report.page', 1);
        $returnUrl = $this->generateUrl('milex_report_index', ['page' => $page]);
        $flashes   = [];
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            foreach ($ids as $objectId) {
                ...
                if (null === $entity) {
                    $flashes[] = [
                        'type'    => 'error',
                        'msg'     => 'milex.report.report.error.notfound',
                        'msgVars' => ['%id%' => $objectId],
                    ];
                } elseif (!$this->container->get('milex.security')->hasEntityAccess(
                    'report:reports:deleteown',
                    'report:reports:deleteother',
                    $entity->getCreatedBy()
                )
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'milex.report.report.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        $page    = $session->get('milex.report.page', 1);
        ...
        //Create the form
        $action = $this->generateUrl('milex_report_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        ...
        if (!$ignorePost && 'POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $template   = 'MilexReportBundle:Report:view';
                } else {
                    //reset old columns
                ...
                $template   = 'MilexReportBundle:Report:index';
            }

            if ($cancelled || ($valid && $form->get('buttons')->get('save')->isClicked())) {
                // Clear session items in case columns changed
                $session->remove('milex.report.'.$entity->getId().'.orderby');
                $session->remove('milex.report.'.$entity->getId().'.orderbydir');
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    'report' => $entity,
                    'form'   => $this->setFormTheme($form, 'MilexReportBundle:Report:form.html.php', 'MilexReportBundle:FormTheme\Report'),
                ],
                'contentTemplate' => 'MilexReportBundle:Report:form.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#milex_report_index',
                    'milexContent' => 'report',
     ...
     *
     * @param \Milex\ReportBundle\Entity\Report|null $entity
     *
     * @return HttpFoundation\JsonResponse|HttpFoundation\RedirectResponse|HttpFoundation\Response
     */
    public function newAction($entity = null)
    {
        ...
        if (!($entity instanceof Report)) {
            /** @var \Milex\ReportBundle\Entity\Report $entity */
        ...
        $action = $this->generateUrl('milex_report_action', ['objectAction' => 'new']);
        $form   = $model->createForm($entity, $this->get('form.factory'), $action);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $returnUrl = $this->generateUrl('milex_report_view', $viewParameters);
                    $template  = 'MilexReportBundle:Report:view';
                }
            } else {
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('milex_report_index', $viewParameters);
                $template       = 'MilexReportBundle:Report:index';
            }
    ...
    public function viewAction($objectId, $reportPage = 1)
    {
        ...
        // Init the forms
        $action = $this->generateUrl('milex_report_action', ['objectAction' => 'view', 'objectId' => $objectId]);
        ...
        if (!empty($dateRangeValues['date_from'])) {
            ...
            $session->set('milex.report.date.from', $from->format($mysqlFormat));
        } elseif ($fromDate = $session->get('milex.report.date.from')) {
            $dateRangeValues['date_from'] = $fromDate;
        }
        if (!empty($dateRangeValues['date_to'])) {
            $to = new \DateTime($dateRangeValues['date_to']);
            $session->set('milex.report.date.to', $to->format($mysqlFormat));
        } elseif ($toDate = $session->get('milex.report.date.to')) {
        ...
        if ('POST' == $this->request->getMethod() && $this->request->request->has('daterange')) {
            if ($this->isFormValid($dateRangeForm)) {
                ...
                $session->set('milex.report.date.to', $dateRangeValues['date_to']);

                $from                         = new \DateTime($dateRangeForm['date_from']->getData());
                $dateRangeValues['date_from'] = $from->format($mysqlFormat);
                $session->set('milex.report.date.from', $dateRangeValues['date_from']);
            }
        }
        ...
        $dynamicFilters = $session->get('milex.report.'.$objectId.'.filters', []);
        $filterSettings = [];
     ...
     * @param object                               $entity
     * @param int                                  $objectId
     * @param \Milex\CoreBundle\Model\CommonModel $model
    ...
    private function checkEntityAccess($postActionVars, $entity, $objectId, array $permissions, $model, $modelName)
    {
        ...
        } elseif (!$this->container->get('milex.security')->hasEntityAccess($permissions[0], $permissions[1], $entity->getCreatedBy())) {
            return $this->accessDenied();
        } elseif ($model->isLocked($entity)) {
    ...
    public function exportAction($objectId, $format = 'csv')
    {
        ...
        $fromDate = $session->get('milex.report.date.from', (new \DateTime('-30 days'))->format('Y-m-d'));
        $toDate   = $session->get('milex.report.date.to', (new \DateTime())->format('Y-m-d'));
    ...
    {
        if ('csv' !== $format) {
            throw new \Exception($this->translator->trans('milex.format.invalid', ['%format%' => $format, '%validFormats%' => 'csv']));
        }
        ...
        /** @var \Milex\ReportBundle\Scheduler\Model\FileHandler $fileHandler */
        $fileHandler = $this->container->get('milex.report.model.file_handler');

        if (empty($report)) {
            return $this->notFound($this->translator->trans('milex.report.notfound', ['%id%' => $reportId]));
        }
        ...
        if (!$fileHandler->compressedCsvFileForReportExists($report)) {
            if ($report->isScheduled()) {
                $message = 'milex.report.download.missing';
            } else {
                $message = 'milex.report.download.missing.but.scheduled';
