...
use Milex\CoreBundle\Helper\FileProperties;
...
class MessageSchedule
{
    ...
    {
        $link = $this->router->generate('milex_report_view', ['objectId' => $report->getId()], UrlGeneratorInterface::ABSOLUTE_URL);

        if ($this->fileCouldBeSend($filePath)) {
            $date = new \DateTime();

            return $this->translator->trans(
                'milex.report.schedule.email.message',
                ['%report_name%' => $report->getName(), '%date%' => $date->format('Y-m-d'), '%link%' => $link]
            );
        }

        return $this->translator->trans(
            'milex.report.schedule.email.message_file_not_attached',
    ...
    {
        $link = $this->router->generate('milex_report_download', ['reportId' => $report->getId()], UrlGeneratorInterface::ABSOLUTE_URL);

        return $this->translator->trans(
            'milex.report.schedule.email.message_file_linked',
            ['%report_name%' => $report->getName(), '%link%' => $link]
        );
    }
    ...
    public function getSubject(Report $report)
    {
        ...
        return $this->translator->trans(
            'milex.report.schedule.email.subject',
            ['%report_name%' => $report->getName(), '%date%' => $date->format('Y-m-d')]
        );
    }

    /**
     * @deprecated 2.16.0 use \Milex\ReportBundle\Scheduler\Model\FileHandler::fileCanBeAttached instead. To be removed in 3.0.0.
     *
     * @param string $filePath
     *
     * @return bool
     *
     * @throws \Milex\CoreBundle\Exception\FileInvalidException
     */
