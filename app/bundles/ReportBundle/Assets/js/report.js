//ReportBundle
Milex.reportOnLoad = function (container) {
    // Activate search if the container exists
    if (mQuery(container + ' #list-search').length) {
        Milex.activateSearchAutocomplete('list-search', 'report');
    }

    // Append an index of the number of filters on the edit form
    if (mQuery('div[id=report_filters]').length) {
        mQuery('div[id=report_filters]').attr('data-index', Milex.getHighestIndex('report_filters'));
        mQuery('div[id=report_tableOrder]').attr('data-index', Milex.getHighestIndex('report_tableOrder'));
        mQuery('div[id=report_aggregators]').attr('data-index', Milex.getHighestIndex('report_aggregators'));

        if (mQuery('.filter-columns').length) {
            mQuery('.filter-columns').each(function () {
                Milex.updateReportFilterValueInput(this, true);
                mQuery(this).on('change', function () {
                    Milex.updateReportFilterValueInput(this);
                });
            });
        }
    } else {
    ...
    Milex.updateReportGlueTriggers();
    Milex.checkSelectedGroupBy();
    Milex.initDateRangePicker();
    ...
    mQuery($isScheduled).change(function () {
        Milex.scheduleDisplay($isScheduled, $unitTypeId, $scheduleDay, $scheduleMonthFrequency);
    });
    mQuery($unitTypeId).change(function () {
        Milex.scheduleDisplay($isScheduled, $unitTypeId, $scheduleDay, $scheduleMonthFrequency);
    });
    mQuery($scheduleDay).change(function () {
        Milex.schedulePreview($isScheduled, $unitTypeId, $scheduleDay, $scheduleMonthFrequency);
    });
...
Milex.scheduleDisplay = function ($isScheduled, $unitTypeId, $scheduleDay, $scheduleMonthFrequency) {
    Milex.checkIsScheduled($isScheduled);
...
Milex.schedulePreview = function ($isScheduled, $unitTypeId, $scheduleDay, $scheduleMonthFrequency) {
...
Milex.checkIsScheduled = function ($isScheduled) {
 ...
 */
Milex.addReportRow = function (elId) {
    ...
    if (elId == 'report_filters') {
        if (typeof Milex.reportPrototypeFilterOptions != 'undefined') {
            // Update the column options if applicable
            mQuery(newColumnId).html(Milex.reportPrototypeFilterOptions);
        }
        ...
        Milex.updateReportFilterValueInput(newColumnId);
        Milex.updateReportGlueTriggers();
    } else if (typeof Milex.reportPrototypeColumnOptions != 'undefined') {
        // Update the column options if applicable
        mQuery(newColumnId).html(Milex.reportPrototypeColumnOptions);
    }

    Milex.activateChosenSelect(mQuery('#' + elId + '_' + index + '_column'));
    mQuery("#" + elId + " *[data-toggle='tooltip']").tooltip({html: true, container: 'body'});

};

Milex.updateReportGlueTriggers = function () {
...
Milex.updateReportFilterValueInput = function (filterColumn, setup) {
    var definitions = (typeof Milex.reportPrototypeFilterDefinitions != 'undefined') ? Milex.reportPrototypeFilterDefinitions : mQuery('#report_filters').data('filter-definitions');
    var operators = (typeof Milex.reportPrototypeFilterOperators != 'undefined') ? Milex.reportPrototypeFilterOperators : mQuery('#report_filters').data('filter-operators');
    ...
    Milex.destroyChosen(mQuery('#' + valueId));
    ...
    if ((filterType == 'multiselect' || filterType == 'select') && typeof definitions[newValue].list != 'undefined') {
        ...
        Milex.activateChosenSelect(newSelect);
    }

    // Activate datetime
    if (filterType == 'datetime' || filterType == 'date' || filterType == 'time') {
        Milex.activateDateTimeInputs('#' + valueId, filterType);
    } else if (mQuery('#' + valueId).hasClass('calendar-activated')) {
        mQuery('#' + valueId).datetimepicker('destroy');
    }
};

Milex.removeReportRow = function (container) {
    mQuery("#" + container + " *[data-toggle='tooltip']").tooltip('destroy');
    mQuery('#' + container).remove();
};

Milex.updateReportSourceData = function (context) {
    Milex.activateLabelLoadingIndicator('report_source');
    mQuery.ajax({
        url: milexAjaxUrl,
        type: 'post',
        ...
        success: function (response) {
            ...
            // Update columns
            Milex.reportPrototypeColumnOptions = mQuery(response.columns);
            ...
            // Update filter list
            Milex.reportPrototypeFilterDefinitions = response.filterDefinitions;
            Milex.reportPrototypeFilterOptions = response.filters;
            Milex.reportPrototypeFilterOperators = response.filterOperators;
...
Milex.checkReportCondition = function (selector) {
...
Milex.checkSelectedGroupBy = function () {
    ...
    if (selectedOption > 0) {
        ...
        existingAggregators.each(function() {
            ...
            Milex.removeReportRow(containerId + '_container');
        });
        mQuery('#aggregators-button').prop('disabled', true);
    }
};

Milex.getHighestIndex = function (selector) {
    var highestIndex = 1;
