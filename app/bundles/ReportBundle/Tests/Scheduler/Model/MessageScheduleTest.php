...
use Milex\ReportBundle\Scheduler\Model\MessageSchedule;
...
class MessageScheduleTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testSendFile($fileSize, $limit)
    {
        $this->translatorMock->expects($this->once())
            ->method('trans')
            ->with('milex.report.schedule.email.message')
            ->willReturn('Subject');
    ...
    public function testDoSendFile($fileSize, $limit)
    {
        $this->translatorMock->expects($this->once())
            ->method('trans')
            ->with('milex.report.schedule.email.message_file_not_attached')
            ->willReturn('Subject');
    ...
    public function testGetMessageForAttachedFile()
    {
        ...
        $this->router->expects($this->once())
            ->method('generate')
            ->with('milex_report_view', ['objectId' => 33], UrlGeneratorInterface::ABSOLUTE_URL)
    ...
    public function testGetMessageForLinkedFile()
    {
        ...
        $this->router->expects($this->once())
            ->method('generate')
            ->with('milex_report_download', ['reportId' => 33], UrlGeneratorInterface::ABSOLUTE_URL)
            ->willReturn('absolute/link');

        $this->translatorMock->expects($this->once())
            ->method('trans')
            ->with('milex.report.schedule.email.message_file_linked')
    ...
    public function testGetSubject()
    {
        ...
        $this->translatorMock->expects($this->once())
            ->method('trans')
            ->with('milex.report.schedule.email.subject')
