...
class ReportBuilderEvent extends AbstractReportEvent
{
    ...
    public function getLeadColumns($prefix = 'l.')
    {
        ...
        foreach ($this->leadFields as $fieldArray) {
            $fields[$prefix.$fieldArray['alias']] = [
                'label' => $this->translator->trans('milex.report.field.lead.label', ['%field%' => $fieldArray['label']]),
                'type'  => $this->reportHelper->getReportBuilderFieldType($fieldArray['type']),
                'alias' => $fieldArray['alias'],
            ];
        }
        $fields[$prefix.'id'] = [
            'label' => 'milex.report.field.lead.id',
            'type'  => 'int',
    ...
    public function getCampaignByChannelColumns()
    {
        return [
            'clel.campaign_id' => [
                'label' => 'milex.campaign.campaign.id',
                'type'  => 'string',
            ],
            'cmp.name' => [
                'label' => 'milex.campaign.campaign',
                'type'  => 'string',
            ],
        ];
    }
    ...
    {
        $channelColumns = [
            MilexReportBuilder::CHANNEL_COLUMN_CATEGORY_ID => [
                'label'       => 'milex.report.campaign.channel.category_id',
                'type'        => 'int',
                'alias'       => 'channel_category_id',
                'channelData' => [],
            ],
            MilexReportBuilder::CHANNEL_COLUMN_CREATED_BY => [
                'label'       => 'milex.report.campaign.channel.created_by',
                'type'        => 'int',
                'alias'       => 'channel_created_by',
                'channelData' => [],
            ],
            MilexReportBuilder::CHANNEL_COLUMN_CREATED_BY_USER => [
                'label'       => 'milex.report.campaign.channel.created_by_user',
                'type'        => 'string',
                'alias'       => 'channel_created_by_user',
                'channelData' => [],
            ],
            MilexReportBuilder::CHANNEL_COLUMN_DATE_ADDED => [
                'label'       => 'milex.report.campaign.channel.date_added',
                'type'        => 'datetime',
                'alias'       => 'channel_date_added',
                'channelData' => [],
            ],
            MilexReportBuilder::CHANNEL_COLUMN_DESCRIPTION => [
                'label'       => 'milex.report.campaign.channel.description',
                'type'        => 'string',
                'alias'       => 'channel_description',
                'channelData' => [],
            ],
            MilexReportBuilder::CHANNEL_COLUMN_NAME => [
                'label'       => 'milex.report.campaign.channel.name',
                'type'        => 'string',
                'alias'       => 'channel_name',
                'channelData' => [],
            ],
