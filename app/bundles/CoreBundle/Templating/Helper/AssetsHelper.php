...
use Milex\CoreBundle\Helper\AssetGenerationHelper;
...
class AssetsHelper
{
    ...
    public function getStyles()
    {
        ...
        if (isset($this->assets[$this->context]['stylesheets'])) {
            foreach (array_reverse($this->assets[$this->context]['stylesheets']) as $s) {
                $styles .= '<link rel="stylesheet" href="'.$this->getUrl($s).'" data-source="milex" />'."\n";
            }
        }

        if (isset($this->assets[$this->context]['styleDeclarations'])) {
            $styles .= "<style data-source=\"milex\">\n";
    ...
    public function outputScripts($location)
    {
        if (isset($this->assets[$this->context]['scripts'][$location])) {
            foreach (array_reverse($this->assets[$this->context]['scripts'][$location]) as $s) {
                list($script, $async) = $s;
                echo '<script src="'.$this->getUrl($script).'"'.($async ? ' async' : '').' data-source="milex"></script>'."\n";
            }
        }

        if (isset($this->assets[$this->context]['scriptDeclarations'][$location])) {
            echo "<script data-source=\"milex\">\n";
            ...
            foreach ($this->assets[$this->context]['headDeclarations'] as $declaration) {
                ...
                switch ($type) {
                    case 'script':
                        ...
                        $headOutput .= "\n".'<script src="'.$this->getUrl($script).'"'.($async ? ' async' : '').' data-source="milex"></script>';
                        break;
                    case 'custom':
                    case 'declaration':
    ...
    public function outputSystemStylesheets()
    {
        ...
        if (isset($assets['css'])) {
            foreach ($assets['css'] as $url) {
                echo '<link rel="stylesheet" href="'.$this->getUrl($url).'" data-source="milex" />'."\n";
            }
        }
    }
    ...
    public function outputSystemScripts($includeEditor = false)
    {
        ...
        if (isset($assets['js'])) {
            foreach ($assets['js'] as $url) {
                echo '<script src="'.$this->getUrl($url).'" data-source="milex"></script>'."\n";
            }
        }

        if ($this->installService->checkIfInstalled()) {
            /**
             * We want to enable JS consumers to simply query Milex.getActiveBuilderName() so they can add logic based on the active builder.
            ...
            echo '<script>Milex.getActiveBuilderName = function() { return \''.$builderName.'\'; }</script>'."\n";
        }
    }
    ...
    public function getSystemScripts($render = false, $includeEditor = false)
    {
        ...
        if ($render) {
            ...
            if (isset($assets['js'])) {
                foreach ($assets['js'] as $url) {
                    $js .= '<script src="'.$this->getUrl($url).'" data-source="milex"></script>'."\n";
                }
            }
     ...
     *
     * @param string $assetFilePath         The path to the file location. Can use full path or relative to milex web root
     * @param string $onLoadCallback        Milex namespaced function to call for the script onload
     * @param string $alreadyLoadedCallback Milex namespaced function to call if the script has already been loaded
     *
     * @return string
     */
    public function includeScript($assetFilePath, $onLoadCallback = '', $alreadyLoadedCallback = '')
    {
        return '<script async="async" type="text/javascript" data-source="milex">Milex.loadScript(\''.$this->getUrl($assetFilePath)."', '$onLoadCallback', '$alreadyLoadedCallback');</script>";
    }
    ...
    {
        return '<script async="async" type="text/javascript" data-source="milex">Milex.loadStylesheet(\''.$this->getUrl($assetFilePath).'\');</script>';
    }
