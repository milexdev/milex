...
class NotificationRepository extends CommonRepository
{
    ...
    {
        $this->_em->getConnection()->update(MILEX_TABLE_PREFIX.'notifications', ['is_read' => 1], ['user_id' => (int) $userId]);
    }
    ...
    public function clearNotificationsForUser($userId, $id = null, $limit = null)
    {
        if (!empty($id)) {
            $this->getEntityManager()->getConnection()->update(
                MILEX_TABLE_PREFIX.'notifications',
                [
                    'is_read' => 1,
                ],
                [
            ...
            $qb->update(MILEX_TABLE_PREFIX.'notifications')
                ->set('is_read', 1)
