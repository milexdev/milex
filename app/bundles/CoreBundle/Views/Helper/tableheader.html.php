                ...
                [
                    'confirm' => [
                        'message' => $view['translator']->hasId($translationBase.'.form.confirmbatchdelete') ?
                            ...
                            $view['translator']->trans('milex.core.form.confirmbatchdelete'),
...
$order        = (!empty($order)) ? $order : $app->getSession()->get("milex.{$sessionVar}.orderby", $defaultOrder);
$dir          = (!empty($dir)) ? $dir : $app->getSession()->get("milex.{$sessionVar}.orderbydir", 'ASC');
$filters      = (!empty($filters)) ? $filters : $app->getSession()->get("milex.{$sessionVar}.filters", []);
$tmpl         = (!empty($tmpl)) ? $tmpl : 'list';
?>
<th<?php echo (!empty($class)) ? ' class="'.$class.'"' : ''; ?>>
    <div class="thead-filter">
        <?php if (!empty($orderBy)): ?>
        <a href="javascript: void(0);" onclick="Milex.reorderTableData('<?php echo $sessionVar; ?>','<?php echo $orderBy; ?>','<?php echo $tmpl; ?>','<?php echo $target; ?>'<?php if (!empty($baseUrl)): ?>, '<?php echo $baseUrl; ?>'<?php endif; ?>);">
            <span><?php echo $view['translator']->trans($text); ?></span>
        ...
        <div class="input-group input-group-sm">
            ...
            <input type="text" placeholder="<?php echo $view['translator']->trans('milex.core.form.thead.filter'); ?>" autocomplete="false" class="form-control input-sm" value="<?php echo $view->escape($value); ?>"<?php echo $toggle; ?> onchange="Milex.filterTableData('<?php echo $sessionVar; ?>','<?php echo $filterBy; ?>',this.value,'<?php echo $tmpl; ?>','<?php echo $target; ?>'<?php if (!empty($baseUrl)): ?>, '<?php echo $baseUrl; ?>'<?php endif; ?>);" />
            <?php $inputClass = (!empty($value)) ? 'fa-times' : 'fa-filter'; ?>
            <span class="input-group-btn">
                <button class="btn btn-default btn-xs" onclick="Milex.filterTableData('<?php echo $sessionVar; ?>','<?php echo $filterBy; ?>',<?php echo (!empty($value)) ? "''," : 'mQuery(this).parent().prev().val(),'; ?>'<?php echo $tmpl; ?>','<?php echo $target; ?>'<?php if (!empty($baseUrl)): ?>, '<?php echo $baseUrl; ?>'<?php endif; ?>);">
