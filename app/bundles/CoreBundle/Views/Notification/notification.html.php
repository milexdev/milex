...
<div class="media pt-sm pb-sm pr-md pl-md nm bdr-b notification" id="notification<?php echo $n['id']; ?>">
    <span class="pull-left mt-xs" style="width:36px">
        ...
        <?php else: ?>
        <?php $tooltip = (!empty($n['type'])) ? ' data-toggle="tooltip" title="'.$view['translator']->trans('milex.notifications.type.'.$n['type']).'"' : ''; ?>
        <i class="fa fa-2x <?php echo (!empty($n['iconClass'])) ? $n['iconClass'] : 'fa-star'; ?>"<?php echo $tooltip; ?>></i>
        <?php endif; ?>
    </span>
    <a href="javascript:void(0);" class="btn btn-default btn-xs btn-nospin pull-right do-not-close" data-toggle="tooltip" title="<?php echo $view['translator']->trans('milex.core.notifications.clear'); ?>" onclick="Milex.clearNotification(<?php echo $n['id']; ?>);"><i class="fa fa-times do-not-close"></i></a>
    <?php if (!$n['isRead']): ?>
