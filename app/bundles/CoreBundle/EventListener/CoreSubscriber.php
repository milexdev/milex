...
use Milex\CoreBundle\Event\IconEvent;
...
class CoreSubscriber implements EventSubscriberInterface
{
    ...
    /**
     * Add milexForms in js script tag for Froala.
     */
    public function onKernelRequestAddGlobalJS(FilterControllerEvent $event)
    {
        if (defined('MILEX_INSTALLER') || $this->userHelper->getUser()->isGuest() || !$event->isMasterRequest()) {
            return;
        }

        $list        = $this->formRepository->getSimpleList();
        $milexForms = json_encode($list, JSON_FORCE_OBJECT | JSON_PRETTY_PRINT);

        $this->assetsHelper->addScriptDeclaration("var milexForms = {$milexForms};");
    }
    ...
    public function onSecurityInteractiveLogin(InteractiveLoginEvent $event)
    {
        ...
        if ($this->securityContext->isGranted('IS_AUTHENTICATED_FULLY') || $this->securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
            ...
            $session->set('milex.user', $user->getId());
    ...
    public function onKernelController(FilterControllerEvent $event)
    {
        ...
        //only affect Milex controllers
        if ($controller[0] instanceof MilexController) {
    ...
    {
        $name    = $event->getType();
        $bundles = $this->bundleHelper->getMilexBundles(true);
            ...
            if (!empty($bundle['config']['routes'][$type])) {
                foreach ($bundle['config']['routes'][$type] as $name => $details) {
                    if ('api' == $type && !empty($details['standard_entity'])) {
                        ...
                        $routeName  = 'milex_api_'.$details['name'].'_';
    ...
    public function onFetchIcons(IconEvent $event)
    {
        ...
        $icons   = $session->get('milex.menu.icons', []);
        ...
        if (empty($icons)) {
            ...
            $session->set('milex.menu.icons', $icons);
        } else {
