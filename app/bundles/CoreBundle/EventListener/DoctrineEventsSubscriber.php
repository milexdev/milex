...
use Milex\CoreBundle\Entity\DeprecatedInterface;
...
class DoctrineEventsSubscriber implements EventSubscriber
{
    ...
    {
        //in the installer
        if (!defined('MILEX_TABLE_PREFIX') && empty($this->tablePrefix)) {
            return;
        } elseif (empty($this->tablePrefix)) {
            $this->tablePrefix = MILEX_TABLE_PREFIX;
        }
        ...
        if (false !== strpos($classMetadata->namespace, 'Milex')) {
