...
use Milex\CoreBundle\Event\CommandListEvent;
use Milex\CoreBundle\Event\GlobalSearchEvent;
use Milex\CoreBundle\Event\UpgradeEvent;
use Milex\CoreBundle\Exception\RecordCanNotUnpublishException;
...
use Milex\CoreBundle\Helper\Update\PreUpdateChecks\PreUpdateCheckError;
use Milex\CoreBundle\Helper\UpdateHelper;
...
class AjaxController extends CommonController
{
            ...
            if (false !== strpos($action, ':')) {
                ...
                if (2 == count($parts)) {
                    ...
                    if (!$classExists = class_exists($namespace.'\\'.$bundle.'Bundle\\Controller\\AjaxController')) {
                        // Check if a plugin is prefixed with Milex
                        $bundle      = 'Milex'.$bundle;
    ...
    protected function globalSearchAction(Request $request)
    {
        ...
        $dataArray['newContent'] = $this->renderView(
            'MilexCoreBundle:GlobalSearch:results.html.php',
    ...
    protected function togglePublishStatusAction(Request $request)
    {
        ...
        if (null !== $entity) {
            ...
            if ($hasPermission) {
                ...
                $this->addFlash('milex.core.error.access.denied');
    ...
    protected function updateSetUpdateLayoutAction(Request $request)
    {
        $dataArray = [
            'success' => 1,
            'content' => $this->renderView('MilexCoreBundle:Update:update.html.php'),
        ];

        // A way to keep the upgrade from failing if the session is lost after
        // the cache is cleared by upgrade.php
        /** @var \Milex\CoreBundle\Helper\CookieHelper $cookieHelper */
        $cookieHelper = $this->container->get('milex.helper.cookie');
        $cookieHelper->setCookie('milex_update', 'setupUpdate', 300);
    ...
    protected function updateRunChecksAction(): JsonResponse
    {
        ...
        if (true === $coreParametersHelper->get('composer_updates', false)) {
            $errors = [$translator->trans('milex.core.update.composer')];
        } else {
        ...
        if (!empty($errors)) {
            $dataArray['success']    = 0;
            $dataArray['stepStatus'] = $translator->trans('milex.core.update.step.failed');
            $dataArray['message']    = $translator->trans('milex.core.update.check.error');
            $dataArray['errors']     = $errors;

            // A way to keep the upgrade from failing if the session is lost after
            // the cache is cleared by upgrade.php
            $cookieHelper->deleteCookie('milex_update');
        } else {
            $dataArray['success']        = 1;
            $dataArray['stepStatus']     = $translator->trans('milex.core.update.step.success');
            $dataArray['nextStep']       = $translator->trans('milex.core.update.step.downloading.package');
            $dataArray['nextStepStatus'] = $translator->trans('milex.core.update.step.in.progress');

            // A way to keep the upgrade from failing if the session is lost after
            // the cache is cleared by upgrade.php
            $cookieHelper->setCookie('milex_update', 'runChecks', 300);
        }
    ...
    protected function updateDownloadPackageAction(Request $request)
    {
        ...
        if ($package['error']) {
            $dataArray['stepStatus'] = $translator->trans('milex.core.update.step.failed');
            $dataArray['message']    = $translator->trans('milex.core.update.error', ['%error%' => $translator->trans($package['message'])]);
            ...
            $dataArray['success']        = 1;
            $dataArray['stepStatus']     = $translator->trans('milex.core.update.step.success');
            $dataArray['nextStep']       = $translator->trans('milex.core.update.step.extracting.package');
            $dataArray['nextStepStatus'] = $translator->trans('milex.core.update.step.in.progress');

            // A way to keep the upgrade from failing if the session is lost after
            // the cache is cleared by upgrade.php
            $cookieHelper->setCookie('milex_update', 'downloadPackage', 300);
        }
    ...
    protected function updateExtractPackageAction(Request $request)
    {
        ...
        $pathsHelper = $this->container->get('milex.helper.paths');
        ...
        if (true !== $archive) {
            ...
            switch ($archive) {
                case \ZipArchive::ER_EXISTS:
                    $error = 'milex.core.update.archive_file_exists';
                    break;
                case \ZipArchive::ER_INCONS:
                case \ZipArchive::ER_INVAL:
                case \ZipArchive::ER_MEMORY:
                    $error = 'milex.core.update.archive_zip_corrupt';
                    break;
                case \ZipArchive::ER_NOENT:
                    $error = 'milex.core.update.archive_no_such_file';
                    break;
                case \ZipArchive::ER_NOZIP:
                    $error = 'milex.core.update.archive_not_valid_zip';
                    break;
                case \ZipArchive::ER_READ:
                case \ZipArchive::ER_SEEK:
                case \ZipArchive::ER_OPEN:
                default:
                    $error = 'milex.core.update.archive_could_not_open';
                    break;
            }

            $dataArray['stepStatus'] = $translator->trans('milex.core.update.step.failed');
            $dataArray['message']    = $translator->trans('milex.core.update.error', ['%error%' => $translator->trans($error)]);
            ...
            if (!$zipper->extractTo(dirname($this->container->getParameter('kernel.root_dir')).'/upgrade')) {
                ...
                $dataArray['message']    = $translator->trans(
                    'milex.core.update.error',
                    ['%error%' => $translator->trans('milex.core.update.error_extracting_package')]
                );
            } else {
                $zipper->close();
                ...
                $dataArray['success']        = 1;
                $dataArray['stepStatus']     = $translator->trans('milex.core.update.step.success');
                $dataArray['nextStep']       = $translator->trans('milex.core.update.step.moving.package');
                $dataArray['nextStepStatus'] = $translator->trans('milex.core.update.step.in.progress');

                // A way to keep the upgrade from failing if the session is lost after
                // the cache is cleared by upgrade.php
                $cookieHelper->setCookie('milex_update', 'extractPackage', 300);
            }
        }
    ...
    public function updateDatabaseMigrationAction(Request $request)
    {
        ...
        $cacheDir = $this->container->get('milex.helper.paths')->getSystemPath('cache');
        ...
        if (file_exists($cacheDir.'/'.MILEX_VERSION.'.zip')) {
            @unlink($cacheDir.'/'.MILEX_VERSION.'.zip');
        }
        ...
        if (0 !== $result) {
            ...
            $this->container->get('monolog.logger.milex')->log('error', '[UPGRADE ERROR] Exit code '.$result.'; '.$outputBuffer);

            $dataArray['stepStatus'] = $translator->trans('milex.core.update.step.failed');
            $dataArray['message']    = $translator->trans(
                    'milex.core.update.error',
                    ['%error%' => $translator->trans('milex.core.update.error_performing_migration')]
                ).' <a href="'.$this->generateUrl('milex_core_update_schema', ['update' => 1])
                .'" class="btn btn-primary btn-xs" data-toggle="ajax">'.$translator->trans('milex.core.retry').'</a>';
            ...
            // the cache is cleared by upgrade.php
            /* @var CookieHelper $cookieHelper */
            $cookieHelper->setCookie('milex_update', 'schemaMigration', 300);

            if ($request->get('finalize', false)) {
                // Go to the finalize step
                $dataArray['success']        = 1;
                $dataArray['stepStatus']     = $translator->trans('milex.core.update.step.success');
                $dataArray['nextStep']       = $translator->trans('milex.core.update.step.finalizing');
                $dataArray['nextStepStatus'] = $translator->trans('milex.core.update.step.in.progress');
            } else {
    ...
    public function updateFinalizationAction(Request $request)
    {
        ...
        $dataArray['success'] = 1;
        $dataArray['message'] = $translator->trans('milex.core.update.update_successful', ['%version%' => $this->factory->getVersion()]);
        ...
        if ($postMessage = $this->container->get('session')->get('post_upgrade_message', false)) {
            ...
            $postMessage              = sprintf('<h4 class="mt-lg">%s</h4><p>%s</p>', $this->container->get('translator')->trans('milex.core.update.post_message'), $postMessage);
            $dataArray['postmessage'] = $postMessage;
        }

        // Execute the milex.post_upgrade event
        ...
        $dataArray['redirect'] = $this->container->get('router')->generate('milex_core_update');
    ...
    protected function downloadIpLookupDataStoreAction(Request $request)
    {
        ...
        if ($request->request->has('service')) {
            ...
            /** @var \Milex\CoreBundle\Factory\IpLookupFactory $ipServiceFactory */
            $ipServiceFactory = $this->container->get('milex.ip_lookup.factory');
            ...
            if ($ipService instanceof AbstractLocalDataLookup) {
                if ($ipService->downloadRemoteDataStore()) {
                    $dataArray['success'] = 1;
                    $dataArray['message'] = $this->container->get('translator')->trans('milex.core.success');
                } else {
            ...
            if ($ipService instanceof AbstractLookup) {
                ...
                if ($ipService instanceof IpLookupFormInterface) {
                    if ($formType = $ipService->getConfigFormService()) {
                        ...
                        $themes[] = 'MilexCoreBundle:FormTheme\Config';

                        $form = $this->get('form.factory')->create($formType, [], ['ip_lookup_service' => $ipService]);
                        $html = $this->renderView(
                            'MilexCoreBundle:FormTheme\Config:ip_lookup_config_row.html.php',
                            [
                                'form' => $this->setFormTheme($form, 'MilexCoreBundle:FormTheme\Config:ip_lookup_config_row.html.php', $themes),
                            ]
                        );
