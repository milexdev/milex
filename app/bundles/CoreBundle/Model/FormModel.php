...
class FormModel extends AbstractCommonModel
{
    ...
    public function setTimestamps(&$entity, $isNew, $unlock = true)
    {
        if ($isNew) {
            ...
            if (method_exists($entity, 'setDateModified')) {
                ...
                if ($setDateModified) {
                    $dateModified = (defined('MILEX_DATE_MODIFIED_OVERRIDE')) ? \DateTime::createFromFormat('U', MILEX_DATE_MODIFIED_OVERRIDE)
                        : new \DateTime();
    ...
    public function getUserContactSubject($subject, $entity)
    {
        switch ($subject) {
            case 'locked':
                $msg = 'milex.user.user.contact.locked';
                break;
            default:
                $msg = 'milex.user.user.contact.regarding';
                break;
        }
