...
use Milex\CoreBundle\Entity\Notification;
use Milex\CoreBundle\Entity\NotificationRepository;
...
class NotificationModel extends FormModel
{
    ...
    public function getNotificationContent($afterId = null, $includeRead = false, $limit = null)
    {
        ...
        if (!$this->disableUpdates && $this->userHelper->getUser()->isAdmin()) {
            ...
            $lastChecked = $this->session->get('milex.update.checked', 0);

            if (time() - $lastChecked > 3600) {
                $this->session->set('milex.update.checked', time());
            ...
            if (isset($updateData['version'])) {
                $announcement = $this->translator->trans(
                    'milex.core.updater.update.announcement_link',
                ...
                $alreadyNotified = $this->session->get('milex.update.notified');

                if (empty($alreadyNotified) || $alreadyNotified != $updateData['version']) {
                    $newUpdate = true;
                    $this->session->set('milex.update.notified', $updateData['version']);
                }
            }
        }
