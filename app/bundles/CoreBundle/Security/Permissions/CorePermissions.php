...
use Milex\CoreBundle\Security\Exception\PermissionBadFormatException;
use Milex\CoreBundle\Security\Exception\PermissionNotFoundException;
use Milex\UserBundle\Entity\Permission;
...
class CorePermissions
{
    ...
    public function isGranted($requestedPermission, $mode = 'MATCH_ALL', $userEntity = null, $allowUnknown = false)
    {
        ...
        foreach ($requestedPermission as $permission) {
            ...
            if (false === in_array(count($parts), [3, 4])) {
                throw new PermissionBadFormatException($this->getTranslator()->trans('milex.core.permissions.badformat', ['%permission%' => $permission]));
            }
            ...
            if ($userEntity->isAdmin()) {
                ...
                if (!$permissionObject->isSupported($parts[1], $parts[2])) {
                    if ($allowUnknown) {
                        ...
                        throw new PermissionNotFoundException($this->getTranslator()->trans('milex.core.permissions.notfound', ['%permission%' => $permission]));
                    }
        ...
        if ('MATCH_ALL' == $mode) {
            ...
            throw new PermissionNotFoundException($this->getTranslator()->trans('milex.core.permissions.mode.notfound', ['%mode%' => $mode]));
        }
    }
     ...
     *
     * It is recommended to define permission objects via DI with tag 'milex.permissions'.
