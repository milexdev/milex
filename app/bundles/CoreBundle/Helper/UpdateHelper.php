...
use Milex\CoreBundle\Helper\Update\Exception\CouldNotFetchLatestVersionException;
use Milex\CoreBundle\Helper\Update\Exception\LatestVersionSupportedException;
use Milex\CoreBundle\Helper\Update\Exception\UpdateCacheDataNeedsToBeRefreshedException;
use Milex\CoreBundle\Helper\Update\Github\Release;
...
use Milex\CoreBundle\Helper\Update\PreUpdateChecks\PreUpdateCheckResult;
...
class UpdateHelper
{
    ...
    public function __construct(
        ...
        $this->milexVersion = defined('MILEX_VERSION') ? MILEX_VERSION : 'unknown';
    ...
    public function fetchPackage($package)
    {
        ...
        try {
            ...
            return [
                'error'   => true,
                'message' => 'milex.core.updater.error.fetching.package',
            ];
        }
    ...
    public function fetchData($overrideCache = false)
    {
        ...
        try {
            ...
            return [
                'error'   => false,
                'message' => 'milex.core.updater.running.latest.version',
            ];
        } catch (CouldNotFetchLatestVersionException $exception) {
            return [
                'error'   => true,
                'message' => 'milex.core.updater.error.fetching.updates',
            ];
        ...
        $data = [
            'error'        => false,
            'message'      => 'milex.core.updater.update.available',
    ...
    public function runPreUpdateChecks(): array
    {
        ...
        if (true === $updateData['error']) {
        ...
        } elseif (false === $updateData['error'] && 'milex.core.updater.running.latest.version' === $updateData['message']) {
            ...
            $checkResults[] = new PreUpdateCheckResult(false, null, [new PreUpdateCheckError('milex.core.updater.running.latest.version')]);
        } elseif (empty($updateData['metadata'])) {
            $checkResults[] = new PreUpdateCheckResult(false, null, [new PreUpdateCheckError('milex.core.update.check.error.release_data')]);
        }
    ...
    private function sendStats()
    {
        ...
        try {
            ...
            $installSource = $this->coreParametersHelper->get('install_source', 'Milex');
    ...
    private function fetchLatestCompatibleVersion(string $updateStability): Release
    {
        ...
        return $this->releaseParser->getLatestSupportedRelease($releases, $this->milexVersion, $updateStability);
    }
