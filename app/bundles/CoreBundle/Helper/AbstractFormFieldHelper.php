...
use Milex\CoreBundle\Helper\ListParser\ArrayListParser;
use Milex\CoreBundle\Helper\ListParser\BarListParser;
use Milex\CoreBundle\Helper\ListParser\Exception\FormatNotSupportedException;
use Milex\CoreBundle\Helper\ListParser\JsonListParser;
use Milex\CoreBundle\Helper\ListParser\ListParserInterface;
use Milex\CoreBundle\Helper\ListParser\ValueListParser;
...
abstract class AbstractFormFieldHelper
{
    ...
    public function getChoiceList($customFields = [])
    {
        ...
        foreach ($this->getTypes() as $v => $type) {
            $choices[$this->translator->transConditional("milex.core.type.{$v}", "{$this->translationKeyPrefix}{$v}")] = $v;
        }
