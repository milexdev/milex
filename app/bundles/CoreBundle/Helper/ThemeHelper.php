...
use Milex\CoreBundle\Exception\FileExistsException;
use Milex\CoreBundle\Exception\FileNotFoundException;
use Milex\CoreBundle\Templating\Helper\ThemeHelper as TemplatingThemeHelper;
...
class ThemeHelper implements ThemeHelperInterface
{
    ...
    public function install($zipFile)
    {
        ...
        if (false === class_exists('ZipArchive')) {
            throw new \Exception('milex.core.ziparchive.not.installed');
        }

        $themeName = basename($zipFile, '.zip');

        if (in_array($themeName, $this->getDefaultThemes())) {
            throw new \Exception($this->translator->trans('milex.core.theme.default.cannot.overwrite', ['%name%' => $themeName], 'validators'));
        }
        ...
        if ($missingFiles = array_diff($requiredFiles, $foundRequiredFiles)) {
            throw new FileNotFoundException($this->translator->trans('milex.core.theme.missing.files', ['%files%' => implode(', ', $missingFiles)], 'validators'));
        }

        // Extract the archive file now
        if (!$zipper->extractTo($themePath, $allowedFiles)) {
            throw new \Exception('milex.core.update.error_extracting_package');
        } else {
            $zipper->close();
            unlink($zipFile);
    ...
    {
        if ('milex_code_mode' !== $template && !in_array($template, array_keys($this->getInstalledThemes($specificFeature)))) {
