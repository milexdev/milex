...
class InputHelper
{
    ...
    private static function getFilter($html = false, $strict = false)
    {
        if (empty(self::$htmlFilter)) {
            // Most of Milex's HTML uses include full HTML documents so use blacklist method
    ...
    public static function _($value, $mask = 'clean', $urldecode = false)
    {
        if (is_array($value)) {
            foreach ($value as $k => &$v) {
                ...
                if (is_array($mask)) {
                    if (array_key_exists($k, $mask)) {
                        if (is_array($mask[$k])) {
                            $useMask = $mask[$k];
                        } elseif (method_exists('Milex\CoreBundle\Helper\InputHelper', $mask[$k])) {
                            $useMask = $mask[$k];
                        }
                    } elseif (is_array($v)) {
                ...
                } elseif (method_exists('Milex\CoreBundle\Helper\InputHelper', $mask)) {
                    $useMask = $mask;
                }
            ...
            return $value;
        } elseif (is_string($mask) && method_exists('Milex\CoreBundle\Helper\InputHelper', $mask)) {
