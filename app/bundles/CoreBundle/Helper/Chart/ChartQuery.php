...
namespace Milex\CoreBundle\Helper\Chart;
...
class ChartQuery extends AbstractChart
{
     ...
     *
     * @see \Milex\CoreBundle\Helper\Chart\DateRangeUnitTrait::getTimeUnitFromDateRange()
     *
        ...
        if ($filters && is_array($filters)) {
            foreach ($filters as $column => $value) {
                ...
                if ('leadlist_id' === $column) {
                    $query->join('t', MILEX_TABLE_PREFIX.'lead_lists_leads', 'lll', 'lll.lead_id = '.$value['list_column_name']);
    ...
    {
        if (MILEX_TABLE_PREFIX && 0 === strpos($table, MILEX_TABLE_PREFIX)) {
            return $table;
        }
        ...
        return MILEX_TABLE_PREFIX.$table;
    }
