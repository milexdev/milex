...
use Milex\CoreBundle\Helper\Language\Installer;
use Monolog\Logger;
use Symfony\Component\Finder\Finder;

/**
 * Helper class for managing Milex's installed languages.
 */
class LanguageHelper
{
    ...
    public function extractLanguagePackage($languageCode)
    {
        ...
        if (!$zipper->extractTo($tempDir)) {
            return [
                'error'   => true,
                'message' => 'milex.core.update.archive_failed_to_extract',
            ];
        }
        ...
        return [
            'error'   => false,
            'message' => 'milex.core.language.helper.language.saved.successfully',
        ];
    }
    ...
    public function fetchLanguages($overrideCache = false, $returnError = true)
    {
        ...
        try {
            ...
            // expected everywhere else inside milex (locale keyed sorted array)
            ...
            return (!$returnError)
                ...
                : [
                    'error'   => true,
                    'message' => 'milex.core.language.helper.error.fetching.languages',
                ];
        }
    ...
    public function fetchPackage($languageCode)
    {
        ...
        if (!isset($cacheData['languages'][$languageCode])) {
            return [
                'error'   => true,
                'message' => 'milex.core.language.helper.invalid.language',
                'vars'    => [
        ...
        try {
            ...
            return [
                'error'   => true,
                'message' => 'milex.core.language.helper.error.fetching.package.exception',
                'vars'    => [
        ...
        if ($data->getStatusCode() >= 300 && $data->getStatusCode() < 400) {
            return [
                'error'   => true,
                'message' => 'milex.core.language.helper.error.follow.redirects',
                'vars'    => [
                    '%url%' => $langUrl,
                ],
            ];
            ...
            return [
                'error'   => true,
                'message' => 'milex.core.language.helper.error.on.language.server.side',
                'vars'    => [
