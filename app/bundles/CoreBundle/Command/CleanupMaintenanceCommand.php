...
class CleanupMaintenanceCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:maintenance:cleanup')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $translator->setLocale($this->getContainer()->getParameter('milex.locale', 'en_US'));
        ...
        if (empty($dryRun) && empty($noInteraction)) {
            ...
            $question = new ConfirmationQuestion(
                '<info>'.$translator->trans('milex.maintenance.confirm_data_purge', ['%days%' => $daysOld]).'</info> ', false
            );
        ...
        $table
            ->setHeaders([$translator->trans('milex.maintenance.header.key'), $translator->trans('milex.maintenance.header.records_affected')])
            ->setRows($rows);
        $table->render();
