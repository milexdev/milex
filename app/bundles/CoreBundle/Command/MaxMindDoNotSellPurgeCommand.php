...
use Milex\CoreBundle\IpLookup\DoNotSellList\MaxMindDoNotSellList;
...
/**
 * CLI Command to purge data from Milex that appears on the
...
class MaxMindDoNotSellPurgeCommand extends Command
{
    ...
    {
        $this->setName('milex:max-mind:purge')
            ...
            ->setHelp(<<<'EOT'
The <info>%command.name%</info> command will purge all data from Milex which is related to any IP found on the MaxMind Do Not Sell List.
    ...
    private function findContactsFromIPs(array $ips): array
    {
        ...
        $sql =
            'SELECT x.lead_id AS id, ip.ip_address AS ip_address '.
             'FROM '.MILEX_TABLE_PREFIX.'lead_ips_xref x '.
             'JOIN '.MILEX_TABLE_PREFIX.'ip_addresses ip ON x.ip_id = ip.id '.
