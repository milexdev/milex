...
/**
 * CLI Command to install Milex sample data.
 */
class InstallDataCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:install:data')
            ->setDescription('Installs Milex with sample data')
            ->setDefinition([
                new InputOption(
            ...
            ->setHelp(<<<'EOT'
The <info>%command.name%</info> command re-installs Milex with sample data.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$force) {
            ...
            $questionString = $translator->trans('milex.core.command.install_data_confirm').' (y = '.$translator->trans('milex.core.form.yes').', n = '.$translator->trans('milex.core.form.no').'): ';
        ...
        $args    = [
            ...
            '--env'    => $env,
            '--quiet'  => true,
            '--group'  => ['group_milex_install_data'],
        ];
        ...
        $output->writeln(
            $translator->trans('milex.core.command.install_data_success')
        );
