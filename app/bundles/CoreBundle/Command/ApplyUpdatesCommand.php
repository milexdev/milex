...
use Milex\CoreBundle\Update\StepProvider;
...
class ApplyUpdatesCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:update:apply')
            ->setDescription('Updates the Milex application')
            ->setDefinition(
                [
                    new InputOption(
            ...
            ->setHelp(
                <<<'EOT'
                The <info>%command.name%</info> command updates the Milex application.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        defined('MILEX_ENV') or define('MILEX_ENV', (isset($options['env'])) ? $options['env'] : 'prod');

        if (true === $this->coreParametersHelper->get('composer_updates', false)) {
            $output->writeln('<error>'.$this->translator->trans('milex.core.command.update.composer').'</error>');
        ...
        try {
            if (empty($options['finish'])) {
                ...
                $output->writeln(
                    "\n\n<warning>".$this->translator->trans('milex.core.command.update.finalize_instructions').'</warning>'
                );
    ...
    private function startUpgrade(): int
    {
        if (!$this->input->getOption('force')) {
            ...
            $question = new ConfirmationQuestion($this->translator->trans('milex.core.update.confirm_application_update').' ', false);

            if (!$helper->ask($this->input, $this->output, $question)) {
                throw new UpdateFailedException($this->translator->trans('milex.core.update.aborted'));
            }
        }
