...
use Milex\CoreBundle\Exception\BadConfigurationException;
use Milex\CoreBundle\Factory\TransifexFactory;
use Milex\CoreBundle\Helper\CoreParametersHelper;
use Milex\Transifex\Connector\Statistics;
use Milex\Transifex\Connector\Translations;
...
class PullTransifexCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:transifex:pull')
            ->setDescription('Fetches translations for Milex from Transifex')
            ->addOption('language', null, InputOption::VALUE_OPTIONAL, 'Optional language to pull', null)
            ->setHelp(<<<'EOT'
The <info>%command.name%</info> command is used to retrieve updated Milex translations from Transifex and writes them to the filesystem.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            $output->writeln($this->translator->trans('milex.core.command.transifex_no_credentials'));
        ...
        foreach ($files as $bundle => $stringFiles) {
            foreach ($stringFiles as $file) {
                ...
                $output->writeln($this->translator->trans('milex.core.command.transifex_processing_resource', ['%resource%' => $name]));

                try {
                    /** @var Statistics $statisticsConnector */
                    $statisticsConnector = $transifex->get('statistics');
                    $response            = $statisticsConnector->getStatistics('milex', $alias);
                    ...
                    foreach ($languageStats as $language => $stats) {
                        ...
                        $output->writeln($this->translator->trans('milex.core.command.transifex_processing_language', ['%language%' => $language]));
                        ...
                        if ($completed >= 80) {
                            ...
                            $response              = $translationsConnector->getTranslation('milex', $alias, $language);
                            ...
                            if (!is_dir($translationDir.$language)) {
                                if (!mkdir($translationDir.$language)) {
                                    $output->writeln(
                                        $this->translator->trans('milex.core.command.transifex_error_creating_directory', [
                            ...
                            if (!file_put_contents($path, $translation['content'])) {
                                $output->writeln(
                                    $this->translator->trans('milex.core.command.transifex_error_creating_file',
                            ...
                            $output->writeln($this->translator->trans('milex.core.command.transifex_resource_downloaded'));
                        }
                    }
                } catch (\Exception $exception) {
                    $output->writeln($this->translator->trans('milex.core.command.transifex_error_pulling_data', ['%message%' => $exception->getMessage()]));
                }
            }
        }
    ...
    /**
     * Returns Milex translation files.
     *
     * @return array
     */
    ...
    {
        $files         = [];
        $milexBundles = $this->getContainer()->getParameter('milex.bundles');
        $pluginBundles = $this->getContainer()->getParameter('milex.plugin.bundles');

        foreach ($milexBundles as $bundle) {
