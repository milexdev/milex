...
namespace Milex\CoreBundle\Tests\Unit\Templating\Helper;
...
class AssetsHelperTest extends TestCase
{
    ...
    public function testGetUrlWithRelativePath(): void
    {
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.milex');

        $assetHelper = new AssetsHelper($this->createPackagesMock());
        $assetHelper->setPathsHelper($pathsHelper);

        Assert::assertEquals('http://some.milex/some/path', $assetHelper->getUrl('some/path'));

        $version = $this->setVersion($assetHelper);

        Assert::assertEquals('http://some.milex/some/path?v'.$version, $assetHelper->getUrl('some/path'));
    }

    public function testGetUrlWithRelativePathWhenMilexInSubFolder(): void
    {
        $pathsHelper = $this->getMockBuilder(PathsHelper::class)
             ->disableOriginalConstructor()
             ->getMock();
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.milex/m');

        $assetHelper = new AssetsHelper($this->createPackagesMock());
        $assetHelper->setPathsHelper($pathsHelper);

        Assert::assertEquals('http://some.milex/m/some/path', $assetHelper->getUrl('some/path'));

        $version = $this->setVersion($assetHelper);

        Assert::assertEquals('http://some.milex/m/some/path?v'.$version, $assetHelper->getUrl('some/path'));
    }
    ...
    public function testGetUrlWithRelativePathWithDevIndex(): void
    {
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.milex/index_dev.php/');
