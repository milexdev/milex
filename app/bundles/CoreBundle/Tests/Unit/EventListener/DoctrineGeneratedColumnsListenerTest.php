...
namespace Milex\CoreBundle\Tests\Unit\EventListener;
...
use Milex\CoreBundle\EventListener\DoctrineGeneratedColumnsListener;
...
class DoctrineGeneratedColumnsListenerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testPostGenerateSchemaWhenTableDoesNotExist(): void
    {
        $this->schema->expects($this->once())
            ->method('hasTable')
            ->with(MILEX_TABLE_PREFIX.'page_hits')
            ->willReturn(false);
    ...
    public function testPostGenerateSchemaWhenColumnDoesNotExist(): void
    {
        ...
        $this->table->expects($this->once())
            ->method('addIndex')
            ->with(['generated_hit_date'], MILEX_TABLE_PREFIX.'generated_hit_date');
