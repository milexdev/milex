 ...
 * @returns {string}
 */
Milex.getUrlParameter = function (name) {
 ...
 * @param actionName
 */
Milex.launchBuilder = function (formName, actionName) {
    var builder = mQuery('.builder');
    Milex.codeMode = builder.hasClass('code-mode');
    Milex.showChangeThemeWarning = true;
    ...
    if (Milex.codeMode) {
        var rawTokens = mQuery.map(Milex.builderTokens, function (element, index) {
            return index
        }).sort();
        Milex.builderCodeMirror = CodeMirror(document.getElementById('customHtmlContainer'), {
            value: themeHtml,
            lineNumbers: true,
            mode: 'htmlmixed',
        ...
        Milex.keepPreviewAlive('builder-template-content');
    } else {
    ...
    applyBtn.off('click').on('click', function(e) {
        Milex.activateButtonLoadingIndicator(applyBtn);
        try {
            Milex.sendBuilderContentToTextarea(function () {
                // Trigger slot:destroy event
                if (typeof document.getElementById('builder-template-content').contentWindow.Milex !== 'undefined') {
                    document.getElementById('builder-template-content').contentWindow.Milex.destroySlots();
                }
                // Clear the customize forms
                mQuery('#slot-form-container, #section-form-container').html('');
                Milex.inBuilderSubmissionOn(form);
                var bgApplyBtn = mQuery('.btn-apply');
                if (0 === bgApplyBtn.length && ("1" === Milex.getUrlParameter('contentOnly') || Milex.isInBuilder)) {
                    var frm = mQuery('.btn-save').closest('form');
                    Milex.inBuilderSubmissionOn(frm);
                    frm.submit();
            ...
            Milex.removeButtonLoadingIndicator(applyBtn);
    ...
    builderPanel.on('scroll', function(e) {
        ...
        if(mQuery.find('.fr-popup:visible').length){
            if(!Milex.isInViewport(builderPanel.find('.fr-view:visible'))) {
    ...
    // Insert the Milex assets to the header
    var assets = Milex.htmlspecialchars_decode(mQuery('[data-builder-assets]').html());
    themeHtml = themeHtml.replace('</head>', assets+'</head>');

    Milex.initBuilderIframe(themeHtml, btnCloseBuilder, applyBtn);
};

Milex.isInViewport = function(el) {
 ...
 */
Milex.inBuilderSubmissionOn = function(form) {
 ...
 * @param jQuery object of form
 */
Milex.inBuilderSubmissionOff = function(form) {
    Milex.isInBuilder = false;
 ...
 */
Milex.processBuilderErrors = function(response) {
 ...
 */
Milex.formatCode = function() {
    Milex.builderCodeMirror.autoFormatRange({line: 0, ch: 0}, {line: Milex.builderCodeMirror.lineCount()});
}

/**
 * Opens Filemanager window
 */
Milex.openMediaManager = function() {
    Milex.openServerBrowser(
        milexBasePath + (typeof milexEnv !== 'undefined' &&  milexEnv === 'dev' ? '/index_dev.php' : '') + '/elfinder',
        screen.width * 0.7,
        screen.height * 0.7
    );
}
 ...
 */
Milex.setFileUrl = function(url, width, height, alt) {
    Milex.insertTextAtCMCursor(url);
}

/**
 * Inserts the text to the cursor position or replace selected range
 */
Milex.insertTextAtCMCursor = function(text) {
    var doc = Milex.builderCodeMirror.getDoc();
 ...
 */
Milex.openServerBrowser = function(url, width, height) {
 ...
 * @param slot
 */
Milex.keepPreviewAlive = function(iframeId, slot) {
    var codeChanged = false;
    // Watch for code changes
    Milex.builderCodeMirror.on('change', function(cm, change) {
        codeChanged = true;
    });

    window.setInterval(function() {
        if (codeChanged) {
            var value = (Milex.builderCodeMirror)?Milex.builderCodeMirror.getValue():'';
            if (!Milex.codeMode) {
                Milex.setCodeModeSlotContent(slot, value);
            }
            Milex.livePreviewInterval = Milex.updateIframeContent(iframeId, value, slot);
            codeChanged = false;
        }
    }, 2000);
};

Milex.isValidHtml = function (html) {
    var doc = document.createElement('div');
    doc.innerHTML = html;
    return (doc.innerHTML === html);
}

Milex.setCodeModeSlotContent = function (slot, content) {
    if (Milex.isValidHtml(content)) {
...
Milex.geCodeModetSlotContent = function (slot) {
    var html = slot.html();
...
Milex.prepareCodeModeBlocksBeforeSave = function(themeHtml) {
...
Milex.killLivePreview = function() {
    window.clearInterval(Milex.livePreviewInterval);
};

Milex.destroyCodeMirror = function() {
    delete Milex.builderCodeMirror;
 ...
 */
Milex.buildBuilderIframe = function(themeHtml, id, onLoadCallback) {
    ...
    Milex.updateIframeContent(id, themeHtml);
};
 ...
 * @returns {*}
 */
Milex.htmlspecialchars_decode = function(encodedHtml) {
 ...
 * @param themeField
 */
Milex.initSelectTheme = function(themeField) {
    var customHtml = mQuery('textarea.builder-html');
    var isNew = Milex.isNewEntity('#page_sessionId, #emailform_sessionId');
    Milex.showChangeThemeWarning = true;
    Milex.builderTheme = themeField.val();

    if (isNew) {
        Milex.showChangeThemeWarning = false;

        // Populate default content
        if (!customHtml.length || !customHtml.val().length) {
            Milex.setThemeHtml(Milex.builderTheme);
        }
    }
    ...
    if (customHtml.length) {
        mQuery('[data-theme]').click(function(e) {
            ...
            var isCodeMode = (theme === 'milex_code_mode');
            Milex.builderTheme = theme;

            if (Milex.showChangeThemeWarning && customHtml.val().length) {
                if (!isCodeMode) {
                    if (confirm(Milex.translate('milex.core.builder.theme_change_warning'))) {
                        customHtml.val('');
                        Milex.showChangeThemeWarning = false;
                    } else {
                        return;
                    ...
                    if (confirm(Milex.translate('milex.core.builder.code_mode_warning'))) {
                    } else {
                        return;
                    }
                }
 ...
 * @param slot
 */
Milex.updateIframeContent = function(iframeId, content, slot) {
    // remove empty lines
    ...
    if (iframeId) {
        ...
        slot.html(content);
        Milex.setEmptySlotPlaceholder(slot.parent());
    }
};
 ...
 * @param theme
 */
Milex.setThemeHtml = function(theme) {
 ...
 * @param model
 */
Milex.closeBuilder = function(model) {
    ...
    try {
        Milex.sendBuilderContentToTextarea(function() {
            if (Milex.codeMode) {
                Milex.killLivePreview();
                Milex.destroyCodeMirror();
                delete Milex.codeMode;
            } else {
                // Trigger slot:destroy event
                if(typeof document.getElementById('builder-template-content').contentWindow.Milex !== 'undefined') {
                    document.getElementById('builder-template-content').contentWindow.Milex.destroySlots();
 ...
 */
Milex.sendBuilderContentToTextarea = function(callback, keepBuilderContent) {
    var customHtml;
    if (Milex.codeMode) {
        customHtml = Milex.builderCodeMirror.getValue();

        // Convert dynamic slot definitions into tokens
        customHtml = Milex.convertDynamicContentSlotsToTokens(customHtml);
        ...
        if (keepBuilderContent) {
            ...
            Milex.cloneHtmlContent(builderHtml, function(themeHtml) {
                Milex.sanitizeHtmlAndStoreToTextarea(themeHtml);
                callback();
            });
        } else {
            Milex.sanitizeHtmlAndStoreToTextarea(builderHtml);
            callback();
        }
    }
};

Milex.sanitizeHtmlAndStoreToTextarea = function(html) {
    var cleanHtml = Milex.sanitizeHtmlBeforeSave(html);

    // Store the HTML content to the HTML textarea
    mQuery('.builder-html').val(Milex.domToString(cleanHtml));
};
 ...
 * @return string
 */
Milex.domToString = function(dom) {
 ...
 */
Milex.sanitizeHtmlBeforeSave = function(htmlContent) {
    // Remove Milex's assets
    htmlContent.find('[data-source="milex"]').remove();
    ...
    var customHtml = Milex.domToString(htmlContent).replace(/url\(&quot;(.+)&quot;\)/g, 'url(\'$1\')');

    // Convert dynamic slot definitions into tokens
    customHtml = Milex.convertDynamicContentSlotsToTokens(customHtml);

    return Milex.prepareCodeModeBlocksBeforeSave(customHtml);
};
 ...
 */
Milex.cloneHtmlContent = function(content, callback) {
    var id = 'iframe-helper';
    var iframeHelper = mQuery('<iframe id="'+id+'" />');
    Milex.buildBuilderIframe(Milex.domToString(content), id, function() {
        callback(mQuery('iframe#'+id).contents());
        iframeHelper.remove();
    });
}

Milex.destroySlots = function() {
    // Trigger destroy slots event
    if (typeof Milex.builderSlots !== 'undefined' && Milex.builderSlots.length) {
        mQuery.each(Milex.builderSlots, function(i, slotParams) {
            mQuery(slotParams.slot).trigger('slot:destroy', slotParams);
        });
        delete Milex.builderSlots;
    }

    // Destroy sortable
    Milex.builderContents.find('[data-slot-container]').sortable().sortable('destroy');

    // Remove empty class="" attr
    Milex.builderContents.find('*[class=""]').removeAttr('class');

    // Remove border highlighted by Froala
    Milex.builderContents = Milex.clearFroalaStyles(Milex.builderContents);

    // Remove style="z-index: 2501;" which Froala forgets there
    Milex.builderContents.find('*[style="z-index: 2501;"]').removeAttr('style');

    // Make sure that the Froala editor is gone
    Milex.builderContents.find('.fr-toolbar, .fr-line-breaker').remove();
...
Milex.clearFroalaStyles = function(content) {
...
Milex.toggleBuilderButton = function (hide) {
...
Milex.initSectionListeners = function() {
    Milex.activateGlobalFroalaOptions();
    Milex.selectedSlot = null;

    Milex.builderContents.on('section:init', function(event, section, isNew) {
        section = mQuery(section);

        if (isNew) {
            Milex.initSlots(section.find('[data-slot-container]'));
        }

        section.on('click', function(e) {
            var clickedSection = mQuery(this);
            var previouslyFocused = Milex.builderContents.find('[data-section-focus]');
            ...
            var focusParts = {
                ...
                'delete': {
                    ...
                    onClick: function() {
                        if (confirm(parent.Milex.translate('milex.core.builder.section_delete_warning'))) {
            ...
            Milex.builderContents.find('[data-slot-focus]').each(function() {
            ...
            if (section.length && section.css('background-color') !== 'rgba(0, 0, 0, 0)') {
                sectionForm.find('#builder_section_content-background-color').val(Milex.rgb2hex(section.css('backgroundColor')));
            }
            ...
            if (sectionWrapper.css('background-color') !== 'rgba(0, 0, 0, 0)') {
                sectionForm.find('#builder_section_wrapper-background-color').val(Milex.rgb2hex(sectionWrapper.css('backgroundColor')));
            }
            ...
            sectionFormContainer.find('input[data-toggle="color"]').each(function() {
                parent.Milex.activateColorPicker(this);
            });
            ...
            sectionForm.on('keyup paste change touchmove', function(e) {
                ...
                switch (field.attr('id')) {
                    case 'builder_section_content-background-color':
                        Milex.sectionBackgroundChanged(section, field.val());
                        break;
                    case 'builder_section_content-background-image':
                        Milex.sectionBackgroundImageChanged(section, field.val());
                        break;
                    ...
                    case 'builder_section_content-background-size':
                        Milex.sectionBackgroundSize(section, field.val());
                        break;
                    ...
                    case 'builder_section_wrapper-background-color':
                        Milex.sectionBackgroundChanged(sectionWrapper, field.val());
                        break;
                    case 'builder_section_wrapper-background-image':
                        Milex.sectionBackgroundImageChanged(sectionWrapper, field.val());
                        break;
                    ...
                    case 'builder_section_wrapper-background-size':
                        Milex.sectionBackgroundSize(sectionWrapper, field.val());
                        break;
            ...
            parent.mQuery('#section-form-container').on('change.minicolors', function(e, hex) {
                ...
                if (focusedSection.length && field.attr('id') === 'builder_section_content-background-color') {
                    Milex.sectionBackgroundChanged(focusedSection, field.val());
                } else if (field.attr('id') === 'builder_section_wrapper-background-color') {
                    Milex.sectionBackgroundChanged(focusedSectionWrapper, field.val());
                }
            });
        });
    });
...
Milex.initSections = function() {
    Milex.initSectionListeners();
    var sectionWrappers = Milex.builderContents.find('[data-section-wrapper]');

    // Make slots sortable
    var bodyOverflow = {};
    Milex.sortActive = false;
    ...
    mQuery('body').sortable({
        ...
        stop: function(event, ui) {
            if (ui.item.hasClass('section-type-handle')) {
                ...
                Milex.builderContents.trigger('section:init', [newSection, true]);
            } else {
                // Restore original overflow
...
Milex.sectionBackgroundChanged = function(element, color) {
    if (color.length) {
        color = '#'+color;
    } else {
        color = 'transparent';
    ...
    Milex.updateOutlookTag(element);
    ...
    mQuery(element).find('[data-slot-focus]').each(function() {
        ...
        if (focusedSlot.attr('data-slot') == 'text') {
            Milex.setTextSlotEditorStyle(parent.mQuery('#slot_text_content'), focusedSlot);
        }
    });
};

Milex.sectionBackgroundImageChanged = function (element, imageUrl) {
...
Milex.sectionBackgroundSize = function (element, size) {
    if (!size) {
        size = 'auto auto';
    }
...
Milex.updateOutlookTag = function (element) {
...
Milex.initSlots = function(slotContainers) {
    if (!slotContainers) {
        slotContainers = Milex.builderContents.find('[data-slot-container]');
    }

    Milex.builderContents.find('a').on('click', function(e) {
        e.preventDefault();
    });
    ...
    var bodyOverflow = {};
    Milex.sortActive = false;
    Milex.parentDocument = parent.document;
        ...
        start: function(event, ui) {
            ...
            Milex.builderContents.find('[data-slot-focus]').each( function() {
                ...
                if (focusedSlot.attr('data-slot') === 'image') {
                    ...
                    Milex.builderContents.find('.fr-image-resizer.fr-active').removeClass('fr-active');
                }
            });

            Milex.builderContents.find('[data-slot-focus]').remove();
        },
        stop: function(event, ui) {
            if (ui.item.hasClass('slot-type-handle')) {
                ...
                Milex.builderContents.trigger('slot:init', newSlot);
            } else {
                // Restore original overflow
                mQuery('body').css(bodyOverflow);
            }
    ...
    mQuery('#slot-type-container .slot-type-handle', parent.document).draggable({
        ...
        helper: function(e, ui) {
            ...
            // Fix body overflow that messes sortable up
            bodyOverflow.overflowX = mQuery('body', Milex.parentDocument).css('overflow-x');
            bodyOverflow.overflowY = mQuery('body', Milex.parentDocument).css('overflow-y');
            mQuery('body', Milex.parentDocument).css({
                overflowX: 'hidden',
                overflowY: 'hidden'
            });
        ...
        cursorAt: {top: 15, left: 15},
        start: function(event, ui) {
            mQuery('#builder-template-content', Milex.parentDocument).css('overflow', 'hidden');
            mQuery('#builder-template-content', Milex.parentDocument).attr('scrolling', 'no');
        ...
        stop: function(event, ui) {
            // Restore original overflow
            mQuery('body', Milex.parentDocument).css(bodyOverflow);

            mQuery('#builder-template-content', Milex.parentDocument).css('overflow', 'visible');
            mQuery('#builder-template-content', Milex.parentDocument).attr('scrolling', 'yes');
            // check if it is initialized first to prevent error
    ...
    iframe.on('scroll', function() {
        mQuery('#slot-type-container .slot-type-handle', Milex.parentDocument).draggable("option", "cursorAt", { top: -1 * iframe.scrollTop() + 15 });
    });
...
Milex.getSlotToolbar = function(type) {
    Milex.builderContents.find('[data-slot-toolbar]').remove();

    var slotToolbar = mQuery('<div/>').attr('data-slot-toolbar', true);
    var deleteLink  = Milex.getSlotDeleteLink();
    var cloneLink = Milex.getSlotCloneLink();
...
Milex.getSlotDeleteLink = function() {
    if (typeof Milex.deleteLink == 'undefined') {
        Milex.deleteLink = mQuery('<a><i class="fa fa-lg fa-times"></i></a>')
            .attr('data-slot-action', 'delete')
            .attr('alt', 'delete')
            .addClass('btn btn-delete btn-default');
    }

    return Milex.deleteLink;
};

Milex.getSlotCloneLink = function() {
    if (typeof Milex.cloneLink == 'undefined') {
        Milex.cloneLink = mQuery('<a><i class="fa fa-lg fa-copy"></i></a>')
            .attr('data-slot-action', 'clone')
            .attr('alt', 'clone')
            .addClass('btn btn-clone btn-clone');
    }
    return Milex.cloneLink;
};

Milex.getSlotFocus = function() {
    Milex.builderContents.find('[data-slot-focus]').remove();

    return mQuery('<div/>').attr('data-slot-focus', true);
};

Milex.cloneFocusForm = function(decId, removeFroala) {
    Milex.reattachDEC();

    var focusForm = parent.mQuery('#emailform_dynamicContent_' + decId);
    Milex.activeDECParent = focusForm.parent();
    // show if hidden
    ...
    Milex.activeDEC = element;
    return element;
};

Milex.initEmailDynamicContentSlotEdit = function (clickedSlot) {
    var decId = clickedSlot.attr('data-param-dec-id');

    var focusForm;

    if (decId || decId === 0) {
        focusForm = Milex.cloneFocusForm(decId);
    }
    ...
    newDynConButton.on('click', function(e) {
        e.stopPropagation();
        Milex.createNewDynamicContentFilter('#dynamicContentFilterTabs_'+decId, parent.mQuery);
        var focusForm = Milex.cloneFocusForm(decId, false);
...
Milex.removeAddVariantButton = function() {
...
Milex.reattachDEC = function() {
    if (typeof Milex.activeDEC !== 'undefined') {
        var element = Milex.activeDEC.detach();
        Milex.activeDECParent.append(element);
    }
};

Milex.isSlotInitiated = function(slot) {
    if (typeof Milex.builderSlots === 'undefined' || Milex.builderSlots.length === 0) return false;
    return typeof Milex.builderSlots.find(function(params) {
        return slot.is(params.slot);
    }) !== 'undefined';
};

Milex.isCodeMode = function() {
    return mQuery('a[data-theme=milex_code_mode]').first().hasClass('hide');
};
...
Milex.initSlotListeners = function() {
    Milex.activateGlobalFroalaOptions();
    Milex.builderSlots = [];
    Milex.selectedSlot = null;

    Milex.builderContents.on('slot:selected', function(event, slot) {
        slot = mQuery(slot);
        Milex.builderContents.find('[data-slot-focus]').remove();
        mQuery(slot).append(Milex.getSlotFocus());
    });

    Milex.builderContents.on('slot:init', function(event, slot) {
        slot = mQuery(slot);
        var type = slot.attr('data-slot');

        // Avoid initialising one slot several times
        if (Milex.isSlotInitiated(slot)) return;

        // initialize the drag handle
        var slotToolbar = Milex.getSlotToolbar(type);
        var deleteLink  = Milex.getSlotDeleteLink();
        var cloneLink   = Milex.getSlotCloneLink();
        var focus       = Milex.getSlotFocus();

        slot.hover(function(e) {
            e.stopPropagation();

            // Get new copies of the focus, toolbar
            slotToolbar = Milex.getSlotToolbar(type);
            focus       = Milex.getSlotFocus();
            ...
            cloneLink.click(function(e) {
                if (type == 'dynamicContent') {
                    var maxId = Milex.getDynamicContentMaxId();

                    slot.clone().attr('data-param-dec-id', maxId + 1).insertAfter(slot);
                    Milex.createNewDynamicContentItem(parent.mQuery);
                } else {
                    slot.clone().insertAfter(slot);
                }

                Milex.initSlots(slot.closest('[data-slot-container="1"]'));
            });
            ...
            Milex.setEmptySlotPlaceholder(slot);
        }, function() {
            if (Milex.sortActive) {
                // don't activate while sorting
        ...
        slot.on('click', function(e) {
            ...
            Milex.deleteCodeModeSlot();
            Milex.removeAddVariantButton();
            ...
            if (focusType == 'dynamicContent') {
                var nff = Milex.initEmailDynamicContentSlotEdit(clickedSlot);
                // replace focusForm
            ...
            for (var i = 0; i < $codeModeSlotTypes.length; i++) {
                if ($codeModeSlotTypes[i] === type) {
                    ...
                    if (element) {
                        Milex.builderCodeMirror = CodeMirror.fromTextArea(element, {
                            lineNumbers: true,
                            mode: 'htmlmixed',
                            extraKeys: {"Ctrl-Space": "autocomplete"},
                            lineWrapping: true,
                        ...
                        html = Milex.geCodeModetSlotContent(elem);
                        Milex.builderCodeMirror.getDoc().setValue(html);
                        Milex.keepPreviewAlive(null, elem);
                    }
                    break;
                }
            }
            ...
            focusForm.find('textarea.editor').each(function () {
                ...
                var froalaOptions = {
                    ...
                    toolbarSticky: false,
                    linkList: [], // TODO push here the list of tokens from Milex.getPredefinedLinks
                ...
                if (focusType !== 'dynamicContent') {
                    ...
                    parent.mQuery(this).on('froalaEditor.initialized', function (e, editor) {
                        parent.Milex.initAtWho(editor.$el, parent.Milex.getBuilderTokensMethod(), editor);
                        Milex.setTextSlotEditorStyle(editor.$el, clickedSlot);
                    });
                }
                ...
                parent.mQuery(this).on('froalaEditor.contentChanged', function (e, editor) {
                    ...
                    if (!(focusType == 'dynamicContent' && mQuery(this).attr('id').match(/filters/))) {
                        ...
                        Milex.setEmptySlotPlaceholder(clickedSlot);
                    }
                });
                ...
                parent.mQuery(this).froalaEditor(parent.mQuery.extend({}, Milex.basicFroalaOptions, froalaOptions));
            });
        ...
        if (type === 'image' || type === 'imagecaption' || type === 'imagecard') {
            ...
            // Init Froala editor
            var froalaOptions = mQuery.extend({}, Milex.basicFroalaOptions, {
                    linkList: [], // TODO push here the list of tokens from Milex.getPredefinedLinks
        ...
        Milex.builderSlots.push({slot: slot, type: type});
    });

    Milex.getPredefinedLinks = function(callback) {
        var linkList = [];
        Milex.getTokens(Milex.getBuilderTokensMethod(), function(tokens) {
            if (tokens.length) {
    ...
    Milex.builderContents.on('slot:change', function(event, params) {
        ...
        Milex.clearSlotFormError(fieldParam);
        ...
        if (fieldParam === 'padding-top' || fieldParam === 'padding-bottom') {
            ...
            if (toInsert === 'url') {
                var videoProvider = Milex.getVideoProvider(insertVal);

                if (videoProvider == null) {
                    Milex.slotFormError(fieldParam, 'Please enter a valid YouTube, Vimeo, or MP4 url.');
                } else {
        ...
        if (params.type == 'text') {
            Milex.setTextSlotEditorStyle(parent.mQuery('#slot_text_content'), params.slot);
        }
    });

    Milex.builderContents.on('slot:destroy', function(event, params) {
        Milex.reattachDEC();
        ...
        if (params.type === 'text') {
            ...
            dynConId = '#emailform_dynamicContent_'+dynConId;
            if (Milex.activeDEC && Milex.activeDEC.attr('id') === dynConId.substr(1)) {
                delete Milex.activeDEC;
                delete Milex.activeDECParent;
            }
            var dynConTarget = parent.mQuery(dynConId);
        ...
        Milex.builderContents.find('.sf-toolbar').remove();
    });
};

Milex.deleteCodeModeSlot = function() {
    Milex.killLivePreview();
    Milex.destroyCodeMirror();
    delete Milex.codeMode;
};

Milex.clearSlotFormError = function(field) {
...
Milex.slotFormError = function (field, message) {
    var customizeSlotField = parent.mQuery('#customize-form-container').find('[data-slot-param="'+field+'"]');
...
Milex.getVideoProvider = function(url) {
    var providers = [
        {
...
Milex.setTextSlotEditorStyle = function(editorEl, slot)
{
    ...
    mQuery.each(['background-color', 'color', 'font-family', 'font-size', 'line-height', 'text-align'], function(key, style) {
        var overrideStyle = Milex.getSlotStyle(slot, style, false);
        if (overrideStyle) {
...
Milex.getSlotStyle = function(slot, styleName, fallback) {
 ...
 * @returns {string}
 */
Milex.getBuilderTokensMethod = function() {
...
Milex.prepareBuilderIframe = function(themeHtml, btnCloseBuilder, applyBtn) {
    ...
    while (match !== null) {
        ...
        if (!Milex.builderTokens.hasOwnProperty(dynConToken)) {
            Milex.builderTokens[dynConToken] = dynConName;
        }
    ...
    themeHtml = Milex.prepareDynamicContentBlocksForBuilder(themeHtml);
    ...
    Milex.buildBuilderIframe(themeHtml, 'builder-template-content', function() {
...
Milex.initBuilderIframe = function(themeHtml, btnCloseBuilder, applyBtn) {
    // Avoid to request the tokens if not necessary
    if (Milex.builderTokensRequestInProgress) {
        // Wait till previous request finish
        var intervalID = setInterval(function(){
            if (!Milex.builderTokensRequestInProgress) {
                clearInterval(intervalID);
                Milex.prepareBuilderIframe(themeHtml, btnCloseBuilder, applyBtn);
            }
        }, 500);
    } else {
        Milex.prepareBuilderIframe(themeHtml, btnCloseBuilder, applyBtn);
...
Milex.prepareDynamicContentBlocksForBuilder = function(builderHtml) {
    for (var token in Milex.builderTokens) {
        // If this is a dynamic content token
        if (Milex.builderTokens.hasOwnProperty(token) && /\{dynamic/.test(token)) {
            var defaultContent = Milex.convertDynamicContentTokenToSlot(token);
...
Milex.convertDynamicContentTokenToSlot = function(token) {
    var dynConData = Milex.getDynamicContentDataForToken(token);
...
Milex.getDynamicContentDataForToken = function(token) {
    ...
    if (dynConContainer.html()) {
        ...
        if (dynConContent.hasClass('editor') && Milex.getActiveBuilderName() === 'legacy') {
...
Milex.convertDynamicContentSlotsToTokens = function (builderHtml) {
    ...
    if (dynConSlots.length) {
        dynConSlots.each(function(i) {
            ...
            builderHtml = Milex.domToString($b);
...
Milex.getDynamicContentMaxId = function() {
...
Milex.setEmptySlotPlaceholder = function (slot) {
...
mQuery(function() {
    if (parent && parent.mQuery && parent.mQuery('#builder-template-content').length) {
        Milex.builderContents = mQuery('body');
        if (!parent.Milex.codeMode) {
            Milex.initSlotListeners();
            Milex.initSections();
            Milex.initSlots();
        }
