Milex.builderTokensForCkEditor = {};
Milex.builderTokens = {};
Milex.dynamicContentTokens = {};
Milex.builderTokensRequestInProgress = false;
Milex.imageManagerLoadURL = milexBaseUrl + 's/file/list';
Milex.imageUploadURL = milexBaseUrl + 's/file/upload';
Milex.imageManagerDeleteURL = milexBaseUrl + 's/file/delete';
Milex.elfinderURL = milexBaseUrl + 'elfinder';


/**
 * Activate Froala options
 */
Milex.activateGlobalFroalaOptions = function() {
    Milex.basicFroalaOptions = {
        enter: mQuery.FroalaEditor.ENTER_BR,
        imageUploadURL: Milex.imageUploadURL,
        imageManagerLoadURL: Milex.imageManagerLoadURL,
        imageManagerDeleteURL: Milex.imageManagerDeleteURL,
        imageDefaultWidth: 0,
        pastePlain: true,
 ...
 */
Milex.initAtWho = function(element, method, froala) {
    ...
    if (Milex.builderTokensRequestInProgress) {
        ...
        var intervalID = setInterval(function(){
            if (!Milex.builderTokensRequestInProgress) {
                ...
                Milex.configureAtWho(element, method, froala);
            }
        }, 500);
    } else {
        Milex.configureAtWho(element, method, froala);
 ...
 * @param froala  Froala Editor
 */
Milex.configureAtWho = function(element, method, froala) {
    Milex.getTokens(method, function(tokens) {
        element.atwho('destroy');

        Milex.configureDynamicContentAtWhoTokens();

        // Add the dynamic content tokens
        mQuery.extend(tokens, Milex.dynamicContentTokens);
 ...
 */
Milex.getTokens = function(method, callback) {
    // Check if the builderTokens var holding the tokens was already loaded
    if (!mQuery.isEmptyObject(Milex.builderTokens)) {
        return callback(Milex.builderTokens);
    }

    Milex.builderTokensRequestInProgress = true;
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.tokens === 'object') {
                ...
                Milex.builderTokens = response.tokens;
 ...
 */
Milex.configureDynamicContentAtWhoTokens = function() {
    Milex.dynamicContentTokens = {};
    ...
    if (dynamicContentTabs.length) {
        dynamicContentTabs.find('a[data-toggle="tab"]').each(function () {
            ...
            Milex.dynamicContentTokens[newOption] = tokenText;
        });
    }
};

Milex.insertTextInEditor = function (obj, text) {
 ...
 */
Milex.customItemRenderer = function (item) {
    let tokenId = item.id;
    const id = item.id;
...
Milex.customItemOutputRenderer = function (item) {
...
Milex.getFeedItems = function (opts, callback) {
    let data = Milex.builderTokensForCkEditor.filter(function(item) {
...
Milex.getTokensForPlugIn = function(method) {
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.tokens === 'object') {
                Milex.builderTokens = response.tokens;
                Milex.configureDynamicContentAtWhoTokens();
                mQuery.extend(Milex.builderTokens, Milex.dynamicContentTokens);
                Milex.builderTokensForCkEditor = mQuery.map(Milex.builderTokens, function(value, i) {
                    return {'id':i, 'name':value, 'token_name': value};
                });
                d.resolve(Milex.builderTokensForCkEditor);
            }
        },
        error: function (request, textStatus, errorThrown) {
            Milex.processAjaxError(request, textStatus, errorThrown);
...
Milex.getCKEditorFonts = function(fonts) {
...
Milex.ConvertFieldToCkeditor  = function(textarea, ckEditorToolbarOptions) {
    ...
    const ckEditorOption = {
        ...
        font_names: Milex.getCKEditorFonts(milexEditorFonts).join(';'),
        filebrowserBrowseUrl : Milex.elfinderURL+'?editor=ckeditor',
    };
    if (ckEditorToolbar[0].indexOf('InsertToken') > -1)
    {
        Milex.getTokensForPlugIn(textarea.attr('data-token-callback')).done(function(tokens) {
            mQuery.extend(ckEditorOption, {
                mentions: [{
                    marker: '{',
                    minChars: 0,
                    feed: Milex.getFeedItems,
                    itemTemplate: Milex.customItemRenderer,
                    outputTemplate: Milex.customItemOutputRenderer,
                }],
                on: {
                    pluginsLoaded: function() {
                        ...
                        editor.ui.addRichCombo( 'InsertToken', {
                            ...
                            init: function() {
                                const me = this;
                                Milex.builderTokensForCkEditor.forEach(function(item){
                                    let key = item.id;
                                    let value = item.name;
            ...
            Milex.InitCkEditor(textarea, ckEditorOption);
        })
    }
    else
    {
...
Milex.InitCkEditor  = function(textarea, options) {
