 ...
 * @param el
 */
Milex.activateSortablePanels = function (container) {
...
console.log(prototype);
            if (mQuery(prototype).length) {
                console.log('exists');
                Milex.appendSortablePanel(sortablesContainer, prototype);
            }
    ...
    mQuery(sortable).find('.sortable-panel-wrapper').each(function() {
        Milex.activateSortablePanel(mQuery(this).closest('.panel'));
    });
};
 ...
 * @param modal
 */
Milex.appendSortablePanel = function(sortablesContainer, modal) {
    ...
    Milex.renameFormElements(newModal, oldIdPrefix, oldNamePrefix, newIdPrefix, newNamePrefix)

    Milex.activateModalEmbeddedForms('#'+mQuery(newModal).attr('id'));

    Milex.showModal(newModal);
 ...
 * @param modal
 */
Milex.updateSortablePanel = function(modalBtn, modal) {
    ...
    Milex.activateSortablePanel(panel);
    ...
    Milex.toggleSortablePanelAddMessage(mQuery(panel).closest('.sortable-panels'));
};
 ...
 * @param panel
 */
Milex.deleteSortablePanel = function(panel) {
    var panelContainer = mQuery(panel).closest('.sortable-panels');
    mQuery(panel).remove();

    Milex.toggleSortablePanelAddMessage(panelContainer);
};
 ...
 * @param modal
 */
Milex.cancelSortablePanel = function(modalBtn, modal) {
    setTimeout(function () {
 ...
 */
Milex.toggleSortablePanelAddMessage = function(panelContainer) {
