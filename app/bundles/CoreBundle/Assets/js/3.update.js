 ...
 * @param state
 */
Milex.processUpdate = function (container, step, state) {
    // Edge case but do it anyway, remove the /index_dev.php from milexBaseUrl to make sure we can always correctly call the standalone upgrader
    var baseUrl = milexBasePath + '/';
    ...
    switch (step) {
        ...
        case 1:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateSetUpdateLayout',
                dataType: 'json',
                success: function (response) {
                    if (response.success) {
                        mQuery('div[id=' + container + ']').html(response.content);
                        Milex.processUpdate(container, step + 1, state);
        ...
        case 2:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateRunChecks',
                dataType: 'json',
                success: function (response) {
        ...
        case 3:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateDownloadPackage',
                dataType: 'json',
                success: function (response) {
                    if (response.redirect) {
                        window.location = response.redirect;
        ...
        case 4:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateExtractPackage',
                dataType: 'json',
                success: function (response) {
                    if (response.redirect) {
                        window.location = response.redirect;
                ...
                success: function (response) {
                    if (response.redirect) {
                        ...
                        if (response.error) {
                            ...
                            Milex.processUpdate(container, step + 1, response.updateState);
                        } else {
                            mQuery('td[id=update-step-moving-status]').append(mQuery('<i></i>').addClass('pull-right fa fa-spinner fa-spin'));

                            // In this section, the step hasn't completed yet so we repeat it
                            Milex.processUpdate(container, step, response.updateState);
                        }
                    }
                },
                error: function (request, textStatus, errorThrown) {
        ...
        case 9:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateDatabaseMigration&finalize=1',
                dataType: 'json',
                success: function (response) {
                    if (response.redirect) {
                        window.location = response.redirect;
                ...
                error: function (request, textStatus, errorThrown) {
                    ...
                    window.location = milexBaseUrl + 's/update/schema?update=1';
                }
            });
            break;
        ...
        case 10:
            mQuery.ajax({
                showLoadingBar: true,
                url: milexAjaxUrl + '?action=core:updateFinalization',
                dataType: 'json',
                success: function (response) {
                    if (response.redirect) {
                        window.location = response.redirect;
                        ...
                        if (response.success) {
                            mQuery('div[id=' + container + ']').html('<div class="alert alert-milex">' + response.message + '</div>');
