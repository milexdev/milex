...
final class PreUpdateChecksStep implements StepInterface
{
    ...
    public function execute(ProgressBar $progressBar, InputInterface $input, OutputInterface $output): void
    {
        ...
        $progressBar->setMessage($this->translator->trans('milex.core.command.update.step.checks'));
        $progressBar->advance();
        ...
        if (!empty($errors)) {
            ...
            throw new UpdateFailedException($this->translator->trans('milex.core.update.check.error')."\n".$errorString);
        }
    }
}
