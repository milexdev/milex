...
final class RemoveDeletedFilesStep implements StepInterface
{
    ...
    public function execute(ProgressBar $progressBar, InputInterface $input, OutputInterface $output): void
    {
        ...
        $progressBar->setMessage($this->translator->trans('milex.core.update.remove.deleted.files'));
        $progressBar->advance();
    ...
    private function deleteFile(string $file)
    {
        ...
        $this->logger->error(
            'UPDATE ERROR: '.$this->translator->trans('milex.core.update.error.removing.file', ['%path%' => $file])
        );
    }
}
