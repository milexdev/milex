...
namespace Milex\CoreBundle\DependencyInjection\Builder;

use Milex\CoreBundle\DependencyInjection\Builder\Metadata\ConfigMetadata;
...
class BundleMetadataBuilder
{
    ...
    private function buildMetadata(): void
    {
        foreach ($this->symfonyBundles as $symfonyBundle => $namespace) {
            // Plugin
            if (false !== strpos($namespace, 'MilexPlugin\\')) {
            ...
            // Core bundle
            if (false !== strpos($namespace, 'Milex\\')) {
                $this->coreMetadata[$symfonyBundle] = $this->buildCoreMetadata($namespace, $symfonyBundle);

                continue;
            }

            // Otherwise not a Milex bundle so ignore
        }

        // Make CoreBundle the first in the core bundle list
        if (!isset($this->coreMetadata['MilexCoreBundle'])) {
            // Not always set for tests
            return;
        }

        $coreBundle = $this->coreMetadata['MilexCoreBundle'];
        unset($this->coreMetadata['MilexCoreBundle']);
        $this->coreMetadata = array_merge(['MilexCoreBundle' => $coreBundle], $this->coreMetadata);
    }
    ...
    {
        $bundleName    = str_replace('Milex', '', $symfonyBundle);
