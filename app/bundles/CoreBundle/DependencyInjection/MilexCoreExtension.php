...
namespace Milex\CoreBundle\DependencyInjection;
...
/**
 * Class MilexCoreExtension.
 *
 * This is the class that loads and manages your bundle configuration
 * To learn more see {@link http://symfony.com/doc/current/cookbook/bundles/extension.html}
 */
class MilexCoreExtension extends Extension
{
    /**
     * {@inheritdoc}
     */
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        // as a service and tag with console.command or include in a Milex config.php services[command] array.
        $loader = new PhpFileLoader($container, new FileLocator(__DIR__.'/../../../config'));
        $loader->load('services.php');

        $bundles = array_merge($container->getParameter('milex.bundles'), $container->getParameter('milex.plugin.bundles'));
                ...
                foreach ($config as $type => $services) {
                    switch ($type) {
                        ...
                        case 'menus':
                            ...
                            break;
                        case 'models':
                            $defaultTag = 'milex.model';
                            break;
                        case 'permissions':
                            $defaultTag = 'milex.permissions';
                            break;
                        case 'integrations':
                            $defaultTag = 'milex.integration';
                            break;
                        case 'command':
                        case 'commands':
                    ...
                    foreach ($services as $name => $details) {
                        ...
                        if ('menus' == $type) {
                            $details = array_merge(
                                [
                                    ...
                                    'factory' => ['@milex.menu.builder', $details['alias'].'Menu'],
                                ],
                                $details
                            );
                        ...
                        if (isset($details['tags'])) {
                            ...
                            if ('events' == $type) {
                                $definition->addTag('milex.event_subscriber');
                            }
                        }
        ...
        foreach ($menus as $alias => $options) {
            $container->setDefinition('milex.menu_renderer.'.$alias, new Definition(
                \Milex\CoreBundle\Menu\MenuRenderer::class,
                [
                    new Reference('knp_menu.matcher'),
                    new Reference('milex.helper.templating'),
                    $options,
                ]
            ))
