(function( window ) {
    ...
    function define_library(){
        ...
        Form.initialize = function(){
            ...
            while(text = re.exec(document.body.innerHTML)) {
                ...
                if (Core.debug()) console.log(replaceArgs.data['style']+' Milex Form: '+replaceText);
        ...
        Form.bindClickEvents = function() {
            ...
            for(var index in Form.clickEvents) {
                ...
                document.querySelector(current.data.element).setAttribute("_milex_form_index", index);
                document.querySelector(current.data.element).addEventListener("click", function(){
                    ...
                    Form.openModal(Form.clickEvents[this.getAttribute('_milex_form_index')]);
                });
            }
        };
        ...
        Form.getFormLink = function(options) {
            ...
            return Core.getMilexBaseUrl() + index + '/form/' + options.data['id'] + '?' + options.params;
        };
        ...
        Form.customCallbackHandler = function(formId, event, data) {
            if (typeof MilexFormCallback !== 'undefined' &&
                typeof MilexFormCallback[formId] !== 'undefined' &&
                typeof MilexFormCallback[formId][event] == 'function'
            ) {
                if (typeof data == 'undefined') {
                    data = null;
                }

                return MilexFormCallback[formId][event](data);
            }
        ...
        Form.prepareForms = function() {
            ...
            for (var i = 0, n = forms.length; i < n; i++) {
                var formId = forms[i].getAttribute('data-milex-form');
                if (formId !== null) {
        ...
        Form.prepareMessengerForm = function(formId) {
            var theForm = document.getElementById('milexform_' + formId);
            ...
            if (!document.getElementById('milexiframe_' + formId)) {
                ...
                ifrm.style.width = 0;
                ifrm.style.heigh = 0;
                ifrm.setAttribute( 'id', 'milexiframe_' + formId);
                ifrm.setAttribute('name', 'milexiframe_' + formId);
                document.body.appendChild(ifrm);

                theForm.target = 'milexiframe_' + formId;
            }

            if (!document.getElementById('milexform_' + formId + '_messenger')) {
                var messengerInput = document.createElement('INPUT');
                messengerInput.type = 'hidden';
                messengerInput.setAttribute('name', 'milexform[messenger]');
                messengerInput.setAttribute('id', 'milexform_' + formId + '_messenger');
        ...
        Form.prepareValidation = function(formId) {
            if (typeof window.MilexFormValidations[formId] == 'undefined') {
                window.MilexFormValidations[formId] = {};
                ...
                [].forEach.call(validations, function (container) {
                    ...
                    window.MilexFormValidations[formId][alias] = {
        ...
        Form.prepareShowOn = function (formId) {
            var theForm = document.getElementById('milexform_' + formId);
            var showOnDataAttribute = 'data-milex-form-show-on';
            ...
            [].forEach.call(showOn, function (container) {
                ...
                var idOnChangeElem  = "milexform_" + formId+"_"+returnArray[0];
        ...
        Form.doShowOn = function (parents, key, selectedValues) {
            ...
            Object.keys((parents[key])).forEach(function(key2) {
                [].forEach.call(selectedValues, function (selectedValue) {
                    ...
                    if (selectedValue) {
                        if (el.getAttribute('data-milex-form-expr') == 'notIn') {
        ...
        Form.preparePagination = function(formId) {
            var theForm        = document.getElementById('milexform_'+formId);
            var pages          = theForm.querySelectorAll('[data-milex-form-page]');
            var lastPageNumber = pages.length;

            [].forEach.call(pages, function (page) {
                var pageNumber       = parseInt(page.getAttribute('data-milex-form-page'));
                var pageBreak        = theForm.querySelector('[data-milex-form-pagebreak="'+pageNumber+'"]');
                ...
                if (pageBreak) {
                    ...
                    var prevButton = pageBreak.querySelector('[data-milex-form-pagebreak-button="prev"]');
                    var nextButton = pageBreak.querySelector('[data-milex-form-pagebreak-button="next"]');
                    ...
                    nextButton.onclick = function(formId, theForm, hidePageNumber, showPageNumber) {
                        return function () {
                            ...
                            var validations = theForm.querySelector('[data-milex-form-page="' + hidePageNumber + '"]').querySelectorAll('[data-validate]');
                            var isValid = true;
                            [].forEach.call(validations, function (container) {
        ...
        Form.switchPage = function(theForm, showPageNumber) {
            ...
            var thePage = theForm.querySelector('[data-milex-form-page="' + showPageNumber + '"]');
            if (thePage) {
                thePage.style.display = 'block'
            }
            var showPageBreak = theForm.querySelector('[data-milex-form-pagebreak="' + showPageNumber + '"]');
            if (showPageBreak) {
        ...
        Form.getPageForField = function (formId, fieldId, switchPage) {
            ...
            var pageBreak = Form.findAncestor(container, 'milexform-page-wrapper');
            if (pageBreak) {
                var page = pageBreak.getAttribute('data-milex-form-page');
                if (switchPage) {
                    Form.switchPage(document.getElementById('milexform_' + formId), page);
                }
        ...
        Form.findAncestor = function (el, cls) {
            ...
            while (true) {
                ...
                if (!parent || Form.hasClass(parent, 'milexform-innerform')) {
                    break;
            ...
            var validator = {
                validateForm: function (submitForm) {
                    ...
                    if (formValid === null) {
                        ...
                        var formContainer = document.getElementById('milexform_wrapper_' + formId);
                        if (formContainer) {
                            formContainer.className = formContainer.className.replace(" milexform-post-success", "");
                        }
                        ...
                        for (var fieldKey in MilexFormValidations[formId]) {
                ...
                validateField: function(theForm, fieldKey) {
                    var field = MilexFormValidations[formId][fieldKey];
                ...
                markError: function(containerId, valid, validationMessage) {
                    ...
                    if (elContainer) {
                        elErrorSpan = elContainer.querySelector('.milexform-errormsg');
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorMark', callbackData)) {
                        if (elErrorSpan) {
                            ...
                            elContainer.className = elContainer.className + " milexform-has-error";
                        }
                    }
                },
                ...
                clearErrors: function() {
                    var theForm    = document.getElementById('milexform_' + formId);
                    var hasErrors  = theForm.querySelectorAll('.milexform-has-error');
                    var that       = this;
                ...
                clearError: function(containerId) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorClear', containerId)) {
                        var elContainer = document.getElementById(containerId);
                        if (elContainer) {
                            var elErrorSpan = elContainer.querySelector('.milexform-errormsg');
                            if (elErrorSpan) {
                                elErrorSpan.style.display = 'none';
                                elContainer.className = elContainer.className.replace(" milexform-has-error", "");
                            }
                        }
                    }
                },
                ...
                parseFormResponse: function (response) {
                    ...
                    var ifrm = document.getElementById('milexiframe_' + formId);
                    if (ifrm) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onResponse', response)) {
                        ...
                        if (response.download) {
                            ...
                            document.getElementById('milexiframe_' + formId).src = response.download;
                            ...
                            for (var field in response.validationErrors) {
                                ...
                                this.markError('milexform_' + formId + '_' + field, false, response.validationErrors[field]);
                            }

                            if (firstPage) {
                                Form.switchPage(document.getElementById('milexform_' + formId), firstPage);
                            }
                        ...
                        if (response.success) {
                            ...
                            var formContainer = document.getElementById('milexform_wrapper_' + formId);
                            if (formContainer) {
                                formContainer.className = formContainer.className + " milexform-post-success";
                            }
                ...
                setMessage: function (message, type) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onMessageSet', {message: message, type: type})) {
                        var container = document.getElementById('milexform_' + formId + '_' + type);
                        if (container) {
                ...
                resetForm: function () {
                    ...
                    Form.switchPage(document.getElementById('milexform_' + formId), 1);

                    document.getElementById('milexform_' + formId).reset();
                ...
                disableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonDisable')) {
                        var submitButton = document.getElementById('milexform_' + formId).querySelector('.milexform-button');

                        if (submitButton) {
                            MilexLang.submitMessage = submitButton.innerHTML;
                            submitButton.innerHTML = MilexLang.submittingMessage;
                ...
                enableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonEnable')) {
                        var submitButton = document.getElementById('milexform_' + formId).querySelector('.milexform-button');
                        if (submitButton) {
                            submitButton.innerHTML = MilexLang.submitMessage;
        ...
        Form.registerFormMessenger = function() {
            window.addEventListener('message', function(event) {
                ...
                if (MilexDomain.indexOf(event.origin) !== 0) return;
        ...
        Form.getFieldContainerId = function(formId, fieldKey) {
            var containerId = 'milexform_' + formId + '_' + fieldKey;
            if (!document.getElementById(containerId)) {
                containerId = 'milexform_' + fieldKey;
            }
        ...
        Modal.loadStyle = function() {
            ...
            s.rel = "stylesheet"
            s.type = "text/css"
            s.href = Core.debug() ? Core.getMilexBaseUrl() + 'media/css/modal.css' : Core.getMilexBaseUrl() + 'media/css/modal.min.css';
        ...
        Modal.open = function() {
            ...
            this.modal.className = this.modal.className + (this.modal.offsetHeight > window.innerHeight ? " milexForm-open milexForm-anchored" : " milexForm-open");
            this.overlay.className = this.overlay.className + " milexForm-open";
        };
        ...
        Modal.buildOut = function() {
            ...
            this.modal.className = "milexForm-modal " + this.options.className;
            ...
            if (this.options.closeButton === true) {
                ...
                this.closeButton.className = "milexForm-close close-button";
            ...
            if (this.options.overlay === true) {
                ...
                this.overlay.className = "milexForm-overlay " + this.options.className;
                docFrag.appendChild(this.overlay);
            }

            // Create content area and append to modal
            contentHolder = document.createElement("div");
            contentHolder.className = "milexForm-content";
        ...
        Modal.close = function() {
            var _ = this;
            this.modal.className = this.modal.className.replace(" milexForm-open", "");
            this.overlay.className = this.overlay.className.replace(" milexForm-open", "");
        ...
        Core.setMilexBaseUrl = function(base_url) {
            config.milex_base_url = base_url.split('/').slice(0,-3).join('/')+'/';
        };

        Core.getMilexBaseUrl = function() {
            return config.milex_base_url;
        };

        Core.initialize = function(base_url) {
            Profiler.startTime();
            if (Core.debug()) console.log('SDK initialized');
            if (typeof(config.milex_base_url) == 'undefined') Core.setMilexBaseUrl(base_url);
            if (Core.debug()) console.log('Automatic setup milex_base_url as: ' + config.milex_base_url);
            Modal.loadStyle();
    ...
    if (typeof(MilexSDK) === 'undefined') {
        window.MilexSDK = define_library();
        var sjs = document.getElementsByTagName('script'), tjs = sjs.length;
        for (var i = 0; i < sjs.length; i++) {
            if (!sjs[i].hasAttribute('src') || sjs[i].getAttribute("src").indexOf('milex-form-src.js') == -1) continue;
            var sParts = sjs[i].getAttribute("src").split("?");
            if (sParts[1] && sParts[1].indexOf("=") !== -1) MilexSDK.setConfig(MilexSDK.parseToObject(sParts[1]));
            MilexSDK.initialize(sParts[0]);
            break;
        }
    }
    if (typeof window.MilexFormValidations == 'undefined') {
        window.MilexFormValidations = {};
    }
})( window );
