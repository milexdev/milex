...
define('MILEX_ROOT', (IN_CLI || $standalone || empty($task)) ? __DIR__ : dirname(__DIR__));
define('MILEX_APP_ROOT', MILEX_ROOT.'/app');

if ($standalone || IN_CLI) {
    if (!file_exists(__DIR__.'/upgrade')) {
        mkdir(__DIR__.'/upgrade');
    }
    define('MILEX_UPGRADE_ROOT', __DIR__.'/upgrade');
} else {
    define('MILEX_UPGRADE_ROOT', __DIR__);
}

// Fail-safe PHP version check
if (file_exists(MILEX_UPGRADE_ROOT.'/app/release_metadata.json')) {
    $metadata = json_decode(file_get_contents(MILEX_UPGRADE_ROOT.'/app/release_metadata.json'), true);
...
$cacheDir        = (isset($localParameters['cache_path'])) ? str_replace('%kernel.root_dir%', MILEX_APP_ROOT, $localParameters['cache_path'].'/prod') : MILEX_APP_ROOT.'/../var/cache/prod';
$logDir          = (isset($localParameters['log_path'])) ? str_replace('%kernel.root_dir%', MILEX_APP_ROOT, $localParameters['log_path'].'/prod') : MILEX_APP_ROOT.'/../var/logs';

define('MILEX_CACHE_DIR', $cacheDir);
define('MILEX_UPGRADE_ERROR_LOG', $logDir.'/upgrade_errors.php');
...
if (isset($_COOKIE['milex_session_name'])) {
    ...
    include MILEX_APP_ROOT.'/config/paths.php';
    $localConfigPath = str_replace('%kernel.root_dir%', MILEX_APP_ROOT, $paths['local_config']);
...
function get_local_config()
{
    ...
    if (null === $parameters) {
        ...
        $localConfig = str_replace('%kernel.root_dir%', MILEX_APP_ROOT, $paths['local_config']);
        ...
        if (file_exists(MILEX_APP_ROOT.'/config/parameters_local.php')) {
            /** @var $parameters */
            include MILEX_APP_ROOT.'/config/parameters_local.php';
...
{
    if (!recursive_remove_directory(MILEX_CACHE_DIR)) {
        process_error_log(['Could not remove the application cache.  You will need to manually delete '.MILEX_CACHE_DIR.'.']);
...
function run_symfony_command($command, array $args)
{
    ...
    require_once MILEX_APP_ROOT.'/autoload.php';
    require_once MILEX_APP_ROOT.'/AppKernel.php';
...
function copy_directory($src, $dest)
{
    ...
    if (!is_dir($dest)) {
        if (!@mkdir($dest, 0777, true)) {
            return sprintf(
                ...
                str_replace(MILEX_UPGRADE_ROOT, '', $src)
            );
        }
    }

    if (!($dh = @opendir($src))) {
        return sprintf('Could not read directory %s to move files.', str_replace(MILEX_UPGRADE_ROOT, '', $src));
    }
    ...
    while (false !== ($file = readdir($dh))) {
        ...
        switch (filetype($sfid)) {
            ...
            case 'file':
                if (!@rename($sfid, $dfid)) {
                    $errorLog[] = sprintf('Could not move file %s to production.', str_replace(MILEX_UPGRADE_ROOT, '', $sfid));
                }
                break;
        }
    }
...
function move_milex_bundles(array $status, $maxCount = 5)
{
    ...
    if (is_dir(MILEX_UPGRADE_ROOT.'/plugins') && !$status['updateState']['pluginComplete']) {
        $iterator = new DirectoryIterator(MILEX_UPGRADE_ROOT.'/plugins');

        // Sanity check, make sure there are actually directories here to process
        $dirs = glob(MILEX_UPGRADE_ROOT.'/plugins/*', GLOB_ONLYDIR);
        ...
        if (count($dirs)) {
            ...
            foreach ($iterator as $directory) {
                ...
                if (!$directory->isDot() && $directory->isDir()) {
                    ...
                    $dest = str_replace(MILEX_UPGRADE_ROOT, MILEX_ROOT, $src);
                    ...
                    if (!$deleteDir) {
                        $errorLog[] = sprintf('Failed to remove the upgrade directory %s folder', str_replace(MILEX_UPGRADE_ROOT, '', $src));
                    }
                }
            }
        }
        ...
        $deleteDir = recursive_remove_directory(MILEX_UPGRADE_ROOT.'/plugins');
    ...
    if (is_dir(MILEX_UPGRADE_ROOT.'/app/bundles') && !$status['updateState']['bundleComplete']) {
        ...
        $completed = true;
        $iterator  = new DirectoryIterator(MILEX_UPGRADE_ROOT.'/app/bundles');

        // Sanity check, make sure there are actually directories here to process
        $dirs = glob(MILEX_UPGRADE_ROOT.'/app/bundles/*', GLOB_ONLYDIR);
        ...
        if ($completed) {
            ...
            $deleteDir = recursive_remove_directory(MILEX_UPGRADE_ROOT.'/app/bundles');
...
function move_milex_core(array $status)
{
    ...
    foreach ($nestedDirectories as $dir) {
        if (is_dir(MILEX_UPGRADE_ROOT.$dir)) {
            copy_directories($dir, $errorLog);

            // At this point, we can remove the media directory
            $deleteDir = recursive_remove_directory(MILEX_UPGRADE_ROOT.$dir);
    ...
    $iterator = new FilesystemIterator(MILEX_UPGRADE_ROOT);
    ...
    foreach ($iterator as $file) {
        ...
        if ($file->isFile() && !in_array($file->getFilename(), ['deleted_files.txt', 'critical_migrations.txt', 'upgrade.php'])) {
            ...
            if (!@rename($src, $dest)) {
                $errorLog[] = sprintf('Could not move file %s to production.', str_replace(MILEX_UPGRADE_ROOT, '', $src));
            }
        }
    }
...
function move_milex_vendors(array $status, $maxCount = 5)
{
    ...
    if (!is_dir(MILEX_UPGRADE_ROOT.'/vendor')) {
        $status['complete']                      = true;
        $status['stepStatus']                    = 'Success';
    ...
    if (is_dir(MILEX_UPGRADE_ROOT.'/vendor/symfony') && !isset($status['updateState']['completedVendors']['symfony'])) {
        ...
        $completed = true;
        $iterator  = new DirectoryIterator(MILEX_UPGRADE_ROOT.'/vendor/symfony');

        // Sanity check, make sure there are actually directories here to process
        $dirs = glob(MILEX_UPGRADE_ROOT.'/vendor/symfony/*', GLOB_ONLYDIR);
        ...
        if ($completed) {
            ...
            $deleteDir = recursive_remove_directory(MILEX_UPGRADE_ROOT.'/vendor/symfony');
    ...
    $completed = true;
    $iterator  = new DirectoryIterator(MILEX_UPGRADE_ROOT.'/vendor');

    // Sanity check, make sure there are actually directories here to process
    $dirs = glob(MILEX_UPGRADE_ROOT.'/vendor/*', GLOB_ONLYDIR);
    ...
    if ($completed) {
        ...
        if (!@rename(MILEX_UPGRADE_ROOT.'/vendor/autoload.php', MILEX_ROOT.'/vendor/autoload.php')) {
        ...
        $deleteDir = recursive_remove_directory(MILEX_UPGRADE_ROOT.'/vendor');
...
{
    if (is_dir(MILEX_UPGRADE_ROOT.$dir)) {
        $iterator = new FilesystemIterator(MILEX_UPGRADE_ROOT.$dir);
...
function copy_directories($dir, &$errorLog, $createDest = true)
{
    ...
    $exists = file_exists(MILEX_ROOT.$dir);
    if ($createDest && !$exists) {
        mkdir(MILEX_ROOT.$dir, 0755, true);
    } elseif (!$exists) {
        $errorLog[] = sprintf('%s does not exist.', MILEX_ROOT.$dir);
    ...
    $iterator = new DirectoryIterator(MILEX_UPGRADE_ROOT.$dir);
...
function process_error_log(array $errorLog)
{
    ...
    if (count($errorLog)) {
        ...
        if (file_exists(MILEX_UPGRADE_ERROR_LOG)) {
            $errors = file_get_contents(MILEX_UPGRADE_ERROR_LOG);
        } else {
        ...
        @file_put_contents(MILEX_UPGRADE_ERROR_LOG, $errors);
    }
}
...
function remove_milex_deleted_files(array $status)
{
    ...
    if (file_exists(MILEX_UPGRADE_ROOT.'/deleted_files.txt')) {
        $deletedFiles = json_decode(file_get_contents(MILEX_UPGRADE_ROOT.'/deleted_files.txt'), true);

        foreach ($deletedFiles as $file) {
            $path = MILEX_ROOT.'/'.$file;
...
function make_request($url, $method = 'GET', $data = null)
{
    ...
    curl_setopt($ch, CURLOPT_CAINFO, MILEX_ROOT.'/vendor/joomla/http/src/Transport/cacert.pem');
    $data = curl_exec($ch);
    curl_close($ch);
