...
class TagController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        //set limits
        $limit = $session->get('milex.tagmanager.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('milex.tags.filter', ''));
        $session->set('milex.tags.filter', $search);

        //do some default filtering
        $orderBy    = $session->get('milex.tags.orderby', 'lt.tag');
        $orderByDir = $session->get('milex.tags.orderbydir', 'ASC');
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('milex.tags.page', $lastPage);
            $returnUrl = $this->generateUrl('milex_tagmanager_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect([
                ...
                'contentTemplate' => 'MilexTagManagerBundle:Tag:index',
                'passthroughVars' => [
                    'activeLink'    => '#milex_tagmanager_index',
                    'milexContent' => 'tagmanager',
                ],
            ]);
        }

        //set what page currently on so that we can return here after form submission/cancellation
        $session->set('milex.tagmanager.page', $page);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexTagManagerBundle:Tag:list.html.php',
            'passthroughVars' => [
                'activeLink'    => '#milex_tagmanager_index',
                'route'         => $this->generateUrl('milex_tagmanager_index', ['page' => $page]),
                'milexContent' => 'tags',
            ],
        ]);
    }
    ...
    {
        if (!$this->get('milex.security')->isGranted('tagManager:tagManager:create')) {
            return $this->accessDenied();
        }
        ...
        $page = $this->get('session')->get('milex.tagmanager.page', 1);
        //set the return URL for post actions
        $returnUrl = $this->generateUrl('milex_tagmanager_index', ['page' => $page]);
        $action    = $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'new']);
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if (0 !== $found) {
                        ...
                        $this->addFlash('milex.core.notice.updated', [
                            '%name%'      => $tag->getTag(),
                            '%menu_link%' => 'milex_tagmanager_index',
                            '%url%'       => $this->generateUrl('milex_tagmanager_action', [
                                'objectAction' => 'edit',
                                'objectId'     => $tag->getId(),
                            ]),
                        ]);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MilexTagManagerBundle:Tag:form.html.php',
            'passthroughVars' => [
                'activeLink'    => '#milex_tagmanager_index',
                'route'         => $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'new']),
                'milexContent' => 'tagmanager',
            ],
        ]);
    }
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        try {
            ...
            return $this->createTagModifyResponse(
                $tag,
                $postActionVars,
                $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
                $ignorePost
            );
            ...
            return $this->postActionRedirect(
                array_merge($postActionVars, [
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'milex.tagmanager.tag.error.notfound',
                            'msgVars' => ['%id%' => $objectId],
                        ],
                    ],
                ])
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($this->isFormValid($form)) {
                    ...
                    if ($form->get('buttons')->get('apply')->isClicked()) {
                        ...
                        $postActionVars['returnUrl']         = $this->generateUrl('milex_tagmanager_action', [
                            'objectAction' => 'edit',
                            'objectId'     => $tag->getId(),
                        ]);
                        ...
                        $editAction = $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'edit', 'objectId' => $tag->getId()]);
                        ...
                        $postActionVars['viewParameters'] = [
                            ...
                            'entity'       => $tag,
                            'objectId'     => $tag->getId(),
                            'form'         => $this->setFormTheme($form, $contentTemplate, 'MilexTagManagerBundle:FormTheme\Filter'),
                        ];
    ...
    private function getPostActionVars($objectId = null)
    {
        //set the return URL
        if ($objectId) {
            $returnUrl       = $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'view', 'objectId'=> $objectId]);
            $viewParameters  = ['objectAction' => 'view', 'objectId'=> $objectId];
            $contentTemplate = 'MilexTagManagerBundle:Tag:view';
        } else {
            //set the page we came from
            $page            = $this->get('session')->get('milex.tagmanager.page', 1);
            $returnUrl       = $this->generateUrl('milex_tagmanager_index', ['page' => $page]);
    ...
    {
        /** @var \Milex\LeadBundle\Model\TagModel $model */
        ...
        } elseif (!$this->get('milex.security')->isGranted('tagManager:tagManager:view')) {
            return $this->accessDenied();
        }

        return $this->delegateView([
            'returnUrl'      => $this->generateUrl('milex_tagmanager_action', ['objectAction' => 'view', 'objectId' => $tag->getId()]),
            'viewParameters' => [
                'tag' => $tag,
            ],
            'contentTemplate' => 'MilexTagManagerBundle:Tag:details.html.php',
            'passthroughVars' => [
                'activeLink'    => '#milex_tagmanager_index',
                'milexContent' => 'tagmanager',
            ],
    ...
    public function deleteAction($objectId)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            } elseif (!$this->get('milex.security')->isGranted('tagManager:tagManager:delete')) {
                return $this->accessDenied();
            }
            ...
            if ($overrideModel->getRepository()->countByLeads([$objectId])[$objectId] > 0) {
                $flashes[] = [
                    'type'    => 'error',
                    'msg'     => 'milex.tagmanager.tag.error.cannotbedeleted',
                ];
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'milex.tagmanager.tag.notice.batch_deleted',
                    'msgVars' => [
