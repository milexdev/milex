...
class PublicController extends FormController
{
    ...
    public function callbackAction()
    {
        ...
        $validatedRequest = $this->get('milex.plugin.clearbit.lookup_helper')->validateRequest($oid, $this->request->request->get('type'));
            ...
            if ('person' === $this->request->request->get('type')) {
                ...
                if ($notify && (!isset($lead->imported) || !$lead->imported)) {
                    ...
                    if ($user = $userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf($this->translator->trans('milex.plugin.clearbit.contact_retrieved'), $lead->getEmail()),
                            'Clearbit Plugin',
                            'fa-search',
                            $user
                        );
                ...
                if ('company' === $this->request->request->get('type', [], true)) {
                    ...
                    if ($notify) {
                        ...
                        if ($user = $userModel->getEntity($notify)) {
                            $this->addNewNotification(
                                sprintf($this->translator->trans('milex.plugin.clearbit.company_retrieved'), $company->getName()),
                                'Clearbit Plugin',
                                'fa-search',
                                $user
                            );
            ...
            try {
                if ($notify) {
                    ...
                    if ($user = $userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf(
                                $this->translator->trans('milex.plugin.clearbit.unable'),
                                $ex->getMessage()
                            ),
                            'Clearbit Plugin',
                            'fa-exclamation',
                ...
                $this->get('monolog.logger.milex')->log('error', 'Clearbit: '.$ex2->getMessage());
            }
        }
