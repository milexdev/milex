...
namespace MilexPlugin\MilexClearbitBundle\Controller;

use Milex\FormBundle\Controller\FormController;
use Milex\LeadBundle\Entity\Company;
use Milex\LeadBundle\Entity\Lead;
use MilexPlugin\MilexClearbitBundle\Form\Type\BatchLookupType;
use MilexPlugin\MilexClearbitBundle\Form\Type\LookupType;
...
class ClearbitController extends FormController
{
    ...
    public function lookupPersonAction($objectId = '')
    {
        ...
        if (!$this->get('milex.security')->hasEntityAccess(
            ...
            $this->addFlash(
                $this->translator->trans('milex.plugin.clearbit.forbidden'),
                [],
                'error'
            );
        ...
        if ('GET' === $this->request->getMethod()) {
            ...
            return $this->delegateView(
                [
                    ...
                    'contentTemplate' => 'MilexClearbitBundle:Clearbit:lookup.html.php',
                    'passthroughVars' => [
            ...
            if ('POST' === $this->request->getMethod()) {
                try {
                    $this->get('milex.plugin.clearbit.lookup_helper')->lookupContact($lead, array_key_exists('notify', $data));
                    $this->addFlash(
    ...
    public function batchLookupPersonAction()
    {
        ...
        if (0 === $count) {
            $this->addFlash(
                $this->translator->trans('milex.plugin.clearbit.empty'),
                [],
                'error'
            );
            ...
            if ($count > 20) {
                $this->addFlash(
                    $this->translator->trans('milex.plugin.clearbit.toomany'),
                    [],
                    'error'
                );
        ...
        if ('GET' === $this->request->getMethod()) {
            ...
            return $this->delegateView(
                [
                    ...
                    'contentTemplate' => 'MilexClearbitBundle:Clearbit:batchLookup.html.php',
                    'passthroughVars' => [
                        'activeLink'    => '#milex_contact_index',
            ...
            if ('POST' === $this->request->getMethod()) {
                ...
                foreach ($lookupEmails as $id => $lookupEmail) {
                    if ($lead = $model->getEntity($id)) {
                        try {
                            $this->get('milex.plugin.clearbit.lookup_helper')->lookupContact($lead, $notify);
                        } catch (\Exception $ex) {
                            $this->addFlash(
                                    $ex->getMessage(),
                                    [],
    ...
    public function lookupCompanyAction($objectId = '')
    {
        ...
        if ('GET' === $this->request->getMethod()) {
            ...
            if (!$website) {
                $this->addFlash(
                    $this->translator->trans('milex.plugin.clearbit.compempty'),
                    [],
                    'error'
                );
            ...
            if ('POST' === $this->request->getMethod()) {
                try {
                    $this->get('milex.plugin.clearbit.lookup_helper')->lookupCompany($company, array_key_exists('notify', $data));
                    $this->addFlash(
    ...
    public function batchLookupCompanyAction()
    {
        ...
        if (0 === $count) {
            ...
            if ($count > 20) {
                $this->addFlash(
                    $this->translator->trans('milex.plugin.clearbit.comptoomany'),
                    [],
                    'error'
                );
            ...
            if ('POST' === $this->request->getMethod()) {
                ...
                foreach ($lookupWebsites as $id => $lookupWebsite) {
                    if ($company = $model->getEntity($id)) {
                        try {
                            $this->get('milex.plugin.clearbit.lookup_helper')->lookupCompany($company, $notify);
                        } catch (\Exception $ex) {
                            $this->addFlash(
                                $ex->getMessage(),
                                [],
