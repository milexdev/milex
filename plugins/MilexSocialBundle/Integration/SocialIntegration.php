...
namespace MilexPlugin\MilexSocialBundle\Integration;
...
abstract class SocialIntegration extends AbstractIntegration
{
        ...
        if ('features' == $formArea) {
            ...
            if ($formType) {
                $builder->add('shareButton', $formType, [
                    'label'    => 'milex.integration.form.sharebutton',
                    'required' => false,
        ...
        if (empty($fields)) {
            ...
            foreach ($available as $field => $details) {
                ...
                switch ($details['type']) {
                    ...
                    case 'boolean':
                        $fields[$fn] = (!$label)
                            ? $this->translator->transConditional("milex.integration.common.{$fn}", "milex.integration.{$s}.{$fn}")
                            : $label;
                        break;
                    case 'object':
                        ...
                        if ('urls' == $field || 'url' == $field) {
                            foreach ($socialProfileUrls as $p => $d) {
                                $fields["{$p}ProfileHandle"] = (!$label)
                                    ? $this->translator->transConditional("milex.integration.common.{$p}ProfileHandle", "milex.integration.{$s}.{$p}ProfileHandle")
                                    : $label;
                            }
                            foreach ($details['fields'] as $f) {
                                $fields["{$p}Urls"] = (!$label)
                                    ? $this->translator->transConditional("milex.integration.common.{$f}Urls", "milex.integration.{$s}.{$f}Urls")
                                    : $label;
                            }
    ...
    {
        return "MilexSocialBundle:Integration/{$this->getName()}/Profile:view.html.php";
    }
