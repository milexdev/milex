...
namespace MilexPlugin\MilexSocialBundle\Helper;
...
use MilexPlugin\MilexSocialBundle\Exception\ExitMonitorException;
use MilexPlugin\MilexSocialBundle\Model\MonitoringModel;
use MilexPlugin\MilexSocialBundle\Model\PostCountModel;
...
class TwitterCommandHelper
{
    ...
    /**
     * Processes a list of tweets and creates / updates leads in Milex.
     *
    ...
    public function createLeadsFromStatuses($statusList, $monitor)
    {
        ...
        if (!$leadField) {
            ...
            $this->output($this->translator->trans('milex.social.monitoring.twitter.field.not.found'));
        ...
        if ($monitorProperties['checknames']) {
            ...
            // handle in Milex but are found during monitoring.
        ...
        foreach ($statusList as $status) {
            ...
            /* @var \Milex\LeadBundle\Entity\Lead $leadEntity */
     ...
     * @param int $mid
     *
     * @return \MilexPlugin\MilexSocialBundle\Entity\Monitoring
     */
    ...
    private function setMonitorLeadStat($monitor, $lead)
    {
        ...
        $monitorLead = new \MilexPlugin\MilexSocialBundle\Entity\Lead();
        $monitorLead->setMonitor($monitor);
        $monitorLead->setLead($lead);
        $monitorLead->setDateAdded(new \DateTime());

        /* @var \MilexPlugin\MilexSocialBundle\Entity\LeadRepository $monitorRepository */
        $monitorRepository = $this->em->getRepository('MilexSocialBundle:lead');
