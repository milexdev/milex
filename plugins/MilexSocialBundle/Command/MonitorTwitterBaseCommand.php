...
use MilexPlugin\MilexSocialBundle\Event\SocialMonitorEvent;
use MilexPlugin\MilexSocialBundle\Helper\TwitterCommandHelper;
use MilexPlugin\MilexSocialBundle\Integration\TwitterIntegration;
...
abstract class MonitorTwitterBaseCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (false === $this->twitter || false === $this->twitter->getIntegrationSettings()->getIsPublished()) {
            $this->output->writeln($this->translator->trans('milex.social.monitoring.twitter.not.published'));

            return 1;
        }

        if (!$this->twitter->isAuthorized()) {
            $this->output->writeln($this->translator->trans('milex.social.monitoring.twitter.not.configured'));
        ...
        if (!$mid) {
            $this->output->writeln($this->translator->trans('milex.social.monitoring.twitter.mid.empty'));
        ...
        if (!$monitor || !$monitor->getId()) {
            $this->output->writeln($this->translator->trans('milex.social.monitoring.twitter.monitor.does.not.exist', ['%id%' => $mid]));
    ...
    protected function processMonitor($monitor)
    {
        ...
        if (count($results['statuses'])) {
            ...
            $this->output->writeln($this->translator->trans('milex.social.monitoring.twitter.no.new.tweets'));
        }
