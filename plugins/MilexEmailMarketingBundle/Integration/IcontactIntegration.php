...
namespace MilexPlugin\MilexEmailMarketingBundle\Integration;

use MilexPlugin\MilexEmailMarketingBundle\Form\Type\IcontactType;
...
class IcontactIntegration extends EmailAbstractIntegration
{
    ...
    {
        return [
            'API-AppId'    => 'milex.icontact.keyfield.appid',
            'API-Username' => 'milex.icontact.keyfield.username',
            'API-Password' => 'milex.icontact.keyfield.password',
        ];
    }
    ...
    public function getAvailableLeadFields($settings = [])
    {
        ...
        if (empty($leadFields)) {
            ...
            foreach ($fields as $f) {
                $leadFields[$f] = [
                    'label'    => $this->translator->trans('milex.icontact.field.'.$f),
                    'type'     => 'string',
