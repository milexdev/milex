...
use MilexPlugin\MilexEmailMarketingBundle\Form\Type\ConstantContactType;
...
class ConstantContactIntegration extends EmailAbstractIntegration
{
    ...
    public function getAvailableLeadFields($settings = [])
    {
        ...
        foreach ($fields as $f) {
            $leadFields[$f] = [
                'label'    => $this->translator->trans('milex.constantcontact.field.'.$f),
                'type'     => 'string',
        ...
        while ($c <= 15) {
            $leadFields['customfield_'.$c] = [
                'label'    => $this->translator->trans('milex.constantcontact.customfield.'.$f),
                'type'     => 'string',
                'required' => false,
            ];
            ++$c;
