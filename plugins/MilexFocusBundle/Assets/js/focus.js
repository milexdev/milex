//@ sourceURL=/plugins/MilexFocusBundle/Assets/js/focus.js

Milex.disabledFocusActions = function(opener) {
    ...
    if (mQuery('.builder').length) {
        ...
        mQuery('.btn-fetch').on('click', function () {
            ...
            if (url) {
                ...
                Milex.launchFocusBuilder();
            } else {
                return;
            }
        });

        Milex.focusInitViewportSwitcher();
...
Milex.launchFocusBuilder = function (forceFetch) {
    ...
    if (!url) {
        ...
        Milex.ajaxActionRequest('plugin:focus:checkIframeAvailability', data, function (response) {
            ...
            Milex.focusCreateIframe(url);

            Milex.ignoreMilexFocusPreviewUpdate = false;
        }, false, false, "GET");
    }
};

// Called when you  click on the show builder button
Milex.focusUpdatePreview = function () {

    // Generate a preview
    var data = mQuery('form[name=focus]').formToArray();
    Milex.ajaxActionRequest('plugin:focus:generatePreview', data, function (response) {
        ...
        if (!mQuery('.mf-bar').length && mQuery('.builder-content').length) {
            mQuery('.builder-content').on('click', function () {
                Milex.closeFocusModal(mQuery('#focus_style').val());
            });
            mQuery('.milex-focus').on('click', function (e) {
                e.stopPropagation();
            });
        }
    });
...
Milex.setFocusDefaultColors = function () {
...
Milex.toggleBarCollapse = function () {
...
Milex.closeFocusModal = function (style) {
...
Milex.closeFocusBuilder = function (el) {
    // Kill preview updates
    if (typeof Milex.ajaxActionXhr != 'undefined' && typeof Milex.ajaxActionXhr['plugin:focus:generatePreview'] != 'undefined') {
        Milex.ajaxActionXhr['plugin:focus:generatePreview'].abort();
        delete Milex.ajaxActionXhr['plugin:focus:generatePreview'];
    }
...
Milex.focusInitViewportSwitcher = function () {
 ...
 */
Milex.focusCreateIframe = function (url) {
