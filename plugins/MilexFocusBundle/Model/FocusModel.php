...
namespace MilexPlugin\MilexFocusBundle\Model;
...
use MilexPlugin\MilexFocusBundle\Event\FocusEvent;
use MilexPlugin\MilexFocusBundle\FocusEvents;
use MilexPlugin\MilexFocusBundle\Form\Type\FocusType;
...
class FocusModel extends FormModel
{
    ...
    /**
     * @var \Milex\FormBundle\Model\FormModel
     */
    protected $formModel;
    ...
    public function __construct(
        \Milex\FormBundle\Model\FormModel $formModel,
     ...
     * {@inheritdoc}
     *
     * @return \MilexPlugin\MilexFocusBundle\Entity\FocusRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexFocusBundle:Focus');
    }

    /**
     * {@inheritdoc}
     *
     * @return \MilexPlugin\MilexFocusBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('MilexFocusBundle:Stat');
    }
    ...
    public function generateJavascript(Focus $focus, $isPreview = false, $byPassCache = false)
    {
        ...
        if ($isPreview || $byPassCache || empty($cached) || !isset($cached['js'])) {
            ...
            $javascript = $this->templating->getTemplating()->render(
                'MilexFocusBundle:Builder:generate.js.php',
                [
                    'focus'    => $focusArray,
                    'preview'  => $isPreview,
                    'clickUrl' => $url,
        ...
        $focusContent = $this->templating->getTemplating()->getEngine('MilexFocusBundle:Builder:content.html.php')->escape($focusContent, 'js');
    ...
    public function getContent(array $focus, $isPreview = false, $url = '#')
    {
        ...
        $content = $this->templating->getTemplating()->render(
            'MilexFocusBundle:Builder:content.html.php',
            [
                'focus'    => $focus,
                'preview'  => $isPreview,
                'htmlMode' => $htmlMode,
        ...
        $formContent        = (!empty($form)) ? $this->templating->getTemplating()->render(
            'MilexFocusBundle:Builder:form.html.php',
            [
                'form'           => $form,
                'pages'          => $pages,
                'lastPage'       => $lastPage,
    ...
    public function addStat(Focus $focus, $type, $data = null, $lead = null)
    {
        ...
        if (is_array($lead)) {
            ...
            $lead = $this->em->getReference('MilexLeadBundle:Lead', $lead['id']);
        }

        switch ($type) {
            case Stat::TYPE_FORM:
                /** @var \Milex\FormBundle\Entity\Submission $data */
                $typeId = $data->getId();
                break;
            ...
            case Stat::TYPE_CLICK:
                /** @var \Milex\PageBundle\Entity\Hit $data */
                $typeId = $data->getId();
                break;
        }
    ...
    public function getStats(Focus $focus, $unit, \DateTime $dateFrom = null, \DateTime $dateTo = null, $dateFormat = null, $canViewOthers = true)
    {
        ...
        $chart->setDataset($this->translator->trans('milex.focus.graph.views'), $data);
        ...
        if ('notification' != $focus->getType()) {
            if ('link' == $focus->getType()) {
                ...
                $data = $query->loadAndBuildTimeData($q);
                $chart->setDataset($this->translator->trans('milex.focus.graph.clicks'), $data);
            } else {
                ...
                $data = $query->loadAndBuildTimeData($q);
                $chart->setDataset($this->translator->trans('milex.focus.graph.submissions'), $data);
            }
        }
