...
use MilexPlugin\MilexCrmBundle\Api\Salesforce\Exception\RetryRequestException;
use MilexPlugin\MilexCrmBundle\Api\Salesforce\Helper\RequestUrl;
...
class SalesforceApi extends CrmApi
{
     ...
     * @throws ApiErrorException
     */
    public function syncMilexToSalesforce(array $data)
    {
    ...
    public function createLeadActivity(array $activity, $object)
    {
        ...
        $mActivityObjectName = $namespace.'milex_timeline__c';
        $activityData        = [];
            ...
            foreach ($activity as $sfId => $records) {
                foreach ($records['records'] as $record) {
                    $body = [
                        ...
                        $namespace.'Milex_url__c'   => $records['leadUrl'],
            ...
            if (!empty($activityData)) {
                ...
                foreach ($chunked as $chunk) {
                    ...
                    if ($chunk) {
                        ...
                        $result                      = $this->syncMilexToSalesforce($request);
                        $results[]                   = $result;
    ...
    public function getCampaignMembers($campaignId, $modifiedSince = null, $queryUrl = null)
    {
        ...
        // Milex IntegrationEntity objects
        $this->requestSettings['headers']['Sforce-Query-Options'] = 'batchSize=200';
