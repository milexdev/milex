...
class SugarcrmApi extends CrmApi
{
    ...
    public function createLead(array $fields, $lead)
    {
        $tokenData       = $this->integration->getKeys();
        $createdLeadData = [];
        //search for Sugar id in milex records first to avoid making an API call
        if (is_object($lead)) {
        ...
        if ('6' == $tokenData['version']) {
            ...
            if (!empty($sugarLeadRecords)) {
                foreach ($sugarLeadRecords as $sLeadRecord) {
                    ...
                    $fieldsToUpdateInSugar = isset($config['update_milex']) ? array_keys($config['update_milex'], 1) : [];
