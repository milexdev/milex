...
use MilexPlugin\MilexCrmBundle\Services\Transport;
use Monolog\Logger;
...
class PipedriveIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'url'      => 'milex.pipedrive.api_url',
            'token'    => 'milex.pipedrive.token',
            'user'     => 'milex.pipedrive.webhook_user',
            'password' => 'milex.pipedrive.webhook_password',
        ];
    }
    ...
    public function getApiHelper()
    {
        if (empty($this->apiHelper)) {
            $class           = '\\MilexPlugin\\MilexCrmBundle\\Api\\'.$this->getName().'Api'; //TODO replace with service
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices'     => [
                        'milex.pipedrive.object.organization'  => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.pipedrive.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            $builder->add(
                ...
                [
                    'choices'     => [
                        'milex.pipedrive.add.edit.contact.import.enabled' => 'enabled',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.pipedrive.add.edit.contact.import',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
    ...
    /**
     * @param array|\Milex\LeadBundle\Entity\Lead $lead
     * @param array                                $config
     *
     * @return mixed
     */
    ...
    public function getFormNotes($section)
    {
        ...
        if ('authorization' == $section) {
            return [
                $translator->trans('milex.pipedrive.webhook_callback').$router->generate(
