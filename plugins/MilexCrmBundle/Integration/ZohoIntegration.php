...
use MilexPlugin\MilexCrmBundle\Api\Zoho\Mapper;
use MilexPlugin\MilexCrmBundle\Api\ZohoApi;
...
class ZohoIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            $this->getClientIdKey()     => 'milex.zoho.form.client_id',
            $this->getClientSecretKey() => 'milex.zoho.form.client_secret',
        ];
    }
     ...
     * @return array
     */
    public function amendLeadDataBeforeMilexPopulate($data, $object = null)
    {
        if ('company' === $object) {
            $object = 'Accounts';
            ...
            foreach ($objects as $recordId => $entityData) {
                ...
                if ('Accounts' === $object) {
                    ...
                    if (count($integrationId)) { // company exists, then update local fields
                        ...
                        $matchedFields = $this->populateMilexLeadData($entityData, $config, 'company');

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMilex = $this->getPriorityFieldsForMilex($config, $object, 'milex_company');
                        if (!empty($fieldsToUpdateInMilex)) {
                            $fieldsToUpdateInMilex = array_intersect_key($config['companyFields'], $fieldsToUpdateInMilex);
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMilex));
                        } else {
                        ...
                        $entity = $this->getMilexCompany($entityData, 'Accounts');
                    }
                    if ($entity) {
                    ...
                    if (count($integrationId)) { // lead exists, then update
                        ...
                        $matchedFields = $this->populateMilexLeadData($entityData, $config, $object);

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMilex = $this->getPriorityFieldsForMilex($config, $object, 'milex');

                        if (!empty($fieldsToUpdateInMilex)) {
                            $fieldsToUpdateInMilex = array_intersect_key($config['leadFields'], $fieldsToUpdateInMilex);
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMilex));
                        } else {
                            $newMatchedFields = $matchedFields;
                        }
                        ...
                        /** @var Lead $entity */
                        $entity = $this->getMilexLead($entityData, true, null, null, $object);
                    }
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                while (true) {
                    ...
                    $result   = $this->amendLeadDataBeforeMilexPopulate($data, $object);
     ...
     * @return array
     */
    public function populateMilexLeadData($data, $config = [], $object = 'Leads')
    {
        // Match that data with mapped lead fields
        $aFields       = $this->getAvailableLeadFields($config);
        $matchedFields = [];
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        ...
        if ('keys' === $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.plugin.zoho.zone_us'     => 'zoho.com',
                        'milex.plugin.zoho.zone_europe' => 'zoho.eu',
                        'milex.plugin.zoho.zone_japan'  => 'zoho.co.jp',
                        'milex.plugin.zoho.zone_china'  => 'zoho.com.cn',
                    ],
                    'label'       => 'milex.plugin.zoho.zone_select',
                    'placeholder' => false,
                    'required'    => true,
                    'attr'        => [
                        'tooltip' => 'milex.plugin.zoho.zone.tooltip',
                    ],
                ]
            );
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.zoho.object.lead'    => 'Leads',
                        'milex.zoho.object.contact' => 'Contacts',
                        'milex.zoho.object.account' => 'company',
                    ],
                    'expanded'    => true,
                    'multiple'    => true,
                    'label'       => $this->getTranslator()->trans('milex.crm.form.objects_to_pull_from', ['%crm%' => 'Zoho']),
                    'label_attr'  => ['class' => ''],
                    'placeholder' => false,
                    'required'    => false,
                ]
    ...
    public function pushLeads($params = [])
    {
        ...
        $config                = $this->mergeConfigToFeatureSettings();
        $integrationEntityRepo = $this->em->getRepository('MilexPluginBundle:IntegrationEntity');
        $fieldsToUpdateInZoho  = isset($config['update_milex']) ? array_keys($config['update_milex'], 0) : [];
        $leadFields            = array_unique(array_values($config['leadFields']));
        $totalUpdated          = $totalCreated = $totalErrors = 0;
        if ($key = array_search('milexContactTimelineLink', $leadFields)) {
            unset($leadFields[$key]);
        }
        if ($key = array_search('milexContactIsContactableByEmail', $leadFields)) {
            unset($leadFields[$key]);
        }
        if (empty($leadFields)) {
            return [0, 0, 0];
            ...
            foreach ($toUpdate as $lead) {
                if (isset($lead['email']) && !empty($lead['email'])) {
                    ...
                    if (isset($isContact[$key])) {
                        ...
                        $leadsToUpdateInZ[$key]     = $lead;
                        $integrationEntity          = $this->em->getReference('MilexPluginBundle:IntegrationEntity', $integrationId[0]['id']);
     ...
     * @return int
     *
     * @throws \MilexPlugin\MilexCrmBundle\Api\Zoho\Exception\MatchingKeyNotFoundException
     */
    private function consumeResponse($response, $zObject, $createIntegrationEntity = false, Mapper $mapper = null)
    {
        ...
        foreach ($rows as $key => $row) {
            $milexId = $mapper->getContactIdByKey($key);
            ...
            if ('SUCCESS' === $row['code'] && $createIntegrationEntity) {
                ...
                if (0 === count($integrationId)) {
                    $this->createIntegrationEntity($zObject, $zohoId, 'lead', $milexId);
                }
            } elseif (isset($row['status']) && 'error' === $row['status']) {
                ++$failed;
                $exception = new ApiErrorException($row['message']);
                $exception->setContactId($milexId);
                $this->logIntegrationError($exception);
            }
        }
