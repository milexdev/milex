...
namespace MilexPlugin\MilexCrmBundle\Integration\Pipedrive\Export;
...
class LeadExport extends AbstractPipedrive
{
    ...
    private function getMappedLeadData(Lead $lead)
    {
        ...
        return $this->convertMilexData($mappedData);
    }
    ...
    {
        $milexOwner = $lead->getOwner();

        if (!$milexOwner) {
            return 0;
        }

        $pipedriveOwner = $this->em->getRepository(PipedriveOwner::class)->findOneByEmail($milexOwner->getEmail());
    ...
    private function convertMilexData($data)
    {
