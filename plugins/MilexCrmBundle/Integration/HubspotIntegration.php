...
use MilexPlugin\MilexCrmBundle\Api\HubspotApi;
use Monolog\Logger;
...
class HubspotIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            $this->getApiKey() => 'milex.hubspot.form.apikey',
        ];
    }
                        ...
                        if (isset($leadFields)) {
                            foreach ($leadFields as $fieldInfo) {
                                ...
                                if (!empty($fieldInfo['readOnlyValue'])) {
                                    $hubsFields[$object][$fieldInfo['name']]['update_milex'] = 1;
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.hubspot.object.contact' => 'contacts',
                        'milex.hubspot.object.company' => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => $this->getTranslator()->trans('milex.crm.form.objects_to_pull_from', ['%crm%' => 'Hubspot']),
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            if ($this->isAuthorized()) {
                ...
                if (isset($data['contacts'])) {
                    foreach ($data['contacts'] as $contact) {
                        if (is_array($contact)) {
                            $contactData = $this->amendLeadDataBeforeMilexPopulate($contact, 'Lead');
                            $contact     = $this->getMilexLead($contactData);
                ...
                if (isset($results['results'])) {
                    foreach ($results['results'] as $company) {
                        if (isset($company['properties'])) {
                            $companyData = $this->amendLeadDataBeforeMilexPopulate($company, null);
                            $company     = $this->getMilexCompany($companyData);
                            if ($id) {
                                return $company;
                            }
                            if ($company) {
    ...
    public function getMilexLead($data, $persist = true, $socialCache = null, $identifiers = null, $object = null)
    {
        ...
        if ($lead = parent::getMilexLead($data, false, $socialCache, $identifiers, $object)) {
            if (isset($stageName)) {
