...
use MilexPlugin\MilexCrmBundle\Api\CrmApi;
...
abstract class CrmAbstractIntegration extends AbstractIntegration
{
    ...
    public function getLeads($params, $query, &$executed, $result = [], $object = 'Lead')
    {
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                return $this->amendLeadDataBeforeMilexPopulate($result, $object);
            }
        } catch (\Exception $e) {
            $this->logIntegrationError($e);
        }
    ...
    public function getApiHelper()
    {
        if (empty($this->helper)) {
            $class        = '\\MilexPlugin\\MilexCrmBundle\\Api\\'.$this->getName().'Api';
    ...
    public function getLeadData(\DateTime $startDate = null, \DateTime $endDate = null, $leadId)
    {
        ...
        foreach ($leadIds as $leadId) {
            ...
            while (true) {
                ...
                $entityToNotDetach = ['Milex\PluginBundle\Entity\Integration', 'Milex\PluginBundle\Entity\Plugin'];
     ...
     * @return Company|null
     */
    public function getMilexCompany($data, $object = null)
    {
        if (is_object($data)) {
        ...
        $config        = $this->mergeConfigToFeatureSettings([]);
        $matchedFields = $this->populateMilexLeadData($data, $config, 'company');
        ...
        if (!empty($existingCompany[2])) {
            $fieldsToUpdate = $this->getPriorityFieldsForMilex($config, $object, 'milex_company');
     ...
     * @return Lead
     */
    public function getMilexLead($data, $persist = true, $socialCache = null, $identifiers = null, $object = null)
    {
        if (is_object($data)) {
            // Convert to array in all levels
            $data = json_encode(json_decode($data, true));
        ...
        $matchedFields = $this->populateMilexLeadData($data, $config);
        ...
        if (!$lead->isNewlyCreated()) {
            ...
            $fieldsToUpdateInMilex = array_intersect_key($leadFields, $fieldsToUpdateInMilex);
            $matchedFields          = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMilex));
            if ((isset($config['updateBlanks']) && isset($config['updateBlanks'][0]) && 'updateBlanks' == $config['updateBlanks'][0])) {
                $matchedFields = $this->getBlankFieldsToUpdateInMilex($matchedFields, $lead->getFields(true), $leadFields, $data, $object);
            }
        }
        ...
        if (isset($data['owner_email']) && isset($config['updateOwner']) && isset($config['updateOwner'][0])
            ...
            if ($milexUser = $this->em->getRepository('MilexUserBundle:User')->findOneBy(['email' => $data['owner_email']])) {
                $lead->setOwner($milexUser);
            }
        }
     ...
     * @return array
     */
    protected function getPriorityFieldsForMilex($config, $entityObject = null, $priorityObject = 'milex')
    {
     ...
     * @return array
     */
    protected function getPriorityFieldsForIntegration($config, $entityObject = null, $priorityObject = 'milex')
    {
        return $this->cleanPriorityFields(
     ...
     * @param $objectFields
     */
    public function getBlankFieldsToUpdateInMilex($matchedFields, $leadFieldValues, $objectFields, $integrationData, $object = 'Lead')
    {
        foreach ($objectFields as $integrationField => $milexField) {
            if (isset($leadFieldValues[$milexField]) && empty($leadFieldValues[$milexField]['value']) && !empty($integrationData[$integrationField.'__'.$object]) && $this->translator->trans('milex.integration.form.lead.unknown') !== $integrationData[$integrationField.'__'.$object]) {
                $matchedFields[$milexField] = $integrationData[$integrationField.'__'.$object];
            }
        }
