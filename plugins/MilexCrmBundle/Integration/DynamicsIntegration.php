...
class DynamicsIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'resource'      => 'milex.integration.dynamics.resource',
            'client_id'     => 'milex.integration.dynamics.client_id',
            'client_secret' => 'milex.integration.dynamics.client_secret',
        ];
    }
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        ...
        if ('features' === $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.dynamics.object.contact'  => 'contacts',
                        'milex.dynamics.object.company'  => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.dynamics.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
    ...
    public function getFormNotes($section)
    {
        if ('custom' === $section) {
            return [
                'template'   => 'MilexCrmBundle:Integration:dynamics.html.php',
                'parameters' => [
                ],
            ];
        }
                ...
                if ('accounts' === $object) {
                    ...
                    if (count($integrationId)) { // company exists, then update local fields
                        ...
                        if (!empty($fieldsToUpdateInMilex)) {
                            $fieldsToUpdateInMilex = array_intersect_key($config['companyFields'], array_flip($fieldsToUpdateInMilex));
                        ...
                        $entity = $this->getMilexCompany($entityData, 'company');
                    }
                    if ($entity) {
                    ...
                    if (count($integrationId)) { // lead exists, then update
                        ...
                        $matchedFields = $this->populateMilexLeadData($entityData, $config);

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMilex = $this->getPriorityFieldsForMilex($config, $object, 'milex');
                        if (!empty($fieldsToUpdateInMilex)) {
                            $fieldsToUpdateInMilex = array_intersect_key($config['leadFields'], array_flip($fieldsToUpdateInMilex));
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMilex));
                        } else {
                            $newMatchedFields = $matchedFields;
                        }
                        ...
                        /** @var Lead $entity */
                        $entity = $this->getMilexLead($entityData);
                    }
    ...
    public function pushLeads($params = [])
    {
        ...
        $config                = $this->mergeConfigToFeatureSettings();
        $integrationEntityRepo = $this->em->getRepository('MilexPluginBundle:IntegrationEntity');
        $fieldsToUpdateInCrm   = isset($config['update_milex']) ? array_keys($config['update_milex'], 0) : [];
        ...
        if (is_array($toUpdate)) {
            ...
            foreach ($toUpdate as $lead) {
                if (isset($lead['email']) && !empty($lead['email'])) {
                    ...
                    $integrationEntity          = $this->em->getReference('MilexPluginBundle:IntegrationEntity', $lead['id']);
