...
class SugarcrmIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'sugarcrm_url'  => 'milex.sugarcrm.form.url',
            'client_id'     => 'milex.sugarcrm.form.clientkey',
            'client_secret' => 'milex.sugarcrm.form.clientsecret',
            'username'      => 'milex.sugarcrm.form.username',
            'password'      => 'milex.sugarcrm.form.password',
        ];
    }
    ...
    public function getCompanies($params = [], $query = null, $executed = null)
    {
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                $executed += $this->amendLeadDataBeforeMilexPopulate($result, $sugarObject);
                if (
        ...
        try {
            if ($this->isAuthorized()) {
                if ('Activity' !== $object and 'company' !== $object) {
                    ...
                    $params['offset'] = $result['next_offset'];
                    $executed += $this->amendLeadDataBeforeMilexPopulate($result, $object);
                    if (
                        (isset($result['total_count']) && $result['total_count'] > $params['offset'])   //Sugar 6
                        || (!isset($result['total_count']) && $params['offset'] > -1)) {            //Sugar 7
    ...
    public function getErrorsFromResponse($response)
    {
        if ('6' == $this->keys['version']) {
            if (!empty($response['name'])) {
                ...
                return $this->translator->trans('milex.integration.error.genericerror', [], 'flashes');
            }
        } else {
    ...
    /**
     * Amend mapped lead data before creating to Milex.
     *
     * @param array  $data
     * @param string $object
     *
    ...
    public function amendLeadDataBeforeMilexPopulate($data, $object)
    {
        ...
        /** @var IntegrationEntityRepository $integrationEntityRepo */
        $integrationEntityRepo = $this->em->getRepository('MilexPluginBundle:IntegrationEntity');
        $companyRepo           = $this->em->getRepository('MilexLeadBundle:Company');
            ...
            foreach ($data[$RECORDS_LIST_NAME] as $record) {
                    ...
                    foreach ($record['name_value_list'] as $item) {
                        if ('Activity' !== $object) {
                            if ($this->checkIfSugarCrmMultiSelectString($item['value'])) {
                                $convertedMultiSelectString         = $this->convertSuiteCrmToMilexMultiSelect($item['value']);
                ...
                if (!empty($dataObject)) {
                    if ('Leads' == $object or 'Contacts' == $object) {
                        ...
                        $company               = null;
                        $this->fetchDncToMilex($entity, $data);
                        ...
                        $entity                = $this->getMilexCompany($dataObject, $object);
    ...
    public function appendToForm(&$builder, $data, $formArea)
    {
        if ('keys' == $formArea) {
            $builder->add('version', ButtonGroupType::class, [
                ...
                'label'             => 'milex.sugarcrm.form.version',
                'constraints'       => [
                    new NotBlank([
        ...
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.sugarcrm.updateOwner' => 'updateOwner',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.sugarcrm.form.updateOwner',
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.sugarcrm.updateDnc' => 'updateDnc',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.sugarcrm.form.updateDnc',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                    'attr'              => [
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'milex.sugarcrm.object.lead'    => 'Leads',
                        'milex.sugarcrm.object.contact' => 'Contacts',
                        'milex.sugarcrm.object.company' => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'milex.sugarcrm.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
    ...
    public function pushLead($lead, $config = [])
    {
        ...
        $fieldsToUpdateInSugar      = isset($config['update_milex']) ? array_keys($config['update_milex'], 0) : [];
    ...
    public function pushLeads($params = [])
    {
        ...
        $config                  = $this->mergeConfigToFeatureSettings();
        $integrationEntityRepo   = $this->em->getRepository('MilexPluginBundle:IntegrationEntity');
        $milexData              = $leadsToUpdate              = $fields              = [];
        $fieldsToUpdateInSugar   = isset($config['update_milex']) ? array_keys($config['update_milex'], 0) : [];
        $leadFields              = $config['leadFields'];
        if (!empty($leadFields)) {
            if ($keys = array_keys($leadFields, 'milexContactTimelineLink')) {
                foreach ($keys as $key) {
                    unset($leadFields[$key]);
                }
            }

            if ($keys = array_keys($leadFields, 'milexContactIsContactableByEmail')) {
                foreach ($keys as $key) {
                    unset($leadFields[$key]);
                }
            }
        ...
        foreach ($checkEmailsInSugar as $object => $checkObjectEmailsInSugar) {
            list($checkEmailsUpdatedInSugar, $deletedRedords) = $this->getObjectDataToUpdate($checkObjectEmailsInSugar, $milexData, $availableFields, $contactSugarFields, $leadSugarFields, $object);
        ...
        if ($checkEmailsInSugar && isset($checkEmailsInSugar['Leads'])) {
            list($checkEmailsInSugar, $deletedSugarLeads) = $this->getObjectDataToUpdate($checkEmailsInSugar['Leads'], $milexData, $availableFields, $contactSugarFields, $leadSugarFields, 'Leads');
        ...
        $apiHelper = $this->getApiHelper();
        if (!empty($milexData)) {
            $result = $apiHelper->syncLeadsToSugar($milexData);
        }
    ...
    private function pushDncToSugar(array $lead, array &$body)
    {
        ...
        /** @var \Milex\LeadBundle\Entity\DoNotContact[] $dncEntries */
    ...
    private function fetchDncToMilex(Lead $lead = null, array $data)
    {
        if (is_null($lead)) {
            return;
        }
        ...
        $reason = \Milex\LeadBundle\Entity\DoNotContact::UNSUBSCRIBED;
        if (!$isContactable) {
     ...
     * @param string $object
     *
     * @return array The first element is made up of records that exist in Milex, but which no longer have a match in CRM.
     ...
     */
    public function getObjectDataToUpdate($checkEmailsInSugar, &$milexData, $availableFields, $contactSugarFields, $leadSugarFields, $object = 'Leads')
    {
        $config     = $this->mergeConfigToFeatureSettings([]);
        ...
        foreach ($sugarLeadRecords as $sugarLeadRecord) {
            if ((isset($sugarLeadRecord) && $sugarLeadRecord)) {
                ...
                $leadOwnerEmails = [];
                if (!empty($checkEmailsInSugar)) {
                    foreach ($checkEmailsInSugar as $emailKey => $milexRecord) {
                        if ($key == $emailKey) {
                            ...
                            $lead['owner_email']                                             = $this->getOwnerEmail($milexRecord);
                            if ($lead['owner_email']) {
     ...
     * @param null $objectId
     */
    protected function buildCompositeBody(&$milexData, $availableFields, $fieldsToUpdateInSugarUpdate, $object, $lead, $onwerAssignedUserIdByEmail = null, $objectId = null)
    {
        $body = [];
        if (isset($lead['email']) && !empty($lead['email'])) {
            //update and create (one query) every 200 records

            foreach ($fieldsToUpdateInSugarUpdate as $sugarField => $milexField) {
                $required = !empty($availableFields[$object][$sugarField.'__'.$object]['required']);
                if (isset($lead[$milexField])) {
                    if (false !== strpos($lead[$milexField], '|')) {
                        // Transform Milex Multi Select into SugarCRM/SuiteCRM Multi Select format
                        $value = $this->convertMilexToSuiteCrmMultiSelect($lead[$milexField]);
                    } else {
                        $value = $lead[$milexField];
                    }
                    $body[] = ['name' => $sugarField, 'value' =>  $value];
                } elseif ($required) {
                    $value  = $this->translator->trans('milex.integration.form.lead.unknown');
                    $body[] = ['name' => $sugarField, 'value' => $value];
                }
            }
            ...
            foreach ($response as $item) {
                ...
                if (isset($item['ko']) && $item['ko']) {
                    ...
                    if ($integrationEntityId) {
                        $integrationEntity = $this->em->getReference('MilexPluginBundle:IntegrationEntity', $integrationEntityId);
                        $integrationEntity->setLastSyncDate(new \DateTime());
    ...
    /**
     * Converts Milex Multi-Select String into the format used to store Multi-Select values used by SuiteCRM / SugarCRM 6.x.
     *
     * @param  string
     *
     * @return string
     */
    public function convertMilexToSuiteCrmMultiSelect($milexMultiSelectStringToConvert)
    {
        //$milexMultiSelectStringToConvert = 'test|enhancedapi|dataservices';
        $multiSelectArrayValues             = explode('|', $milexMultiSelectStringToConvert);
    ...
    /**
     * Converts a SuiteCRM / SugarCRM 6.x Multi-Select String into the format used to store Multi-Select values used by Milex.
     *
     * @param  string
     *
     * @return string
     */
    public function convertSuiteCrmToMilexMultiSelect($suiteCrmMultiSelectStringToConvert)
    {
        // Milex Multi Select format - 'choice1|choice2|choice_3'
