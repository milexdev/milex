...
use MilexPlugin\MilexCrmBundle\Api\PipedriveApi;
...
class FetchPipedriveDataCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('milex:integration:pipedrive:fetch')
            ->setDescription('Pulls the data from Pipedrive and sends it to Milex')
            ->addOption(
                '--restart',
                null,
    ...
    private function getData($type, $endPoint, $integrationObject)
    {
        ...
        while (true) {
            ...
            $service = $container->get('milex_integration.pipedrive.import.'.$type);
