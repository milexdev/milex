...
use MilexPlugin\MilexCrmBundle\Tests\Stubs\StubIntegration;
...
class CrmAbstractIntegrationTest extends AbstractIntegrationTestCase
{
    public function testFieldMatchingPriority()
    {
        ...
        $methodMilex = new \ReflectionMethod(StubIntegration::class, 'getPriorityFieldsForMilex');
        $methodMilex->setAccessible(true);
        ...
        $fieldsForMilex = $methodMilex->invokeArgs($integration, [$config]);

        $this->assertSame(
            ['email', 'address_1', 'address_2'],
            $fieldsForMilex,
            'Fields to update in Milex should return fields marked as 1 in the integration priority config.'
        );
    ...
    public function testCompanyDataIsMappedForNewCompanies()
    {
        ...
        $integration = $this->getMockBuilder(StubIntegration::class)
            ...
            ->setMethodsExcept(['getMilexCompany', 'setCompanyModel', 'setFieldModel', 'hydrateCompanyName'])
            ->getMock();

        $integration->expects($this->once())
            ->method('populateMilexLeadData')
            ->willReturn($data);

        $company = $integration->getMilexCompany($data);
