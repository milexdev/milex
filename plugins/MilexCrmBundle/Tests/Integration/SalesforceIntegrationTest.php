...
namespace MilexPlugin\MilexCrmBundle\Tests\Integration;
...
use MilexPlugin\MilexCrmBundle\Integration\SalesforceIntegration;
...
class SalesforceIntegrationTest extends AbstractIntegrationTestCase
{
    ...
    const SC_MULTIPLE_MAUTIC_CONTACTS = 'multiple_milex_contacts';
    ...
    public function testThatLeadsAreOnlyCreatedIfEnabled()
    {
        ...
        $this->sfMockMethods = ['makeRequest', 'getMilexContactsToCreate'];
        ...
        $sf->expects($this->never())
            ->method('getMilexContactsToCreate');
    ...
    {
        $this->sfMockMethods = ['makeRequest', 'getMilexContactsToCreate', 'getMilexContactsToUpdate', 'getSalesforceSyncLimit'];

        $sf      = $this->getSalesforceIntegration();
        $counter = 0;
        $sf->expects($this->exactly(2))
            ->method('getMilexContactsToUpdate')
            ->will(
                $this->returnCallback(
    ...
    public function testLeadsAreNotCreatedInSfIfFoundToAlreadyExistAsContacts()
    {
        ...
        $this->sfMockMethods = ['makeRequest', 'getSalesforceObjectsByEmails', 'prepareMilexContactsToCreate'];
        ...
        $sf->expects($this->never())
            ->method('prepareMilexContactsToCreate');
    ...
    public function testExportingContactActivity()
    {
        ...
        /*
         * Ensures that makeRequest is called with the milex_timeline__c endpoint.
    ...
    public function testMilexContactTimelineLinkPopulatedsPayload()
    {
        $this->sfObjects     = ['Contact'];
        $this->sfMockMethods = ['makeRequest', 'getSalesforceObjectsByEmails'];
        $sf                  = $this->getSalesforceIntegration(2, 2);
        ...
        /*
         * When checking if contacts need to be created, the milexContactTimelineLink
            ...
            ->with(
                ...
                [
                    'contact1@sftest.com' => [
                        ...
                        'milexContactTimelineLink'         => 'milex_plugin_timeline_view',
                        'milexContactIsContactableByEmail' => 1,
                        'milexContactId'                   => 1,
                    ],
                    'contact2@sftest.com' => [
        ...
        foreach ($objects as $object) {
            $mappedData = [
                ...
                'contact2@sftest.com' => [
                    ...
                    'email'                             => 'Contact2@sftest.com',
                    'milexContactTimelineLink'         => 'milex_plugin_timeline_view',
                    'milexContactIsContactableByEmail' => 0,
                ],
            ];
            ...
            foreach ($mappedData as $assertion) {
                $this->assertArrayHasKey('milexContactIsContactableByEmail', $assertion);
            }
        }
    }

    public function testUpdateDncByMilexDate()
    {
        $this->sfMockMethods = ['makeRequest', 'updateDncByDate', 'getDncHistory'];
        ...
        foreach ($objects as $object) {
            ...
            $sf = $this->getSalesforceIntegration(2, 2);
            $sf->expects($this->any())->method('updateDncByDate')->willReturn(true);
            $sf->expects($this->any())->method('getDncHistory')->willReturn($this->getSalesforceDNCHistory($object, 'Milex'));

            $sf->pushLeadDoNotContactByDate('email', $mappedData, $object, ['start' => '2017-10-15T10:00:00.000000']);
            foreach ($mappedData as $assertion) {
                $this->assertArrayNotHasKey('milexContactIsContactableByEmail', $assertion);
            }
        }
    }
        ...
        $this->em->method('getRepository')
            ->willReturnMap(
                [
                    ...
                    ['MilexCoreBundle:AuditLog', $auditLogRepo],
                ]
            );
            ...
            ->willReturnCallback(
                function () {
                    switch (func_get_arg(0)) {
                        case 'MilexPluginBundle:IntegrationEntity':
                        ...
                        switch (true) {
                            case false !== strpos($args[0], '/query'):
                                if (isset($args[1]['q']) && false !== strpos($args[0], 'from CampaignMember')) {
                                    ...
                                    return $this->getSalesforceDNCHistory($updateObject, 'Milex');
                                } else {
                                    // Extract emails
    ...
    /**
     * Simulate looping over Milex leads to update.
     *
     * @param $object
     * @param $limit
     * @param $max
    ...
    protected function getLeadsToUpdate($object, $limit, $max, $specificObject)
    {
        ...
        $this->milexContacts = array_merge($entities[$object], $this->milexContacts);

        return $entities;
    }

    /**
     * Simulate looping over Milex leads to create.
     *
     * @param $limit
     * @param $max
     *
    ...
    protected function getLeadsToCreate($limit, $max = 200)
    {
        ...
        $this->milexContacts = array_merge($entities, $this->milexContacts);
    ...
    protected function getSalesforceDNCHistory($object, $priority)
    {
        $datePriority = [
            ...
            'Milex' => '2017-10-16T18:43:43.000+0000',
            ];
    ...
    protected function getMilexContacts()
    {
        $contacts             = $this->milexContacts;
        $this->milexContacts = [
            'Contact' => [],
            'Lead'    => [],
        ];
