...
namespace MilexPlugin\MilexCrmBundle\Tests\Pipedrive;
...
abstract class PipedriveTest extends MilexMysqlTestCase
{
    ...
    protected function installPipedriveIntegration($published = false, array $settings = [], array $apiKeys = ['url' => '', 'token' => ''], array $features = ['push_lead'], $addCredential = true)
    {
        $plugin = new Plugin();
        $plugin->setName('CRM');
        $plugin->setDescription('Enables integration with Milex supported CRMs.');
        $plugin->setBundle('MilexCrmBundle');
    ...
    protected function createUser($isAdmin = true, $email = 'admin@pipedrive-admin.com', $username = 'pipedrive-admin')
    {
        ...
        $userModel = $this->client->getContainer()->get('milex.model.factory')->getModel('user');
    ...
    protected function getIntegrationObject()
    {
        ...
        $integrationHelper = self::$container->get('milex.helper.integration');
