...
use Milex\CoreBundle\Helper\ThemeHelper;
...
class GrapesJsController extends CommonController
{
    ...
    public function builderAction($objectType, $objectId)
    {
        ...
        $this->logger     = $this->get('monolog.logger.milex');

        /** @var \Milex\EmailBundle\Model\EmailModel|\Milex\PageBundle\Model\PageModel $model */
        ...
        if (false !== strpos($objectId, 'new')) {
            ...
            if (!$this->get('milex.security')->isGranted($aclToCheck.'create')) {
                return $this->accessDenied();
            }

            /** @var \Milex\EmailBundle\Entity\Email|\Milex\PageBundle\Entity\Page $entity */
        ...
        $themeHelper  = $this->get('milex.helper.theme');
        ...
        if ($logicalName = $this->checkForMjmlTemplate($templateName.'.mjml.twig')) {
            ...
            $newContent = $this->get('session')->get('milex.'.$objectType.'builder.'.$objectId.'.content', []);
        ...
        foreach ($slots as $slot => $slotConfig) {
            ...
            if ('slideshow' == $slotConfig['type']) {
                ...
                if (method_exists($renderingEngine, 'getEngine')) {
                    $renderingEngine->getEngine('MilexPageBundle:Page:Slots/slideshow.html.php');
                }
    ...
    {
        $templatingHelper = $this->get('milex.helper.templating');
