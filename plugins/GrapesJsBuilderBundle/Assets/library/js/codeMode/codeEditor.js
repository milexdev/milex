...
class CodeEditor {
  ...
  buildCodePopup() {
    ...
    btnEdit.innerHTML = Milex.translate('grapesjsbuilder.sourceEditBtnLabel');
    btnEdit.className = `${cfg.stylePrefix}btn-prim ${cfg.stylePrefix}btn-code-edit`;
    btnEdit.onclick = this.updateCode.bind(this);

    btnCancel.innerHTML = Milex.translate('grapesjsbuilder.sourceCancelBtnLabel');
  ...
  showCodePopup(editor) {
    ...
    editor.Modal.setTitle(Milex.translate('grapesjsbuilder.sourceEditModalTitle'));
    editor.Modal.open();

    editor.Modal.onceClose(() => editor.stopCommand('preset-milex:code-edit'));
  }
  ...
  updateCode() {
    ...
    try {
      ...
      window.alert(`${Milex.translate('grapesjsbuilder.sourceSyntaxError')} \n${e.message}`);
    }
  }
