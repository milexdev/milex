...
namespace MilexPlugin\MilexCitrixBundle\Model;

use Milex\CampaignBundle\Model\EventModel;
use Milex\CoreBundle\Model\FormModel;
use Milex\LeadBundle\Entity\Lead;
use Milex\LeadBundle\Model\LeadModel;
use MilexPlugin\MilexCitrixBundle\CitrixEvents;
use MilexPlugin\MilexCitrixBundle\Entity\CitrixEvent;
use MilexPlugin\MilexCitrixBundle\Entity\CitrixEventTypes;
use MilexPlugin\MilexCitrixBundle\Event\CitrixEventUpdateEvent;
...
class CitrixModel extends FormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \MilexPlugin\MilexCitrixBundle\Entity\CitrixEventRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('MilexCitrixBundle:CitrixEvent');
    }
    ...
    public function getDistinctEventNames($product)
    {
        ...
        $dql = sprintf(
            "SELECT DISTINCT(c.eventName) FROM MilexCitrixBundle:CitrixEvent c WHERE c.product='%s'",
            $product
        );
    ...
    public function getDistinctEventNamesDesc($product)
    {
        ...
        $dql = sprintf(
            "SELECT DISTINCT c.eventName, c.eventDesc FROM MilexCitrixBundle:CitrixEvent c WHERE c.product='%s'",
            $product
        );
        $query  = $this->em->createQuery($dql);
        $items  = $query->getResult();
    ...
    public function countEventsBy($product, $email, $eventType, array $eventNames = [])
    {
        ...
        $dql = 'SELECT COUNT(c.id) as cant FROM MilexCitrixBundle:CitrixEvent c '.
