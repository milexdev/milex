...
class PublicController extends FormController
{
    ...
    public function callbackAction()
    {
        ...
        $validatedRequest = $this->get('milex.plugin.fullcontact.lookup_helper')->validateRequest($oid);
            ...
            if ($notify && (!isset($lead->imported) || !$lead->imported)) {
                ...
                if ($user = $userModel->getEntity($notify)) {
                    $this->addNewNotification(
                        sprintf($this->translator->trans('milex.plugin.fullcontact.contact_retrieved'), $lead->getEmail()),
                        'FullContact Plugin',
                        'fa-search',
                        $user
                    );
            ...
            try {
                if ($notify && $lead && (!isset($lead->imported) || !$lead->imported)) {
                    ...
                    if ($user = $userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf(
                                $this->translator->trans('milex.plugin.fullcontact.unable'),
                                $lead->getEmail(),
                                $ex->getMessage()
                            ),
                            'FullContact Plugin',
                ...
                $this->get('monolog.milex.logger')->log('error', 'FullContact: '.$ex2->getMessage());
            }
        }
    ...
    private function compcallbackAction($result, $validatedRequest)
    {
        ...
        try {
            ...
            if ($notify) {
                ...
                if ($user = $userModel->getEntity($notify)) {
                    $this->addNewNotification(
                        sprintf($this->translator->trans('milex.plugin.fullcontact.company_retrieved'), $company->getName()),
                        'FullContact Plugin',
                        'fa-search',
                        $user
                    );
